...
- Problem cue: Our method is able to solve a hierarchy of only equalities ten times faster than the iterativeprojection hierarchical solvers and can consider inequalities at any level while running ...
- Method cue: Our method is able to solve a hierarchy of only equalities ten times faster than the iterativeprojection hierarchical solvers and can consider inequalities at any level while running ...
- Result/evaluation cue: Some typical trajectories of the joints are shown in Fig.

위 cue는 기존 official abstract 또는 local text extraction에서 보존한 것이다. 수치·조건·인과적 해석은 full-text 정독 전까지 `UNVERIFIED`다.

## Dependency Position

`Dynamic Whole-Body Motion Generation under Rigid Contacts and Other Unilateral Constraints → Hierarchical Quadratic Programming: Fast Online Humanoid-Robot Motion Generation → Whole-Body Nonlinear Model Predictive Control Through Contacts for Quadrupeds`
