...
| Observation / input | Y In other words, the ~ value is the expected discounted reward for executing action a at state x and following policy ... | state 또는 observation, action, reward와 transition history | exact sensor/frame/preprocessing from PDF body |
...
PDF body contribution framing (p. 3 (2. The task for ~-learning), p. 4 (3. The convergence proof), p. 4 (3. The convergence proof), p. 1 (1. Introduction), p. 1 (Body text (section boundary not confidently recovered))): In O~-learning, the agent's experience consists of a sequence of distinct stages or episodes.
...
rl writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. Evidence interface anchors: p. 2 (2. The task for ~-learning), p. 2 (2. The task for ~-learning), p. 3 (2. The task for ~-learning), p. 1 (Body text (section boundary not confidently recovered)). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **Evidence anchors reviewed:** problem p. 1 (1. Introduction), interface p. 2 (2. The task for ~-learning), p. 2 (2. The task for ~-learning), p. 3 (2. The task for ~-learning), p. 1 (Body text (section boundary not confidently recovered)), objective p. 3 (2. The task for ~-learning), p. 6 (3.2. The theorem), p. 7 (3.2. The theorem), p. 7 (3.2. The theorem), p. 2 (2. The task for ~-learning), p. 3 (2. The task for ~-learning).
...
- **Target problem:** Section 2 describes the problem, the method, and the notation, section 3 gives an overview of the proof, and section 4 discusses two extensions. (p. 1, 1. Introduction).
- **Formulation-changing contribution:** O~-learning (Watkins, 1989) is a form of model-free reinforcement learning. (p. 1, 1. Introduction).
- **Assumption/failure evidence:** Unfortunately, the theorem does not extend trivially to this case, and alternative proof methods such as those in Kushner and Clark (1978) may be required. (p. 8, 4. Discussions and conclusions).
