...
- **p. 6 / 3.1 Preliminaries - extractive PDF cue:** The point-plane error metric in combination with correspondences obtained using projective data association was first demonstrated in a real time modelling system by [23] where ...
- **p. 4 / 3 METHOD - extractive PDF cue:** Surface reconstruction update: The global scene fusion process, where given the pose determined by tracking the depth data from a new sensor frame, the surface ...
- **p. 6 / 3.1 Preliminaries - extractive PDF cue:** Many tracking algorithms use feature selection to improve speed by reducing the number of points for which data association need be performed.
- **p. 4 / 3.1 Preliminaries - extractive PDF cue:** We compute an L = 3 level multi-scale representation of the surface measurement in the form of a vertex and normal map pyramid.
- **p. 5 / 3.1 Preliminaries - extractive PDF cue:** Also, the support is increased linearly with distance from the sensor center to support correct representation of noisier measurements.
- **p. 5 / 3.1 Preliminaries - extractive PDF cue:** Although efficient algorithms exist for computing the true discrete SDF for a given set of point measurements (complexity is linear in the the number of ...
- **p. 7 / 3.1 Preliminaries - extractive PDF cue:** (21) An iteration is obtained by solving: min x∈R6 ∑ Ωk(u)̸=null ∥E∥2 2 (22) E = ˆNg k-1(ˆu)⊤ G(u)x+ eVg k(u)-ˆVg k-1(ˆu)  (23) By ...
- **p. 5 / 3.1 Preliminaries - extractive PDF cue:** Storing a weight Wk(p) with each value allows an important aspect of the global minimum of the convex L2 de-noising metric to be exploited for ...

## Design Rationale

- **p. 2 / 1 INTRODUCTION - extractive PDF cue:** In this paper we present a detailed method with analysis of what we believe is the first system which permits real-time, dense volumetric reconstruction of ...
- **p. 6 / 3.1 Preliminaries - extractive PDF cue:** Second, modern GPU hardware enables a fully parrallelised processing pipeline, so that the data association and point-plane optimisation can use all of the available surface ...
- **p. 7 / 3.1 Preliminaries - extractive PDF cue:** The symmetry of the system enables operations and memory to be saved and the final sum is obtained using a parallel tree-based reduction [13], to ...
...
- **p. 5 / 3.1 Preliminaries - extractive PDF cue:** Storing a weight Wk(p) with each value allows an important aspect of the global minimum of the convex L2 de-noising metric to be exploited for ...
- **p. 7 / 3.1 Preliminaries - extractive PDF cue:** Outliers from compatibility checks (Equation 17) using a surface measurement with (center) and without (right) bilateral filtering applied to the raw depth map. Ω(u) = ...
- **p. 4 / 3.1 Preliminaries - extractive PDF cue:** Subsequently each level in a vertex and normal map pyramid Vl∈[1...L], Nl∈[1...L] is computed with Equations 3 and 4 using the corresponding depth map level.
- **p. 5 / 3.1 Preliminaries - extractive PDF cue:** (5) A dense surface measurement (such as the raw depth map Rk) provides two important constraints on the surface being reconstructed.
- **p. 6 / 3.1 Preliminaries - extractive PDF cue:** 3.5 Sensor Pose Estimation Live camera localisation involves estimating the current camera pose Tw,k ∈SE3 (Equation 1) for each new depth image.
- **p. 7 / 3.1 Preliminaries - extractive PDF cue:** (21) An iteration is obtained by solving: min x∈R6 ∑ Ωk(u)̸=null ∥E∥2 2 (22) E = ˆNg k-1(ˆu)⊤ G(u)x+ eVg k(u)-ˆVg k-1(ˆu)  (23) By ...
...
- **p. 2 / 2 BACKGROUND - extractive PDF cue:** Later, it was discovered to be practically advantageous to abandon the propagation of a full probabilistic state and instead to run two procedures in alternation ...
- **p. 4 / 3.1 Preliminaries - extractive PDF cue:** We will also use a dot notation to denote homogeneous vectors ˙u := (u⊤/1)⊤ 3.2 Surface Measurement At time k a measurement comprises a raw ...
- **p. 6 / 3.1 Preliminaries - extractive PDF cue:** 3.5 Sensor Pose Estimation Live camera localisation involves estimating the current camera pose Tw,k ∈SE3 (Equation 1) for each new depth image.
...
- **p. 5 / 3.1 Preliminaries - extractive PDF cue:** For a raw depth map Rk with a known pose Tg,k, its global frame projective TSDF [FRk,WRk] at a point p in the global frame ...
- **p. 6 / 3.1 Preliminaries - extractive PDF cue:** In this work, we take advantage of two factors to allow us instead to make use of all of the data in a depth image ...
- **p. 7 / 3.1 Preliminaries - extractive PDF cue:** Relocalisation Our current implementation uses an interactive re-localisation scheme, whereby if the sensor loses track, the last known sensor pose is used to provide a ...
...
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** 4.1 Metrically Consistent Reconstruction Our tracking and mapping system provides a constant time algorithm for a given area of reconstruction, and we are interested in ...
...
- **p. 8 / 4 EXPERIMENTS - extractive PDF cue:** While the turntable experiments demonstrate interesting convergence of the system without an explicit global optimisation, the real power in integrating every frame of data is ...
- **p. 9 / 4 EXPERIMENTS - extractive PDF cue:** But the frame-model tracking results in drift-free operation without explicit global optimisation.
...
- **p. 9 / 4 EXPERIMENTS - extractive PDF cue:** 4.2 Processing Time Figure 13 shows results from an experiment where timings were taken of the main system components and the reconstruction voxel resolution was ...
