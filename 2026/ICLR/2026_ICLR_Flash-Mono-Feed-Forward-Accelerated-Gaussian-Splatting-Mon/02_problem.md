...
- Abstract problem cue: These issues stem from the time-consuming $\textit{Train-from-Scratch}$ optimization and the lack of inter-frame scale consistency from single-frame geometry priors.
...
- 이 논문은 두 축을 결합하는 흐름 안에서, `Gaussian Splatting, geometry, depth, 3D Vision` 관점의 개선을 제안한다.
