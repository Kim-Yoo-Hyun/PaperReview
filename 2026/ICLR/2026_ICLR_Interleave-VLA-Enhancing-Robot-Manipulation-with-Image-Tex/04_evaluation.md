...
- **p. 8 / 4 EXPERIMENTS - extractive PDF cue:** Notably, although the pretraining dataset does not include FANUC robot arm data, it still enables strong cross-embodiment transfer to FANUC.
- **p. 8 / 4 EXPERIMENTS - extractive PDF cue:** Unlike the SimplerEnv experiments, where large-scale BridgeData V2 supports strong performance, the real-robot setup relies on a smaller self-collected dataset.
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** 4.2 REAL ROBOT COMPARISON Task setup.
- **p. 9 / 4 EXPERIMENTS - extractive PDF cue:** The interleaved instruction format naturally accommodates diverse input types, making human-robot interaction more intuitive by removing the need for users to precisely describe complex objectives ...
- **p. 10 / 4 EXPERIMENTS - extractive PDF cue:** This occurs because many common robot-instruction templates, e.g., "Put object A [near / to the left of / on] object B", become ambiguous when expressed ...
- **p. 10 / 4 EXPERIMENTS - extractive PDF cue:** Instruction image diversity is crucial as well, Table 5 demonstrates that combining Internet images with task-specific images cropped from robot observations yields the best overall ...
- **p. 6 / 4 EXPERIMENTS - extractive PDF cue:** Performances are tested on SimplerEnv-Bridge setup, which uses a WidowX robot configuration compatible with the BridgeData V2 (Walke et al., 2023b).
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** We evaluate on a FANUC LRMate 200iD/7L robotic arm equipped with an SMC gripper.
...
- **p. 1 / Figure/Table caption - extractive PDF cue:** Figure 1: (a) Our Interleaved X-Embodiment Dataset features diverse, high-quality object-centric images automatically generated from real-world robot demonstrations. (b) Interleave-VLA achieves 2× stronger out-of-domain generalization ...
...
- **p. 5 / Figure/Table caption - extractive PDF cue:** Figure 2: Overview of the Interleave-VLA paradigm, featuring an extendable adaptation of Text- VLA to handle interleaved inputs, scalable training on a constructed large interleaved ...
- **p. 5 / Figure/Table caption - extractive PDF cue:** Figure 3: Left: Our open interleaved X-Embodiment dataset features a large number of high-quality cropped images with diversity across objects. Right: Interleave dataset generation pipeline: ...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Figure 4: Left: Illustration of generalization settings in SIMPLER. (a) Visual generalization: unseen environments, tablecloths, and lighting conditions. (b) Semantic generalization with novel objects from ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Table 2: Interleave-VLA and Text-VLA comparison on SimplerEnv. In-Domain includes 4 tasks following SimplerEnv-Bridge setup. We add 3 Out-of-Domain evaluation suites, namely: Visual, Novel Object, ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Figure 5: Qualitative analysis of Interleave-VLA's improved performance over the Text-VLA paradigm. In out-of-domain SimplerEnv tasks with unfamiliar objects, Text-VLA displays at- tentional hallucination, which ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Table 3: Comparison of success rates (Succ) and correct object picking rates (Acc) in real-robot experiments. All the baselines use the base VLA model π0. ...
...
| Published as a conference paper at ICLR 2026 Table 4: Interleave-VLA unlocks powerful zero-shot generalization to diverse instruction modalities, including hand-drawn sketches, user-cropped images, ... | component/input/data sensitivity | p. 9 (4 EXPERIMENTS) |
...
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** In-Domain includes 4 tasks following SimplerEnv-Bridge setup.
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** RT-1-X (O'Neill et al., 2024) Text-VLA Text/Text 1.1 ± 0.5 0.0 ± 0.0 3.5 ± 0.4 5.8 ± 0.3 3.2 ± 0.2 Octo (Team et ...
- **p. 10 / 4 EXPERIMENTS - extractive PDF cue:** Data In-Domain Out-of-Domain Internet Only 59.2 69.1 Task-specific Only 67.5 67.1 Mixed 71.0 71.7 Table 6: Interleaved instructions contribute through both format and content.
...
- **p. 22 / Figure/Table caption - extractive PDF cue:** Figure 9: Interleave-VLA Inference time w.r.t number of images. When number of images is 1 - 2, it is typically the cost of Text-VLA model. ...
- **p. 6 / 4 EXPERIMENTS - extractive PDF cue:** (2) What are the common failure modes of Text-VLA, and how does Interleave-VLA address them?
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** For quantitative breakdown of failure modes, please refer to Figure 11.
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** These failures can be attributed to semantic ambiguity in cluttered visual contexts and distributional bias in the training data.
- **p. 9 / 4 EXPERIMENTS - extractive PDF cue:** For more styles of sketches and potential failure modes, please refer to Table 8 and 9 in Apppendix C.
- **p. 19 / Figure/Table caption - extractive PDF cue:** Table 9: Example sketches for each style. Yellow-cube and green-cube sketches representing each drawing style. The examples are sourced from multiple individuals and are designed ...
