...
- **p. 3 / 3 METHOD - extractive PDF cue:** First, we introduce the mean velocity policy (MVP), showing how its integration with a "generateand-select" mechanism enables a direct mapping from noise to optimal actions.
- **p. 5 / 3 METHOD - extractive PDF cue:** Inspired by this, we introduce the instantaneous velocity constraint (IVC), a training objective that explicitly enforces a boundary condition at t.
- **p. 4 / 3 METHOD - extractive PDF cue:** The resulting action, a⋆, then serves three purposes: (1) interacting with the environment, (2) acting as the target action for policy training, and (3) calculating ...
- **p. 6 / 3 METHOD - extractive PDF cue:** The policy training loss Lpolicy combines the mean velocity model loss in Eq.
- **p. 4 / 3 METHOD - extractive PDF cue:** In practice, at any given state s, the agent first generate N diverse candidate actions as ai = ai k(1) = ϵi + uθ(ϵi, 0, ...
- **p. 3 / 3 METHOD - extractive PDF cue:** In RL, a policy π(·/s) defines a distribution over actions given a state s.
- **p. 6 / 3 METHOD - extractive PDF cue:** (20) end for ▷Phase 2: Online Interaction and Fine-tuning for online training step k = 1, 2, . . . do Observe sk, execute a∗ ...
- **p. 4 / 3 METHOD - extractive PDF cue:** Let θ denote the learnable parameters, the training objective is to minimize the residual of the mean flow identity in Eq.

## Design Rationale

- **p. 2 / 1 INTRODUCTION - extractive PDF cue:** Our contributions are summarized threefold: • We propose a new flow-based policy, namely mean velocity policy (MVP), that enables fastest one-step action generation.
- **p. 3 / 3 METHOD - extractive PDF cue:** First, we introduce the mean velocity policy (MVP), showing how its integration with a "generateand-select" mechanism enables a direct mapping from noise to optimal actions.
- **p. 1 / 1 INTRODUCTION - extractive PDF cue:** In this paper, we propose the mean velocity policy (MVP) as an affirmative answer.
...
- **p. 4 / 3 METHOD - extractive PDF cue:** Let θ denote the learnable parameters, the training objective is to minimize the residual of the mean flow identity in Eq.
- **p. 5 / 3 METHOD - extractive PDF cue:** Because the LMF loss is blind to the boundary, it cannot provide a gradient to force C(a, r) to zero.
- **p. 5 / 3 METHOD - extractive PDF cue:** Minimizing the IVC loss, LIVC = E[∥∆v∥2], forces the integration constant C(a, r) in Theorem 2 to zero.
- **p. 6 / 3 METHOD - extractive PDF cue:** (19) Update critic Qϕ by minimizing LQ(ϕ) with Eq.
- **p. 6 / 3 METHOD - extractive PDF cue:** To keep the IVC loss finite, the optimization must prevent this divergence, which requires C(a, r) = 0.
- **p. 3 / 3 METHOD - extractive PDF cue:** We then present the instantaneous velocity constraint (IVC) and theoretically justify its role in improving the learning accuracy.
...
- **p. 3 / 3 METHOD - extractive PDF cue:** For standard flow-based policies, this mapping is framed as a generative process: a velocity model, v(a(t), t, s), transforms a standard Gaussian noise (source) into ...
- **p. 2 / 2 PRELIMINARIES - extractive PDF cue:** (1) Grounded in the off-policy learning paradigm, our approach utilizes an action-value function (Qfunction) to guide policy improvement, which denotes the expected cumulative return for ...
- **p. 3 / 3 METHOD - extractive PDF cue:** In RL, a policy π(·/s) defines a distribution over actions given a state s.
- **p. 4 / 3 METHOD - extractive PDF cue:** (11) Then the critic function Qϕ parameterized by ϕ is employed to evaluate all candidates, and the action yielding the highest Q-value is identified as ...
- **p. 2 / 1 INTRODUCTION - extractive PDF cue:** Our contributions are summarized threefold: • We propose a new flow-based policy, namely mean velocity policy (MVP), that enables fastest one-step action generation.
- **p. 4 / 3 METHOD - extractive PDF cue:** The resulting action, a⋆, then serves three purposes: (1) interacting with the environment, (2) acting as the target action for policy training, and (3) calculating ...
- **p. 6 / 3 METHOD - extractive PDF cue:** Algorithm 1 Mean Flow RL Input: mean velocity policy πθ, where θ is the parameters of uθ, Critic Qϕ, offline dataset Doffline Initialize replay buffer ...
...
- **p. 6 / 3 METHOD - extractive PDF cue:** The policy training loss Lpolicy combines the mean velocity model loss in Eq.
- **p. 6 / 3 METHOD - extractive PDF cue:** (20) end for ▷Phase 2: Online Interaction and Fine-tuning for online training step k = 1, 2, . . . do Observe sk, execute a∗ ...
- **p. 8 / 4 EXPERIMENTS - extractive PDF cue:** (3) Training and inference time analysis.
- **p. 9 / 4 EXPERIMENTS - extractive PDF cue:** The results are listed in Table 3. our MVP and FQL exhibit very similar inference times, with both approaches being significantly faster than BFN and ...
...
- **p. 8 / 4 EXPERIMENTS - extractive PDF cue:** Our full version (λ = 1.0) was compared against variants with a reduced constraint (λ = 0.5) and without the constraint (λ = 0.0).
...
- **p. 9 / 4 EXPERIMENTS - extractive PDF cue:** To simulate a more realistic deployment scenario without hardware acceleration, we disabled JAX's Just-In-Time (JIT) compilation during all evaluations.
- **p. 18 / Figure/Table caption - extractive PDF cue:** Table 4: Ablation on the impact of IVC. Task MVP (λ = 0.0) MVP (λ = 0.5) MVP (λ = 1.0) Cube-triple-task3 0.65 ± 0.05 ...
...
- **PDF anchors reviewed:** method p. 3 (3 METHOD), p. 5 (3 METHOD), p. 4 (3 METHOD), p. 6 (3 METHOD), p. 4 (3 METHOD), p. 3 (3 METHOD), objective p. 4 (3 METHOD), p. 5 (3 METHOD), p. 5 (3 METHOD), p. 6 (3 METHOD), p. 6 (3 METHOD), p. 3 (3 METHOD), temporal p. 6 (3 METHOD), p. 7 (4 EXPERIMENTS), p. 7 (4 EXPERIMENTS), p. 8 (4 EXPERIMENTS), p. 8 (4 EXPERIMENTS), p. 1 (1 INTRODUCTION).
