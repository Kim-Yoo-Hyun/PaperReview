...
- **p. 1 / 1 INTRODUCTION - extractive body cue:** This trend has also begun to influence robotics, where scaling transformer-based architectures and training them on large, heterogeneous robot datasets have produced "robot foundation models" ...
- **p. 1 / ABSTRACT - extractive body cue:** To mitigate this, we introduce an embodiment-based grouping strategy in which robots are clustered by morphological similarity and the model is updated with a group ...
- **p. 2 / 1 INTRODUCTION - extractive body cue:** To mitigate these conflicts, we propose a simple, static grouping strategy that represents each robot as a morphology graph and clusters robots by graph-based distances; ...
...
- **p. 2 / 1 INTRODUCTION - extractive body cue:** 2 RELATED WORKS 2.1 OFFLINE RL Offline RL aims to learn a policy that maximizes cumulative reward using only a static dataset of environment interactions ...
...
- **p. 2 / 1 INTRODUCTION - extractive body cue:** 2 RELATED WORKS 2.1 OFFLINE RL Offline RL aims to learn a policy that maximizes cumulative reward using only a static dataset of environment interactions ...
- **p. 3 / 1 INTRODUCTION - extractive body cue:** Unlike the standard multi-task RL setting, where a single robot embodiment solves multiple tasks with different rewards or goals, here multiple robot embodiments solve a ...
- **p. 5 / 1 INTRODUCTION - extractive body cue:** These findings indicate that effective crossembodiment learning requires methods that minimize negative transfer while maximizing positive transfer.
...
- **p. 3 / 1 INTRODUCTION - extractive body cue:** 3 EXPERIMENTAL SETUP 3.1 PROBLEM SETTING We study multi-embodiment offline RL, where a single policy must control multiple robot morphologies under a common state-action interface.
- **p. 2 / 1 INTRODUCTION - extractive body cue:** Finally, the policy πϕ(a / s) is extracted via advantage-weighted BC, avoiding any need to evaluate out-of-distribution actions.
- **p. 2 / 1 INTRODUCTION - extractive body cue:** 2 RELATED WORKS 2.1 OFFLINE RL Offline RL aims to learn a policy that maximizes cumulative reward using only a static dataset of environment interactions ...
- **p. 3 / 1 INTRODUCTION - extractive body cue:** In this way, the same policy πθ(a / s, f morph(τ)) can generalize across embodiments by conditioning on these universal state features.
- **p. 4 / 1 INTRODUCTION - extractive body cue:** The central challenge is to train a single network across robots whose state and action dimensions differ.
- **p. 4 / 1 INTRODUCTION - extractive body cue:** To facilitate offline RL, we further extend URMA by introducing a state-action value function (Q-function).
- **p. 6 / 1 INTRODUCTION - extractive body cue:** (1) in which Q(s, a) denotes the learned state-action value function and V (s) denotes the learned state value function.
...
- **p. 5 / 1 INTRODUCTION - extractive body cue:** We pre-train via offline RL on a dataset excluding one robot, then finetune that robot with pre-trained networks, comparing it to a model trained without ...
- **p. 1 / 1 INTRODUCTION - extractive body cue:** This trend has also begun to influence robotics, where scaling transformer-based architectures and training them on large, heterogeneous robot datasets have produced "robot foundation models" ...
- **p. 10 / 1 INTRODUCTION - extractive body cue:** 5, the wall-clock training time grows substantially with M.
...
- **p. 10 / 1 INTRODUCTION - extractive body cue:** (ii) Sensitivity to the group count M We evaluate the effect of the number of Embodiment Grouping clusters M by sweeping M over {1, 2, ...
...
- **p. 10 / 1 INTRODUCTION - extractive body cue:** To remove this effect, we also run a compute-normalized comparison in which, for the IQL baseline, we multiply the total number of optimizer steps K ...
- **p. 8 / 1 INTRODUCTION - extractive body cue:** Finally, to assess the effect of our embodiment-based grouping strategy across different learning backbones, we report Embodiment Grouping (EG) counterparts of BC, TD3+BC, and IQL.
- **p. 8 / 1 INTRODUCTION - extractive body cue:** We denote these variants as BC+EG, TD3+BC+EG, and IQL+EG (ours), respectively.
- **p. 9 / 1 INTRODUCTION - extractive body cue:** The variant that combines IQL with Embodiment Grouping achieves the best average performance.
- **p. 9 / 1 INTRODUCTION - extractive body cue:** 6.2 ABLATIONS & COMPUTE-NORMALIZED ANALYSIS This section disentangles design choices in Embodiment Grouping (EG) and the impact of compute.
...
- **Evidence anchors reviewed:** method p. 4 (1 INTRODUCTION), p. 2 (1 INTRODUCTION), p. 5 (1 INTRODUCTION), p. 1 (1 INTRODUCTION), p. 1 (ABSTRACT), p. 2 (1 INTRODUCTION), objective p. 2 (1 INTRODUCTION), p. 3 (1 INTRODUCTION), p. 5 (1 INTRODUCTION), p. 8 (1 INTRODUCTION), p. 1 (ABSTRACT), p. 2 (1 INTRODUCTION), temporal p. 3 (1 INTRODUCTION), p. 5 (1 INTRODUCTION), p. 2 (1 INTRODUCTION), p. 3 (1 INTRODUCTION), p. 4 (1 INTRODUCTION), p. 8 (1 INTRODUCTION).
