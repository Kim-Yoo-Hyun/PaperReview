...
| Observation / input | Based on these observations, we propose a general recipe for effective batch online RL: train an expressive IL policy as the actor, ... | multi-view observation, language/task label과 action trajectory | exact sensor/frame/preprocessing from PDF body |
...
robot_data writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. Evidence interface anchors: p. 2 (1 Introduction), p. 6 (3 Preliminaries), p. 8 (3 Preliminaries), p. 3 (3 Preliminaries). The downstream handoff is claimed only when the body describes it.
