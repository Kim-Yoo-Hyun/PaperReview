...
- **p. 7 / 3 Preliminaries - extractive body cue:** For Gaussian policies, since the action distribution is less expressive, we use explicit policy extraction.
- **p. 8 / 3 Preliminaries - extractive body cue:** This instantiation recovers the IDQL algorithm [26] for one iteration of batch online RL, though the recipe defines a category of methods and can be ...
- **p. 4 / 3 Preliminaries - extractive body cue:** The policy can then be recovered from the Q-function and value function via a policy extraction step.
- **p. 3 / 3 Preliminaries - extractive body cue:** As in traditional RL, the objective is to find a policy π that maximizes the expected sum of discounted rewards Eτ∼pπ(τ)[P t γtr(st, at)] where ...
...
- **p. 3 / 3 Preliminaries - extractive body cue:** As in traditional RL, the objective is to find a policy π that maximizes the expected sum of discounted rewards Eτ∼pπ(τ)[P t γtr(st, at)] where ...
- **p. 4 / 3 Preliminaries - extractive body cue:** To do so, it aims to minimize the following objectives for learning a parameterized Q-function Qϕ (with target Q-function Qϕ′) and value function Vψ: LQ(ϕ) ...
- **p. 6 / 3 Preliminaries - extractive body cue:** This is done through maximizing a RL objective and an IL objective offline to learn the policy.
- **p. 7 / 3 Preliminaries - extractive body cue:** Both use supervised IL objectives and the value function objectives from IQL.
- **p. 4 / 3 Preliminaries - extractive body cue:** Imitation learning is often formulated as behavior cloning, which uses supervised learning to learn a policy πθ parameterized by θ to maximize the log-likelihood of ...
- **p. 6 / 3 Preliminaries - extractive body cue:** In contrast to a separate policy extraction step that explicitly extracts a policy to maximize Q-values, an alternative for policy extraction is to optimize for ...
...
- **p. 8 / 3 Preliminaries - extractive body cue:** We use RGB images and robot proprioceptive state (joint and end-effector positions) as input, and use a ResNet-18 [32] as the vision backbone.
- **p. 3 / 3 Preliminaries - extractive body cue:** S denotes the state space and A denotes the action space. r(s, a) is the reward function mapping from state and action pairs to rewards, ...
- **p. 4 / 3 Preliminaries - extractive body cue:** The policy can then be recovered from the Q-function and value function via a policy extraction step.
- **p. 4 / 3 Preliminaries - extractive body cue:** As in IQL, we consider Advantage-Weighted Regression (AWR) [16] as a canonical example of a policy extraction method.
- **p. 5 / 3 Preliminaries - extractive body cue:** We examine policy extraction choices individually in more detail in Section 4.2 and policy expressivity in Section 4.3.
...
- **p. 7 / 3 Preliminaries - extractive body cue:** To control for the advantages of the implicit policy extraction method in batch online RL that we observed in Section 4.2, we additionally run a ...
...
- **p. 6 / 3 Preliminaries - extractive body cue:** We separate policy extraction into two distinct categories, explicit policy extraction and implicit policy extraction, to analyze the effect of extraction method on performance.
...
- **p. 8 / Figure/Table caption - extractive body cue:** Figure 8: Normalized re- turns of value-based RL with and without temporally corre- lated noise at different data scales, averaged over all tasks. Improving diversity ...
- **p. 9 / 6 Discussion - extractive body cue:** Batch online RL provides a paradigm for just that-enabling policies to leverage their own rollouts for self-improvement without the complications of online RL.
- **p. 9 / 6 Discussion - extractive body cue:** For researchers, we bring to attention open questions for future work to optimize each component of the recipe further.
