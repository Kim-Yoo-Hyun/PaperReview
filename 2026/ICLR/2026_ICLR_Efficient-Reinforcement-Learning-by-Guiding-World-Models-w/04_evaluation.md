...
- **p. 8 / 4 EXPERIMENTS - extractive body cue:** I show comparison results on 22 locomotion and 50 robotic manipulation tasks with pixel inputs from DMControl and Meta-World benchmarks.
- **p. 7 / 4 EXPERIMENTS - extractive body cue:** Dataset Our dataset consists of data from two benchmarks: DMControl and Meta-World, visualized in Sec.
- **p. 7 / 4 EXPERIMENTS - extractive body cue:** As the unsupervised RL agents are trained to maximize the agent's curiosity rather than a specific reward signal, the dataset for DMControl does not contain ...
- **p. 8 / 4 EXPERIMENTS - extractive body cue:** We train an RL agent to control an Ant robot from DMControl to complete a series of tasks incrementally.
- **p. 9 / 4 EXPERIMENTS - extractive body cue:** NCRL demonstrates the effectiveness of using execution guidance over uncertainty-based reward labeling on challenging robotic manipulation tasks.
- **p. 9 / 4 EXPERIMENTS - extractive body cue:** Setup & Baselines We set our continual adaptation experiment based on the Quadruped robot from DMControl.
- **p. 10 / 4 EXPERIMENTS - extractive body cue:** We use the Quadruped Walk task as a representative task for the investigation.
- **p. 10 / 4 EXPERIMENTS - extractive body cue:** Fine-tuning the full world model yields the best performance on the tested task.
...
- **p. 2 / Figure/Table caption - extractive body cue:** Figure 1: Overview of NCRL (Non-curated offline data for efficient RL). NCRL leverages non- curated offline data-reward-free, mixed-quality, and multi-embodiment-to enable efficient RL. It uses ...
- **p. 3 / Figure/Table caption - extractive body cue:** Table 1: Comparison with different policy learning methods that leverage offline data. Offline RL Off2On RL RLPD MT Offline RL NCRL (ours) Reward-free offline data ...
- **p. 5 / Figure/Table caption - extractive body cue:** Figure 2: Visualization of Distribution Mismatch. Left: At the early stage of fine-tuning, there is a distribution shift between offline data used for world model ...
- **p. 6 / Figure/Table caption - extractive body cue:** Figure 3: Left: Quantitative comparison across 72 diverse tasks from Meta-World (Yu et al., 2020) and DMControl (Tassa et al., 2018) with the same sample ...
- **p. 8 / Figure/Table caption - extractive body cue:** Figure 4: Comparison with other world model pre-training methods. NCRL outperforms state- of-the-art model-based methods without relying on techniques used in iVideoGPT, such as reward ...
- **p. 8 / Figure/Table caption - extractive body cue:** Figure 5: NCRL enables fast task adaptation. We train an RL agent to control an Ant robot from DMControl to complete a series of tasks ...
- **p. 9 / Figure/Table caption - extractive body cue:** Figure 6: Ablation study on key components. "P" represents world model pre-training, "ER" means experience rehearsal, and "G" represents execution guidance. The combination of a ...
- **p. 9 / Figure/Table caption - extractive body cue:** Figure 7: Comparison of execution guidance versus uncertainty-based reward labeling. NCRL demonstrates the effectiveness of using execution guidance over uncertainty-based reward label- ing on challenging ...
...
| Output/decision | shielded, recovery 또는 safe action | action frame, controller and termination | p. 2 (3. Train), p. 1 (1 INTRODUCTION) |
...
- **p. 7 / 4 EXPERIMENTS - extractive body cue:** As the unsupervised RL agents are trained to maximize the agent's curiosity rather than a specific reward signal, the dataset for DMControl does not contain ...
- **p. 7 / 4 EXPERIMENTS - extractive body cue:** For example, on Quadruped Walk, NCRL benefits from exploratory offline data, enabling pixel-based control within just 100 trials.
