...
- **p. 1 / ABSTRACT - extractive PDF cue:** Vision-language models (VLMs) are essential to Embodied AI, enabling robots to perceive, reason, and act in complex environments.
- **p. 1 / ABSTRACT - extractive PDF cue:** They also serve as the foundation for the recent Vision-Language-Action (VLA) models.
- **p. 1 / ABSTRACT - extractive PDF cue:** Yet most evaluations of VLMs focus on single-view settings, leaving their ability to integrate multi-view information underexplored.
- **p. 1 / ABSTRACT - extractive PDF cue:** At the same time, multi-camera setups are increasingly standard in robotic platforms, as they provide complementary perspectives to mitigate occlusion and depth ambiguity.
- **p. 1 / ABSTRACT - extractive PDF cue:** Whether VLMs can effectively leverage such multi-view inputs for robotic reasoning therefore remains an open question.
- **p. 10 / 1 INTRODUCTION - extractive PDF cue:** By isolating failure modes in multi-view grounding rather than in isolated perception, MVRoboBench exposes the precise bottlenecks that future embodied AI systems must overcome.
- **p. 2 / 1 INTRODUCTION - extractive PDF cue:** Singleview inputs are inherently limited by challenges like occlusion, depth ambiguity, and restricted fields of view.
...
| Observation / input | Top: original inputs from left gripper, head, and right gripper cameras; Bottom: blurry synthesized view from interpolated extrinsics. "text": "Image context: Corresponding ... | standardized observation, action, task state와 evaluation split | exact sensor/frame/preprocessing from PDF |
...
- **p. 2 / 1 INTRODUCTION - extractive PDF cue:** Singleview inputs are inherently limited by challenges like occlusion, depth ambiguity, and restricted fields of view.
- **p. 10 / 1 INTRODUCTION - extractive PDF cue:** To address these challenges, specialized approaches (Cheng et al., 2024; Ma et al., 2025; Zhou et al., 2025; Fan et al., 2025; Liu et al., ...
- **p. 2 / 1 INTRODUCTION - extractive PDF cue:** Multi-view observations, by contrast, offer complementary perspectives that help overcome these limitations.
- **p. 3 / 1 INTRODUCTION - extractive PDF cue:** These findings highlight the unique challenges of multi-view reasoning in robotics and the need for specialized benchmarks like MV-RoboBench.

## What the Paper Changes

PDF contribution framing (p. 2 (1 INTRODUCTION), p. 3 (1 INTRODUCTION), p. 2 (1 INTRODUCTION), p. 1 (1 INTRODUCTION), p. 23 (C.4 STRUCTURAL AUGMENTATION VIA DEPTH PRIORS)): To fill this gap, we introduce MV-RoboBench, a benchmark specifically designed to evaluate multiview spatial reasoning in robotic manipulation scenarios.
...
- **p. 1 / 1 INTRODUCTION - extractive PDF cue:** Vision-language models (VLMs) (OpenAI, 2024; Team et al., 2023; Anthropic, 2024; Zhu et al., 2025; Bai et al., 2025; Liu et al., 2023b) play a ...
- **p. 23 / C.4 STRUCTURAL AUGMENTATION VIA DEPTH PRIORS - extractive PDF cue:** This design enables rapid prototyping of spatial arrangements and provides a consistent interface for generating QA items that require reasoning about relative positions and geometric ...
