...
PDF body evaluation/result cue (p. 14 (A.1 NETWORK ARCHITECTURES), p. 8 (4 EXPERIMENTS), p. 7 (4 EXPERIMENTS), p. 9 (4 EXPERIMENTS), p. 9 (4 EXPERIMENTS), p. 7 (4 EXPERIMENTS)): The efficacy of these features is demonstrated on the ScanNet benchmark, where they achieve 72.5% mIoU with linear probing, substantially outperforming 2D-lifted features from models like DINOv2 (63.1% mIoU).

## Evaluation Body Digest

- **p. 15 / A.2 DATASET DETAILS AND SUBSET SELECTION - extractive body cue:** For all experiments, we adhere to the official training, validation, and testing splits for the ScanNetV2 and Matterport3D datasets to ensure fair comparison with prior ...
- **p. 15 / A.2 DATASET DETAILS AND SUBSET SELECTION - extractive body cue:** To align with the categorical diversity of each dataset, the number of clusters K is set based on the variety of real-world spaces documented in ...
- **p. 7 / 4 EXPERIMENTS - extractive body cue:** As shown in Table 2, GeoPurify establishes a new stateof-the-art on long-tail benchmarks like ScanNet200 and the challenging M160 split.
- **p. 6 / 4 EXPERIMENTS - extractive body cue:** This strategy yields a training set that is not merely small but is a curated distillation of the dataset's core semantic diversity.
- **p. 5 / 4 EXPERIMENTS - extractive body cue:** We evaluate our method on three prominent 3D indoor scene understanding benchmarks.
- **p. 6 / 4 EXPERIMENTS - extractive body cue:** We posit that the most informative scenes are not simply those with many object types, but those where these objects appear in balanced proportions.
- **p. 8 / 4 EXPERIMENTS - extractive body cue:** Method ScanNet200 Matterport40 Matterport80 Matterport160 f-mIoU f-mAcc mIoU mAcc mIoU mAcc mIoU mAcc PLA (Ding et al., 2023) 1.8 3.1 - - - - - ...
- **p. 14 / A.1 NETWORK ARCHITECTURES - extractive body cue:** We hypothesize that features learned via this multi-modal, multi-task objective provide richer semantic information for 3D scene understanding than those from networks trained solely on ...
...
| 4 EXPERIMENTS | EMPIRICAL / REAL-ROBOT OR HARDWARE | While applying GeoPurify to a standard LSeg backbone achieves a respectable 51.2 mIoU, replacing it with the generalist X-Decoder boosts performance to 55.1 mIoU. | p. 9 (4 EXPERIMENTS) |
...
- **p. 4 / Figure/Table caption - extractive body cue:** Figure 2: GeoPurify: A Data-Efficient Pipeline for Geometric Purification of 3D Semantic Features. Our method consists of two stages. 1) Training (left, dotted path): A ...
- **p. 7 / Figure/Table caption - extractive body cue:** Table 1: Quantitative results for open-vocabulary 3D semantic segmentation on ScanNetV2 and Matterport3D. We compare GeoPurify, trained with only ∼1.5% of the 3D data, against ...
- **p. 8 / Figure/Table caption - extractive body cue:** Table 2: Zero-shot segmentation performance on long-tail benchmarks. We report key metrics (f-mIoU, f-mAcc) on ScanNet200 and (mIoU, mAcc) on frequency-based splits of Matterport3D (top ...
- **p. 8 / Figure/Table caption - extractive body cue:** Table 3: Zero-shot cross-dataset evaluation. Models are trained on the source dataset and evalu- ated directly on the target without fine-tuning.
- **p. 9 / Figure/Table caption - extractive body cue:** Table 4: Ablation studies of GeoPurify on the ScanNetV2 validation set. We analyze the impact of our core geometric purification module, the choice of 2D ...
- **p. 14 / Figure/Table caption - extractive body cue:** Table 5: Student Network Architecture (ϕS). The network operates on sparse 3D tensors. The feature dimension is maintained at 512 throughout the body. Stage Operator ...
- **p. 16 / Figure/Table caption - extractive body cue:** Table 6: Final scene and region IDs constituting our compact training subsets for ScanNetV2 and Matterport3D. ScanNetV2 Selected Scenes Matterport3D Selected Regions scene0126 00 VzqfbhrpDEA ...

## Embodiment / Environment

| Dimension | PDF body-grounded cue | Unresolved condition | Anchor |
|---|---|---|---|
| Robot/hardware/simulator | For all experiments, we adhere to the official training, validation, and testing splits for the ScanNetV2 and Matterport3D datasets to ensure fair comparison with ... | embodiment, simulator version and control stack | p. 15 (A.2 DATASET DETAILS AND SUBSET SELECTION), p. 15 (A.2 DATASET DETAILS AND SUBSET SELECTION) |
| Task/environment | To align with the categorical diversity of each dataset, the number of clusters K is set based on the variety of real-world spaces documented ... | reset, timeout, object/scene variation | p. 15 (A.2 DATASET DETAILS AND SUBSET SELECTION), p. 7 (4 EXPERIMENTS) |
...
| As shown in Table 3, GeoPurify significantly outperforms existing methods in both transfer directions. | comparison identity and matched condition | p. 8 (4 EXPERIMENTS) |
...
- **p. 6 / 4 EXPERIMENTS - extractive body cue:** We sample a highly compact subset of original training data, comprising merely 20 scenes (∼1.6%) from ScanNetV2 and 20 scene regions (∼1.3%) from Matterport3D.
- **p. 6 / 4 EXPERIMENTS - extractive body cue:** Once this compact subset is selected, the subsequent distillation training is performed without using any 3D semantic labels, relying only on the raw point cloud ...
- **p. 6 / 4 EXPERIMENTS - extractive body cue:** The student network is trained for 50 epochs using the AdamW optimizer with a learning rate of 1 × 10-3, which is decayed using a ...
- **p. 8 / 4 EXPERIMENTS - extractive body cue:** The advantage is particularly pronounced when transferring from Matterport3D →ScanNetV2, where our method achieves 54.9 mIoU, surpassing the next-best competitor by a large margin of ...
- **p. 9 / 4 EXPERIMENTS - extractive body cue:** Component Setting mIoU mAcc Geometric Purification w/o Purification (Aggregated 2D features) 50.2 68.1 + GeoPurify (Ours) 55.1 72.5 2D Semantic Backbone LSeg 48.6 61.6 LSeg ...
- **p. 9 / 4 EXPERIMENTS - extractive body cue:** Training Subset Size We evaluate the method's sensitivity to data quantity by training on subsets of 10, 20, 30, and 50 scenes.
...
| This section provides the implementation details for our experiments to ensure reproducibility. | p. 14 (A EXPERIMENTAL SETUP AND IMPLEMENTATION DETAILS) |
...
- **Evidence anchors reviewed:** datasets p. 15 (A.2 DATASET DETAILS AND SUBSET SELECTION), p. 15 (A.2 DATASET DETAILS AND SUBSET SELECTION), p. 7 (4 EXPERIMENTS), p. 6 (4 EXPERIMENTS), p. 5 (4 EXPERIMENTS), p. 6 (4 EXPERIMENTS), metrics p. 6 (4 EXPERIMENTS), p. 21 (Figure/Table caption), p. 6 (4 EXPERIMENTS), p. 7 (4 EXPERIMENTS), p. 9 (4 EXPERIMENTS), p. 15 (A.2 DATASET DETAILS AND SUBSET SELECTION), baselines p. 8 (4 EXPERIMENTS), p. 20 (Figure/Table caption), p. 9 (4 EXPERIMENTS), p. 6 (4 EXPERIMENTS), p. 7 (4 EXPERIMENTS), p. 8 (4 EXPERIMENTS), results p. 14 (A.1 NETWORK ARCHITECTURES), p. 8 (4 EXPERIMENTS), p. 7 (4 EXPERIMENTS), p. 9 (4 EXPERIMENTS), p. 9 (4 EXPERIMENTS), p. 7 (4 EXPERIMENTS).
