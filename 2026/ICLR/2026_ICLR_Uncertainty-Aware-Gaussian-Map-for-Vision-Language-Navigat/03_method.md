...
- **p. 6 / 3 METHOD - extractive PDF cue:** Following the conventional procedure [11, 17, 30], our agent is optimized with a two-stage training scheme: pretraining with auxiliary objectives such as masked language modeling ...
- **p. 6 / 3 METHOD - extractive PDF cue:** To supervise SGM construction, we apply a pixel-wise rendering loss between the rendered outputs and ground-truth observations.
- **p. 3 / 3 METHOD - extractive PDF cue:** Based on these observations, the agent learns a navigation policy π(at/X, It, Dt) that predicts actions at ∈At, which includes navigable neighbor nodes, previously observed ...
- **p. 4 / 3 METHOD - extractive PDF cue:** Given multi-view RGB-D observations Ot = {It, Dt} at step t, the agent first generates a sparse pseudo-lidar point cloud via camera-to-world transformation.
- **p. 4 / 3 METHOD - extractive PDF cue:** For s, we apply SAM2 [68] to segment the panoramic observation I into spatially coherent regions {mk}K k=1 and extract their CLIP [69] embeddings, which ...
- **p. 5 / 3 METHOD - extractive PDF cue:** Published as a conference paper at ICLR 2026 Since log p(O) is constant with respect to χ, minimizing KL divergence is equivalent to maximizing ELBO, ...
- **p. 5 / 3 METHOD - extractive PDF cue:** Because computing this matrix directly is infeasible, like [63, 67], we adopt the Fisher Information as a tractable approximation: ∇2 GLr = ∇G ˆI ∇G ...
- **p. 7 / 3 METHOD - extractive PDF cue:** Once the 3D Value Map is established, action prediction incurs negligible additional cost compared to existing VLN agents [11].

## Design Rationale

- **p. 4 / 3 METHOD - extractive PDF cue:** To approximate it, like [66], we introduce variational distributions qϕ(χ) = {qϕµ i (χµ i ), qϕe i (χe i)}i and optimize them by minimizing ...
...
- **p. 6 / 3 METHOD - extractive PDF cue:** Following the conventional procedure [11, 17, 30], our agent is optimized with a two-stage training scheme: pretraining with auxiliary objectives such as masked language modeling ...
- **p. 7 / 3 METHOD - extractive PDF cue:** Once the 3D Value Map is established, action prediction incurs negligible additional cost compared to existing VLN agents [11].
- **p. 4 / 3 METHOD - extractive PDF cue:** Therefore, after several rounds of differentiable rendering optimization, we further refine SGM by retaining only Gaussians subject to the constraints ∥ei∥2 > τe ∧αi > ...
- **p. 4 / 3 METHOD - extractive PDF cue:** To approximate it, like [66], we introduce variational distributions qϕ(χ) = {qϕµ i (χµ i ), qϕe i (χe i)}i and optimize them by minimizing ...
- **p. 5 / 3 METHOD - extractive PDF cue:** Following the same variational inference framework, we learn a posterior qϕs(χs) by maximizing the corresponding ELBO, regularized by a zero-mean Gaussian prior p(χs) = N(0, ...
...
| Objective/constraint | Published, conference, ICLR, Since, constant, respect, minimizing, divergence, equivalent, maximizing | path cost, risk or goal utility | equation anchor required |

## Observation–State–Action Interface

- **p. 3 / 3 METHOD - extractive PDF cue:** Based on these observations, the agent learns a navigation policy π(at/X, It, Dt) that predicts actions at ∈At, which includes navigable neighbor nodes, previously observed ...
- **p. 6 / 3 METHOD - extractive PDF cue:** Following the conventional procedure [11, 17, 30], our agent is optimized with a two-stage training scheme: pretraining with auxiliary objectives such as masked language modeling ...
- **p. 7 / 3 METHOD - extractive PDF cue:** At each navigable viewpoint, our agent constructs a SGM from panoramic observations and extends it into a 3D Value Map for reliable action prediction.
- **p. 1 / 1 INTRODUCTION - extractive PDF cue:** Early agents adopted sequence-to-sequence frameworks [1, 10], directly mapping language and visual observations into actions.
- **p. 6 / 3 METHOD - extractive PDF cue:** To supervise SGM construction, we apply a pixel-wise rendering loss between the rendered outputs and ground-truth observations.
- **p. 3 / 3 METHOD - extractive PDF cue:** At each step t, the agent receives a panoramic observation composed of multiple RGB views It = {It,k ∈RH×W ×3}K k=1 and associated depth maps ...
- **p. 4 / 3 METHOD - extractive PDF cue:** Given multi-view RGB-D observations Ot = {It, Dt} at step t, the agent first generates a sparse pseudo-lidar point cloud via camera-to-world transformation.
...
- **p. 6 / 3 METHOD - extractive PDF cue:** Following the conventional procedure [11, 17, 30], our agent is optimized with a two-stage training scheme: pretraining with auxiliary objectives such as masked language modeling ...
- **p. 5 / 3 METHOD - extractive PDF cue:** Published as a conference paper at ICLR 2026 Since log p(O) is constant with respect to χ, minimizing KL divergence is equivalent to maximizing ELBO, ...
- **p. 6 / 3 METHOD - extractive PDF cue:** Pretraining is conducted for 100k iterations with a batch size of 64, optimized by Adam [78] with a learning rate of 1e-4.
- **p. 8 / 4 EXPERIMENT - extractive PDF cue:** These cases show how uncertainty helps disambiguate confounding structures and encode traversability constraints.
...
- **p. 9 / 4 EXPERIMENT - extractive PDF cue:** Components R2R [1] REVERIE [28] # SGM 3DVM SR ↑ SPL ↑ SR ↑ RGS ↑ RGSPL ↑ 1 - - 72.22 60.41 46.98 32.15 ...
- **p. 8 / 4 EXPERIMENT - extractive PDF cue:** (b) Our agent bypasses the obstacle and enters the designated region, while VER halts at the "table" without completing the task.
- **p. 9 / 4 EXPERIMENT - extractive PDF cue:** For row #2, the scores are obtained by using SGM as the 3D scene representation without uncertainty values.
- **p. 20 / Figure/Table caption - extractive PDF cue:** Table 10: Sensitivity Analysis of uncertainty-related hyperparameters on R2R val unseen split. (a) δ and (b) η regulate geometric uncertainty, while (c) ε governs semantic ...
...
- **PDF anchors reviewed:** method p. 6 (3 METHOD), p. 6 (3 METHOD), p. 3 (3 METHOD), p. 4 (3 METHOD), p. 4 (3 METHOD), p. 5 (3 METHOD), objective p. 5 (3 METHOD), p. 6 (3 METHOD), p. 7 (3 METHOD), p. 4 (3 METHOD), p. 4 (3 METHOD), p. 5 (3 METHOD), temporal p. 6 (3 METHOD), p. 1 (1 INTRODUCTION), p. 2 (2 RELATED WORK), p. 2 (2 RELATED WORK), p. 3 (3 METHOD), p. 3 (3 METHOD).
