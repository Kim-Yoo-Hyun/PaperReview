# Insights — Spatial Forcing: Implicit Spatial Representation Alignment for Vision-language-action Model
...
- **Method cue:** The final training objective combines both the standard training loss for action generation and the 3D foundation model alignment loss with a weighting factor α: LSF = Laction ...
...
- **Registry interface:** `VLA, Vision-Language Model, 3D Vision` is the paper's recorded topic/interface, not evidence that the full robotics loop was evaluated.
...
- Registry position: `VLA and Generalist Robot Policies`; tags: `VLA, Vision-Language Model, 3D Vision`.
...
- **Protocol carried forward from the legacy note (candidate, not a verified paper evaluation):**
  - 논문 내 evaluation 단서: LIBERO, RoboTwin / accuracy, SR, success rate
