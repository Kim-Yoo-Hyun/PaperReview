...
PDF body framing (p. 3 (X. Wang et al), p. 4 (X. Wang et al), p. 2 (X. Wang et al), p. 1 (Front matter), p. 2 (X. Wang et al)): The main contributions of this work are summarized as follows: - We identify policy efficiency as a critical yet overlooked deployment bottleneck for VLA models and distinguish it from pure ...
...
- **p. 4 / X. Wang et al - extractive PDF cue:** However, existing GRPO approaches for VLAs universally rely on binary success rewards [6, 14, 21, 28], which create two fundamental limitations.
...
- **p. 1 / Front matter - extractive PDF cue:** Vision-Language-Action (VLA) models provide a unified paradigm for robotic manipulation, yet their real-world deployment is often bottlenecked by execution efficiency.
- **p. 2 / X. Wang et al - extractive PDF cue:** Consequently, intrinsic policy efficiency remains the primary bottleneck for deployed VLA systems.
...
| Observation / input | At an arbitrary decision step t, the policy πθ processes the current visual observation ot and language instruction l to predict a ... | image/video, language instruction, proprioception과 history | exact sensor/frame/preprocessing from PDF |
...
|---|---|---|
| State / observation variable | multimodal context o,l,p/history; body terms: arbitrary, decision, step, policy, processes, current, visual, observation, language, instruction | p. 5 (3 Method), p. 4 (X. Wang et al), p. 1 (Front matter) |
| Decision / output variable | action, pose, option or chunk a; body terms: main, contributions, summarized, follows, identify, policy, efficiency, critical | p. 3 (X. Wang et al), p. 5 (3 Method), p. 1 (1 Introduction) |
...
PDF contribution framing (p. 3 (X. Wang et al), p. 5 (3 Method), p. 1 (1 Introduction), p. 1 (Front matter), p. 3 (X. Wang et al)): The main contributions of this work are summarized as follows: - We identify policy efficiency as a critical yet overlooked deployment bottleneck for VLA models and distinguish it from pure ...

- **p. 5 / 3 Method - extractive PDF cue:** We propose a two-stage posttraining framework that extends the executable action horizon per inference and reduces the number of steps required to complete a task ...
- **p. 1 / 1 Introduction - extractive PDF cue:** Vision-Language-Action (VLA) models integrate visual perception, language understanding, and action generation into a single end-to-end framework, establishing a scalable paradigm for general-purpose robotic manipulation [2-4,10-12,19, ...
- **p. 1 / Front matter - extractive PDF cue:** Ultimately, our framework delivers up to a 5.83× end-to-end deployment speedup without compromising task success rates.
- **p. 3 / X. Wang et al - extractive PDF cue:** PolicyTrim 3 In this paper, we propose PolicyTrim, a two-stage RL-based post-training framework that enhances the policy efficiency of VLA models through reliable chunk extension ...
...
vla writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 5 (3 Method), p. 4 (X. Wang et al), p. 1 (Front matter), p. 15 (2.48 Method). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 3 (X. Wang et al), p. 4 (X. Wang et al), p. 2 (X. Wang et al), p. 1 (Front matter), p. 2 (X. Wang et al), interface p. 5 (3 Method), p. 4 (X. Wang et al), p. 1 (Front matter), p. 15 (2.48 Method), objective p. 5 (3 Method), p. 21 (B Implementation Details).
