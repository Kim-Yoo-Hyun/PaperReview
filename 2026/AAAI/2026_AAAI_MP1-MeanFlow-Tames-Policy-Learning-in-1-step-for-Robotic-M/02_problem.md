...
| Observation / input | MP1: One-Step Trajectory Generation In the context of robot learning, the policy's task is to map a sequence of observations, including 3D ... | observation history와 expert trajectory/action | exact sensor/frame/preprocessing from PDF |
...
PDF contribution framing (p. 2 (Abstract), p. 1 (Abstract), p. 1 (Abstract), p. 2 (Abstract), p. 3 (Abstract)): Our contributions are as follows: • We introduce MP1, the first MeanFlow-based robot learning framework.
...
il writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 3 (Abstract), p. 3 (Abstract), p. 4 (Abstract), p. 4 (Abstract). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (Abstract), p. 2 (Abstract), p. 1 (Abstract), p. 1 (Abstract), p. 3 (Abstract), interface p. 3 (Abstract), p. 3 (Abstract), p. 4 (Abstract), p. 4 (Abstract), objective p. 1 (Abstract), p. 4 (Abstract), p. 4 (Abstract), p. 1 (Abstract), p. 2 (Abstract), p. 2 (Abstract).
