# Compact Object-Level Representations with Open-Vocabulary Understanding for Indoor Visual Relocalization

- Year/Venue: 2026 / RA-L
- Category: 3D Vision-Language Understanding
- Tags: semantic
- Paper link: ./2026/RA-L/2026_RA-L_Compact-Object-Level-Representations-with-Open-Vocabulary/paper.pdf
- Code/Project: not identified
- Source audit: regenerated from local `paper.pdf` on 2026-07-02; survey-keyword template text removed.

## Problem
- Building fail to generalize to long-tail categories and complex scenarios. on this design, a DIOU-based (Distance-IOU) retrieval strategy Recent progress in 2D vision-language foundation models, is also deriv

## Core Idea
- To this end, we propose OpenReLoc, a camera relocalization system designed to provide scene understanding and accurate pose estimation capabilities.
- Compared to PixLoc, our method This factor explains GoReloc’s poor success rate.

## Input / Output
- 본문 기반 자동 추출에서는 입력/출력 schema를 확정하지 않는다. 위 method/evaluation 단서와 `paper.pdf`의 method section을 함께 확인해야 한다.

## Main Claims
- Experimental results demonstrate that OpenReLoc achieves superior relocalization recall and accuracy across various datasets.
- 1: OpenReLoc, an open-vocabulary visual relocalization system, can achieve robust and accurate relocalization performance on various indoor scenes, based on an object-level map.
- As shown in the figure, in an extremely large multi-floor scene, the robot observes a tiny corner containing a small radio and long-tailed animal ornament, and our system ...

## Limitation
- 자동 추출 실패. `paper.pdf` 본문 수동 확인 필요.

## Contribution
- Experimental results demonstrate that OpenReLoc achieves superior relocalization recall and accuracy across various datasets.
- To this end, we propose OpenReLoc, a camera relocalization system designed to provide scene understanding and accurate pose estimation capabilities.
- 1: OpenReLoc, an open-vocabulary visual relocalization system, can achieve robust and accurate relocalization performance on various indoor scenes, based on an object-level map.

## Abstract Cue
- —Indoor visual relocalization plays a critical role in emerging spatial and embodied AI applications.
