...
- **p. 1 / Abstract - extractive PDF cue:** To teach robots complex manipulation tasks, a common approach is to fine-tune a pre-trained vision-languageaction model (VLA) on task-specific data.
- **p. 1 / Abstract - extractive PDF cue:** However, since this recipe updates existing representations, it is unsuitable for longterm operation in the real world, where robots must continually adapt to new tasks ...
- **p. 1 / Abstract - extractive PDF cue:** Existing continual learning methods for robotics commonly require storing previous data (exemplars), struggle with long task sequences, or rely on task identifiers for deployment.
- **p. 1 / Abstract - extractive PDF cue:** To address these limitations, we propose CLARE, a general, parameter-efficient framework for exemplar-free continual learning with VLAs.
- **p. 1 / Abstract - extractive PDF cue:** CLARE introduces lightweight modular adapters into selected VLA modules and autonomously expands the model only where necessary when learning a new task, guided by layer-wise ...
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** However, state-of-the-art VLAs still cannot adapt reliably to unseen tasks without fine-tuning on task-specific data [6]- [8].
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** This long-term adaptability, known as continual or lifelong learning [1], remains an open challenge in robotics despite decades of research [2]-[4].
...
| Observation / input | 23: Train Dn ℓof all layers ℓ∈E from Dn via (5). consisting of camera images I1 t , . . . , ... | image/video, language instruction, proprioception과 history | exact sensor/frame/preprocessing from PDF |
...
| Decision / output variable | action, pose, option or chunk a; body terms: architecture-agnostic, keep, following, sections, general, continual, learning, setting | p. 3 (IV. METHODOLOGY), p. 1 (I. INTRODUCTION), p. 5 (IV. METHODOLOGY) |
...
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** This long-term adaptability, known as continual or lifelong learning [1], remains an open challenge in robotics despite decades of research [2]-[4].
- **p. 3 / III. PROBLEM SETUP - extractive PDF cue:** Pre-training has provided the base VLA with general visual, language, and action representations, but it cannot solve new tasks zero-shot [6], [7].
- **p. 3 / III. PROBLEM SETUP - extractive PDF cue:** 17: else 18: Link Dn ℓto an existing adapter via (8).

## What the Paper Changes

PDF contribution framing (p. 3 (IV. METHODOLOGY), p. 1 (I. INTRODUCTION), p. 5 (IV. METHODOLOGY)): As our method is architecture-agnostic, we keep the following sections general.
...
vla writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 3 (III. PROBLEM SETUP), p. 2 (III. PROBLEM SETUP), p. 3 (III. PROBLEM SETUP), p. 2 (III. PROBLEM SETUP). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (I. INTRODUCTION), p. 1 (I. INTRODUCTION), p. 3 (III. PROBLEM SETUP), p. 3 (III. PROBLEM SETUP), interface p. 3 (III. PROBLEM SETUP), p. 2 (III. PROBLEM SETUP), p. 3 (III. PROBLEM SETUP), p. 2 (III. PROBLEM SETUP), objective p. 3 (IV. METHODOLOGY), p. 4 (IV. METHODOLOGY), p. 4 (IV. METHODOLOGY), p. 5 (IV. METHODOLOGY).
