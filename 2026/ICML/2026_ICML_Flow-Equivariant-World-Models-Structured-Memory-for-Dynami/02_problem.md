...
| Observation / input | A sequence-to-sequence model Φ defines a map f 7→h for outputs h = {ht}T t=0, ht : X′ →RK′, such as mapping ... | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF body |
...
| Decision / output variable | geometry/map/query r; body terms: support, more, complex, tasks, partially, observed, world, modeling | p. 3 (3.1. Generalized Flow Equivariance), p. 4 (3.1. Generalized Flow Equivariance), p. 4 (3.1. Generalized Flow Equivariance) |
...
PDF body contribution framing (p. 3 (3.1. Generalized Flow Equivariance), p. 4 (3.1. Generalized Flow Equivariance), p. 4 (3.1. Generalized Flow Equivariance), p. 5 (3.1. Generalized Flow Equivariance), p. 2 (1. Introduction)): To support more complex tasks, such as 3D partially observed world modeling, we introduce an abstract version of the flow equivariant recurrence relation which supports arbitrary encoders and update operations.
