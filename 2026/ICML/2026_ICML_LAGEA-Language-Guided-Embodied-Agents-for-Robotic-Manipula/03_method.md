...
> Analysis basis: full-text PDF body checked on 2026-09-03 (23 pages; PyMuPDF text; extraction quality: high); canonical paper source: https://openreview.net/forum?id=watVfFbZGF; PDF retrieval source: https://openreview.net/pdf/28f8573440fbd9bb2ac48d0e31f3573d128fcf46.pdf. The note is an evidence-anchored PDF body analysis; exact tables/equations remain at the cited page anchors. Evidence boundary: selected PDF body sentences, captions and section anchors were used; exact table/equation values remain at those anchors. Reading tracker status remains user-controlled; registry source evidence is reconciled separately.
...
- **p. 5 / 3.2. Reward Generation - extractive body cue:** (a) A Goal Potential ϕt is formed by aligning the current state zt with the goal image zg and instruction zy.
- **p. 5 / 3.3. Dynamic Reward Shaping - extractive body cue:** We apply shaping only on failures using the mask mt = 1[ rtask t < 0 ], and we down-weight shaping as the policy improves.
- **p. 4 / 3.2. Reward Generation - extractive body cue:** With the shared space in place, we convert progress toward the task and movement toward the feedback into dense, directional rewards.
- **p. 4 / 3.2. Reward Generation - extractive body cue:** In parallel, we reward movement toward the feedback direction and concentrate credit to causal moments via the key-frame weights ˆwt.
...
- **p. 4 / 3.2. Reward Generation - extractive body cue:** With the shared space in place, we convert progress toward the task and movement toward the feedback into dense, directional rewards.
- **p. 4 / 3.2. Reward Generation - extractive body cue:** In parallel, we reward movement toward the feedback direction and concentrate credit to causal moments via the key-frame weights ˆwt.
- **p. 5 / 3.2. Reward Generation - extractive body cue:** The computation of our delta-based rewards.
- **p. 5 / 3.2. Reward Generation - extractive body cue:** The temporal difference of these potentials creates the fused feedback-VLM rewards.
- **p. 3 / 3.1.1. STRUCTURED FEEDBACK - extractive body cue:** The model is required to return only a schema-constrained JSON.
- **p. 3 / 3.1.2. KEY FRAME GENERATION - extractive body cue:** We therefore identify a small set of key frames and diffuse their influence locally in time, so learning focuses on causal moments (approach, contact, reversal).
...
- **p. 4 / 3.2. Reward Generation - extractive body cue:** We project images, instruction text, and feedback with Ei, Et, Ef and use unit-norm embeddings for the current state zt, the goal image zg, the ...
- **p. 4 / 3.1.3. FEEDBACK ALIGNMENT - extractive body cue:** Key-frame weights ˆwt identify when gradients should matter; the remaining step is to make the episodic feedback f actionable by aligning it with visual states ...
- **p. 5 / 3.2. Reward Generation - extractive body cue:** (a) A Goal Potential ϕt is formed by aligning the current state zt with the goal image zg and instruction zy.
- **p. 1 / 1. Introduction - extractive body cue:** Prior self-reflection paradigms (Shinn et al., 2023) show that textual self-critique can improve decision making, but these demonstrations largely live in text-only environments such as ...
...
- **p. 2 / 1. Introduction - extractive body cue:** As smaller VLMs can hallucinate or drift in free-form text (Guan et al., 2024; Chen et al., 2024b), feedback is structured and aligned with goal ...
- **p. 1 / 1. Introduction - extractive body cue:** To reduce engineering overhead, a pragmatic trend is to treat VLMs as zero-shot reward models (Rocamonde et al., 2023), scoring progress from natural-language goals and ...
...
- **p. 7 / 4.1.2. RESULTS ON FETCH TASKS - extractive body cue:** 0.0M 0.2M 0.4M 0.6M 0.8M 1.0M Environment Steps 400 200 0 200 400 VLM Reward VLM Reward Signal Over Training 0.0M 0.2M 0.4M 0.6M 0.8M ...
...
- **p. 6 / 4.1.2. RESULTS ON FETCH TASKS - extractive body cue:** To validate our design choices and disentangle the individual contributions of our core components, we conduct a series of comprehensive ablation studies.
- **p. 7 / 4.3.1. SYNERGY OF DELTA REWARDS AND ADAPTIVE - extractive body cue:** SHAPING To isolate the contributions of our key reward components, we performed a targeted ablation study on both observable random goal and hidden fixed goal ...
- **p. 5 / 4. Experiments - extractive body cue:** We also include LIV (Ma et al., 2023), a robotics reward model pre-trained on large-scale datasets, and a variant, LIV-Proj, which utilizes randomly initialized and ...
- **p. 5 / 4. Experiments - extractive body cue:** We evaluate LAGEA on a suite of simulated embodied manipulation tasks, comparing against baseline RL agents and ablated LAGEA variants to measure the contributions of ...
- **p. 7 / 4.1.2. RESULTS ON FETCH TASKS - extractive body cue:** No, rgoal t No, rfb t No, LaGEA 79% 80% 80% 99% 19% (b) Reward shaping ablation.
