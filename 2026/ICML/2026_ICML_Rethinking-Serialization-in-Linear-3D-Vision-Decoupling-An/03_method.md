...
- **p. 5 / 3.1. Overview - extractive body cue:** Unlike standard DeltaNet (Yang et al., 2024) which maintains a matrix state S ∈RD×D to capture cross-dimensional interactions, we employ a dimension-wise vector state st ...
- **p. 5 / 3.1. Overview - extractive body cue:** The small-decay limit therefore serves only as intuition for this high-retention regime; it should not be read as strict order invariance of intermediate states or ...
- **p. 3 / 3.1. Overview - extractive body cue:** The objective of LAGM is to capture local anisotropy within k-NN neighborhoods, independent of any global serialization order.
- **p. 3 / 3.1. Overview - extractive body cue:** For a point cloud P = {pi}N i=1 with coordinates pi ∈R3, the local neighborhood of point pi is defined as Nk(i) = arg min ...
...
- **p. 3 / 3.1. Overview - extractive body cue:** The objective of LAGM is to capture local anisotropy within k-NN neighborhoods, independent of any global serialization order.
- **p. 3 / 3.1. Overview - extractive body cue:** For a point cloud P = {pi}N i=1 with coordinates pi ∈R3, the local neighborhood of point pi is defined as Nk(i) = arg min ...
- **p. 4 / 3.1. Overview - extractive body cue:** Right: gated state update mechanism with effective decay coefficient α and write gate β, enabling content-based retrieval through query projection.
- **p. 5 / 3.1. Overview - extractive body cue:** By contrast, GISA uses the content-based update in Eq.
- **p. 5 / 3.1. Overview - extractive body cue:** Although the GISA update has a recurrent form, it can be evaluated with a hardwarefriendly chunk-wise scan over Eq.
...
- **p. 5 / 3.1. Overview - extractive body cue:** In Mamba (Gu & Dao, 2024), the state evolves as: ht = ¯A · ht-1 + ¯Bt · xt, yt = Ct · ht, (9) ...
- **p. 5 / 3.1. Overview - extractive body cue:** Unlike standard DeltaNet (Yang et al., 2024) which maintains a matrix state S ∈RD×D to capture cross-dimensional interactions, we employ a dimension-wise vector state st ...
- **p. 3 / 3.1. Overview - extractive body cue:** Current 3D State-Space Models (SSMs) typically force point clouds into a single 1D sequence via space-filling curves, conflating local geometric modeling with global semantic aggregation.
- **p. 3 / 3.1. Overview - extractive body cue:** LAGM: Local Anisotropy Geometric Modeling Our framework is built on a simple observation: 3D point cloud understanding requires local anisotropy for geometry but global isotropy ...
- **p. 2 / 1. Introduction - extractive body cue:** Based on this observation, we propose AnIsoNet, which decouples local anisotropic geometry modeling from global semantic aggregation.
- **p. 4 / 3.1. Overview - extractive body cue:** The key point is that LAGM performs local anisotropic feature extraction before any global semantic aggregation.
- **p. 4 / 3.1. Overview - extractive body cue:** Right: gated state update mechanism with effective decay coefficient α and write gate β, enabling content-based retrieval through query projection.
...
- **p. 6 / 4.2. Main Results - extractive body cue:** Underline denotes second-best without pre-training.
- **p. 6 / 4.2. Main Results - extractive body cue:** Compared with PTv3, it is 1.0% higher on the validation split without external pre-training.
- **p. 7 / 4.3. Analysis and Ablation - extractive body cue:** Replacing the spherical local encoding with the ellipsoidal variant improves the baseline from 73.48% to 74.44% (+0.96%), while adding GISA alone yields a larger gain ...
...
- **p. 8 / 4.4. Efficiency Analysis - extractive body cue:** Its latency is not the lowest in the table-PointMamba is faster per forward pass-but AnIsoNet offers a stronger accuracy-resource trade-off, combining clearly better accuracy with ...
- **p. 12 / Figure/Table caption - extractive body cue:** Table 10. Cross-module ablation across benchmarks. ESE denotes the LAGM local encoder. S3DIS and ScanNetV2 report mIoU, while ScanObjectNN reports OA and mAcc.
...
- **Evidence anchors reviewed:** method p. 4 (3.1. Overview), p. 4 (3.1. Overview), p. 3 (3.1. Overview), p. 3 (3.1. Overview), p. 5 (3.1. Overview), p. 5 (3.1. Overview), objective p. 3 (3.1. Overview), p. 3 (3.1. Overview), p. 4 (3.1. Overview), p. 5 (3.1. Overview), p. 5 (3.1. Overview), temporal p. 5 (3.1. Overview), p. 3 (3.1. Overview), p. 3 (3.1. Overview), p. 4 (3.1. Overview), p. 4 (3.1. Overview), p. 5 (3.1. Overview).
