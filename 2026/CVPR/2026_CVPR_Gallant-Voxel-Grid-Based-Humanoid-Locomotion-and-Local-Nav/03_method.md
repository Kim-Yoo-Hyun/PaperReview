...
- **p. 3 / 3.1. Problem Formulation - extractive PDF cue:** Episodes end on fall, harsh collision (contact on the torso, hip, or knee links with a force exceeding 100 N), or timeout.
- **p. 3 / 3.1. Problem Formulation - extractive PDF cue:** We formulate humanoid perceptive locomotion as a partially observable Markov decision process (POMDP) M = (S, A, O, P, R, Ω, γ) and train an ...
- **p. 3 / 3.1. Problem Formulation - extractive PDF cue:** The objective is to maximize expected return J(π) = E[PH-1 t=0 γtrt].
- **p. 3 / 3.2. Efficient LiDAR Simulation - extractive PDF cue:** Traditional raycasting builds a Bounding Volume Hierarchy (BVH) over scene geometry, which becomes costly if updated at every simulation step due to dynamics.
- **p. 2 / 1. Introduction - extractive PDF cue:** In contrast, 3D LiDAR provides detailed scene geometry with a wide FoV, but its raw point clouds are sparse and noisy, which bottlenecks sample-efficient policy ...
- **p. 3 / 3.1. Problem Formulation - extractive PDF cue:** Actor and critic share all features except privileged inputs, which are critic-only.
- **p. 3 / 3. Method - extractive PDF cue:** Together, these components form a fullstack pipeline-from data generation to perception to control-that trains a single policy to robustly traverse all-space obstacles and deploy zero-shot ...
- **p. 2 / 1. Introduction - extractive PDF cue:** Experiments also highlight the importance of our high-fidelity LiDAR simulation, which dynamically generates realistic observations essential for scalable, LiDAR-based training.

## Design Rationale

- **p. 2 / 1. Introduction - extractive PDF cue:** To scale training and narrow the simulation-to-reality (simto-real) gap, we develop a LiDAR simulation pipeline that models sensor noise and latency and enables realistic scanning ...
...
- **p. 3 / 3.1. Problem Formulation - extractive PDF cue:** We formulate humanoid perceptive locomotion as a partially observable Markov decision process (POMDP) M = (S, A, O, P, R, Ω, γ) and train an ...
- **p. 5 / 4.2.1. Metrics - extractive PDF cue:** We train every policy for 4{,} 000 iterations, then run 5 independent evaluations (each run evaluates over 1{,} 000 complete episodes), reporting mean \pm standard ...
- **p. 7 / 4.3.1. Deployment - extractive PDF cue:** These results highlight Gallant's ability to encode spatial constraints from perception and translate them into robust, real-time whole-body behaviors.
...
- **p. 5 / 4.2.2. Baselines - extractive PDF cue:** To assess the effectiveness of core components in Gallant, we compare against the following ablations: • Self-scan.
- **p. 8 / 4.3.2. Ablation - extractive PDF cue:** The NoDR variant performs reasonably well on Ceiling and Door, suggesting low sensitivity to sensing latency in these cases.
- **p. 7 / 4.3.1. Deployment - extractive PDF cue:** We directly deploy the Gallant-trained policy onto the real Unitree G1 humanoid without any fine-tuning.
- **p. 7 / 4.3.2. Ablation - extractive PDF cue:** To evaluate sim-to-real performance, we deploy three policies on the 29-DoF Unitree G1 and compare success rates across terrains: (i) HeightMap, which replaces the voxel ...
- **p. 5 / 4.2.3. Result - extractive PDF cue:** Ablationspecific analyses are summarized as follow: 28090
- **p. 6 / 4.2.3. Result - extractive PDF cue:** For each ablation setting, the best-performing value per metric on each terrain is highlighted in bold.
- **p. 6 / 4.2.3. Result - extractive PDF cue:** With all other settings fixed, Gallant achieves much higher success rates than the variant that ignores dynamic objects (w/o-Self-Scan) across all tasks.
...
- **PDF anchors reviewed:** method p. 3 (3.1. Problem Formulation), p. 3 (3.1. Problem Formulation), objective p. 3 (3.1. Problem Formulation), p. 3 (3.2. Efficient LiDAR Simulation), temporal p. 3 (3.1. Problem Formulation), p. 4 (C Voxel Grid), p. 3 (3.1. Problem Formulation), p. 5 (4.2.1. Metrics), p. 7 (4.3.1. Deployment), p. 8 (4.4. Further Analyses).
