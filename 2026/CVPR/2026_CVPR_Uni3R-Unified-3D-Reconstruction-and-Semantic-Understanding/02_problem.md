...
- **p. 1 / Abstract - extractive PDF cue:** Reconstructing and semantically interpreting 3D scenes from sparse 2D views remains a fundamental challenge in computer vision.
- **p. 1 / Abstract - extractive PDF cue:** Conventional methods often decouple semantic understanding from reconstruction or necessitate costly per-scene optimization, thereby restricting their scalability and generalizability.
- **p. 1 / Abstract - extractive PDF cue:** In this paper, we introduce Uni3R, a novel feed-forward framework that jointly reconstructs a unified 3D scene representation enriched with open-vocabulary semantics, directly from unposed ...
- **p. 1 / Abstract - extractive PDF cue:** Our approach leverages a Cross-View Transformer to robustly integrate information across arbitrary *Equal contribution.
- **p. 1 / Abstract - extractive PDF cue:** Intern at Horizon Robotics †Equal contribution.
- **p. 2 / 1. Introduction - extractive PDF cue:** Recent efforts, including LangSplat [28] and Feature-3DGS [45], have incorporated semantic fields into 3D Gaussian Splatting, yet remain constrained by scene-specific optimization and lack scalability ...
...
| Observation / input | Qualitative comparison of novel view synthesis on RealEstate10k test set with 8 input images. | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF |
...
PDF contribution framing (p. 2 (1. Introduction), p. 2 (1. Introduction), p. 3 (3. Method), p. 4 (3.1.2. Cross-View Transformer Encoder), p. 5 (3.3. Training Objectives)): Our contributions are summarized as follows: • We introduce Uni3R, a novel feed-forward architecture that unifies 3D reconstruction and semantic understanding.
...
- **p. 5 / 3.3. Training Objectives - extractive PDF cue:** Given that the predictions from VGGT are not uniformly reliable, especially in challenging regions such as reflective surfaces or areas with heavy occlusion, we introduce ...
