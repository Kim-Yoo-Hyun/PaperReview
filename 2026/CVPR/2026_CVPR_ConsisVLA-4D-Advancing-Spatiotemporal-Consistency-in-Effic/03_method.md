...
- **p. 2 / 3) Cross-Scene - extractive PDF cue:** Our contributions are summarized as follows: • We propose ConsisVLA-4D, an efficient and innovative framework that advances spatiotemporal consistency in 3D-Perception and 4D-Reasoning. • We ...
- **p. 4 / 4.1. Proposed Framework - extractive PDF cue:** (5) On the other hand, we use the aggregated geometric relation zagg-3D L′ to infer the depth representations of future multiview perspectives as actions unfold: ...
- **p. 5 / 4.4. Cross-Scene Spatiotemporal Consistency - extractive PDF cue:** In the SC-Attn module, each dynamic token 0dyn-4D i is independently guided by its corresponding object representation zobj-3D i and the instruction embedding t: \ ...
- **p. 1 / Abstract - extractive PDF cue:** Current Vision-Language-Action (VLA) models primarily focus on mapping 2D observations to actions but exhibit notable limitations in spatiotemporal perception and reasoning: 1) spatial representations often ...
- **p. 5 / 4.4. Cross-Scene Spatiotemporal Consistency - extractive PDF cue:** Cross-Scene Thinker with Spatiotemporal Consistency Attention (SC-Attn) ensures: 1) Three sets of initialized dynamic tokens decode dynamic object representations for one view (CoTracker [26, 27] ...
- **p. 3 / 3. Preliminary & Problem Definition - extractive PDF cue:** 3) VGGT [64] takes M RGB images (xi)M i=1 from the same 3D scene and outputs depth maps Di, point maps Pi, and feature grids ...
- **p. 1 / Abstract - extractive PDF cue:** Building upon these, we introduce 3) CS-Thinker to achieve Cross-Scene spatiotemporal consistency as actions unfold.
- **p. 6 / 4.4. Cross-Scene Spatiotemporal Consistency - extractive PDF cue:** During this process, the initialized action tokens 0A are decoded in parallel to yield ˆA, optimized using the L1 loss Laction.

## Design Rationale

- **p. 2 / 3) Cross-Scene - extractive PDF cue:** Our contributions are summarized as follows: • We propose ConsisVLA-4D, an efficient and innovative framework that advances spatiotemporal consistency in 3D-Perception and 4D-Reasoning. • We ...
- **p. 2 / 1. Introduction - extractive PDF cue:** To this end, we propose ConsisVLA-4D, a unified and efficient framework that enhances spatiotemporal consistency in 3D-perception and 4D-reasoning, as shown in Fig.
- **p. 1 / Abstract - extractive PDF cue:** Building upon these, we introduce 3) CS-Thinker to achieve Cross-Scene spatiotemporal consistency as actions unfold.
...
- **p. 6 / 4.4. Cross-Scene Spatiotemporal Consistency - extractive PDF cue:** During this process, the initialized action tokens 0A are decoded in parallel to yield ˆA, optimized using the L1 loss Laction.
- **p. 2 / 1. Introduction - extractive PDF cue:** 2) How can we enhance spatiotemporal consistency to optimize action prediction through 4D visual reasoning?
- **p. 3 / 3. Preliminary & Problem Definition - extractive PDF cue:** 2) DINOv2 [48] aligns augmented different views of the same image using contrastive loss, allowing zgeo = f DINOv2 v (x) to capture geometric consistency.
- **p. 3 / 3. Preliminary & Problem Definition - extractive PDF cue:** 2, this establishes the foundation for feature selection and fusion design: 1) SigLIP [74] employs visual and text encoders f SigLIP v (·) and f ...
- **p. 5 / 4.4. Cross-Scene Spatiotemporal Consistency - extractive PDF cue:** The dynamic-object loss is formulated as: \smal l \mathcal { L} _{\text {d yn-4D}} = \b ig \/ (\ ha t {\ mathbf {z}}_{i^*}^{\text {dyn-4D}} ...
- **p. 6 / 4.4. Cross-Scene Spatiotemporal Consistency - extractive PDF cue:** The global-depth loss is defined as: \smal l \ m ath cal {L}_{ \ te x t { dep-4D}} =
...
- **p. 1 / Abstract - extractive PDF cue:** Current Vision-Language-Action (VLA) models primarily focus on mapping 2D observations to actions but exhibit notable limitations in spatiotemporal perception and reasoning: 1) spatial representations often ...
- **p. 1 / 1. Introduction - extractive PDF cue:** D): 1) CV-Aligner extracts instructionrelated and cross-correlated spatial objects; 2) CO-Fuser aggregates multi-view geometric relation; 3) CS-Thinker infers actions based on implicit knowledge of future ...
- **p. 4 / 4.1. Proposed Framework - extractive PDF cue:** We concatenate the initialized action chunk 0A at the end of the sequence and apply our proposed Spatiotemporal Consistency Attention (SC-Attn) to ultimately obtain the ...
- **p. 3 / 3. Preliminary & Problem Definition - extractive PDF cue:** 3) VGGT [64] takes M RGB images (xi)M i=1 from the same 3D scene and outputs depth maps Di, point maps Pi, and feature grids ...
- **p. 2 / 3) Cross-Scene - extractive PDF cue:** During inference, these pre-learned knowledge sets account for less than 10% of the observation-instruction sequence, enabling efficient 4D visual reasoning.
- **p. 5 / 4.4. Cross-Scene Spatiotemporal Consistency - extractive PDF cue:** In the SC-Attn module, each dynamic token 0dyn-4D i is independently guided by its corresponding object representation zobj-3D i and the instruction embedding t: \ ...
- **p. 3 / 3. Preliminary & Problem Definition - extractive PDF cue:** Analogous to how the human retina captures 2D images while the brain integrates spatial cues and predicts future states during manipulation, VLA models should inherit ...
...
- **p. 7 / 5.2. Overall Performance & Efficiency - extractive PDF cue:** Compared to simulation, the real-world setup adds a new viewpoint input, increases the action chunk size from 8 to 25, and reduces the training batch ...
- **p. 6 / 4.4. Cross-Scene Spatiotemporal Consistency - extractive PDF cue:** Latency and Throughput (T-put) represent the inference delay and the number of predicted actions per second, while Cost indicates the time required for every 10k ...
...
- **p. 7 / 5.1. Experimental Setup - extractive PDF cue:** Ablation components include ES-Selection, Single-Fusion from CVAligner, and Group-Fusion, IG-Aggregation from CO-Fuser.
- **p. 7 / 5.1. Experimental Setup - extractive PDF cue:** Ablation Study on CV-Aligner and CO-Fuser.
- **p. 8 / 5.3. Ablation Studies - extractive PDF cue:** Ablation Study on sparsification ratio (Spf.Ratio). "†" denotes reproduced results of FastV and SliME.
- **p. 8 / Figure/Table caption - extractive PDF cue:** Table 6. Ablation Study on CS-Thinker. Dyn. O. and Glob. D. represent the training-only dynamic objects and global depth representations in 4D-Reasoning, respectively. Dyn. O. ...
