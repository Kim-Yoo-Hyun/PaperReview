...
- **p. 1 / Abstract - extractive PDF cue:** Robotic manipulation in complex 3D environments requires unifying spatial reasoning with intuitive visual perception, which is a capability that current Vision-Language-Action paradigms address separately.
- **p. 1 / Abstract - extractive PDF cue:** While 3D VLAs excel in geometric and physical reasoning, they lack intuitive, imagelevel understanding and dense visual semantics; conversely, 2D VLAs (even with depth image) ...
- **p. 1 / Abstract - extractive PDF cue:** We introduce DiffRender-VLA, a differentiable rendering-based framework that bridges 3D and 2D VisionLanguage-Action models through gradient-consistent visual mediation.
- **p. 1 / Abstract - extractive PDF cue:** It generates differentiable images by lo- *Corresponding author is Xiu Su and Xiaoheng Deng calizing the next end-effector target with a world-aligned cube marker, differentiably ...
- **p. 1 / Abstract - extractive PDF cue:** These differentiable images serve as visual bridges, embedding spatial semantics while allowing gradients from 2D VLAs to backpropagate into 3D representations, thereby coupling geometric reasoning ...
- **p. 2 / 1. Introduction - extractive PDF cue:** The key difficulty lies in coupling geometric reasoning with semantic perception-robots must not only reason about 3D spatial structures but also interpret visual cues in ...
- **p. 2 / 1. Introduction - extractive PDF cue:** Robotic manipulation in complex 3D environments remains a central challenge in artificial intelligence and robotics.
...
| Observation / input | Given a natural language instruction I which is transformed to elang by VLM, like [43], and multi-view RGB-D observations O = {oi}Mobs ... | image/video, language instruction, proprioception과 history | exact sensor/frame/preprocessing from PDF |
...
- **p. 4 / 3.3. Rendering Adaptive Viewpoint - extractive PDF cue:** This enables joint reasoning about the target location and observation perspective as a coupling that is difficult to achieve through separate optimization.
...
vla writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 3 (3.1. Localizing Coarse Target Region), p. 2 (1. Introduction), p. 6 (3.4. Fine-Grained Action Prediction), p. 2 (1. Introduction). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (1. Introduction), p. 2 (1. Introduction), interface p. 3 (3.1. Localizing Coarse Target Region), p. 2 (1. Introduction), p. 6 (3.4. Fine-Grained Action Prediction), p. 2 (1. Introduction), objective p. 4 (3.3. Rendering Adaptive Viewpoint), p. 3 (3. Method), p. 4 (3.3. Rendering Adaptive Viewpoint), p. 6 (3.4. Fine-Grained Action Prediction), p. 6 (3.4. Fine-Grained Action Prediction).
