...
- **p. 1 / Abstract - extractive PDF cue:** Humans naturally allocate more time before acting when handling complex tasks in the physical world.
- **p. 1 / Abstract - extractive PDF cue:** This paradigm has recently led to remarkable advances in boosting Large Language Models (LLMs) on complex tasks in digital domains.
- **p. 1 / Abstract - extractive PDF cue:** However, the potential of test-time computing remains largely unexplored for robotic foundation models that interact with the physical world.
- **p. 1 / Abstract - extractive PDF cue:** FM-Steer first introduces an intermediate flow verifier to estimate state-action values for candidate actions.
- **p. 1 / Abstract - extractive PDF cue:** At test time, the policy iteratively samples multiple noisy action proposals and retains the one with the highest predicted value, yielding value-aligned, high-quality actions without ...
- **p. 2 / 1. Introduction - extractive PDF cue:** However, robot control has stricter real-time requirements than text generation: extra inference computation can introduce delays, causing jitter or even task failure.
- **p. 2 / 1. Introduction - extractive PDF cue:** To address these challenges, we introduce FM-Steer, a framework that enhances flow-based VLA models at test time with value-guided test-time sampling and cascaded action denoising.
...
| Observation / input | A flow-based VLA aims to model the data distribution p(At/ot), mapping the observation ot, which consists of images it, language instructions ℓt, ... | observation, uncertainty/risk estimate와 task command | exact sensor/frame/preprocessing from PDF |
...
- **p. 2 / 1. Introduction - extractive PDF cue:** To address this issue, we propose a cascaded action denoising mechanism that distributes the denoising computation across the original VLA and a separate Lite-Flow denoiser, ...
- **p. 3 / 3. Preliminaries - extractive PDF cue:** The model typically consists of a VLM backbone and a flow matching expert.
- **p. 3 / 3. Preliminaries - extractive PDF cue:** A flow-based VLA aims to model the data distribution p(At/ot), mapping the observation ot, which consists of images it, language instructions ℓt, and robot state ...
- **p. 6 / Model - extractive PDF cue:** We present the success rate (SR) and standard error for each method across four task suites.
...
safety writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 3 (3. Preliminaries), p. 4 (4.2. Cascaded Action Denoising), p. 4 (4.1. Value-Guided Test-Time Sampling), p. 2 (1. Introduction). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (1. Introduction), p. 2 (1. Introduction), interface p. 3 (3. Preliminaries), p. 4 (4.2. Cascaded Action Denoising), p. 4 (4.1. Value-Guided Test-Time Sampling), p. 2 (1. Introduction), objective p. 3 (4.1. Value-Guided Test-Time Sampling), p. 4 (4.1. Value-Guided Test-Time Sampling).
