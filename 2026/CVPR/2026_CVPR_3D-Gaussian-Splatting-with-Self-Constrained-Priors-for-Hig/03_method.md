...
- **p. 4 / 3.3. Loss Functions - extractive PDF cue:** We use planar Gaussians in 3DGS for better geometry representation.
- **p. 3 / 3.1. Learning Self-Constrained Priors - extractive PDF cue:** Moreover, we also progressively reduce the width of the narrow band to strengthen the constraints along with stabilizing the optimization.
- **p. 3 / 3. Method - extractive PDF cue:** With the learned Gaussians {gj}, we can render {gj} into depth maps {d′} and fuse them into a TSDF for surface extraction, or RGB images ...
- **p. 4 / 3.2. Constraints with a Self-Constrained Prior - extractive PDF cue:** Although the field f t remains an estimation, it is adequate to present a coarse surface inference.
- **p. 4 / 3.3. Loss Functions - extractive PDF cue:** Overall, we minimize the loss function L by, L = LRGB + λ1LDepth + λ2LNS + λ3LNM + λ4LSCP , (10) where {λ1, λ2, λ3, ...
- **p. 3 / 3. Method - extractive PDF cue:** Additionally, we will shrink the range of area around estimated surfaces each time to progressively impose tightened constraints on the learning of Gaussians.
- **p. 4 / 3.2. Constraints with a Self-Constrained Prior - extractive PDF cue:** With the interpolated signed distance sj and the gradient ∇f t(µj), we update the position by µj ←µj -sj ∗∇f t(µj).
- **p. 4 / 3.3. Loss Functions - extractive PDF cue:** Specifically, we use LRGB to evaluate the error of rendering v′ to the input image v with a mean absolute error (MAE), a structural similarity ...

## Design Rationale

- **p. 1 / 1. Introduction - extractive PDF cue:** Our contributions are listed below, • We propose a self-constrained prior to impose constraints on the learning of 3D Gaussians in a geometry-aware manner.
...
- **p. 3 / 3. Method - extractive PDF cue:** With the learned Gaussians {gj}, we can render {gj} into depth maps {d′} and fuse them into a TSDF for surface extraction, or RGB images ...
- **p. 4 / 3.3. Loss Functions - extractive PDF cue:** Specifically, we use LRGB to evaluate the error of rendering v′ to the input image v with a mean absolute error (MAE), a structural similarity ...
- **p. 3 / 3.1. Learning Self-Constrained Priors - extractive PDF cue:** The error map indicates the distance to the ground truth surface. the depth fusion operation F, f t = F({d′ i(t)}).
- **p. 1 / 1. Introduction - extractive PDF cue:** We conduct evaluations on widely used benchmark to justify our idea and report our superiority over the state-of-the-art methods.
- **p. 2 / 1. Introduction - extractive PDF cue:** We also apply Gaussian geometric constraints (GC) that are related to interpolated distance s, centers µ and gradients ∇f t for high fidelity surface reconstruction. ...
- **p. 4 / 3.3. Loss Functions - extractive PDF cue:** 3, we also use loss terms to evaluate the RGB rendering errors and multi-view consistency on depth maps.
- **p. 1 / 1. Introduction - extractive PDF cue:** To resolve this issue, we propose a self-constrained prior to impose constraints on 3D Gaussians in a geometry-aware manner.
...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Figure 10. Effect of the self-constrained prior.
...
- **PDF anchors reviewed:** method p. 4 (3.3. Loss Functions), p. 3 (3.1. Learning Self-Constrained Priors), p. 3 (3. Method), p. 4 (3.2. Constraints with a Self-Constrained Prior), objective p. 4 (3.3. Loss Functions), p. 3 (3.1. Learning Self-Constrained Priors), p. 3 (3. Method), p. 4 (3.2. Constraints with a Self-Constrained Prior), temporal p. 2 (2. Related Work), p. 2 (2. Related Work).
