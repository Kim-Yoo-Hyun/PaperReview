...
- **p. 6 / 4.1. Setup - extractive body cue:** To adapt it to our dynamic 4D benchmark as a strong baseline for timeagnostic queries, we train the model utilizing the exact same instance masks ...
- **p. 6 / 4.1. Setup - extractive body cue:** We extend the HyperNeRF [27] dataset for the evaluation of STRS-4DGS task.
- **p. 7 / 4.2. Results - extractive body cue:** Quantitative comparisons on the HyperNeRF dataset.
- **p. 7 / 4.2. Results - extractive body cue:** 2 presents examples from two scenes, visualizing the activation heatmaps and the resulting segmentation masks (red contours) with non-target regions grayed out.
- **p. 8 / 4.2. Results - extractive body cue:** The performance is evaluated using the mIoU metric for time-agnostic queries on the HyperNeRF dataset.
...
- **p. 6 / 4.1. Setup - extractive body cue:** To comprehensively assess both temporal accuracy and segmentation quality, we adopt the vIoU metric, defined as vIoU = 1 /Su/ P t∈Si IoU(ˆst, st), where ...
- **p. 6 / 4.1. Setup - extractive body cue:** For time-sensitive referring queries, we assess temporal performance using an accuracy metric, defined as Acc = ncorrect/nall, where ncorrect represents the number of correctly predicted ...
...
- **p. 7 / Figure/Table caption - extractive body cue:** Figure 2. Qualitative comparison of time-agnostic referring querying results between 4DLangSplat [22] and our method. For both methods, we visualize the activation heatmaps and the ...
- **p. 7 / Figure/Table caption - extractive body cue:** Table 1. Quantitative comparisons on the HyperNeRF dataset. We report (a) time-agnostic referring querying (mIoU in %) and (b) time- sensitive referring querying (Acc and ...
- **p. 8 / Figure/Table caption - extractive body cue:** Figure 3. Qualitative comparison of time-sensitive query results in novel viewpoints between 4DLangSplat [22] and our method. Our method accurately resolves these spatial relations and ...
- **p. 8 / Figure/Table caption - extractive body cue:** Table 2. Ablation study of different components in our ST4R-Splat framework. The performance is evaluated using the mIoU metric for time-agnostic queries on the HyperNeRF ...
...
- **p. 6 / 4.1. Setup - extractive body cue:** For evaluation, we provide 52 time-agnostic referring queries and 8 time-sensitive referring queries for 26 objects in 6 different scenes.
