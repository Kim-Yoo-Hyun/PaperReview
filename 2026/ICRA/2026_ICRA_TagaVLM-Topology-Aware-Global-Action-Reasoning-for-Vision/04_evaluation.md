...
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** For testing, we utilize 1,021 navigation paths from the val seen split and 2,349 paths from the val unseen split in the R2R dataset.
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** To rearrange the dataset into SAP task for training, 14,093 trajectories from the R2R training split are segmented into discrete navigation steps.
- **p. 7 / IV. EXPERIMENTS - extractive PDF cue:** All ablation experiments are conducted on the TagaVLM-0.5B model and the val unseen split of R2R dataset.
- **p. 7 / IV. EXPERIMENTS - extractive PDF cue:** By using the rich scene diversity provided by HM3D, an additional 500K augmented data samples are utilized for the first-stage fine-tuning, which improves the model's ...
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** In these metrics, Trajectory Length (TL) denotes average path length in meters; Navigation Error (NE) represents the average distance in meters between the agent's final ...
- **p. 7 / IV. EXPERIMENTS - extractive PDF cue:** The global action space introduces backtracking capability that improves fault tolerance in navigation processes, enabling the agent to execute direct backtracking upon encountering navigation errors ...
- **p. 7 / IV. EXPERIMENTS - extractive PDF cue:** II, the model achieves large performance gains of 12.26% in SR and 14.8 in SPL with the Interleaved Navigation Prompt, as shown in row (c).
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** Notably, compared to MapGPT[15], our approach achieves an absolute improvement of 3.39% in SR and 9.08 in SPL on the val unseen split.
...
- **p. 4 / Figure/Table caption - extractive PDF cue:** Fig. 3. In the navigation process, if an unvisited candidate node is observed multiple times at different positions, it is represented by stitching the images ...
- **p. 5 / Figure/Table caption - extractive PDF cue:** Fig. 4. A successful case demonstrates TagaVLM's spatial topological awareness and path correction ability. (a) shows the navigation instruction containing two key landmarks: black chairs ...
...
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** To rearrange the dataset into SAP task for training, 14,093 trajectories from the R2R training split are segmented into discrete navigation steps.
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** Furthermore, we leverage the data augmentation method [22] to generate 500K SAP samples (around 90K trajectories) across 800 scenes from the HM3D [39] dataset.
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** Our training employs a two-stage strategy: 1) pre-training for 12,500 steps on a mixture of R2R and augmented HM3D data, followed by 2) fine-tuning for ...
...
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** However, due to computational resource limitations, TagaVLM-7B is fine-tuned with only 200K augmented samples.
- **p. 7 / IV. EXPERIMENTS - extractive PDF cue:** However, owing to the limitation of computational resources, the amount of training data used for the proposed method is significantly smaller than that of NaviLLM[16], ...
