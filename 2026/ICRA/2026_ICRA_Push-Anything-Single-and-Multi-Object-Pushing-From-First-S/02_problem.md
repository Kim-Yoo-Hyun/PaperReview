...
| Observation / input | The set D comprises all feasible z satisfying the coupled constraints across time: the linear dynamics (5b), the slack-variable equality (5c), and ... | joint/task state, reference와 sensor feedback | exact sensor/frame/preprocessing from PDF body |
...
PDF body contribution framing (p. 1 (I. INTRODUCTION), p. 3 (IV. METHODS), p. 3 (IV. METHODS), p. 4 (IV. METHODS)): We introduce Push Anything, a manipulation pipeline for real-time planar pushing of a wide variety of objects, including multi-object scenes.
