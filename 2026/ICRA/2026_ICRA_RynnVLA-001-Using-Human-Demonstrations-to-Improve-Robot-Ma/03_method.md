...
- **p. 4 / 3 Methodology - extractive body cue:** In this work, we propose RynnVLA-001, a Vision-Language-Action (VLA) model built upon large-scale video generation pretraining.
...
- **p. 5 / 3 METHODOLOGY - extractive body cue:** The training of the action head is supervised by L1 loss, which is computed exclusively for the outputs at token positions of <ACTION_PLACEHOLDER>.
...
- **p. 5 / 3 METHODOLOGY - extractive body cue:** The training is supervised by the cross-entropy loss over discrete visual tokens and language tokens.
- **p. 6 / 3 METHODOLOGY - extractive body cue:** 2) Future Visual Prediction: The model continues the autoregressive prediction of visual tokens for the next frame, supervised by cross-entropy loss.
...
- **p. 5 / 3 METHODOLOGY - extractive body cue:** The training is supervised by the cross-entropy loss over discrete visual tokens and language tokens.
- **p. 6 / 3 METHODOLOGY - extractive body cue:** 2) Future Visual Prediction: The model continues the autoregressive prediction of visual tokens for the next frame, supervised by cross-entropy loss.
- **p. 4 / 3 Methodology - extractive body cue:** The objective of this stage is to train an I2V model that closely mimics the inference process of a VLA model.
- **p. 5 / 3 METHODOLOGY - extractive body cue:** The training of the action head is supervised by L1 loss, which is computed exclusively for the outputs at token positions of <ACTION_PLACEHOLDER>.
- **p. 4 / 3 Methodology - extractive body cue:** The model is progressively trained through three stages, as illustrated in Fig.
...
- **p. 4 / 3 Methodology - extractive body cue:** In a typical VLA setting, actions are predicted conditioned on current observations (e.g., visual inputs and robot states) and a language instruction.
- **p. 4 / 3 Methodology - extractive body cue:** 3) Robot-Centric Vision-Language-Action Modeling: The VLA model inherits the weights from the previous stages and is trained on robot data using language instructions and current ...
- **p. 2 / 1 INTRODUCTION - extractive body cue:** (3) Robot-Centric Vision-Language-Action Modeling employs robot datasets paired with language instructions to learn mappings from visual observations and language to robotic actions. image and a ...
- **p. 6 / 3 METHODOLOGY - extractive body cue:** At each step of the cycle, the model receives the language instruction, the current RGB observation from the robot's cameras and current robot states as ...
- **p. 2 / 1 INTRODUCTION - extractive body cue:** During inference, given an observation and a language instruction, the model outputs a single action embedding, which is subsequently decoded by ActionVAE into a sequence ...
- **p. 6 / 3 METHODOLOGY - extractive body cue:** This sequence explicitly provides four components: (1) the high-level goal (language), (2) robot-centric visual observations (front and wrist views), (3) the current robot state, and ...
- **p. 5 / 3 METHODOLOGY - extractive body cue:** The input sequence is now structured as: [language, visual tokenst, state embeddingt, <ACTION_PLACEHOLDER>, ...], where <ACTION_PLACEHOLDER> is the signal to generate continuous action embeddings.
...
- **p. 6 / 3 METHODOLOGY - extractive body cue:** During training, the model is optimized with two concurrent objectives: 1) Robot Action Prediction: The hidden state corresponding to the output of the <ACTION_PLACEHOLDER> token ...
- **p. 5 / 3 METHODOLOGY - extractive body cue:** The training of the action head is supervised by L1 loss, which is computed exclusively for the outputs at token positions of <ACTION_PLACEHOLDER>.
- **p. 9 / 5 EXPERIMENTS - extractive body cue:** Benefiting from the pretrained T2I checkpoint, the RynnVLA-001-Chameleon model achieves reasonable results on simple grasping.
- **p. 10 / 5 EXPERIMENTS - extractive body cue:** All models are trained with reduced epochs for efficiency; scores are for relative comparison.
...
- **p. 9 / 5 EXPERIMENTS - extractive body cue:** To investigate the effectiveness of our proposed two-stage pretraining pipeline, we conduct a comprehensive ablation study, with results presented in Table 3 and Table 4.
- **p. 10 / 5 EXPERIMENTS - extractive body cue:** To evaluate the effectiveness of the component, we conduct an ablation study on the Calvin ABC->D benchmark, comparing the performance of predicting VAE embeddings against ...
- **p. 12 / 5 EXPERIMENTS - extractive body cue:** A variant of RynnVLA-001 is trained solely on data without distractor objects.
- **p. 9 / 5 EXPERIMENTS - extractive body cue:** By incorporating this second pretraining stage where the model learns to predict human trajectories, our full model, RynnVLA-001, achieves the best performance among all variants.
- **p. 10 / 5 EXPERIMENTS - extractive body cue:** For experimental efficiency, our full model and the ablated variants are trained from the pretraining weights of RynnVLA-001-Video but for a reduced number of epochs.
- **p. 11 / 5 EXPERIMENTS - extractive body cue:** 5.4 Ablation Study on Model Designs
- **p. 12 / 5 EXPERIMENTS - extractive body cue:** To validate this hypothesis, we perform an ablation study.
...
- **Evidence anchors reviewed:** method p. 4 (3 METHODOLOGY), p. 6 (3 METHODOLOGY), p. 4 (3 Methodology), p. 6 (3 METHODOLOGY), p. 5 (3 METHODOLOGY), p. 5 (3 METHODOLOGY), objective p. 6 (3 METHODOLOGY), p. 5 (3 METHODOLOGY), p. 6 (3 METHODOLOGY), p. 4 (3 Methodology), p. 5 (3 METHODOLOGY), p. 4 (3 Methodology), temporal p. 5 (3 METHODOLOGY), p. 5 (3 METHODOLOGY), p. 6 (3 METHODOLOGY), p. 6 (3 METHODOLOGY), p. 7 (5 EXPERIMENTS), p. 10 (5 EXPERIMENTS).
