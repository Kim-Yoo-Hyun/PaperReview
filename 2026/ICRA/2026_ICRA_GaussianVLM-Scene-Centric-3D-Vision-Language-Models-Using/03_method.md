...
- **p. 3 / III. METHOD - extractive PDF cue:** The sparsifier takes as input the dense language features and outputs sparse task-aware tokens.
...
- **p. 3 / III. METHOD - extractive PDF cue:** To sparsify the resulting dense language features with a task-awareness, we introduce a dual sparsifier module.
...
- **p. 3 / III. METHOD - extractive PDF cue:** Both stages share a unified training objective.
- **p. 3 / III. METHOD - extractive PDF cue:** This pre-training stage uses a one-sided contrastive objective [34], encouraging the output embedding of the task-guided sparsifier si to match its corresponding label embedding li, ...
- **p. 3 / III. METHOD - extractive PDF cue:** GaussianVLM relies on three key innovations: (1) a language-aware Gaussian splatting backbone [27] that predicts language features for each Gaussian, enabling direct language-based alignment between ...
- **p. 4 / III. METHOD - extractive PDF cue:** Our sparsifier employs the language task to generate queries that guide the filtering of visual input via depthwise cross-attention [5].
...
- **p. 2 / I. INTRODUCTION - extractive PDF cue:** Across the board, GaussianVLM achieves state-of-the-art performance, outperforming the SOTA baselines [9], [23] on every benchmark.
- **p. 2 / I. INTRODUCTION - extractive PDF cue:** The taskguided sparsifier takes as input the dense scene tokens and the task tokens, using the latter in cross-attention to guide the sparsification process.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** In this work, we propose to shift from object-centric to scene-centric representations by embedding language features directly into the spatial structure of the environment.
...
- **p. 5 / IV. EXPERIMENTS - extractive PDF cue:** To encourage linguistic diversity, we use GPT-4o to generate 40 paraphrased variants per prompt with varied syntax and vocabulary.
...
- **PDF anchors reviewed:** method p. 3 (III. METHOD), p. 4 (III. METHOD), p. 3 (III. METHOD), p. 4 (III. METHOD), objective p. 3 (III. METHOD), p. 3 (III. METHOD), temporal p. 3 (III. METHOD), p. 3 (III. METHOD), p. 4 (III. METHOD), p. 5 (IV. EXPERIMENTS).
