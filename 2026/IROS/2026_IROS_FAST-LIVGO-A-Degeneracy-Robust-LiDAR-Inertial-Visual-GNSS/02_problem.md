...
- **p. 1 / Abstract - extractive PDF cue:** Robust state estimation and mapping in long-term, large-scale, and highly dynamic environments remains a key challenge in robotics.
- **p. 1 / Abstract - extractive PDF cue:** Existing LiDAR-Inertial-Visual Odometry (LIVO) systems achieve strong local accuracy but suffer from accumulated drift over long distances and may fail in geometrically degraded or textureless ...
- **p. 1 / Abstract - extractive PDF cue:** Meanwhile, GNSS-aided fusion frameworks often rely on LiDAR or visual odometry for state prediction and outlier rejection, making them vulnerable when odometry degenerates.
- **p. 1 / Abstract - extractive PDF cue:** To address these limitations, we propose a tightly coupled LiDAR-Inertial-Visual-GNSS fusion framework based on an Error-State Iterated Kalman Filter.
- **p. 1 / Abstract - extractive PDF cue:** An online spatiotemporal alignment module using Dynamic Time Warping is introduced for highly dynamic conditions.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** However, in large-scale field missions or high-speed Unmanned Aerial Vehicle (UAV) flights, pure LIVO systems still face critical limitations.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** Existing loosely-coupled schemes underutilize the high precision of GNSS carrier phases, while traditional tightly-coupled frameworks lack adaptive integrity monitoring under alternating LIVO and GNSS degradation.
...
| Observation / input | To unify spatial references, GNSS ephemeris and observations (e.g., position, velocity) are transformed from the EarthCentered, Earth-Fixed (ECEF) frame to the local ... | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF |
...
PDF contribution framing (p. 1 (I. INTRODUCTION), p. 2 (III. METHODOLOGY), p. 1 (I. INTRODUCTION)): To address these issues, we propose a tightly-coupled multi-sensor fusion system formulated within an Error-State Iterated Kalman Filter (ESIKF), aiming at globally consistent mapping in highly dynamic scenarios and robust ...
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 2 (III. METHODOLOGY), p. 2 (III. METHODOLOGY), p. 3 (III. METHODOLOGY), p. 1 (I. INTRODUCTION). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (I. INTRODUCTION), p. 1 (I. INTRODUCTION), interface p. 2 (III. METHODOLOGY), p. 2 (III. METHODOLOGY), p. 3 (III. METHODOLOGY), p. 1 (I. INTRODUCTION), objective p. 2 (III. METHODOLOGY), p. 3 (III. METHODOLOGY), p. 2 (III. METHODOLOGY), p. 3 (III. METHODOLOGY), p. 4 (III. METHODOLOGY).
