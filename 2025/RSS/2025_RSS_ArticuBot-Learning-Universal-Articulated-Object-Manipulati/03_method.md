...
- **p. 4 / 2. Hierarchical Policy Learning -- Low-level Policy Architecture - extractive PDF cue:** Bottom: We propose a goal-conditioned 3D diffusion policy for the low-level policy, which first applies attention between the current end-effector points, the scene points, and ...
- **p. 5 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** Instead, we propose to use a hilrarchical policy representation, which consists of 4 high-level policy and a low-level policy.
- **p. 7 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** The final latent embedding used for diffusion is the concatenation of the above two features: [fpoo™, fom"! yess, po This latent embedding is used ‘as ...
- **p. 8 / B. Is a Hierarchical Policy Needed? - extractive PDF cue:** takes 3D point cloud as input and outputs delta endeffector transformations as the actions. + DP3 Transformer, which replaces the simplified PointNet encoder in DP3 ...
...
- **p. 6 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** Specifically, instead of representing the sub-goal endeffector pose as a position and an SO(3) orientation (e.g., 4 quaternion or a 6D orientation representation [65}) and ...
- **p. 6 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** As in DP3, the network has two parts: a point cloud encoder that encodes the point cloud observation into a latent embedding, and a diffusion ...
- **p. 5 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** Formally, the high-level policy 7! is learned via minimizing the following loss:
...
- **p. 5 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** Formally, the high-level policy 7! is learned via minimizing the following loss:
- **p. 5 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** Our goal is to find a neural network policy =, parameterized by 8, t0 minimize the following imitation learning loss:
- **p. 4 / 2. Hierarchical Policy Learning -- Low-level Policy Architecture - extractive PDF cue:** Among the collision-free solutions, we choose the one solution that has the shortest distance in the joint angle space to the current joint configuration, so ...
- **p. 6 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** We train it with the following two losses, which supervises the per-point displacement prediction, and the weighted average prediction:
- **p. 7 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** We perform cross tention between the current end-effector points and the goal end-effector points, also with Rotary Position Embedding (RoPE) (4), which produces another set ...
...
- **p. 7 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** The final latent embedding used for diffusion is the concatenation of the above two features: [fpoo™, fom"! yess, po This latent embedding is used ‘as ...
- **p. 4 / 2. Hierarchical Policy Learning -- Low-level Policy Architecture - extractive PDF cue:** Bottom: We propose a goal-conditioned 3D diffusion policy for the low-level policy, which first applies attention between the current end-effector points, the scene points, and ...
- **p. 5 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** Specifically, the high-level policy 7}! learns to predict the sub-goal end-effector pose given an observation.
- **p. 6 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** We note that the low-level policy is not trained to reach the sub-goal end-effector pose; it is trained to solve the task, and the sub-goal ...
- **p. 1 / 2. Hierarchical Policy Learning - extractive PDF cue:** simulator, and distill them into a hierarchical policy with point cloud observations.
- **p. 5 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** We use the robot base frame as our reference frame, ie. all point cloud observations, and robot actions, are expressed inthe robot base frame.
...
- **p. 6 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** At inference time, the final predicted subgoal endheffector pose is the averaged prediction from all points in the scene: e¢,(0) = 31 (p;-+8)(0)).
- **p. 6 / B. Policy Learning with a Hierarchical Policy Representation - extractive PDF cue:** At inference time, the final prediction of the sub-goal end-effector Points is then the weighted average of the displacement from each point: €€,(0) = 2)", ...
...
- **p. 13 / C. Mobile X-Arm Results - extractive PDF cue:** We think adding a force-torque sensor on the X-Arm to enable impedance control could help alleviate this issue; fine-tuning the policy in the real-world via ...
