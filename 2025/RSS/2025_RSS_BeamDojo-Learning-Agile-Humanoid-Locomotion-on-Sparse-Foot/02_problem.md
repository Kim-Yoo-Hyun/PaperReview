...
| Observation / input | 1) Observation Space and Action Space: ‘The policy observations, denoted a8 o,, consist of four components: 0 = [61 0f°"*, of", a ... | proprioception, reference pose/motion, visual or language command | exact sensor/frame/preprocessing from PDF body |
...
PDF body contribution framing (p. 1 (Abstract), p. 2 (1. INrRopucTION), p. 2 (1. INrRopucTION), p. 3 (A. Foothold Reward), p. 1 (Body text (section not recovered))): To address these challenges, we introduce BEAMDOJO, a reinforcement learning (RL) framework designed for enabling agile humanoid locomotion on sparse footholds.
