...
- **Registry interface:** `Robotics, quadruped locomotion, safe locomotion, torque control` is the paper's recorded topic/interface, not evidence that the full robotics loop was evaluated.
- Reuse the paper by preserving its input/output boundary and testing downstream success, failure, and latency under a matched baseline budget.

### Dependency and evolution

- Registry position: `Locomotion, Whole-Body, and Mobile Manipulation`; tags: `Robotics, quadruped locomotion, safe locomotion, torque control`.
...
고정된 state, action, compute budget에서 Robotics, quadruped locomotion, safe locomotion, torque control formulation이 task cost 또는 success를 유지하면서 perturbation 이후 recovery를 개선하는가?
