...
- **p. 5 / 4.1. Dataset and Task Description - extractive PDF cue:** For zero-shot occupancy estimation, we evaluate on the widely known Occ3D-nuScenes benchmark [41], which provides semantic voxel labels for the nuScenes dataset.
- **p. 5 / 4.1. Dataset and Task Description - extractive PDF cue:** We conduct all experiments on the nuScenes dataset [3].
- **p. 6 / 4.4. Zero-shot Semantic Occupancy Estimation - extractive PDF cue:** We evaluate our approach against other recent approaches on the Occ3D-nuScenes dataset [41] and show the results in Tab.
- **p. 6 / 4.2. Implementation Details - extractive PDF cue:** We use the same fixed vocabulary to train the reducer U for each experiment, which is based on the target classes of the Occ3D-nuScenes dataset.
- **p. 7 / 4.4. Zero-shot Semantic Occupancy Estimation - extractive PDF cue:** Semantic occupancy estimation results on the Occ3DnuScenes benchmark [41] in terms of geometric IoU and semantic mIoU.
- **p. 7 / 4.4. Zero-shot Semantic Occupancy Estimation - extractive PDF cue:** Qualitative results showing open vocabulary retrieval on nuScenes [3].
...
- **p. 7 / 4.5. Ablations - extractive PDF cue:** Loss Function MSE CosSim Cos-guided MSE IoU 50.29 49.88 51.59 mIoU 9.41 9.89 10.71 mAP (v) 20.1 22.6 22.7 using either the MSE loss or ...
...
- **p. 4 / Figure/Table caption - extractive PDF cue:** Figure 1. Architecture of the proposed model. A set of images is first transformed to 3D voxel features via BEVStereo [24] and a 3D CNN ...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Table 1. 3D open vocabulary retrieval results on the bench- mark provided by [43]. mAP (v) is calculated only on points visible to one of ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Figure 2. Qualitative results showing open vocabulary retrieval on nuScenes [3]. Given a text query, we compute similarities between the text embedding and each estimated ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Table 2. Semantic occupancy estimation results on the Occ3D- nuScenes benchmark [41] in terms of geometric IoU and se- mantic mIoU. The Mode indicates the ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Table 3. Ablation on the loss function used for Llang.
...
- **p. 5 / 4.1. Dataset and Task Description - extractive PDF cue:** It consists of 105 samples, each with an open vocabulary text query and corresponding binary labels for the LiDAR point cloud, with the goal of ...
- **p. 6 / 4.2. Implementation Details - extractive PDF cue:** We train each network with a batch size of 4 for 18 epochs.
- **p. 6 / 4.2. Implementation Details - extractive PDF cue:** For each ray, we sample 100 points, and use the nerfacc [23] package for rendering.
- **p. 7 / 4.4. Zero-shot Semantic Occupancy Estimation - extractive PDF cue:** Input images Similarity Map "Traffic light" FRONT-LEFT FRONT FRONT-RIGHT BACK-LEFT BACK BACK-RIGHT "Metal Pole" FRONT-LEFT FRONT FRONT-RIGHT BACK-LEFT BACK BACK-RIGHT "Human being" FRONT-LEFT FRONT FRONT-RIGHT ...
- **p. 3 / 3.3. Volume Rendering Supervision - extractive PDF cue:** Specifically, a rendering weight w(r(t)) is computed for each sampled point on the ray by accumulating the interpolated density: w(r(t)) = T (r(t)) (1 -exp(-σ(r(t))δt)) ...
