...
| Observation / input | BEV Branch: Looking around for scene-level representation The multi-view sequential images with the previous T frames are first input into the 2D ... | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF body |
...
PDF body contribution framing (p. 2 (1. Introduction), p. 2 (1. Introduction), p. 3 (3. Methodology), p. 3 (3. Methodology), p. 5 (3. Methodology)): Our main contributions can be concluded as follows: i) We proposed LSSInst, a two-stage framework that improves the geometric details in LSS-based BEV perception with instance representations; ii) We proposed ...
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. Evidence interface anchors: p. 4 (3. Methodology), p. 4 (3. Methodology), p. 2 (1. Introduction), p. 3 (3. Methodology). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **Evidence anchors reviewed:** problem p. 1 (1. Introduction), p. 2 (1. Introduction), p. 2 (1. Introduction), interface p. 4 (3. Methodology), p. 4 (3. Methodology), p. 2 (1. Introduction), p. 3 (3. Methodology), objective p. 4 (3. Methodology), p. 4 (3. Methodology), p. 5 (3. Methodology), p. 6 (3. Methodology).
