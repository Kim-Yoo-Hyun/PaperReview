...
- **p. 4 / 3. Methodology - extractive PDF cue:** Backbone Multi-frame Multi-view Features Multi-view Images with Previous T Frames Depth Distribution Map BEV Feature BEV Temporal Encoder BEV Branch Temporally-shared View Transformation BEV Sequence ...
- **p. 4 / 3. Methodology - extractive PDF cue:** BEV Branch: Looking around for scene-level representation The multi-view sequential images with the previous T frames are first input into the 2D image backbone network ...
...
- **p. 5 / 3. Methodology - extractive PDF cue:** Instance Adapter: Scene-to-instance adaptation For the sake of preserving a coherent and solid semantic consistency between BEV and instance representations, we propose the instance adapter ...
- **p. 5 / 3. Methodology - extractive PDF cue:** To that end, the proposed adapter module first performs a reprojection of the proposal box coordinates Po ∈RNβ×3 obtained through the BEV proposal head, returning ...
- **p. 6 / 3. Methodology - extractive PDF cue:** Then the multi-frame features are fed into the sparse temporal encoder fenc, a naive three-layer MLP, for temporal iterative fusion.
- **p. 6 / 3. Methodology - extractive PDF cue:** Then the per-frame sampled feature Fδt, t ∈{0, 1, ..., Tχ} is formulated by Fδt = Wχ K X k=1 Akt·W ′ χΦ[Fimg, (Mt(pχ -τt ...
- **p. 4 / 3. Methodology - extractive PDF cue:** Lastly, the model makes the final prediction based on the updated output. briefly introduces the BEV branch.

## Design Rationale

- **p. 2 / 1. Introduction - extractive PDF cue:** Our main contributions can be concluded as follows: i) We proposed LSSInst, a two-stage framework that improves the geometric details in LSS-based BEV perception with ...
...
- **p. 4 / 3. Methodology - extractive PDF cue:** Backbone Multi-frame Multi-view Features Multi-view Images with Previous T Frames Depth Distribution Map BEV Feature BEV Temporal Encoder BEV Branch Temporally-shared View Transformation BEV Sequence ...
- **p. 5 / 3. Methodology - extractive PDF cue:** To that end, the proposed adapter module first performs a reprojection of the proposal box coordinates Po ∈RNβ×3 obtained through the BEV proposal head, returning ...
- **p. 5 / 3. Methodology - extractive PDF cue:** Instance Branch: Looking back for instancelevel representation Given the sequential image features {F t img}Tχ t=0 (Tχ ≤T) from the image backbone network and the ...
- **p. 6 / 3. Methodology - extractive PDF cue:** Spatiotemporal Sampling and Fusion The sparse feature Fχ with the box embedding Gχ will be updated by the spatial and temporal sampling after being fed ...
...
- **p. 4 / 3. Methodology - extractive PDF cue:** BEV Branch: Looking around for scene-level representation The multi-view sequential images with the previous T frames are first input into the 2D image backbone network ...
- **p. 4 / 3. Methodology - extractive PDF cue:** Backbone Multi-frame Multi-view Features Multi-view Images with Previous T Frames Depth Distribution Map BEV Feature BEV Temporal Encoder BEV Branch Temporally-shared View Transformation BEV Sequence ...
- **p. 2 / 1. Introduction - extractive PDF cue:** The instance branch focuses on fine-grained sparse feature extraction and geometric matching using prepared inputs, such as box embeddings and spatiotemporal sampling and fusion.
- **p. 3 / 3. Methodology - extractive PDF cue:** In this work, we propose LSSInst, which looks back for the more geometry-aware and finegrained target feature extraction to bridge the adaptation between scene-level and ...
...
- **p. 2 / 1. Introduction - extractive PDF cue:** Specifically, it outperforms BEVDet by 5.0%, BEVDepth by 2.2%, BEVStereo by 2.6%, and surpasses the state-ofthe-art LSS-based method SOLOFusion by 1.6%.
- **p. 5 / 3. Methodology - extractive PDF cue:** With it combined with the sparse instance features, there will be more geometric priors and implicit compensation in subsequent attention interactions.
...
- **p. 6 / 4.2. Benchmark Results - extractive PDF cue:** On the test set, our LSSInst achieves an mAP of 54.6% and an NDS of 62.9% without any additional augmentation, outperforming all LSS-based methods.
...
- **p. 7 / 4.3. Generalization Ability and Geometric-Wise - extractive PDF cue:** Comparison results of LSS-based and two-stage detectors on 3D detection on the nuScenes val set. † denotes the performance without future frames for a fair ...
...
- **p. 14 / Figure/Table caption - extractive PDF cue:** Table 13. Point Ablation Points mAP↑ NDS↑ 1 0.365 0.477 2
- **p. 14 / Figure/Table caption - extractive PDF cue:** Table 14. Weight Ablation Weight mAP↑ NDS↑ 1 0.365 0.477 2
- **p. 7 / 4.5. Multiplicate Queries Ablations - extractive PDF cue:** We can observe that on the one hand, relying solely on the potential queries cannot play a major role, and even utilizing all 900 queries ...
...
- **PDF anchors reviewed:** method p. 4 (3. Methodology), p. 4 (3. Methodology), p. 3 (3. Methodology), p. 5 (3. Methodology), p. 5 (3. Methodology), p. 6 (3. Methodology), objective p. 4 (3. Methodology), p. 4 (3. Methodology), p. 5 (3. Methodology), p. 5 (3. Methodology), p. 6 (3. Methodology), temporal p. 4 (3. Methodology), p. 4 (3. Methodology), p. 6 (3. Methodology), p. 2 (1. Introduction), p. 3 (3. Methodology), p. 5 (3. Methodology).
