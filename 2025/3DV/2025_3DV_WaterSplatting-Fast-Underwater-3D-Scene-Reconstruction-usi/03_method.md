...
- **p. 4 / 3.3. Loss Function Alignment - extractive PDF cue:** For the case of our 3DGS-based model, we propose a regularized loss function LReg: we apply pixel-wise weight W = {wi,j} on both rendered estimate ...
- **p. 3 / 3.1. Preliminaries - extractive PDF cue:** For scene rendering in scattering media we use the revised underwater image formation model from [1] where the final image I is separated into a ...
- **p. 5 / 3.3. Loss Function Alignment - extractive PDF cue:** Integrating regularization into the LReg-DSSIM formulation becomes particularly critical for 3DGS optimization due to the discrete nature of its primitives, necessitating structural regularization to maintain ...
- **p. 3 / 3. Method - extractive PDF cue:** Then, we illustrate our proposed rendering model combining 3DGS with medium encoding in Sec.
- **p. 4 / 3.3. Loss Function Alignment - extractive PDF cue:** In low-light situations, [25] proposed a regularized L2 loss LReg-L2 = ((sg(ˆy) + ϵ)-1 ⊙(ˆy -y))2, (19) to boost the weight of the dark regions ...
- **p. 5 / 3.3. Loss Function Alignment - extractive PDF cue:** To model the smoothness of volumetric medium, we employ LReg-L2 as our pixel-level loss.
- **p. 3 / 3.1. Preliminaries - extractive PDF cue:** During optimization, 3DGS periodically densify Gaussians with high average gradient on 2D coordinates ˆµi across frames via splitting large ones and duplicating small ones.
- **p. 5 / 3.3. Loss Function Alignment - extractive PDF cue:** and the regularized D-SSIM loss LReg-DSSIM = LDSSIM(W ⊙y, W ⊙ˆy).

## Design Rationale

- **p. 2 / 1. Introduction - extractive PDF cue:** Loss Function Alignment: We propose a novel loss function designed to align 3DGS with human perception of High Dynamic Range (HDR) and low-light scenes.
- **p. 2 / 1. Introduction - extractive PDF cue:** Splatting with Medium: We introduce a novel approach that combines the strengths of Gaussian Splatting (GS) and volume rendering.
- **p. 3 / 3.2. Splatting with Medium - extractive PDF cue:** We illustrate the pipeline of our method in Fig.
...
- **p. 3 / 3.1. Preliminaries - extractive PDF cue:** During optimization, 3DGS periodically densify Gaussians with high average gradient on 2D coordinates ˆµi across frames via splitting large ones and duplicating small ones.
- **p. 5 / 3.3. Loss Function Alignment - extractive PDF cue:** and the regularized D-SSIM loss LReg-DSSIM = LDSSIM(W ⊙y, W ⊙ˆy).
- **p. 3 / 3. Method - extractive PDF cue:** At last, we explain our proposed loss function to align 3DGS with human perception of HDR scenes in Sec.
...
- **p. 3 / 3.2. Splatting with Medium - extractive PDF cue:** The input to our model is a set of images with scattering medium and corresponding camera poses.
- **p. 3 / 3.1. Preliminaries - extractive PDF cue:** In the meantime, 3DGS prunes primitives with low opacity for acceleration and periodically set αi close to zero for all Gaussians to moderate the increase ...
- **p. 4 / 3.2. Splatting with Medium - extractive PDF cue:** (11) The contributed color of the Gi to final output is Cobj i (r) = T obj i T med(si)αici = T obj i αiciexp(-σmedsi), ...
- **p. 4 / 3.3. Loss Function Alignment - extractive PDF cue:** For the case of our 3DGS-based model, we propose a regularized loss function LReg: we apply pixel-wise weight W = {wi,j} on both rendered estimate ...
- **p. 5 / 3.3. Loss Function Alignment - extractive PDF cue:** Our final proposed loss function is LReg = (1 -λ)LReg-L2 + λLReg-DSSIM .
- **p. 2 / 1. Introduction - extractive PDF cue:** The results of our evaluation demonstrate the effectiveness of our proposed method in achieving high-quality, efficient underwater reconstruction.
- **p. 2 / 1. Introduction - extractive PDF cue:** To address this issue, a NeRF approach, SeaThru-NeRF [18], was proposed, which uses two fields: one for the geometry and one for the volume in ...
...
- **p. 7 / 4.2. Ablation Study - extractive PDF cue:** We conduct a quantitative analysis on different combination of loss functions, between pixel-wise component {L1, L2, LReg-L1, LReg-L2} and frame-wise {LDSSIM, LReg-DSSIM}, as well as ...
...
- **p. 6 / 4.1. Results - extractive PDF cue:** For Restoration, we further show the rendered medium without rendering objects.
- **p. 6 / 4.1. Results - extractive PDF cue:** We compare our method with SeaThru-NeRF by showing both the full image and the rendering without the medium.
- **p. 7 / 4.1. Results - extractive PDF cue:** Our rendering without medium and depth maps significantly outperform those from the SeaThru-NeRF, especially in scenes that are farther from the camera.
- **p. 8 / Figure/Table caption - extractive PDF cue:** Figure 8. Limitation: insufficient supervision. Our method (left) has low-detail visuals in regions not sufficiently covered by train- ing views. SeaThru-NeRF [18] (right) is blurry ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Table 3. Ablation Study Avg. over SeaThru-NeRF Scenes Configuration PSNR↑ SSIM↑ LPIPS↓ L1+LDSSIM 29.219 0.915
