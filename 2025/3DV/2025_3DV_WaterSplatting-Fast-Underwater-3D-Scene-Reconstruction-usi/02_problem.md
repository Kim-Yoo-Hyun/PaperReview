...
- **p. 1 / Abstract - extractive PDF cue:** The underwater 3D scene reconstruction is a challenging, yet interesting problem with applications ranging from naval robots to VR experiences.
- **p. 1 / Abstract - extractive PDF cue:** The problem was successfully tackled by fully volumetric NeRF-based methods which can model both the geometry and the medium (water).
- **p. 1 / Abstract - extractive PDF cue:** Unfortunately, these methods are slow to train and do not offer real-time rendering.
- **p. 1 / Abstract - extractive PDF cue:** More recently, 3D Gaussian Splatting (3DGS) method offered a fast alternative to NeRFs.
- **p. 1 / Abstract - extractive PDF cue:** However, because it is an explicit method that renders only the geometry, it cannot render the medium and is therefore unsuited for underwater reconstruction.
...
| Observation / input | The input to our model is a set of images with scattering medium and corresponding camera poses. | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF |
...
PDF contribution framing (p. 2 (1. Introduction), p. 2 (1. Introduction), p. 3 (3.2. Splatting with Medium), p. 4 (3.3. Loss Function Alignment), p. 3 (3.2. Splatting with Medium)): Loss Function Alignment: We propose a novel loss function designed to align 3DGS with human perception of High Dynamic Range (HDR) and low-light scenes.
...
- **p. 3 / 3.2. Splatting with Medium - extractive PDF cue:** Under the occlusion of both primitives and medium, our model acquires the transmittance along the ray and is capable of synthesizing medium component and object ...
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 3 (3.2. Splatting with Medium), p. 3 (3.1. Preliminaries), p. 4 (3.2. Splatting with Medium), p. 4 (3.3. Loss Function Alignment). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (Abstract), p. 1 (Abstract), interface p. 3 (3.2. Splatting with Medium), p. 3 (3.1. Preliminaries), p. 4 (3.2. Splatting with Medium), p. 4 (3.3. Loss Function Alignment), objective p. 4 (3.3. Loss Function Alignment), p. 3 (3. Method), p. 3 (3.1. Preliminaries), p. 4 (3.3. Loss Function Alignment), p. 5 (3.3. Loss Function Alignment), p. 5 (3.3. Loss Function Alignment).
