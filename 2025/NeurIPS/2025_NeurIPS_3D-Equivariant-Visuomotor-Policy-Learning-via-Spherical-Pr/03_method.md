...
- **p. 16 / C Implementation of Our Policy - extractive body cue:** Our model consists of an SO(3)-equivariant observation encoder followed by an SO(3)-equivariant diffusion module, both implemented using escnn [3] and e3nn [57].
- **p. 16 / C Implementation of Our Policy - extractive body cue:** Given an observation x ∈X, the SO(2)-equivariant image encoder λ first maps the RGB image I into a regular representation, which is then mapped to ...
- **p. 6 / 4 Method - extractive body cue:** First, due to the SO(3)-equivariant encoder (Proposition 1) and the SO(3)-equivariant diffusion model (Section 4.2), our policy has end-to-end symmetry to global scene SO(3) rotations.
...
- **p. 5 / 4 Method - extractive body cue:** We represent this spherical signal in the spectral domain as truncated Fourier coefficients calculated using the spherical Fourier transform (Equation 1).
...
- **p. 5 / 4 Method - extractive body cue:** We represent this spherical signal in the spectral domain as truncated Fourier coefficients calculated using the spherical Fourier transform (Equation 1).
- **p. 6 / 4 Method - extractive body cue:** These properties are inherently preserved without requiring additional constraints.
- **p. 7 / 4 Method - extractive body cue:** Under a rotation of the gripper with respect to the workspace, no a priori constraint can be placed on how the action trajectory should transform.
- **p. 6 / 4 Method - extractive body cue:** Since the image is recorded in the camera frame, the spherical signal is unaffected, i.e. Φ(g·x) = Φ(x), while the camera pose updates as Rx ...
- **p. 7 / 4 Method - extractive body cue:** The advantage is empirically validated in Section 5.
- **p. 17 / C Implementation of Our Policy - extractive body cue:** We train all models using the AdamW [38] optimizer with Exponential Moving Average, and adopt the DDPM [15] framework with 100 denoising steps for both ...
...
- **p. 4 / 3 Background - extractive body cue:** Fourier Coefficients Gripper Orientation Figure 2: Overview of Image-to-Sphere Policy (ISP) (a) An SO(3)-equivariant observation encoder extracts features from the RGB input, projects them onto ...
- **p. 4 / 4 Method - extractive body cue:** The observation x ∈X consists of two parts, an eye-in-hand RGB image I, that captures visual information, and proprioceptive data, P ∈R7, including the end-effector's ...
- **p. 3 / 3 Background - extractive body cue:** Formally, given an observation O and diffusion timestep k, the policy predicts a noise estimate ϵk from a corrupted action sequence ak = a0 + ...
- **p. 3 / 3 Background - extractive body cue:** Given an observation sequence O = {ot-k+1, ..., ot} at timestep t, the learned policy predicts an action chunk A = {at+1, ..., at+n}, where ...
- **p. 1 / 1 Introduction - extractive body cue:** g Figure 1: We propose the first SO(3)-equivariant policy learning framework based on a single eyein-hand RGB image, where the predicted action sequence transforms equivariantly ...
- **p. 5 / 4 Method - extractive body cue:** First, we encode the input image I from the observation x using a standard SO(2)-equivariant image encoder λ.
- **p. 5 / 4 Method - extractive body cue:** Equivariance Correction is similar to a canonicalization map c : X →G, where fcano = c(x)f(c(x)-1x) transforms the input to a canonical frame, then transforms ...
...
- **p. 17 / C Implementation of Our Policy - extractive body cue:** In both cases, the denoising network outputs a sequence of 16 action steps, which are used for optimization during training, while only the first 8 ...
- **p. 17 / C Implementation of Our Policy - extractive body cue:** For all baselines, we retain their original hyperparameter settings for evaluation and only adjust the number of training steps to ensure consistency across methods.
...
- **p. 7 / 5 Experiments - extractive body cue:** For each task, we train three independent models with different random seeds (0, 1, and 2) for each of the 100- and 200-demonstration settings.
- **p. 8 / 68.7 58.7 58.0 32.0 54.3 (-6.7) - extractive body cue:** We compare the ISP-SO(2) with two variants: Pretraining, which initializes the image encoder with an 8
...
- **p. 7 / 5 Experiments - extractive body cue:** To ensure a fair comparison, all experiments in the following sections, including ablations and method variants, consistently apply SO(2) data augmentation during training by rotating ...
...
- **p. 8 / 68.7 58.7 58.0 32.0 54.3 (-6.7) - extractive body cue:** (3) EquiU: With or without an equivariant temporal denoising U-Net in the diffusion module.
- **p. 9 / 68.7 58.7 58.0 32.0 54.3 (-6.7) - extractive body cue:** These results confirm the effectiveness of our equivariant design in addressing diverse manipulation challenges in the real world.
- **p. 18 / C Implementation of Our Policy - extractive body cue:** Specifically, we evaluated (a) a variant of our method without rotation correction that uses delta control, and (b) the original Diffusion Policy with delta control.
- **p. 7 / 5 Experiments - extractive body cue:** For eyein-hand control, we replace its image encoder with a standard ResNet [12], so only proprioception and denoising remain equivariant.
- **p. 9 / 68.7 58.7 58.0 32.0 54.3 (-6.7) - extractive body cue:** ImageNet-1k [50]-pretrained equivariant ResNet-18, and Scratch, which trains the entire model from random initialization.
...
- **Evidence anchors reviewed:** method p. 4 (4 Method), p. 17 (C Implementation of Our Policy), p. 7 (4 Method), p. 16 (C Implementation of Our Policy), p. 16 (C Implementation of Our Policy), p. 6 (4 Method), objective p. 5 (4 Method), p. 6 (4 Method), p. 7 (4 Method), p. 6 (4 Method), p. 7 (4 Method), p. 17 (C Implementation of Our Policy), temporal p. 3 (3 Background), p. 17 (C Implementation of Our Policy), p. 5 (4 Method), p. 17 (C Implementation of Our Policy), p. 18 (C Implementation of Our Policy), p. 2 (2 Related Work).
