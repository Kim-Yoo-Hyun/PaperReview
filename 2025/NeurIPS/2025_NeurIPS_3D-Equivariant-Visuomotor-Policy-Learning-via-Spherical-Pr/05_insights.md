...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: The observation x ∈ X consists of two parts, an eye-in-hand RGB image I, that captures visual information, and proprioceptive data, P ∈ R7 , including the end-effector’s ...
- 출발 문제 단서: When used with RGB data, current equivariant policies are unable to fully leverage the SO(3) structure present in the problem and underperform the point cloud version significantly .
- 주장된 효과 단서: We perform extensive experiments in both simulation and the real world that demonstrate that our method consistently outperforms strong baselines in terms of both performance and sample efficiency.
...
- Result cue: We perform extensive experiments in both simulation and the real world that demonstrate that our method consistently outperforms strong baselines in terms of both performance and sample efficiency.
