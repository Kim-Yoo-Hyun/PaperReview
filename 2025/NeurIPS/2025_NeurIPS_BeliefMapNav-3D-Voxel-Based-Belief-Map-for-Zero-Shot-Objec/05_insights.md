...
- **Claim/result cue:** Experiments on HM3D and HSSD benchmarks show that BeliefMapNav achieves state-of-the-art (SOTA) Success Rate (SR) and Success weighted by Path Length (SPL), with a notable 9.7 SPL improvement ...
...
- **Registry interface:** `3D Vision, Navigation` is the paper's recorded topic/interface, not evidence that the full robotics loop was evaluated.
...
- Registry position: `Embodied Navigation and Mapping`; tags: `3D Vision, Navigation`.
