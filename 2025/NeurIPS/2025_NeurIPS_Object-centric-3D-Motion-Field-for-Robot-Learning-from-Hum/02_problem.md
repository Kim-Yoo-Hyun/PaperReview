...
- **p. 1 / Abstract - extractive PDF cue:** Learning robot control policies from human videos is a promising direction for scaling up robot learning.
- **p. 1 / Abstract - extractive PDF cue:** However, how to extract action knowledge (or action representations) from videos for policy learning remains a key challenge.
...
- **p. 1 / 1 Introduction - extractive PDF cue:** Recently, human-object interaction videos stand out as a particularly promising avenue to overcome this challenge.
- **p. 1 / 1 Introduction - extractive PDF cue:** Data is the primary bottleneck in robot learning - collecting large-scale high quality robotic data in real world at scale for training control policies is ...
...
| Observation / input | Dual Head UNet UNet Blocks concat Depth 3D PixelFlow Intrinsics Map Depth Motion Camera Intrinsics Phase I Phase II Input concat Output ... | multi-view observation, language/task label과 action trajectory | exact sensor/frame/preprocessing from PDF |
...
- **p. 1 / 1 Introduction - extractive PDF cue:** Data is the primary bottleneck in robot learning - collecting large-scale high quality robotic data in real world at scale for training control policies is ...
- **p. 2 / 1 Introduction - extractive PDF cue:** Point-cloud 3D flow is noisy and cannot represent motion accurately.
- **p. 2 / 1 Introduction - extractive PDF cue:** Due to this data collection challenge, many works look into the feasibility of using real-world actionfree videos for robot learning.
- **p. 4 / 2 Preliminaries - extractive PDF cue:** We first discuss a very simple pipeline for this purpose as suggested by latest works [55] and its fundamental limitations, and then we introduce our ...

## What the Paper Changes

PDF contribution framing (p. 3 (1 Introduction), p. 3 (1 Introduction), p. 1 (Abstract), p. 1 (Abstract), p. 4 (2 Preliminaries)): We present a simple and novel architecture that can learn to see and predict object-centric 3D motion field in the real world for control.
...
- **p. 1 / Abstract - extractive PDF cue:** We introduce two novel components in its implementation.
- **p. 1 / Abstract - extractive PDF cue:** Experiments show that our method reduces 3D motion estimation error by over 50% compared to the latest method, achieve 55% average success rate in diverse ...
- **p. 4 / 2 Preliminaries - extractive PDF cue:** We first discuss a very simple pipeline for this purpose as suggested by latest works [55] and its fundamental limitations, and then we introduce our ...
...
robot_data writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 5 (2 Preliminaries), p. 2 (1 Introduction), p. 3 (2 Preliminaries), p. 5 (2 Preliminaries). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (1 Introduction), p. 1 (1 Introduction), p. 2 (1 Introduction), p. 2 (1 Introduction), p. 4 (2 Preliminaries), interface p. 5 (2 Preliminaries), p. 2 (1 Introduction), p. 3 (2 Preliminaries), p. 5 (2 Preliminaries), objective p. 1 (Abstract), p. 3 (2 Preliminaries), p. 6 (2 Preliminaries), p. 6 (2 Preliminaries), p. 7 (2 Preliminaries), p. 7 (2 Preliminaries).
