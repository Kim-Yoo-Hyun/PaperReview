...
- **p. 5 / 3 Method - extractive body cue:** To enhance the model's flexibility and generalization, we introduce a time encoding token eT , which captures the relative temporal offset between the historical and ...
- **p. 4 / 3 Method - extractive body cue:** 𝑥 𝑦 𝑧 Spatial PE … Vision encoder … Sampled inputs LLM Transformer … … … Tokens from frame k Last hidden feature Tokens from ...
- **p. 8 / Method - extractive body cue:** We use 4 fixed cameras to capture each demonstration from different angles, collecting 50 trajectories per task per camera-resulting in a total of 200 trajectories ...
- **p. 5 / 3 Method - extractive body cue:** Our total training loss can be written as follows: L = Lt + Lr + Lg + λdLd, (3) where the translation loss Lt = ...
...
- **p. 5 / 3 Method - extractive body cue:** 3.4 Loss functions Algorithm 1: memory bank sampling Input: t, {It-j / j = 0, 1, . . . , n -1}, sample size k, ...
- **p. 5 / 3 Method - extractive body cue:** Our total training loss can be written as follows: L = Lt + Lr + Lg + λdLd, (3) where the translation loss Lt = ...
- **p. 3 / 3 Method - extractive body cue:** These multimodal tokens are then fed into the decoder D for next-token prediction.
- **p. 4 / 3 Method - extractive body cue:** 3.2 Spatial-aware visual tokens A reasonable action prediction requires awareness of both semantic perception and spatial perception of the scene.
- **p. 8 / Method - extractive body cue:** This task emphasizes the need for fine-grained action prediction.
- **p. 4 / 3 Method - extractive body cue:** Specifically, given current timestamp t, all image observations {It-j / j = 0, 1, 2, . . . , n-1} with a temporal window n, ...
...
- **p. 3 / 3 Method - extractive body cue:** Specifically, VLA with a low-level control policy refers to a class of models that use the current observations as input to predict an action for ...
- **p. 5 / 3 Method - extractive body cue:** 3.4 Loss functions Algorithm 1: memory bank sampling Input: t, {It-j / j = 0, 1, . . . , n -1}, sample size k, ...
- **p. 3 / 3 Method - extractive body cue:** 3.1 Preliminary Problem definition The vision-language action (VLA) model takes a language instruction as input and aims to control a robot to accomplish the specified ...
- **p. 4 / 3 Method - extractive body cue:** 𝑥 𝑦 𝑧 Spatial PE … Vision encoder … Sampled inputs LLM Transformer … … … Tokens from frame k Last hidden feature Tokens from ...
- **p. 2 / 1 Introduction - extractive body cue:** A real-world action distribution can be interpreted as a response function conditioned on observations or input, denoted as At(input).
- **p. 8 / Method - extractive body cue:** We set InternVL-4B with single RGB image inputs followed by an action head as our Base VLA model.
- **p. 2 / 1 Introduction - extractive body cue:** Previous approaches, such as OpenVLA [1], use only a single RGB image and a textual instruction as input.
...
- **p. 3 / 3 Method - extractive body cue:** Vision-language model backbone We leverage a pretrained large vision-language model (VLM) as the backbone, specifically InternVL-4B [12], which consists of a text tokenizer T , ...
- **p. 8 / Method - extractive body cue:** We use 4 fixed cameras to capture each demonstration from different angles, collecting 50 trajectories per task per camera-resulting in a total of 200 trajectories ...
- **p. 7 / 4 Experiments - extractive body cue:** We employed a cosine learning rate scheduler with a learning rate of 4e-5, using a batch size of 128 and training for 20 epochs. λd ...
...
- **p. 15 / Figure/Table caption - extractive body cue:** Table 7: Ablations on heads and inputs (Libero-Long). Left: action head vs. FPS and success (MLP, autoregressive, diffusion). Right: effect of pretraining, 3D coordinate embedding, ...
- **p. 15 / Figure/Table caption - extractive body cue:** Table 8: Frame sampling ablations on Libero-Spatial. MBS attains the highest success (0.866) with competitive efficiency, while single-frame is fastest and most memory-light but less ...
- **p. 6 / 4 Experiments - extractive body cue:** We remove frames with unchanged proprioception, specifically the stationary frames, and exclude trajectories with a total action count exceeding 600.
- **p. 7 / 4 Experiments - extractive body cue:** Unlike the pretraining phase, we used the simplest input settings to enable our model to learn the interaction effects between 3D information and historical data ...
...
- **p. 7 / 4 Experiments - extractive body cue:** 4.3 LIBERO evaluation After pretraining, we fine-tune and conduct close-loop testing in the LIBERO simulation environment, using task success rate as the metric to evaluate ...
...
- **Evidence anchors reviewed:** method p. 5 (3 Method), p. 3 (3 Method), p. 3 (3 Method), p. 8 (Method), p. 5 (3 Method), p. 4 (3 Method), objective p. 5 (3 Method), p. 5 (3 Method), p. 3 (3 Method), p. 4 (3 Method), p. 8 (Method), p. 4 (3 Method), temporal p. 8 (Method), p. 4 (3 Method), p. 4 (3 Method), p. 5 (3 Method), p. 5 (3 Method), p. 6 (4 Experiments).
