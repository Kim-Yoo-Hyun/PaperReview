...
- **p. 1 / Abstract - extractive PDF cue:** Pose-free Neural Radiance Field (NeRF) aims at novel view synthesis (NVS) without relying on accurate poses, exhibiting significant practical value.
- **p. 1 / Abstract - extractive PDF cue:** Image and LiDAR point cloud are two pivotal modalities in autonomous driving scenarios.
- **p. 1 / Abstract - extractive PDF cue:** While demonstrating impressive performance, single-modality pose-free NeRFs often suffer from local optima due to the limited geometric information provided by dense image textures or the ...
- **p. 1 / Abstract - extractive PDF cue:** Although prior methods have explored the complementary strengths of both modalities, they have only leveraged inherently sparse point clouds for discrete, nonpixel-wise depth supervision, and ...
- **p. 1 / Abstract - extractive PDF cue:** As a result, a Multimodal Unified Pose-free framework remains notably absent.
- **p. 2 / 1 Introduction - extractive PDF cue:** Nevertheless, prior research [37] has faced challenges due to the significant domain gap and uncoordinated convergence problems [27, 42, 34] between these modalities.
- **p. 2 / 1 Introduction - extractive PDF cue:** Compared to continuous LiDAR-Camera Fields, projecting LiDAR point clouds onto images as discrete depth priors fails to provide continuous, pixel-wise supervision.
...
| Observation / input | For the image modality, we use a lightweight MLP to refine the geo-MLP output, helping reduce modality conflicts. | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF |
...
- **p. 2 / 1 Introduction - extractive PDF cue:** Compared to continuous LiDAR-Camera Fields, projecting LiDAR point clouds onto images as discrete depth priors fails to provide continuous, pixel-wise supervision.
- **p. 1 / 1 Introduction - extractive PDF cue:** However, existing pose-free NeRFs have largely concentrated on single modalities, particularly on images.
- **p. 1 / 1 Introduction - extractive PDF cue:** Nevertheless, due to the lack of geometric consistency, relying solely on rich texture with39th Conference on Neural Information Processing Systems (NeurIPS 2025).
- **p. 3 / 1 Introduction - extractive PDF cue:** Comprehensive experiments demonstrate that MUP significantly outperforms prior state-of-the-art techniques and single-modality approaches by a large margin in both registration and NVS.

## What the Paper Changes

PDF contribution framing (p. 3 (1 Introduction), p. 2 (1 Introduction), p. 2 (1 Introduction), p. 3 (1 Introduction), p. 5 (4 Methodology)): In summary, our primary contributions can be delineated as follows: (1) We propose MUP, a unified pose-free framework that combines the advantages of two modalities for pose estimation and multimodal ...
...
- **p. 3 / 1 Introduction - extractive PDF cue:** We evaluate our method across diverse scenarios using the KITTI-360 [15] and NuScenes [4] autonomous driving datasets.
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 5 (4 Methodology), p. 5 (4 Methodology), p. 7 (4 Methodology), p. 2 (1 Introduction). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (1 Introduction), p. 2 (1 Introduction), p. 1 (1 Introduction), p. 1 (1 Introduction), p. 3 (1 Introduction), interface p. 5 (4 Methodology), p. 5 (4 Methodology), p. 7 (4 Methodology), p. 2 (1 Introduction), objective p. 5 (4 Methodology), p. 6 (4 Methodology), p. 5 (4 Methodology), p. 6 (4 Methodology), p. 7 (4 Methodology), p. 7 (4 Methodology).
