...
PDF body evaluation/result cue (p. 9 (4 Experiments), p. 8 (4 Experiments), p. 8 (4 Experiments), p. 16 (Figure/Table caption), p. 18 (Figure/Table caption), p. 7 (4 Experiments)): On the sparse unbounded-drone dataset, our approach significantly outperforms the SoTA methods with improvements of 2.54 dB in PSNR, 0.089 in SSIM, and cuts LPIPS and DSIM by 33.8% and ...

## Evaluation Body Digest

- **p. 7 / 4 Experiments - extractive body cue:** 4.1 Datasets We evaluate the performance of MS-GS and current SoTA methods on three real-world scenes with sparse inputs-one with single appearance and two with ...
- **p. 7 / 4 Experiments - extractive body cue:** Sparse Mip-NeRF 360 Dataset [33] contains 4 outdoor and 4 indoor scenes with a complex central object or area and a detailed background.
- **p. 15 / A.1.2 Comparison with prior benchmark - extractive body cue:** Additionally, our dataset contains scenes with 360-degree coverage by perspective cameras, whereas Phototourism is covered by face-forward images.
- **p. 8 / 4 Experiments - extractive body cue:** 4.4 Comparisons Table 2: Quantitative Comparison on sparse Mip-NeRF 360 dataset; bold numbers are the best, underscored second best.
- **p. 8 / 4 Experiments - extractive body cue:** In the Wild GT Pose Method LPIPSÓ DSIMÓ PSNRÒ SSIMÒ LPIPSÓ DSIMÓ DRGS[23] 0.588 0.273 19.16 0.516 0.544 0.253 DNGS[9] 0.503 0.193 19.79 0.588 0.466 ...
- **p. 9 / 4 Experiments - extractive body cue:** 5 present results on these benchmarks.
...
- **p. 15 / A.1 Sparse unbounded drone dataset - extractive body cue:** (a) Sunny (b) Snowy (c) Cloudy (d) Low-light Figure 6: Dataset visualizations
...
- **Detected evaluation headings:** 4 Experiments (p. 7); A.1 Sparse unbounded drone dataset (p. 15); A.1.2 Comparison with prior benchmark (p. 15); A.2 Experiments and visualizations (p. 15); A.3 Implementation details (p. 17); A.6 In-the-wild evaluation (p. 19); A.6.2 Evaluation metrics (p. 20).
...
| Figure/Table caption | EMPIRICAL / REAL-ROBOT OR HARDWARE | Figure 8: Visualizations of rendering with different point cloud initializations. dense point cloud, usually millions of points. To utilize DUSt3R points, we align them ... | p. 16 (Figure/Table caption) |
| Figure/Table caption | EMPIRICAL / REAL-ROBOT OR HARDWARE | Table 7: Experiments in 12-view setting, where each appearance has 3 images. MS-GS continues to outperform other methods. The metrics are reported as the ... | p. 18 (Figure/Table caption) |
...
- **p. 2 / Figure/Table caption - extractive body cue:** Figure 1: With 20 input views, DNGS and FSGS produce overly smooth rendering in regions lacking support from sparse point cloud initialization. For scenes with ...
- **p. 4 / Figure/Table caption - extractive body cue:** Figure 2: Overview of our depth prior initialization of MS-GS. Semantic masks and corresponding SfM point depth within each mask are obtained through our SfM-prompted ...
- **p. 6 / Figure/Table caption - extractive body cue:** Figure 3: Overview of our multi-view geometry-guided supervision of MS-GS. Initialized from our proposed dense point cloud, we first create virtual views between training cameras. ...
- **p. 7 / Figure/Table caption - extractive body cue:** Table 1: Ablation studies on different components of MS-GS. The metrics are reported as the average on the Sparse Unbounded drone dataset; bold numbers are ...
- **p. 8 / Figure/Table caption - extractive body cue:** Figure 4: Novel view synthesis results when components are added sequentially. Please zoom in if possible for better visualization. 4.3
- **p. 8 / Figure/Table caption - extractive body cue:** Table 2: Quantitative Comparison on sparse Mip-NeRF 360 dataset; bold numbers are the best, underscored second best. We only evaluate LPIPS and DSIM for in-the-wild ...
- **p. 9 / Figure/Table caption - extractive body cue:** Table 3: Quantitative Comparison on sparse unbounded drone dataset. Methods: renders each test view with the appearance embedding taken from the training image that is ...
- **p. 9 / Figure/Table caption - extractive body cue:** Table 4: Quantitative Comparison on sparse Phototourism dataset. Methods; optimize appearance embedding on the left half of the test image and evaluate on the other ...
...
| Task/environment | Sparse Mip-NeRF 360 Dataset [33] contains 4 outdoor and 4 indoor scenes with a complex central object or area and a detailed background. | reset, timeout, object/scene variation | p. 7 (4 Experiments), p. 15 (A.1.2 Comparison with prior benchmark) |
...
| A previous approach [11] has tried to perform re-triangulation based on known train poses, but does not account for pose inaccuracy. | definition/direction/unit from same section | p. 19 (A.6 In-the-wild evaluation) |
| This analysis indicates that DSIM is an appropriate metric for in-the-wild evaluations: it avoids over-penalising inevitable alignment errors while still capturing real perceptual degradation. | definition/direction/unit from same section | p. 21 (A.6.2 Evaluation metrics) |
| Figure 9: Error maps after alignment. Brighter means higher error. A.2.2 Edge cases (a) GT (b) Render (c) GT (d) Render | definition/direction/unit from same section | p. 17 (Figure/Table caption) |
| Figure 12: Illustration of Coordinate Alignment. We first compute the transformation M ˚ between train cameras in two coordinate systems Cref train and Cinput ... | definition/direction/unit from same section | p. 20 (Figure/Table caption) |
...
| The baseline introduced in our ablation study Section 4.3 uses the same Splatfacto model. | comparison identity and matched condition | p. 17 (A.3 Implementation details) |
| Figure 8: Visualizations of rendering with different point cloud initializations. dense point cloud, usually millions of points. To utilize DUSt3R points, we align them ... | comparison identity and matched condition | p. 16 (Figure/Table caption) |
| Table 7: Experiments in 12-view setting, where each appearance has 3 images. MS-GS continues to outperform other methods. The metrics are reported as the ... | comparison identity and matched condition | p. 18 (Figure/Table caption) |
...
| The baseline introduced in our ablation study Section 4.3 uses the same Splatfacto model. | component/input/data sensitivity | p. 17 (A.3 Implementation details) |
| Figure 10: As MS-GS favors more accurate local alignment, areas without dense initialization can introduce artifacts in (a) and (b). Specular highlights can be ... | component/input/data sensitivity | p. 17 (Figure/Table caption) |

## Main Results / Claim–Evidence Map

| Claim or target | Result/condition cue | Evidence strength | Anchor |
|---|---|---|---|
| In summary, the main contributions of our work are: • We introduce a Semantic Depth Alignment approach, which leverages monocular depths in local semantic ... | On the sparse unbounded-drone dataset, our approach significantly outperforms the SoTA methods with improvements of 2.54 dB in PSNR, 0.089 in SSIM, and cuts ... | PDF body cue; verify exact table/figure and matched conditions | p. 9 (4 Experiments), p. 8 (4 Experiments), p. 8 (4 Experiments), p. 16 (Figure/Table caption), p. 18 (Figure/Table caption), p. 7 (4 Experiments) |
...
- **p. 17 / A.3 Implementation details - extractive body cue:** The appearance MLP consists of 3 layers of 64 hidden units.
- **p. 17 / A.3 Implementation details - extractive body cue:** The appearance MLP consists of 3 layers of 64 hidden units.
...
| body limitation/failure cue | A previous approach [11] has tried to perform re-triangulation based on known train poses, but does not account for pose inaccuracy. | p. 19 (A.6 In-the-wild evaluation) |
| body limitation/failure cue | This analysis indicates that DSIM is an appropriate metric for in-the-wild evaluations: it avoids over-penalising inevitable alignment errors while still capturing real perceptual degradation. | p. 21 (A.6.2 Evaluation metrics) |
...
| Results are obtained with the NVIDIA RTX A5500 GPU. | p. 17 (A.3 Implementation details) |
| The same hyperparameters are maintained throughout the experiments. | p. 17 (A.3 Implementation details) |
...
- **Evidence anchors reviewed:** datasets p. 7 (4 Experiments), p. 7 (4 Experiments), p. 15 (A.1.2 Comparison with prior benchmark), p. 8 (4 Experiments), p. 8 (4 Experiments), p. 9 (4 Experiments), metrics p. 8 (4 Experiments), p. 7 (4 Experiments), p. 8 (4 Experiments), p. 9 (4 Experiments), p. 19 (A.6 In-the-wild evaluation), p. 21 (A.6.2 Evaluation metrics), baselines p. 9 (4 Experiments), p. 8 (4 Experiments), p. 17 (A.3 Implementation details), p. 16 (Figure/Table caption), p. 18 (Figure/Table caption), p. 7 (4 Experiments), results p. 9 (4 Experiments), p. 8 (4 Experiments), p. 8 (4 Experiments), p. 16 (Figure/Table caption), p. 18 (Figure/Table caption), p. 7 (4 Experiments).
