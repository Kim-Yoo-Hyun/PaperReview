...
- **p. 3 / 3 Methodology - extractive PDF cue:** In robotic action prediction, most visual tokens remain static across frames except for key regions like the manipulator or target object.
- **p. 3 / 3 Methodology - extractive PDF cue:** By avoiding redundant computation of unchanged static tokens between adjacent frames, our approach directly alleviates the computation bottleneck of language decoder in VLA models while ...
- **p. 3 / 3 Methodology - extractive PDF cue:** However, most existing Vision-Language-Action (VLA) 3
- **p. 3 / 3 Methodology - extractive PDF cue:** (3) While KV caching is effective for language decoding within a single query in vision-language models, this technique does not address redundancy in the visual ...
- **p. 1 / 1 Introduction - extractive PDF cue:** Leveraging large-scale real-world robotic datasets [6, 7], pioneering works [8-11] have introduced Vision-Language-Action (VLA) models, which integrate vision and language modalities to directly generate robotic ...
...
- **p. 2 / 1 Introduction - extractive PDF cue:** To address the inefficiency introduced by repeatedly processing static visual information, we present VLA-Cache, a training-free inference acceleration method that exploits temporal continuity in robotic ...
- **p. 3 / 3 Methodology - extractive PDF cue:** To address this, we propose a method that identifies visually static tokens and filters out semantically important ones based on attention scores from the VLA ...
- **p. 3 / 3 Methodology - extractive PDF cue:** In the following sections, we introduce its core mechanisms: static token selection, task-relevance filtering, and layer-adaptive reuse to accelerate VLA inference while preserving action accuracy.
...
- **p. 1 / 1 Introduction - extractive PDF cue:** Leveraging large-scale real-world robotic datasets [6, 7], pioneering works [8-11] have introduced Vision-Language-Action (VLA) models, which integrate vision and language modalities to directly generate robotic ...
- **p. 1 / 1 Introduction - extractive PDF cue:** Learning a robust and generalizable policy for robotic manipulation through policy learning has long been a challenging problem [1], with traditional reinforcement learning approaches [2, ...
- **p. 2 / 1 Introduction - extractive PDF cue:** This motivates our proposed token caching mechanism, which explicitly exploits temporal redundancy in visual inputs to reduce redundant computation without compromising decision quality.
- **p. 2 / 1 Introduction - extractive PDF cue:** result, they struggle to achieve an optimal balance between inference speed and action accuracy.
...
- **p. 3 / 3 Methodology - extractive PDF cue:** In the following sections, we introduce its core mechanisms: static token selection, task-relevance filtering, and layer-adaptive reuse to accelerate VLA inference while preserving action accuracy.
- **p. 9 / 5 Experiment - extractive PDF cue:** The method also achieves considerable reductions in FLOPs and inference time.
- **p. 9 / 5 Experiment - extractive PDF cue:** The efficiency gains are evident in the FLOPs and inference time measurements.
...
| Action / skill decoding | Specifically, we adopt two state-of-the-art token-level acceleration techniques SparseVLM [30] and FastV [29] on OpenVLA as compared methods in the LIBERO benchmark. | p. 7 (5 Experiment), p. 22 (Figure/Table caption) |
...
- **p. 23 / Figure/Table caption - extractive PDF cue:** Table 10: Varying the relevance threshold τ (with k=100). Overall, efficiency (FLOPs and latency) improves monotonically with larger k and τ, while success rate remains ...
- **p. 7 / 5 Experiment - extractive PDF cue:** The SIMPLER simulator [18] offers two settings, Visual Matching and Variant Aggregation, designed to bridge simulation-to-reality gaps.
- **p. 8 / 5 Experiment - extractive PDF cue:** Ablation on Token Reusing/Pruning Rate.
...
- **p. 22 / Figure/Table caption - extractive PDF cue:** Figure 5: VLA-Cache test results and attention heat map in a simulated environment E.3 Additional Ablations and Comparisons Attention vs. object-mask proxies for task relevance. ...
- **p. 23 / Figure/Table caption - extractive PDF cue:** Table 8: Attention vs. object-mask proxies for task relevance on LIBERO-SPATIAL. While object masks provide spatial localization, they can miss fine-grained or contextual signals essential ...
...
- **PDF anchors reviewed:** method p. 3 (3 Methodology), p. 3 (3 Methodology), objective p. 3 (3 Methodology), p. 3 (3 Methodology), temporal p. 8 (5 Experiment), p. 4 (X X), p. 6 (V Hl), p. 6 (V Hl), p. 2 (1 Introduction), p. 3 (3 Methodology).
