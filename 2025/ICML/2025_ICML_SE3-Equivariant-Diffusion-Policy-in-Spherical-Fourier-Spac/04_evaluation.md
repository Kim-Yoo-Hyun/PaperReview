...
> Analysis basis: full-text PDF body checked on 2026-09-01 (20 pages; PyMuPDF text; extraction quality: high); canonical paper source: https://openreview.net/forum?id=U5nRMOs8Ed; PDF retrieval source: https://openreview.net/pdf/c5f710d0d4c4b6918752b49ce065c915bf7bdde3.pdf. The note is an evidence-anchored body analysis; exact tables/equations remain at the cited page anchors. Reading tracker status/evidence was not changed.
...
- **p. 6 / 5.1. Simulation Experiments - extractive PDF cue:** To evaluate robustness, we modify four MimicGen tasks with SE(3) initialization by randomly tilting the table within a defined range and randomly placing objects on ...
- **p. 7 / 5.2. Physical Experiments - extractive PDF cue:** Training Dataset from Human Demonstrations We use Gello (Wu et al., 2023) to collect demonstrations with objects initialized in random SE(3) poses.
- **p. 6 / 5.1. Simulation Experiments - extractive PDF cue:** Experimental Settings We conduct simulation experiments using the MimicGen (Mandlekar et al., 2023) environment, built on the Mujoco simulator (Todorov et al., 2012), which features ...
- **p. 7 / 5.2. Physical Experiments - extractive PDF cue:** Five physical robotic manipulation tasks.
- **p. 8 / 5.2. Physical Experiments - extractive PDF cue:** Success rate (%) of 5 physical experiments over 20 evaluation episodes.
- **p. 8 / 5.2. Physical Experiments - extractive PDF cue:** The action space and number of training demonstrations are listed under each task.
- **p. 6 / 5.1. Simulation Experiments - extractive PDF cue:** We report the maximum test success rate throughout training, averaging results over 50 rollouts for each of the three seeds.
- **p. 7 / 5.2. Physical Experiments - extractive PDF cue:** Evaluation success rate on 12 MimicGen tasks with SE(2) initialization.
...
- **p. 1 / Figure/Table caption - extractive PDF cue:** Figure 1. SDP enforces that the policy is SO(3) equivariant. Specifically, in the second row, an SO(3) rotation that is applied to the scene leads ...
- **p. 4 / Figure/Table caption - extractive PDF cue:** Figure 2. Method overview. During inference, SDP first embeds state St into a spherical scene feature Ct by the encoder enc. Then, SDTU ϵθ estimates ...
- **p. 5 / Figure/Table caption - extractive PDF cue:** Figure 3. Spherical denoising temporal U-net (SDTU). Left: The SDTU ϵθ estimates the noise ϵ, based on the noisy actions Ak t , denoising step ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Figure 4. MimicGen tasks with SE(3) initialization ((a)-(c), showing 1 of 4 tasks) and SE(2) initialization ((d)-(f), showing 3 of 12 tasks).
- **p. 7 / Figure/Table caption - extractive PDF cue:** Table 1. Evaluation success rate on 4 MimicGen tasks with 3 levels of SE(3) initialization. We train all the baselines on progressively tilted environments with ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Table 2. Evaluation success rate on 12 MimicGen tasks with SE(2) initialization. We train all the baselines with 100 demonstrations. SDP demonstrates the best performance ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Figure 5. Five physical robotic manipulation tasks. Push Eraser we also tested 2048 points). The actions are the 6 DoF gripper poses for single-arm tasks ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Table 3. Success rate (%) of 5 physical experiments over 20 evalu- ation episodes. The action space and number of training demon- strations are listed ...
...
- **p. 6 / 5.1. Simulation Experiments - extractive PDF cue:** The RGB images have a resolution of 84 × 84 × 3, while RGBD data can be used to reconstruct either 3D colored voxels (843) ...
- **p. 6 / 5.1. Simulation Experiments - extractive PDF cue:** We report the maximum test success rate throughout training, averaging results over 50 rollouts for each of the three seeds.
- **p. 6 / 5.1. Simulation Experiments - extractive PDF cue:** Results on Tasks with SE(2) Initialization Table 2 shows that SDP outperforms all baselines across 10 tasks, except for Coffee and Coffee Preparation.
- **p. 6 / 5.2. Physical Experiments - extractive PDF cue:** Point clouds with 1024 points are reconstructed from the RGBD images (for 6
- **p. 7 / 5.2. Physical Experiments - extractive PDF cue:** MimicGen tasks with SE(3) initialization ((a)-(c), showing 1 of 4 tasks) and SE(2) initialization ((d)-(f), showing 3 of 12 tasks).
- **p. 7 / 5.2. Physical Experiments - extractive PDF cue:** SDP demonstrates the best performance on 10 tasks.
...
- **p. 9 / 6. Conclusion and Limitations - extractive PDF cue:** One limitation of the proposed method is that it operates in position control, ignoring contact forces, which leads to protective stops in the Flip Book ...
- **p. 9 / 6. Conclusion and Limitations - extractive PDF cue:** Another limitation is the lowresolution point cloud processing in the observation encoder, which struggles to capture fine details, such as these in the Push Eraser ...
- **p. 4 / Figure/Table caption - extractive PDF cue:** Figure 2. Method overview. During inference, SDP first embeds state St into a spherical scene feature Ct by the encoder enc. Then, SDTU ϵθ estimates ...
- **p. 5 / Figure/Table caption - extractive PDF cue:** Figure 3. Spherical denoising temporal U-net (SDTU). Left: The SDTU ϵθ estimates the noise ϵ, based on the noisy actions Ak t , denoising step ...
- **p. 6 / 5.2. Physical Experiments - extractive PDF cue:** The observations are captured by two stationary RGBD cameras positioned above the workspace to minimize occlusion.
- **p. 6 / 5.1. Simulation Experiments - extractive PDF cue:** We hypothesize that this drop is caused by pointcloud occlusion and object instability due to gravity, both of which disrupt SE(3) equivariance.

- **PDF anchors reviewed:** datasets p. 6 (5.1. Simulation Experiments), p. 7 (5.2. Physical Experiments), p. 6 (5.1. Simulation Experiments), p. 7 (5.2. Physical Experiments), p. 8 (5.2. Physical Experiments), p. 8 (5.2. Physical Experiments), metrics p. 6 (5.1. Simulation Experiments), p. 7 (5.2. Physical Experiments), p. 8 (5.2. Physical Experiments), p. 9 (Figure/Table caption), p. 9 (Figure/Table caption), p. 6 (5.1. Simulation Experiments), baselines p. 6 (5.1. Simulation Experiments), p. 6 (5.1. Simulation Experiments), p. 7 (5.2. Physical Experiments), p. 7 (5.2. Physical Experiments), p. 8 (Figure/Table caption), p. 9 (Figure/Table caption), results p. 6 (5.1. Simulation Experiments), p. 9 (Figure/Table caption), p. 6 (5.1. Simulation Experiments), p. 7 (Figure/Table caption), p. 8 (5.2. Physical Experiments), p. 9 (Figure/Table caption).
