...
- **p. 5 / 4.1. Environment Setup - extractive body cue:** We collect robotic datasets on multi-task scenes using a Franka robot, where there are multiple tasks that can be completed in the same scene.
- **p. 6 / 5.1. Real-world Experiments - extractive body cue:** This enables better visual grounding and generalization capabilities of OTTER to perform better on training and unseen tasks, despite being trained on a small robotic ...
...
- **p. 2 / 3. Empirical results suggest that OTTER's performance - extractive body cue:** on unseen tasks scales along multiple axes: through larger pre-trained vision-language encoders, increased policy network capacity, and pre-training on larger robot datasets.
- **p. 5 / 4.1. Environment Setup - extractive body cue:** The training tasks involve objects encountered during model training, whereas the unseen tasks test the model's ability to generalize to unseen objects or scenes.
- **p. 6 / 5.1. Real-world Experiments - extractive body cue:** Single Primitive We first evaluate all models on both the training and unseen tasks of the pick and place primitive in the real robot environment, ...
- **p. 7 / 5.2. Simulation Experiments - extractive body cue:** From Table 3, we found all the models perform similarly on training tasks in LIBERO due to the limited variations of the tasks and sufficient ...
- **p. 5 / 4.1. Environment Setup - extractive body cue:** The overall performance is measured by calculating the average success rate with standard error across all trials for the training and unseen tasks.
...
- **p. 1 / Figure/Table caption - extractive body cue:** Figure 1: (Top) Different feature extraction approaches in VLA models. (a) Direct Feature Passing: existing approaches, exem- plified by Octo and OpenVLA, extract and pass ...
- **p. 2 / Figure/Table caption - extractive body cue:** Figure 2: OTTER Model architecture. At each timestep t, text- aware visual features fvl are extracted from a pre-trained CLIP model (see Figure 3). Then ...
- **p. 4 / Figure/Table caption - extractive body cue:** Figure 3: Text-aware Visual Features Extraction We calcu- late the similarity between the visual patch features and per-token language features, then take the softmax over ...
- **p. 5 / Figure/Table caption - extractive body cue:** Figure 4: Example scenes in the simulation (left) and in the physical environments (right) using a Franka robot. to the proprioception representation, we express the ...
- **p. 6 / Figure/Table caption - extractive body cue:** Table 1: Physical Single Primitive Multi-task Experiments. For each model, we conduct physical robot pick and place exper- iments, with 100 trials on in-distribution training ...
...
- **p. 8 / Figure/Table caption - extractive body cue:** Table 4: Ablation results on LIBERO Object tasks and unseen tasks. We evaluate OTTER and other baselines on 100 trials of in- distribution LIBERO-Object tasks, ...
...
- **p. 5 / 4.1. Environment Setup - extractive body cue:** The in-distribution tasks are the 30 tasks in the original LIBERO-Spatial/Object/Goal, which can evaluate the model's multi-task learning capabilities.
...
- **p. 7 / 5.1. Real-world Experiments - extractive body cue:** The results further suggest that OTTER's generalization capabilities can be enhanced through increased model capacity (OTTER-L) and pre-training on large robotic datasets (OTTER-OXE). on unseen ...
- **p. 7 / 5.2. Simulation Experiments - extractive body cue:** For the unseen tasks, we change 10 tasks in LIBERO-90 with different objects and distractors to test the generalizability of all the models on unseen ...
