...
- **p. 1 / Abstract - extractive PDF cue:** We introduce Referring 3D Gaussian Splatting Segmentation (R3DGS), a new task that aims to segment target objects in a 3D Gaussian scene based on natural ...
- **p. 1 / Abstract - extractive PDF cue:** This task requires the model to identify newly described objects that may be occluded or not directly visible in a novel view, posing a significant ...
- **p. 1 / Abstract - extractive PDF cue:** Developing this capability is crucial for advancing embodied AI.
- **p. 1 / Abstract - extractive PDF cue:** To support research in this area, we construct the first R3DGS dataset, Ref-LERF.
- **p. 1 / Abstract - extractive PDF cue:** Our analysis reveals that 3D multimodal understanding and spatial relationship modeling are key challenges for R3DGS.
- **p. 2 / 1. Introduction - extractive PDF cue:** However, these methods face significant limitations when applied to R3DGS.
...
- **p. 1 / 1. Introduction - extractive PDF cue:** 1, R3DGS requires the model to identify newly described objects, even when occluded or not directly visible in the novel view, posing a significant challenge ...
- **p. 1 / 1. Introduction - extractive PDF cue:** To bridge this gap, we introduce a new task: Referring 3D Gaussian Splatting Segmentation (R3DGS), aims at segmenting objects in a 3D Gaussian scene based ...
- **p. 2 / 1. Introduction - extractive PDF cue:** One major drawback is the lack of interaction between the text query and Gaussian representations during training.
- **p. 3 / 3.2. Problem Statement and Method Overview - extractive PDF cue:** The key challenge lies in segmenting the target object in this unseen view, where it may be partially occluded or even entirely invisible.

## What the Paper Changes

PDF contribution framing (p. 1 (1. Introduction), p. 2 (1. Introduction), p. 2 (1. Introduction), p. 3 (3.2. Problem Statement and Method Overview), p. 4 (3.3. 3D Gaussian Referring Fields)): To bridge this gap, we introduce a new task: Referring 3D Gaussian Splatting Segmentation (R3DGS), aims at segmenting objects in a 3D Gaussian scene based on natural language expressions that ...
...
- **p. 3 / 3.2. Problem Statement and Method Overview - extractive PDF cue:** To infuse languageawareness into the 3D Gaussians, we introduce a new property called referring features.
- **p. 4 / 3.3. 3D Gaussian Referring Fields - extractive PDF cue:** 2, our method surpasses existing approaches, establishing a superior referring segmentation framework in 3D Gaussian scenes.
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 5 (3.5. Gaussian-Text Contrastive Learning), p. 2 (1. Introduction), p. 4 (3.3. 3D Gaussian Referring Fields), p. 4 (3.3. 3D Gaussian Referring Fields). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (1. Introduction), p. 1 (1. Introduction), p. 1 (1. Introduction), p. 2 (1. Introduction), p. 3 (3.2. Problem Statement and Method Overview), interface p. 5 (3.5. Gaussian-Text Contrastive Learning), p. 2 (1. Introduction), p. 4 (3.3. 3D Gaussian Referring Fields), p. 4 (3.3. 3D Gaussian Referring Fields), objective p. 6 (3.5. Gaussian-Text Contrastive Learning), p. 4 (3.3. 3D Gaussian Referring Fields), p. 5 (3.4. Position-aware Cross-Modal Interaction), p. 5 (3.4. Position-aware Cross-Modal Interaction).
