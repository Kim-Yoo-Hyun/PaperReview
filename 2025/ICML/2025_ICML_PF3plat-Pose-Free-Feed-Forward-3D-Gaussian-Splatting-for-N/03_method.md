...
- **p. 5 / 3.3. Loss Function - extractive body cue:** Combining the three loss functions, we define our final objective function: Limg + L2D-3D +λ3D-3DL3D-3D, where we set λ3D-3D = 0.05.
- **p. 5 / 3.3. Loss Function - extractive body cue:** We identify that provided good coarse alignments, RGB loss is sufficient, as similarly observed in (Ye et al., 2024), but with larger baselines, the training ...
- **p. 3 / 3.2.1. COARSE ALIGNMENT OF 3D GAUSSIANS - extractive body cue:** However, these solutions are incompatible with our goal of achieving a single feed-forward process with training solely from unposed images.
- **p. 5 / 3.3. Loss Function - extractive body cue:** Specifically, while the multi-view consistent surface loss projects the Gaussian center from one view to another using the estimated depth and camera pose, e.g., from ...
- **p. 4 / 3.2.4. 3D GAUSSIAN PARAMTER PREDICTIONS - extractive body cue:** We finally obtain an aggregated cost volume Cagg i by feeding Cmulti i and Cguide i to a series of cross-attention layers to update the ...
...
- **p. 5 / 3.3. Loss Function - extractive body cue:** Combining the three loss functions, we define our final objective function: Limg + L2D-3D +λ3D-3DL3D-3D, where we set λ3D-3D = 0.05.
- **p. 4 / 3.2.4. 3D GAUSSIAN PARAMTER PREDICTIONS - extractive body cue:** We finally obtain an aggregated cost volume Cagg i by feeding Cmulti i and Cguide i to a series of cross-attention layers to update the ...
- **p. 3 / 3.2.1. COARSE ALIGNMENT OF 3D GAUSSIANS - extractive body cue:** Such misalignments and sprase gradients can either cause severe performance degradation or disrupt the learning process.
- **p. 3 / 3.1. Problem Formulation - extractive body cue:** Our objective is to reconstruct a 3D scene from a set of N unposed images {Ii}N i=1 with Ii ∈RH×W ×3 and corresponding camera intrinsic ...
- **p. 4 / 3.2.4. 3D GAUSSIAN PARAMTER PREDICTIONS - extractive body cue:** Multi-View and Guidance Cost Volume Construction and Aggregation.
...
- **p. 3 / 3.2.1. COARSE ALIGNMENT OF 3D GAUSSIANS - extractive body cue:** This issue is particularly exacerbated when widebaseline images are given as input or the absence of groundtruth pose or depth prevents alignments of 3D Gaussians.
- **p. 3 / 3.1. Problem Formulation - extractive body cue:** To render, we output the depth maps Di ∈RH×W for each image Ii, along with their corresponding camera poses Pi ∈R3×4, consisting of a rotation ...
- **p. 5 / 3.2.4. 3D GAUSSIAN PARAMTER PREDICTIONS - extractive body cue:** This feedback loop enhances the accuracy of both depth and pose estimations, resulting in more consistent and reliable 3D reconstructions.
- **p. 4 / 3.2.3. CAMERA POSE REFINEMENT - extractive body cue:** These coordinates, along with the feature maps Fi ∈Rh×w×d and a pose token PCLS ∈Rd, are input into a series of self- and cross-attention layers.
- **p. 4 / 3.2.2. MULTI-VIEW CONSISTENT DEPTH ESTIMATION - extractive body cue:** Our refinement module includes a pixel-wise depth offset estimation that uses the feature maps Fi from the depth network (Piccinelli et al., 2024) as the ...
- **p. 5 / 3.2.4. 3D GAUSSIAN PARAMTER PREDICTIONS - extractive body cue:** To condition the prediction of Gaussian parameters, such as opacity, covariance, and color, we incorporate Sgeo as additional input.
- **p. 2 / 1. Introduction - extractive body cue:** We summarize our contributions below: • We propose PF3plat, a feed-forward network that reconstructs 3D scenes, parameterized by 3D Gaussians, from sparse, unposed views without ...
...
- **p. 5 / 3.3. Loss Function - extractive body cue:** We identify that provided good coarse alignments, RGB loss is sufficient, as similarly observed in (Ye et al., 2024), but with larger baselines, the training ...
- **p. 3 / 3.2.1. COARSE ALIGNMENT OF 3D GAUSSIANS - extractive body cue:** However, these solutions are incompatible with our goal of achieving a single feed-forward process with training solely from unposed images.
- **p. 5 / 4.1. Implementation Details - extractive body cue:** Our model is trained on 4 NVIDIA A100 GPU for 50,000 iterations using the Adam optimizer (Kingma, 2014), with a learning rate set to 8 ...
- **p. 8 / 4.5. Analysis and More Results - extractive body cue:** Finally, we provide the inference time of each of our components: overall inference time, UniDepth processing time, and decoder time.
- **p. 5 / 4.1. Implementation Details - extractive body cue:** The code and pretrained weights will be made publicly available.
...
- **p. 8 / 4.4. Ablation Study - extractive body cue:** In this ablation study, we aim to investigate the effectiveness of each component of our method.
- **p. 8 / 4.3. Experimental Results - extractive body cue:** Component ablations on RealEstate10K.
- **p. 6 / 4.2. Experimental Setting - extractive body cue:** For novel view synthesis, we compare our approach against established generalized NeRF and 3DGS variants, including PixelNeRF (Yu et al., 2021), (Du et al., 2023), ...
