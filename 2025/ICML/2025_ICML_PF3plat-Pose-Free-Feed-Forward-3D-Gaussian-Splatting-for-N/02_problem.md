...
- **p. 1 / Abstract - extractive PDF cue:** We tackle the problem of view synthesis from sparse, unposed images in a single feed-forward pass.
- **p. 1 / Abstract - extractive PDF cue:** Our method builds on 3DGS and relaxes common requirements such as dense views, accurate camera poses or depth, and large image overlaps.
- **p. 1 / Abstract - extractive PDF cue:** However, the main challenge arises from the parametrization of pixel-aligned 3D Gaussians, as their misalignments inevitably yield noisy or sparse gradients that destabilize training.
- **p. 1 / Abstract - extractive PDF cue:** To address this, we leverage pretrained monocular depth estimation and visual correspondence networks for coarse alignment, then refine depth and pose via lightweight learnable modules.
- **p. 1 / Abstract - extractive PDF cue:** We further estimate geometry confidence scores, driven by aggregated monocular and multi-view depth, to assess the reliability of 3D Gaussian centers and condition the prediction ...
- **p. 2 / 1. Introduction - extractive PDF cue:** However, a unique challenge emerges in the parametrization of pixel-aligned 3DGS.
- **p. 1 / 1. Introduction - extractive PDF cue:** To address some of these limitations, recent efforts (Yu et al., 2021; Johari et al., 2022; Chen et al., 2021; Yang et al., 2023) have ...
...
| Observation / input | This issue is particularly exacerbated when widebaseline images are given as input or the absence of groundtruth pose or depth prevents alignments ... | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF |
...
- **p. 1 / 1. Introduction - extractive PDF cue:** In real-world scenarios, casually captured images contain sparse and distant viewpoints, and lack precise camera poses, making it impractical to assume densely captured views with ...
- **p. 3 / 3.1. Problem Formulation - extractive PDF cue:** Note that, in line with existing pose-free view synthesis methods (Fu et al., 2023; Ye et al., 2024; Hong et al., 2024; Chen & Lee, ...

## What the Paper Changes

PDF contribution framing (p. 2 (1. Introduction), p. 1 (1. Introduction), p. 2 (1. Introduction), p. 5 (3.2.4. 3D GAUSSIAN PARAMTER PREDICTIONS), p. 3 (3.2.1. COARSE ALIGNMENT OF 3D GAUSSIANS)): We summarize our contributions below: • We propose PF3plat, a feed-forward network that reconstructs 3D scenes, parameterized by 3D Gaussians, from sparse, unposed views without requiring groundtruth depth or pose ...
...
- **p. 5 / 3.2.4. 3D GAUSSIAN PARAMTER PREDICTIONS - extractive PDF cue:** A key idea of our approach is that Sgeo enables supervision signals to flow from the Gaussian parameters back to the depth and pose estimates.
- **p. 3 / 3.2.1. COARSE ALIGNMENT OF 3D GAUSSIANS - extractive PDF cue:** To this end, we propose to provide coarse alignment of 3D Gaussians.
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 3 (3.2.1. COARSE ALIGNMENT OF 3D GAUSSIANS), p. 3 (3.1. Problem Formulation), p. 5 (3.2.4. 3D GAUSSIAN PARAMTER PREDICTIONS), p. 4 (3.2.3. CAMERA POSE REFINEMENT). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (1. Introduction), p. 1 (1. Introduction), p. 2 (1. Introduction), p. 1 (1. Introduction), p. 3 (3.1. Problem Formulation), interface p. 3 (3.2.1. COARSE ALIGNMENT OF 3D GAUSSIANS), p. 3 (3.1. Problem Formulation), p. 5 (3.2.4. 3D GAUSSIAN PARAMTER PREDICTIONS), p. 4 (3.2.3. CAMERA POSE REFINEMENT), objective p. 5 (3.3. Loss Function), p. 5 (3.3. Loss Function), p. 3 (3.2.1. COARSE ALIGNMENT OF 3D GAUSSIANS), p. 3 (3.1. Problem Formulation), p. 4 (3.2.2. MULTI-VIEW CONSISTENT DEPTH ESTIMATION), p. 4 (3.2.4. 3D GAUSSIAN PARAMTER PREDICTIONS).
