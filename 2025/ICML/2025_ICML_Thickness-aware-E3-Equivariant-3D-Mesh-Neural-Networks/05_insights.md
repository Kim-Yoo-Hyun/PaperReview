...
- **Claim/result cue:** Evaluations on a real-world industrial dataset demonstrate the superior performance of T-EMNN in accurately predicting node-level 3D deformations, effectively capturing thickness effects while maintaining computational efficiency.
...
- **Registry interface:** `3D reconstruction, equivariant, 3D Vision` is the paper's recorded topic/interface, not evidence that the full robotics loop was evaluated.
...
- Registry position: `3D Geometry, Registration, and Equivariance`; tags: `3D reconstruction, equivariant, 3D Vision`.
