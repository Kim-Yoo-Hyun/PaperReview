...
- **p. 1 / Abstract - extractive PDF cue:** We present LOCATE 3D, a model for localizing objects in 3D scenes from referring expressions like "the small coffee table between the sofa and the ...
- **p. 1 / Abstract - extractive PDF cue:** Notably, LOCATE 3D operates directly on sensor observation streams (posed RGB-D frames), enabling real-world deployment on robots and AR devices.
- **p. 1 / Abstract - extractive PDF cue:** Key to our approach is 3D-JEPA, a novel self-supervised learning (SSL) algorithm applicable to sensor point clouds.
- **p. 1 / Abstract - extractive PDF cue:** It takes as input a 3D pointcloud featurized using 2D foundation models (CLIP, DINO).
- **p. 1 / Abstract - extractive PDF cue:** Subsequently, masked prediction in latent space is employed as a pretext task to aid the self-supervised learning of contextualized pointcloud features.
- **p. 1 / 1. Introduction - extractive PDF cue:** They often require human annotation at inference time in the form of detailed 3D meshes or object instance segmentation, making them difficult to deploy on ...
- **p. 3 / 1. Introduction - extractive PDF cue:** We found directly reconstructing such fine-grained and high-dimensional features to be difficult.
...
| Observation / input | Preprocessing: Lifting 2D Foundation Model Features into 3D Point Clouds We begin by preprocessing the inputs (posed RGB-D images) by constructing a ... | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF |
...
- **p. 3 / 1. Introduction - extractive PDF cue:** We found directly reconstructing such fine-grained and high-dimensional features to be difficult.
- **p. 4 / 1. Introduction - extractive PDF cue:** This allows for faster mixing of information from the start, due to lack of an explicit grouping.
- **p. 2 / 1. Introduction - extractive PDF cue:** Crucially, LOCATE 3D achieves these impressive results with fewer assumptions compared to prior models.
- **p. 2 / 1. Introduction - extractive PDF cue:** It further exhibits strong generalization capabilities on held-out scenes and annotations in ScanNet++.

## What the Paper Changes

PDF contribution framing (p. 4 (2.3.1. LANGUAGE-CONDITIONED 3D DECODER), p. 5 (2.3.1. LANGUAGE-CONDITIONED 3D DECODER), p. 1 (1. Introduction), p. 1 (1. Introduction), p. 5 (2.3.1. LANGUAGE-CONDITIONED 3D DECODER)): Specifically, each decoder module consists of three attention blocks: (1) a self-attention block that enables queries to refine their representations through mutual interaction, (2) a cross-attention block where queries extract ...
...
- **p. 1 / 1. Introduction - extractive PDF cue:** For AI systems to effectively assist us in the physical world, such as through robots or smart glasses, an understanding of *Equal contribution 1FAIR at ...
- **p. 5 / 2.3.1. LANGUAGE-CONDITIONED 3D DECODER - extractive PDF cue:** For bounding boxes, we developed a novel architecture (Figure 7).
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 3 (1. Introduction), p. 1 (1. Introduction), p. 2 (1. Introduction), p. 3 (1. Introduction). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (1. Introduction), p. 3 (1. Introduction), p. 4 (1. Introduction), p. 2 (1. Introduction), p. 2 (1. Introduction), interface p. 3 (1. Introduction), p. 1 (1. Introduction), p. 2 (1. Introduction), p. 3 (1. Introduction), objective p. 5 (2.3.2. TRAINING LOCATE 3D), p. 5 (2.3.2. TRAINING LOCATE 3D).
