...
- **p. 3 / 2.2. State Space Models and Visual Applications - extractive PDF cue:** Vim [72] presents the first pure SSM-based model that efficiently compress the vision representation for intensive prediction tasks.
- **p. 4 / 3.2. Multi-Modal Mamba Mixer Block - extractive PDF cue:** To establish the correspondence between 3D vision and task prompts, we first construct a hybrid feature chain by channel-wisely concatenating 3D instance queries and prompt ...
- **p. 3 / 3.1. Overall Framework - extractive PDF cue:** For the point cloud, we use the pre-trained PointNet++ [47] to obtain point features P={p0, p1, ..., pS} of the segments.
...
- **p. 5 / 3.4. Output Heads and Losses - extractive PDF cue:** The model is optimized through multi-task learning with combined losses.
- **p. 5 / Method - extractive PDF cue:** The output ¯xi of IDPA is incorporated into the mamba-based query decoder branch via a residual connection.
- **p. 5 / 3.4. Output Heads and Losses - extractive PDF cue:** We compute the cross entropy loss Lgrd and Lgen for both grounding and generation heads, while the dice loss Lmask is applied to the mask ...
- **p. 3 / 2.2. State Space Models and Visual Applications - extractive PDF cue:** Recent studies [32, 36, 65, 66] investigate the applicability of mamba on 3D tasks by employing distinct point cloud ordering policy.

## Design Rationale

- **p. 2 / 1. Introduction - extractive PDF cue:** To capture spatial relationships of 3D object sequences while enhancing fine-grained interactions of 3D-VL interaction, we develop a Mamba Mixer module, which consists of a ...
...
- **p. 3 / 2.2. State Space Models and Visual Applications - extractive PDF cue:** Recent studies [32, 36, 65, 66] investigate the applicability of mamba on 3D tasks by employing distinct point cloud ordering policy.
- **p. 2 / 1. Introduction - extractive PDF cue:** Leveraging State Space Models (SSMs) as its core, a flux of mamba proposes a selection scanning mechanism, enabling it to handle long-range sequences and spatial ...
- **p. 2 / 1. Introduction - extractive PDF cue:** Drawing inspiration from 2D vision-language (2D-VL) models [10, 14, 28, 33, 34], contemporary 3D-VL models rely heavily on transformer-based cross-modality interaction to fuse 3D scene ...
- **p. 3 / 3.1. Overall Framework - extractive PDF cue:** Each query is ultimately routed through three universal output heads to predict the instance mask, task-related score, and scene-grounded texts.
- **p. 4 / 3.2. Multi-Modal Mamba Mixer Block - extractive PDF cue:** Consider the prompt-based Mamba Mixer as an exemplar: (1) Intra-modality interactions.
- **p. 4 / 3.2. Multi-Modal Mamba Mixer Block - extractive PDF cue:** The spatial scanning exhibits channel independence, failing to capture multi-modal interactions.
- **p. 5 / 3.4. Output Heads and Losses - extractive PDF cue:** 3, our Mamba-3VL utilizes three output heads to support 3D-VL tasks.
...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Table 9. Ablation study of proposed modules' effectiveness, with average performance evaluated under IoU@0.5. in Tab. 2, Mamba-3VL establishes new competitive bench- marks for 3D ...
...
- **p. 7 / 4.2. Results on 3D Vision-Language Tasks - extractive PDF cue:** Ablations of the selection of scanning mechanism.
- **p. 8 / 4.3. Ablation Study and In-depth Analysis - extractive PDF cue:** A), more framework ablations of Mamba-3VL (Tab.
- **p. 8 / 4.3. Ablation Study and In-depth Analysis - extractive PDF cue:** Removing either component (i.e., w/o.
