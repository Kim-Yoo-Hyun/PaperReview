...
- **p. 5 / 4.1. Dataset and Evaluation Metrics - extractive body cue:** The benchmark utilizes real-world scenes from the SceneVerse [26], incorporating indoor scans from 5 different datasets - ScanNet [11], 3RScan [40], MultiScan [31], ARKitScenes [3] ...
- **p. 6 / 4.2. Implementation Details - extractive body cue:** Comparisons on SG3D benchmark across five datasets.
- **p. 6 / 4.3. Comparison on SG3D Benchmark - extractive body cue:** It is pre-trained on an extensive range of 3D tasks, including object captioning [30, 51], object referring [1, 18, 48], 3D QA [12, 15, 47] ...
...
- **p. 7 / 4.4. Ablation Study - extractive body cue:** In the SG3D benchmark, the step count of a task ranges from 2 to 10.
...
- **p. 8 / 4.5. Qualitative Visualization - extractive body cue:** T represents the task description, St and Ot denote the step instruction and corresponding referred target object in step t.
- **p. 8 / 4.5. Qualitative Visualization - extractive body cue:** In the second example, there are multiple chairs in the scene and the last step instruction, "Walk back to your chair", refers to the chair ...
...
- **p. 1 / Figure/Table caption - extractive body cue:** Figure 1. An example of SG3D task (above) and a comparison be- tween previous visual grounding framework (bottom left) and our recurrent framework (bottom right) ...
- **p. 3 / Figure/Table caption - extractive body cue:** Figure 2. The overview of two 3DVG baseline models (blue background) integrated with our proposed plug-in temporal fusion module - GroundFlow (orange background). Unlike baseline ...
...
- **p. 6 / Figure/Table caption - extractive body cue:** Table 1. Comparisons on SG3D benchmark across five datasets. The values for the metrics s-acc (step accuracy) and t-acc (task accuracy) are expressed as percentages ...
- **p. 7 / Figure/Table caption - extractive body cue:** Table 2. Comparison of temporal fusion methods for 3D-VisTA and PQ3D. The ∆improvement in the last two columns is relative to the original 3DVG baselines. ...
- **p. 7 / Figure/Table caption - extractive body cue:** Table 3. Comparison of different short-term and long-term mem- ory settings for 3D-VisTA and PQ3D. Aligned with section 3.2, ˆJm denotes the aggregation of the ...
- **p. 7 / Figure/Table caption - extractive body cue:** Figure 4. Improvements after GroundFlow module is integrated in terms of task accuracy of 3D-VisTA and PQ3D across different step count subsets. various settings of ...
- **p. 8 / Figure/Table caption - extractive body cue:** Figure 5. Visualization results from PQ3D and PQ3D+GroundFlow. T represents the task description, St and Ot denote the step instruction and corresponding referred target object ...
...
- **p. 5 / 4.2. Implementation Details - extractive body cue:** The models are trained for 50 epochs with batch size of 32 and evaluated on the last epoch using evaluation split of the SG3D benchmark.
- **p. 7 / 4.4. Ablation Study - extractive body cue:** Tasks with step counts greater than 7 are combined into one subset, as creating separate subsets for them would result in fewer than 100 tasks, ...
...
- **Evidence anchors reviewed:** datasets p. 5 (4.1. Dataset and Evaluation Metrics), p. 6 (4.2. Implementation Details), p. 6 (4.3. Comparison on SG3D Benchmark), p. 5 (4.2. Implementation Details), p. 7 (4.4. Ablation Study), p. 7 (4.4. Ablation Study), metrics p. 7 (4.4. Ablation Study), p. 5 (4.1. Dataset and Evaluation Metrics), p. 6 (4.3. Comparison on SG3D Benchmark), p. 6 (4.3. Comparison on SG3D Benchmark), p. 7 (4.4. Ablation Study), p. 5 (4.1. Dataset and Evaluation Metrics), baselines p. 6 (4.3. Comparison on SG3D Benchmark), p. 5 (4.2. Implementation Details), p. 6 (4.4. Ablation Study), p. 7 (4.4. Ablation Study), p. 7 (4.4. Ablation Study), p. 3 (Figure/Table caption), results p. 6 (4.3. Comparison on SG3D Benchmark), p. 6 (4.3. Comparison on SG3D Benchmark), p. 7 (4.4. Ablation Study), p. 7 (4.4. Ablation Study), p. 5 (4.1. Dataset and Evaluation Metrics), p. 5 (4.1. Dataset and Evaluation Metrics).
