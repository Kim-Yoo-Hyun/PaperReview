...
| Observation / input | MM-Spatial-3B achieves SOTA with both image-only input and tool-use monocular depth, outperforming SpatialRGPT-VILA-1.5-8B (which fully encodes depth). | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF body |
...
| Decision / output variable | geometry/map/query r; body terms: MM1, architecture, focusing, mobile-friendly, variant, comprising, DFN-CLIP, image | p. 4 (4.1. Model Architecture), p. 4 (4.2. Data and Training) |
...
PDF body contribution framing (p. 4 (4.1. Model Architecture), p. 4 (4.2. Data and Training)): We use the MM1.5 architecture [85, 128] (focusing on the mobile-friendly 3B variant), comprising of a DFN-CLIP [34, 89] image encoder and a decoder-only LLM backbone, which are bridged via ...
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. Evidence interface anchors: p. 8 (Model), p. 1 (1. Introduction), p. 4 (4.1. Model Architecture), p. 4 (4.1. Model Architecture). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **Evidence anchors reviewed:** problem p. 1 (Abstract), p. 1 (Abstract), interface p. 8 (Model), p. 1 (1. Introduction), p. 4 (4.1. Model Architecture), p. 4 (4.1. Model Architecture), objective no optimization/equation sentence selected.
