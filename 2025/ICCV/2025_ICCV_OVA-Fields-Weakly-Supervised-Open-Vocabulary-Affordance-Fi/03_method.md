...
- **p. 3 / 3.1. Multi-Modal Affordance Perception - extractive body cue:** We combine affordance detection with visual feature extraction to create the comprehensive object representations.
- **p. 5 / 3.3. Query Mapping - extractive body cue:** We compute the similarity between vq and the feature representations of each point to identify manipulative regions: s(p) = sim(vq, f(p)) = vq · f(p) ...
- **p. 5 / 3.2. Spatial Feature Extraction and Feature Fusion - extractive body cue:** (5) This balanced loss optimizes the OVA-Fields to generate embeddings that capture both affordance and visual features, supporting robust affordance detection and visual recognition in ...
- **p. 4 / 3.2. Spatial Feature Extraction and Feature Fusion - extractive body cue:** This loss maximizes the similarity between the correct affordance and point features while minimizing it for incorrect affordances.
- **p. 3 / 3.1. Multi-Modal Affordance Perception - extractive body cue:** To address this, we apply BSRGAN [46] for super-resolution, with the objectives of enhancing the resolution of small object bounding boxes and improving the detection ...
...
- **p. 5 / 3.2. Spatial Feature Extraction and Feature Fusion - extractive body cue:** (5) This balanced loss optimizes the OVA-Fields to generate embeddings that capture both affordance and visual features, supporting robust affordance detection and visual recognition in ...
- **p. 3 / 3.1. Multi-Modal Affordance Perception - extractive body cue:** To address this, we apply BSRGAN [46] for super-resolution, with the objectives of enhancing the resolution of small object bounding boxes and improving the detection ...
- **p. 4 / 3.2. Spatial Feature Extraction and Feature Fusion - extractive body cue:** The total loss L combines these two terms by 6388
- **p. 3 / 3.1. Multi-Modal Affordance Perception - extractive body cue:** These heatmaps show the likelihood of specific affordances associated with the object, producing heatmaps Hi for each affordance type aj (e.g., "open" or "hold").
...
- **p. 3 / 3.2. Spatial Feature Extraction and Feature Fusion - extractive body cue:** In the OVA-Fields, our approach uses a sequence of RGB-D images, along with pose data and camera intrinsics, as input to build a point cloud ...
- **p. 2 / 1. Introduction - extractive body cue:** This module detects key parts like handles with low computational cost, supporting robust and scalable real robot manipulation. • We enable seamless integration between semantic ...
- **p. 4 / 3.2. Spatial Feature Extraction and Feature Fusion - extractive body cue:** A learnable temperature parameter further optimizes the OVA-Fields' ability to distinguish affordance types, allowing accurate detection of actionable regions in real 3D environments based on ...
- **p. 5 / 3.3. Query Mapping - extractive body cue:** In the final step, we map the user's complex natural language instructions to specific affordances in the 3D scene to identify the corresponding manipulative regions.
- **p. 2 / 1. Introduction - extractive body cue:** Such a model would allow robots to detect actionable areas and interact effectively with specific parts in complex and multi-object environments, guided by open-ended user ...
- **p. 3 / 3.1. Multi-Modal Affordance Perception - extractive body cue:** (1) These high-response regions indicate potential actionable areas on the image plane.
- **p. 4 / 3.2. Spatial Feature Extraction and Feature Fusion - extractive body cue:** Negative samples contain mismatched pairs, such as "handle" points with unrelated actions ("grab the cup") or visual contexts, like refrigerator handles paired with distant image ...
...
- **p. 5 / 4.1. Experiment Settings - extractive body cue:** OpenScene and CLIPFO3D are adjusted with frozen encoders and retrained projections using our affordance labels.
- **p. 3 / 3.1. Multi-Modal Affordance Perception - extractive body cue:** The scaled values H are then passed through a sigmoid function to compute the final weight, ensuring balanced training responses across regions.
- **p. 3 / 3.1. Multi-Modal Affordance Perception - extractive body cue:** Using a pre-trained CLIP model [28], visual embeddings are computed for each bounding box, embedding both object and spatial information into the 3D scene.
- **p. 4 / 3.2. Spatial Feature Extraction and Feature Fusion - extractive body cue:** OVA-Fields integrates feature fusion and training in three key steps.
...
- **p. 7 / 4.3. Ablation Study - extractive body cue:** Comparison of the small object processing procedure in the ablation study. a systematic ablation study on the dynamic weight mechanism by comparing four variants (Tab.
- **p. 6 / 4.3. Ablation Study - extractive body cue:** To evaluate the effectiveness of our small object handling mechanism, we conduct a comparative experiment by studying the OVAFields' performance with and without the super-resolution ...
...
- **p. 7 / 4.3. Ablation Study - extractive body cue:** All variants share identical training protocols on the Lab and Home dataset and are evaluated on two metrics: mIoU and instruction grounding accuracy (the success ...
- **p. 5 / 4.1. Experiment Settings - extractive body cue:** For ClosedSet Affordance Detection Model (3D AffordanceNet [5], Mask3D [31]), we adjust Mask3D's architectures while preserving its core mechanisms and replace Mask3D's output head with ...
- **p. 5 / 4.1. Experiment Settings - extractive body cue:** For Open-Vocabulary Affordance Detection Models (OpenAD [36], OpenMask3D [35], CLIP-Fields [32], OpenScene [26], CLIP-FO3D [45]), we maintain OpenMask3D's and CLIP-Fields's original architectures but unify text-conditional ...
