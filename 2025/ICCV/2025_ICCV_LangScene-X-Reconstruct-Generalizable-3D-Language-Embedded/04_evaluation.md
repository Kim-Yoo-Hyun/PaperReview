...
- **p. 6 / 4.1. Experiment Setup - extractive body cue:** The LERF dataset is an in-the-wild dataset captured by a handheld device, while ScanNet is a large scene dataset captured by RGB-D devices in complex ...
...
- **p. 7 / 4.2. Main Results - extractive body cue:** 2D Segmentation Results on Scannet [7] Dataset.
- **p. 8 / 4.2. Main Results - extractive body cue:** On the ScanNet dataset, the improvement upon the best existing method comes to 14.92% in terms of mIoU.
...
- **p. 8 / 4.3. Ablations - extractive body cue:** 7, it is evident that our quantized technique demonstrates superior performance in both loss convergence rate and accuracy in terms of L2 loss (from 1e^{-3} ...
...
- **p. 8 / 4.3. Ablations - extractive body cue:** 8, where our method enable to perform sharper boundaries and more accurate activation scores within the query objects.
...
- **p. 1 / Figure/Table caption - extractive body cue:** Figure 1. LangScene-X: Given sparse views as input (e.g., as few as two images), we design a generative paradigm to build the 3D generalizable language-embedded ...
- **p. 4 / Figure/Table caption - extractive body cue:** Figure 2. Pipeline of LangScene-X. Given two sparse-view images as input, we first generate a sequence of 3D consistent RGB images, normal maps, and segmentation ...
- **p. 4 / Figure/Table caption - extractive body cue:** Figure 3. The illustration of Language Quantized Compressor (LQC). By leveraging learnable embedding and vector quantisa- tion strategy, it compresses high-dimensional language features into discrete ...
- **p. 6 / Figure/Table caption - extractive body cue:** Table 1. 2D Quantitative Results on LERF-OVS Dataset. We report the open-vocabulary localization accuracy (%) and 2D semantic segmentation (IoU scores). LSeg [19] is a ...
- **p. 6 / Figure/Table caption - extractive body cue:** Table 2. 2D Quantitative Results on ScanNet Dataset. We report the open-vocabulary localization accuracy (%) and 2D semantic segmentation (IoU scores). The bold denotes the ...
- **p. 7 / Figure/Table caption - extractive body cue:** Figure 4. 2D Segmentation Results on LERF-OVS [17] Dataset. Here, we showcase two cases (i.e., Teatime, Kitchen) with multiple segmentation masks with text query. On ...
- **p. 7 / Figure/Table caption - extractive body cue:** Figure 5. 2D Segmentation Results on Scannet [7] Dataset. Here, we showcase two cases (i.e., 0085 00, 0114 00) with multiple segmentation masks with text ...
- **p. 8 / Figure/Table caption - extractive body cue:** Figure 6. Feature Matching comparison between our method and vanilla video diffusion mdoel . L2 Loss (log) Step
...
- **p. 6 / 4.1. Experiment Setup - extractive body cue:** Then we finetune it on 3D-consistent real data (i.e., RealEstate-10K [55] and ACID [24]) with 2000 steps on the learning rate 1 × 10-5.
- **p. 6 / 4.1. Experiment Setup - extractive body cue:** Next, we apply StableNormal to annotate 200 normal video clips of 3D scene data from RealEstate-10k and finetune TriMap video diffusion along with RGB videos ...
...
- **p. 6 / 4.1. Experiment Setup - extractive body cue:** All videos are center-cropped and resized to 720 × 480 resolution with 49 frames.
- **p. 6 / 4.1. Experiment Setup - extractive body cue:** Then, we train our model on large-scale open-world dataset COCO [23] with a batch size of 16 and 500,000 steps.
- **p. 6 / 4.1. Experiment Setup - extractive body cue:** For Language Surface Fields training, we first train the Gaussian model with only RGB and normal loss \protect \mathcal {L}_{normal} for 5,000 steps, and then ...
