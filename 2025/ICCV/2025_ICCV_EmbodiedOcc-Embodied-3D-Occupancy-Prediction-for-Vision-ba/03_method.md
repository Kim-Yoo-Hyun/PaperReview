...
- **p. 3 / 3.1. Embodied 3D Occupancy Prediction - extractive body cue:** Then we detach and put these updated Gaussians back into the memory.
- **p. 3 / 3.1. Embodied 3D Occupancy Prediction - extractive body cue:** Different subscripts may correspond to similar positions and perspectives, indicating that the agent has returned to a previously explored location.
- **p. 3 / 3.1. Embodied 3D Occupancy Prediction - extractive body cue:** Depth Aware Predicted Depth Map … … Input T-1 Input T … … … … Gaussian Memory T Gaussian Memory T-1 Occupancy T Occupancy T-1 ...
- **p. 2 / 3.1. Embodied 3D Occupancy Prediction - extractive body cue:** Conventional methods in indoor scenarios for occupancy prediction accepted RGB-D as inputs to predict the semantic occupancy of a 3D scene which requires depth sensors.
- **p. 3 / 3.1. Embodied 3D Occupancy Prediction - extractive body cue:** Recent methods begin to consider endowing models with the same competence, which accept a monocular RGB image as input and derive a 3D occupancy prediction ...
- **p. 2 / 1. Introduction - extractive body cue:** Most existing methods [2, 54, 56] still focus on local 3D occupancy prediction by integrating semantic and depth information extracted from the visual inputs.
...
- **p. 7 / 4.4. Experimental Analysis - extractive body cue:** Effect of Continuous Online Updating.
- **p. 7 / 4.4. Experimental Analysis - extractive body cue:** We analyze the effect of our depth-aware branch in Table 5 using the 26366
...
- **p. 6 / 4.2. Implementation Details - extractive body cue:** The depth prediction network used in the depth-aware branch is a fine-tuned DepthAnything-V2 model [51] that remains frozen during the training, and the depth-aware layer ...
- **p. 8 / 4.4. Experimental Analysis - extractive body cue:** Due to space limitations, we will use a more diverse set of samples to further show the visual effect of our EmbodiedOcc in the supplementary ...
