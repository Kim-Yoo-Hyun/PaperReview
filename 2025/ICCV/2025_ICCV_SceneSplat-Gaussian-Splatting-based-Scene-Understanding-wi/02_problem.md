...
- **p. 1 / Abstract - extractive PDF cue:** Recognizing arbitrary or previously unseen categories is essential for comprehensive real-world 3D scene understanding.
- **p. 1 / Abstract - extractive PDF cue:** Currently, all existing methods rely on 2D or textual modalities during training or together at inference.
- **p. 1 / Abstract - extractive PDF cue:** This highlights the clear absence of a model capable of processing 3D data alone for learning semantics end-to-end, along with the necessary data to train ...
- **p. 1 / Abstract - extractive PDF cue:** Meanwhile, 3D Gaussian Splatting (3DGS) has emerged as the de facto standard for 3D scene representation across various vision tasks.
- **p. 1 / Abstract - extractive PDF cue:** However, effectively integrating semantic reasoning into 3DGS in a generalizable manner remains an open challenge.
- **p. 2 / 1. Introduction - extractive PDF cue:** This highlights a key limitation: the absence of a robust model for processing 3D data end-to-end for semantic learning, along with the lack of sufficient ...
- **p. 2 / 1. Introduction - extractive PDF cue:** To address this gap, current methods resort to multi-modality fusion, distilling knowledge from 2D vision-language models into 3D data.
...
| Observation / input | (5) The output tokens ˆ Tm are mapped to the input Gaussian space with the reconstruction projector ˆGm = Φ( ˆTm) ∈RN′×F ... | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF |
...
- **p. 2 / 1. Introduction - extractive PDF cue:** To address this gap, current methods resort to multi-modality fusion, distilling knowledge from 2D vision-language models into 3D data.
- **p. 1 / 1. Introduction - extractive PDF cue:** The ability to interpret arbitrary queries rather than being limited to a closed set of categories is crucial for 3D understanding models to generalize across ...

## What the Paper Changes

PDF contribution framing (p. 2 (1. Introduction), p. 2 (1. Introduction), p. 6 (4.3. Self Supervised Pretraining), p. 4 (4. Methodology), p. 6 (4.3. Self Supervised Pretraining)): Our contributions can be summarized as follows: • We present SceneSplat-7K, a high-quality large-scale Gaussian splats dataset spanning 7K indoor scenes, which boosts 3DGS scene understanding research. • We propose ...
...
- **p. 4 / 4. Methodology - extractive PDF cue:** Building upon the SceneSplat-7K dataset, we carry out both vision-language 3DGS pretraining, which enables openvocabulary scene understanding, and self-supervised pretraining, which regularizes the latent space ...
- **p. 6 / 4.3. Self Supervised Pretraining - extractive PDF cue:** 4.2, the precomputed language feature enables effective knowledge distillation.
