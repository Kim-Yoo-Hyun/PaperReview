...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: In this work, we present Generative Gaussian Splatting (GGS) – a novel approach that integrates a 3D representation with a pre-trained latent video diffusion model.
- 출발 문제 단서: Introduction Synthesizing consistent and photorealistic 3D scenes is an open problem in computer vision.
- 주장된 효과 단서: We evaluate our approach on two common benchmark datasets for scene synthesis, RealEstate10K and ScanNet++, and find that our proposed GGS model significantly improves both the 3D consistency ...
...
- Result cue: We evaluate our approach on two common benchmark datasets for scene synthesis, RealEstate10K and ScanNet++, and find that our proposed GGS model significantly improves both the 3D consistency ...
