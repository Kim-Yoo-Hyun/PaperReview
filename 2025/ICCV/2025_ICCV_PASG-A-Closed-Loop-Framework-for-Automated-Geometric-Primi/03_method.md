...
- **p. 3 / 2.2. Spatial Reasoning for Manipulation - extractive body cue:** Through automated extraction of primitives using VFMs and hierarchical semantic alignment with VLMs, PASG achieves closed-loop optimization of spatial reasoning paradigms in open-world scenarios.
- **p. 4 / 3.2. Geometry Primitive Extraction - extractive body cue:** Keypoint Extraction For geometric keypoint ( \mathcal {K}_{\text {raw}} ) detection, we extract representative geometric positions including centers ( \ math cal {C} = \{c_1,...,c_n\} ...
- **p. 5 / 3.2. Geometry Primitive Extraction - extractive body cue:** Principal Axis Calibration For standardized axis representation, most 3D objects in datasets provide pre-aligned main axes.
- **p. 6 / 3.4. Semantic-guide Reasoning in Manipulation - extractive body cue:** Specifically, through our manipulation experiments (in Section 4.2), we validate that the PASG pipeline can reliably identify interaction primitives across diverse object categories.
...
- **p. 3 / Method - extractive body cue:** OmniManip employs computational constraint optimization and scene rendering for VLM validation, while our method directly detects annotation-primitive misalignment for efficient self-correction. addresses this limitation by ...
- **p. 3 / 2.2. Spatial Reasoning for Manipulation - extractive body cue:** Spatial reasoning in manipulation involves inferring interaction constraints from object's spatial primitives to guide robot actions.
- **p. 4 / 3.1. Semantic Primitives in Robotic Manipulation - extractive body cue:** These specify object orientation and motion constraints: • Primary Axis ( a_p ) The principal orientation axis of the object, usually dictated by its geometry ...
- **p. 5 / 3.3. Task-Oriented Semantic Annotation - extractive body cue:** \mat h cal { R}_ {i j}^o(P_{ij}^o, A_{ij}^o) \implies g_{ij} \label {eq:constraint} (2) where P _{ i j}^o \subseteq \mathcal {P}^o denotes point primitives and ...
- **p. 5 / 3.3. Task-Oriented Semantic Annotation - extractive body cue:** Each task t_i is decomposed into sub-stages with explicit operation goals per stage: \ mathc al { G}_i = \{g_{i1},...,g_{ik}\} \label {eq:subgoals} (1) To identify ...
- **p. 4 / 3.1. Semantic Primitives in Robotic Manipulation - extractive body cue:** (e.g. the spout tip over a cup for pouring) • Grasp Point ( p_g ): A location on the object optimized for a secure hold ...
...
- **p. 2 / 1. Introduction - extractive body cue:** Our contributions are as follows: • We propose a novel framework that automatically annotates hierarchical semantics for object interaction primitives, bridging the gap between low-level ...
- **p. 3 / 2.2. Spatial Reasoning for Manipulation - extractive body cue:** Spatial reasoning in manipulation involves inferring interaction constraints from object's spatial primitives to guide robot actions.
- **p. 3 / 3.1. Semantic Primitives in Robotic Manipulation - extractive body cue:** In robotic manipulation tasks, spatial primitives of objects serve as fundamental building blocks for planning and executing actions.
- **p. 4 / 3.1. Semantic Primitives in Robotic Manipulation - extractive body cue:** Point Interaction Primitives ( \mathcal {P} ) denote specific object locations critical for manipulation. • Anchor Point ( p_a ): A reference position that determines ...
- **p. 4 / 3.1. Semantic Primitives in Robotic Manipulation - extractive body cue:** Overview of PASG To further integrate operational task semantics, we categorize interaction primitives into two functionally distinct classes based on manipulation requirements: point-based ( \mathcal ...
- **p. 6 / 3.3. Task-Oriented Semantic Annotation - extractive body cue:** Keypoints and Axes Annotated Output.
- **p. 6 / 3.4. Semantic-guide Reasoning in Manipulation - extractive body cue:** Specifically, through our manipulation experiments (in Section 4.2), we validate that the PASG pipeline can reliably identify interaction primitives across diverse object categories.
...
- **p. 7 / 4.3. Object-based Spatial-Semantic Reasoning - extractive body cue:** We first generate 6,979 questions from a designated pool of base objects, allocating 80% (5,583 questions) as the fine-tuning training set to establish a foundational ...
- **p. 8 / 4.3. Object-based Spatial-Semantic Reasoning - extractive body cue:** Finetune We fine-tuned Qwen-2.5VL [6] using Low-Rank Adaptation (LoRA) to assess whether the VQA benchmark supports knowledge transfer in primitive compositional reasoning.
