...
- **p. 4 / 4. Simulation Experiments - extractive body cue:** To assess the capabilities of the pretrained model, we conduct evaluations across four simulation benchmarks in this section: 1) SimplerEnv [37] (Google Robot) demonstrates the ...
- **p. 5 / 4.3. LIBERO - extractive body cue:** LIBERO [40] is a comprehensive benchmark for knowledge transfer in multitask and lifelong robot learning.
- **p. 7 / 5.1. Real-Robot Task Finetuning - extractive body cue:** Given the data domain gap between our robot platform and the pretrain dataset, we primarily evaluate Dita on 10-shot generalization for the following challenging tasks ...
- **p. 7 / 5. Real-Robot Experiments - extractive body cue:** For real-robot experiments, We employ 10-shot finetuning to assess the model's adaptability in complex, long-horizon, multi-modal tasks within unseen robot environments.
- **p. 5 / 4.2. SimplerEnv - extractive body cue:** SimplerEnv [37] is a Real-to-Sim platform designed to evaluate policies learned from real robot data within a simulation environment.
- **p. 6 / 4.6. Ablation Study - extractive body cue:** We argue that 2-frame observations strike an optimal balance, providing sufficient visual distinction between objects in the workspace and the robot states.
...
- **p. 5 / Figure/Table caption - extractive body cue:** Table 1. Success rate comparison with RT-1-X [8], Octo-Base [72] and OpenVLA-7B [32] on SimplerEnv (both match and variant results of Google Robot [8]).
- **p. 5 / Figure/Table caption - extractive body cue:** Table 2. Comparison with Diffusion Policy (denoted as DP*, training from scratch) [17], Octo [72], and OpenVLA [32] on LIBERO [40]. Except for Dita results, ...
- **p. 6 / Figure/Table caption - extractive body cue:** Table 3. The comparisons with state-of-the-art approaches on Calvin (ABC→D) with the metrics of success rate and average success length. The abbreviations denote different input ...
- **p. 6 / Figure/Table caption - extractive body cue:** Table 4. Comparison of our model with two baseline methods (dis- cretization and diffusion head) on ManiSkill2 success rate. The abbreviations denote the task names: ...
- **p. 7 / Figure/Table caption - extractive body cue:** Table 5. Ablation on ManiSkill2 about the observation length (# obs) and the trajectory length (# traj). # obs # traj All PickC StackC S-YCB ...
...
- **p. 5 / 4.4. CALVIN - extractive body cue:** In contrast, employing diffusion head underperforms Dita by 0.45 points with similar pretrained weights, highlighting the 7690
- **p. 6 / 4.4. CALVIN - extractive body cue:** To construct the benchmark, we select 5 tasks (PickCube-v0, StackCube-v0, PickSingleYCB-v0, PickClutterYCB-v0, PickSingleEGAD-v0) from ManiSkill2 and create a camera pool comprising 300K random cameras.
- **p. 7 / 5.1. Real-Robot Task Finetuning - extractive body cue:** The system is operating under control frequency of 3Hz.
- **p. 7 / 5.1. Real-Robot Task Finetuning - extractive body cue:** 10 samples are collected for each task, with position variances introduced during evaluation to assess generalization performance. • Pour.
- **p. 7 / 5.1. Real-Robot Task Finetuning - extractive body cue:** We further devise several longhorizon tasks (more than 3 steps), including "Pick up the bowl within the drawer and pour the coffee beans into the ...
- **p. 7 / 5.1. Real-Robot Task Finetuning - extractive body cue:** We finetune the Dita on the aforementioned multiple manipulation tasks, with data collected on the same platform, using LoRA [26] for fair comparison and AdamW ...
