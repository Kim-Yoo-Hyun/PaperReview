...
- **p. 1 / Abstract - extractive PDF cue:** We propose a novel zero-shot approach for keypoint detection on 3D shapes.
- **p. 1 / Abstract - extractive PDF cue:** Point-level reasoning on visual data is challenging as it requires precise localization capability, posing problems even for powerful models like DINO or CLIP.
- **p. 1 / Abstract - extractive PDF cue:** Traditional methods for 3D keypoint detection rely heavily on annotated 3D datasets and extensive supervised training, limiting their scalability and applicability to new categories or ...
- **p. 1 / Abstract - extractive PDF cue:** In contrast, our method utilizes the rich knowledge embedded within Multi-Modal Large Language Models (MLLMs).
- **p. 1 / Abstract - extractive PDF cue:** Specifically, we demonstrate, for the first time, that pixel-level annotations used to train recent MLLMs can be exploited for both extracting and naming salient keypoints ...
- **p. 2 / 1. Introduction - extractive PDF cue:** In general, we can observe an increased level of difficulty when going from complete objects to object parts and, finally, to specific points or small ...
- **p. 2 / 1. Introduction - extractive PDF cue:** Through this study, we characterize the strengths and limitations of the 3D awareness imparted to models through training with pixel-level annotations.
...
| Observation / input | It processes both images and text as input and generates text as output. | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF |
...
- **p. 2 / 1. Introduction - extractive PDF cue:** Through this study, we characterize the strengths and limitations of the 3D awareness imparted to models through training with pixel-level annotations.
- **p. 4 / 3. Motivation - extractive PDF cue:** Furthermore, the recent MLLMs that incorporate 3D data [18, 51, 58] are typically trained with explicit alignment against pre-trained traditional vision-language models, and thus inherit ...
- **p. 4 / 3. Motivation - extractive PDF cue:** Existing methods for the 3D keypoint detection problem typically formulate the problem as either a supervised learning task, by exploiting the ground truth annotations, e.g., ...
- **p. 3 / 3. Motivation - extractive PDF cue:** Localization and naming of points in an image or a 3D shape is an extremely challenging problem.

## What the Paper Changes

PDF contribution framing (p. 2 (1. Introduction), p. 2 (1. Introduction), p. 4 (4.2. Prompting Molmo to Detect 2D Keypoints), p. 7 (Method), p. 7 (Method)): Inspired by these recent developments, we propose investigating MLLMs endowed with point-level reasoning in the context of 3D shape understanding and specifically for zero-shot keypoint detection.
...
- **p. 7 / Method - extractive PDF cue:** We then lift these 2D keypoints to 3D using the same backprojection technique described in our method.
- **p. 7 / Method - extractive PDF cue:** We lift the prediction of this method to 3D using the same lifting procedure used in our method to compare 3D Zero-shot keypoint detection.
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 7 (Method), p. 4 (4.2. Prompting Molmo to Detect 2D Keypoints), p. 4 (4.2. Prompting Molmo to Detect 2D Keypoints), p. 7 (Method). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (1. Introduction), p. 2 (1. Introduction), p. 4 (3. Motivation), p. 4 (3. Motivation), p. 3 (3. Motivation), interface p. 7 (Method), p. 4 (4.2. Prompting Molmo to Detect 2D Keypoints), p. 4 (4.2. Prompting Molmo to Detect 2D Keypoints), p. 7 (Method), objective p. 5 (4.3. Zero-Shot 3D Keypoint Detection).
