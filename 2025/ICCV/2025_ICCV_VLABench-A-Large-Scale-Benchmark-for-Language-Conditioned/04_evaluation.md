...
- **p. 6 / 3.4. Dataset Construction - extractive PDF cue:** Following the approach of previous benchmarks built on Mujoco [38, 47], the dataset is stored in the same format, with similar visual rendering quality and ...
- **p. 5 / 3.3. Benchmark - extractive PDF cue:** The evaluation episodes use objects seen in the training set, with minimal differences. - Track 2: Cross-category visual generalization.
- **p. 5 / 3.3. Benchmark - extractive PDF cue:** The evaluation episodes use unseen categories of objects as the target entity for evaluation. - Track 3: Common sense application.
...
- **p. 6 / 3.3. Benchmark - extractive PDF cue:** The progress score refers to the completion level of subtasks in a long-horizon task and serves as a softer process supervision metric compared to the ...
- **p. 5 / 3.3. Benchmark - extractive PDF cue:** In addition to the success rate (SR), considering the long-horizon nature and high difficulty level of our tasks, we introduce the intention score (IS) and ...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Figure 3. Evaluation pipeline for VLMs. Step 1: Sample the required four-view images, as well as those segmented with numerical information, from the simulation. Meanwhile, ...
- **p. 5 / 3.3. Benchmark - extractive PDF cue:** Intention score is a useful soft metric measuring the correctness trend in selecting the target object for the operation especially when sub-tasks are not yet ...
...
- **p. 1 / Figure/Table caption - extractive PDF cue:** Figure 1. Overview of VLABench. VLABench is a large-scale language-conditioned manipulation benchmark to evaluate the compre- hensive skill learning and generalization ability of action policies ...
- **p. 3 / Figure/Table caption - extractive PDF cue:** Table 1. Comparison of popular benchmarks in robot learning. SemLang: semantically non-template rich language instructions. LogiRea- son: task logic and relevant information reasoning. Knowledge: tasks ...
- **p. 5 / Figure/Table caption - extractive PDF cue:** Figure 2. Task examples in each dimension. The first row showcases examples of primitive tasks from Section 3.1, while the second row presents examples of ...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Figure 3. Evaluation pipeline for VLMs. Step 1: Sample the required four-view images, as well as those segmented with numerical information, from the simulation. Meanwhile, ...
...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Figure 5. Radar charts depicting the performance of all VLM mod- els across six dimensions. The reason why only GLM-4V-9B is evaluated in a zero-shot ...
...
- **p. 6 / 3.3. Benchmark - extractive PDF cue:** The red part represents the error output of the model, where the red solid arrows represent the dependencies generated by the error, and red dashed ...
- **p. 7 / Model - extractive PDF cue:** We then assessed the models across 50 episodes for each task, resulting in a total of 250 trials per track.
...
- **p. 6 / 3.4. Dataset Construction - extractive PDF cue:** To enhance sample efficiency, reject sampling and failure-triggered early termination are applied.
- **p. 8 / 5. Conclusion - extractive PDF cue:** We hope that VLABench will inspire both the future research on robotics pertaining recipe and promote more robust VLA architectures development.
- **p. 8 / Figure/Table caption - extractive PDF cue:** Figure 5. Radar charts depicting the performance of all VLM mod- els across six dimensions. The reason why only GLM-4V-9B is evaluated in a zero-shot ...
- **p. 5 / 3.3. Benchmark - extractive PDF cue:** We also extend the evaluation to cover various skills and long-horizon tasks to assess the overall capability and execution robustness of the workflow.
