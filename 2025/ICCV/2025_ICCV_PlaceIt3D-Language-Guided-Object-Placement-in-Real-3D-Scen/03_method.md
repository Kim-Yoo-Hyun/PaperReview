...
- **p. 6 / 4.4. Losses - extractive PDF cue:** We use a combination of Binary Cross Entropy (BCE) and Dice [43] losses when comparing a ground truth mask ¯ M with a predicted mask ...
- **p. 6 / 4.4. Losses - extractive PDF cue:** Finally, our total loss is defined as \Lo = \Lo _ {seg}(\bar {\mas k }_{l oc}, \mask _{loc}) + \Lo _{rot} + \Lo _{seg}(\bar {\mask ...
- **p. 1 / 1. Introduction - extractive PDF cue:** As in the shoe example, the goal is to find a valid placement of the object among multiple configurations that satisfy the instruction.
- **p. 1 / 1. Introduction - extractive PDF cue:** At two to three years old, neurotypical children learn to follow two-step instructions like "Get your shoes and put them on the shelf" [42].
- **p. 2 / 1. Introduction - extractive PDF cue:** Among the valid options, the model must follow the user's stated intent rather than default to common sense priors [36, 56].
- **p. 2 / 1. Introduction - extractive PDF cue:** The asset's size and shape restrict feasible placements; given the same scene and instruction, a large object has fewer valid locations than a small one.
- **p. 6 / 4.4. Losses - extractive PDF cue:** (2) The loss for the rotation prediction is given by \m a thca l {L}_ {rot} = \text {BCE}(\bar {\mask }_{rot}, \mask _{rot}), (3) where ...

## Design Rationale

- **p. 2 / 1. Introduction - extractive PDF cue:** To advance research in this area, we make three key contributions, summarized here: • We introduce PLACEIT3D-benchmark for languageguided placement with 3,500 evaluation examples, each ...
- **p. 2 / 1. Introduction - extractive PDF cue:** Like the benchmark, it uses ScanNet scenes and PartObjaverse-Tiny assets. • We propose PLACEWIZARD, a proto-method for this task built on recent 3D LLMs [25].
- **p. 1 / 1. Introduction - extractive PDF cue:** In this paper, we focus on the novel task of languageguided 3D object placement in a reconstructed real 3D scene.
...
- **p. 4 / 3.2.2. Benchmark metrics - extractive PDF cue:** To evaluate placement performance, we compute metrics that capture constraint validity overall and by subgroup: • Global Constraint Accuracy: The percentage of all constraints (across ...
...
- **p. 7 / 5.1.1. Ablations - extractive PDF cue:** We describe the different variants below.
- **p. 7 / 5.1.1. Ablations - extractive PDF cue:** This variant uses our proposed uniform spatial pooling approach instead of the original superpoints pooling.
...
- **p. 5 / 3.2.3. Benchmark statistics - extractive PDF cue:** For the visibility constraint we use the same procedure as the benchmark, but use two approximations for efficiency: the asset is replaced by its bounding ...
...
- **PDF anchors reviewed:** method p. 6 (4.4. Losses), p. 6 (4.4. Losses), objective p. 6 (4.4. Losses), p. 6 (4.4. Losses), temporal p. 1 (1. Introduction), p. 3 (3. Language-Guided 3D Object Placement), p. 3 (3.1. Physical plausibility and language constraints), p. 5 (3.2.3. Benchmark statistics), p. 5 (3.2.3. Benchmark statistics), p. 6 (4.2. Asset encoding).
