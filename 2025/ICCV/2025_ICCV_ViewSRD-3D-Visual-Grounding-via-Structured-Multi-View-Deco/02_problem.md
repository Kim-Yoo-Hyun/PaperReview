...
- **p. 1 / Abstract - extractive PDF cue:** 3D visual grounding aims to identify and localize objects in a 3D space based on textual descriptions.
- **p. 1 / Abstract - extractive PDF cue:** However, existing methods struggle with disentangling targets from anchors in complex multi-anchor queries and resolving inconsistencies in spatial descriptions caused by perspective variations.
- **p. 1 / Abstract - extractive PDF cue:** To tackle these challenges, we propose ViewSRD, a framework that formulates 3D visual grounding as a structured multi-view decomposition process.
- **p. 1 / Abstract - extractive PDF cue:** First, the Simple Relation Decoupling (SRD) module restructures complex multianchor queries into a set of targeted single-anchor statements, generating a structured set of perspective-aware descriptions ...
...
- **p. 1 / 2. The nightstand is closest to the wall - extractive PDF cue:** Large language models (LLMs) often have difficulty interpreting such descriptions [17, 51], yet resolving these ambiguities is crucial for improving grounding accuracy [20].
- **p. 2 / 2. The nightstand is closest to the wall - extractive PDF cue:** Ultimately, both the inherent complexity of multi-anchor queries and the challenges introduced by perspective shifts hinder the accurate interpretation of positional relationships in 3DVG, limiting ...
...
| Observation / input | These decomposed representations serve as the foundation for the Multi-view Textual-Scene Interaction (Multi-TSI) module, which integrates textual and scene features across multiple ... | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF |
...
- **p. 2 / 2. The nightstand is closest to the wall - extractive PDF cue:** Ultimately, both the inherent complexity of multi-anchor queries and the challenges introduced by perspective shifts hinder the accurate interpretation of positional relationships in 3DVG, limiting ...
- **p. 1 / 2. The nightstand is closest to the wall - extractive PDF cue:** Compounding this challenge, inconsistenThis ICCV paper is the Open Access version, provided by the Computer Vision Foundation.
- **p. 2 / 2. The nightstand is closest to the wall - extractive PDF cue:** To tackle these challenges, we propose ViewSRD, a framework that formulates 3D visual grounding as a structured multi-view decomposition process.
- **p. 3 / 3. ViewSRD - extractive PDF cue:** This multi-view setup introduces significant challenges for 3D visual grounding: (1) language-grounded spatial relations must remain consistent across view-dependent variations, and (2) object referents may ...

## What the Paper Changes

PDF contribution framing (p. 2 (2. The nightstand is closest to the wall), p. 2 (2. The nightstand is closest to the wall), p. 3 (3. ViewSRD), p. 4 (3.2. Textual Aggregation), p. 5 (3.3. Multi-view Textual-Scene Interaction Module)): In summary, our contributions are fourfold: • We propose ViewSRD, a framework that formulates 3D visual grounding as a structured multi-view decomposition process, effectively handling complex multi-anchor queries and mitigating ...
...
- **p. 4 / 3.2. Textual Aggregation - extractive PDF cue:** To enable the model to effectively learn from diverse sentence representations, we introduce a textual feature aggregation strategy.
- **p. 5 / 3.3. Multi-view Textual-Scene Interaction Module - extractive PDF cue:** At the final Transformer layer, the output consists of both [object] tokens and [view] tokens.
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 1 (Abstract), p. 2 (2. The nightstand is closest to the wall), p. 2 (2. The nightstand is closest to the wall), p. 3 (3. ViewSRD). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (2. The nightstand is closest to the wall), p. 2 (2. The nightstand is closest to the wall), p. 1 (2. The nightstand is closest to the wall), p. 2 (2. The nightstand is closest to the wall), p. 3 (3. ViewSRD), interface p. 1 (Abstract), p. 2 (2. The nightstand is closest to the wall), p. 2 (2. The nightstand is closest to the wall), p. 3 (3. ViewSRD), objective p. 6 (3.5. Overall Loss Functions), p. 6 (3.5. Overall Loss Functions).
