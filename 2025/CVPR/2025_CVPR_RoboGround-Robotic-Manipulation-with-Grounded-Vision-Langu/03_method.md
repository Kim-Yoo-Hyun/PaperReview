...
- **p. 6 / 4.4. Training and Inference - extractive PDF cue:** Since arm actions are continuous, we use Smooth-L1 loss Larm for optimization.
- **p. 5 / 4.3. Grounded Policy Network - extractive PDF cue:** For the policy network, we employ a language-conditioned transformer architecture, following the GR-1 model [43].
- **p. 4 / 4.1. Overview - extractive PDF cue:** We then incorporate this grounding knowledge into the low-level policy network, where the grounded masks function as both an attention mechanism within the Grounded Perceiver ...
- **p. 5 / 4.3. Grounded Policy Network - extractive PDF cue:** This sequence is then processed by a transformer decoder, which predicts the nextstep action tokens through the output <ACT> tokens.
- **p. 6 / 4.4. Training and Inference - extractive PDF cue:** Thus, the total training loss for the policy network is: Ltotal = Larm + Lgripper.
- **p. 4 / 4.1. Overview - extractive PDF cue:** Finally, we outline the training and evaluation procedures for the complete framework in Section 4.4.
- **p. 6 / 4.4. Training and Inference - extractive PDF cue:** For binary gripper actions, we apply Binary Cross Entropy (BCE) loss Lgripper.
- **p. 5 / 4.3. Grounded Policy Network - extractive PDF cue:** However, this token resampling process may lead to information loss, potentially limiting policy learning by failing to capture critical details about the target objects and ...

## Design Rationale

- **p. 2 / 1. Introduction - extractive PDF cue:** In this work, we introduce grounding masks as a promising intermediate representation that balances two key aspects: (1) Effective spatial guidance, which not only specifies ...
- **p. 2 / 1. Introduction - extractive PDF cue:** To address dataset limitations, we propose an automated pipeline for generating simulated manipulation data with a diverse set of objects and instructions.
- **p. 5 / 4.3. Grounded Policy Network - extractive PDF cue:** To address this, we propose guiding attention toward regions defined by grounded masks, ensuring that essential information is preserved for effective manipulation.
...
- **p. 6 / 4.4. Training and Inference - extractive PDF cue:** Since arm actions are continuous, we use Smooth-L1 loss Larm for optimization.
- **p. 5 / 4.3. Grounded Policy Network - extractive PDF cue:** However, this token resampling process may lead to information loss, potentially limiting policy learning by failing to capture critical details about the target objects and ...
- **p. 5 / 4.3. Grounded Policy Network - extractive PDF cue:** To enable action prediction, a learnable ACT token with feature Za is also included.
...
- **p. 5 / 4.3. Grounded Policy Network - extractive PDF cue:** As shown in Figure 3(b), this model processes a sequence of historical image observations, robot states and a language instruction as input to predict future ...
- **p. 5 / 4.2. Grounded Vision-Language Model - extractive PDF cue:** The grounded vision-language model takes an image observation and a language instruction as input and outputs binary masks for target objects and/or target placement areas ...
- **p. 6 / 4.4. Training and Inference - extractive PDF cue:** In each forward pass, the policy network receives image observations, robot states over N consecutive timesteps, and the corresponding language instruction.
- **p. 4 / 4.1. Overview - extractive PDF cue:** We then incorporate this grounding knowledge into the low-level policy network, where the grounded masks function as both an attention mechanism within the Grounded Perceiver ...
- **p. 6 / 4.4. Training and Inference - extractive PDF cue:** These initial masks are provided to the grounded policy network to predict the next action tokens.
- **p. 1 / 1. Introduction - extractive PDF cue:** Research in this area typically falls into two categories: accessible yet coarse-grained representations, such as language instructions [2, 49], which are easy to generate but ...
- **p. 2 / 1. Introduction - extractive PDF cue:** As shown in Figure 1, existing datasets [10, 26-28, 40] often suffer from limited instruction diversity and scene complexity, leading policy networks to overfit to ...
...
- **p. 6 / 4.4. Training and Inference - extractive PDF cue:** Thus, the total training loss for the policy network is: Ltotal = Larm + Lgripper.
- **p. 4 / 4.1. Overview - extractive PDF cue:** Finally, we outline the training and evaluation procedures for the complete framework in Section 4.4.
- **p. 6 / 4.4. Training and Inference - extractive PDF cue:** To optimize inference time, segmentation masks are extracted from the grounded VLM only once, at the beginning of the episode.
- **p. 5 / 4.3. Grounded Policy Network - extractive PDF cue:** The result is then fed into a pre-trained ViTMAE encoder [16].
...
- **p. 8 / 5.4. Ablation Study - extractive PDF cue:** Ablation Study on Grounded VLM. "Zero-shot" refers to the zero-shot evaluation of the grounded VLM. "Sim. data" and "VLM data" denotes the use of simulated ...
...
- **p. 7 / 5.4. Ablation Study - extractive PDF cue:** We perform ablation studies by training models with different datasets and mask configurations.
- **p. 7 / 5.2. Main Results - extractive PDF cue:** Since the pre-trained model is unavailable, we reproduce it here without large-scale pre-training or image prediction.
- **p. 8 / 5.4. Ablation Study - extractive PDF cue:** Ablation Study on Training Data and Grounded Masks. "Ori.
...
- **PDF anchors reviewed:** method p. 6 (4.4. Training and Inference), p. 5 (4.3. Grounded Policy Network), p. 4 (4.1. Overview), p. 5 (4.3. Grounded Policy Network), p. 6 (4.4. Training and Inference), p. 4 (4.1. Overview), objective p. 6 (4.4. Training and Inference), p. 6 (4.4. Training and Inference), p. 5 (4.3. Grounded Policy Network), p. 5 (4.3. Grounded Policy Network), temporal p. 5 (4.3. Grounded Policy Network), p. 5 (4.3. Grounded Policy Network), p. 6 (4.4. Training and Inference), p. 6 (4.4. Training and Inference), p. 4 (4.1. Overview), p. 4 (3.2. Diverse Instructions).
