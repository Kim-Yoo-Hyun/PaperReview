...
- **p. 5 / 4.1. Pre-training - extractive body cue:** For scene-level pre-training, we utilize the real-world ScanNetV2 dataset [10] with more than 1,500 scans of indoor scenes.
- **p. 6 / 4.2.1. Object-level Fine-tuning - extractive body cue:** When fine-tuning object models for classification, we experiment on the real-world ScanObjectNN [47] dataset, which comprises 15 categories and includes three splits: OBJ BG, OBJ ...
...
- **p. 5 / 4.1. Pre-training - extractive body cue:** For object-level pre-training, we adhere to established practices [31, 67] to use the synthetic ShapeNet dataset [2].
...
- **p. 7 / 4.2.2. Scene-level Fine-tuning - extractive body cue:** Semantic segmentation results on the scene-level datasets.
...
- **p. 1 / Figure/Table caption - extractive body cue:** Figure 1. Pre-training paradigm comparison. Existing object- level pre-training methods usually follow a generative masked auto-encoding (MAE) paradigm. Their scene-level counterparts mostly leverage the contrastive ...
- **p. 3 / Figure/Table caption - extractive body cue:** Figure 2. UniPre3D pre-training pipeline. Our proposed pre- training task involves predicting Gaussian parameters from the in- put point cloud. The 3D backbone network is ...
- **p. 4 / Figure/Table caption - extractive body cue:** Table 6. Our analysis suggests that this is primarily due to the sparsity of the scene point cloud and the exponential in- crease in complexity ...
- **p. 6 / Figure/Table caption - extractive body cue:** Figure 3. Visualization of UniPre3D pre-training outputs. The first row presents the input point clouds, followed by the reference view images in the second row. ...
- **p. 6 / Figure/Table caption - extractive body cue:** Table 1. Classification results on the ScanObjectNN dataset. We report the overall accuracy (%) on three data splits.
- **p. 7 / Figure/Table caption - extractive body cue:** Table 2. Part segmentation results on the ShapeNetPart dataset. We report the mean IoU across all part categories mIoUC, and the mean IoU across all ...
- **p. 7 / Figure/Table caption - extractive body cue:** Table 3. Semantic segmentation results on the scene-level datasets. We report the mean IoU on the validation set. The Standard Transformer results are from PCP-MAE ...
- **p. 8 / Figure/Table caption - extractive body cue:** Table 4. Instance segmentation results on the scene-level datasets. We use PointGroup [19] as the baseline model, follow- ing previous papers. We report the mean ...
...
- **p. 5 / 4.1. Pre-training - extractive body cue:** ShapeNet contains over 50,000 CAD models, from each of which we randomly sample point clouds of 1,024 points and evenly render 36 images via DISN ...
- **p. 5 / 4.1. Pre-training - extractive body cue:** Each scene contains a point cloud of over 100,000 points and hundreds of associated projected images.
...
- **p. 6 / 4.2.1. Object-level Fine-tuning - extractive body cue:** For part segmentation fine-tuning, we utilize the ShapeNetPart [66] dataset that contains over 16,000 samples across 16 classes, featuring fine-grained part annotations for 50 categories.
- **p. 6 / 4.2.1. Object-level Fine-tuning - extractive body cue:** Model Pre-train OBJ BG OBJ ONLY PB T50 RS Standard Transformer [48] ✗ 79.86 80.55 77.24 OcCo [51] 84.85 85.54 78.79 Point-BERT [67] 87.43 88.12 ...
