...
- **p. 4 / 3.3. Compound 3D-Informed Design - extractive body cue:** 3.2.1, we consider two main aspects in our compound 3D-informed design - the architecture design that leads to visual encoders with strong 3D awareness and ...
- **p. 6 / 3.3.1. Design space - extractive body cue:** We investigate the 3D-awareness of mixed visual encoders, and incorporate 3D-informed data at each training stage across all architecture components.
- **p. 6 / 3.3.1. Design space - extractive body cue:** We then study a series of design decisions. • + mixed vision encoder.
- **p. 3 / 3.1. Preliminary of LMMs - extractive body cue:** This stage focuses on developing foundational visual representations, often with reconstructionbased objectives (e.g., MAE [23], DINOv2 [47]).
...
- **p. 3 / 3.1. Preliminary of LMMs - extractive body cue:** This stage focuses on developing foundational visual representations, often with reconstructionbased objectives (e.g., MAE [23], DINOv2 [47]).
- **p. 3 / 3.1. Preliminary of LMMs - extractive body cue:** The success of deep learning in vision is largely due to well-curated data [20, 56], and LMMs similarly depend on this foundation, albeit with diverse ...
- **p. 5 / 3.3.1. Design space - extractive body cue:** We consider two types of visual encoders: (i) Frozen & pretrained visual encoder CLIP [48] following [41] but with the option to mix a wider ...
- **p. 8 / Model - extractive body cue:** Roadmap progression towards the best-performing model.
- **p. 8 / Model - extractive body cue:** Thorough exploration of the design space and roadmap progression.
...
- **p. 4 / 3.2.1. Challenges of 3D spatial reasoning - extractive body cue:** Existing pretraining and visual instruction tuning data for LMMs [41, 58] focused on detailed descriptions and conversations about scenes, appearances, and actions, while being vague ...
- **p. 3 / 3.1. Preliminary of LMMs - extractive body cue:** At this stage, the model is trained to describe images in details to align visual and language representations in the same space. • Visual instruction ...
- **p. 5 / 3.3. Compound 3D-Informed Design - extractive body cue:** This compound design simultaneously considers 3D-informed data, architecture, and training methods to search for the best-performing models for spatial reasoning. image 2D bbox image caption ...
- **p. 6 / 3.3.1. Design space - extractive body cue:** (b) design of LLaVA-v1.5 (CVPR'24) Alignment Instruction tuning Visual Encoder CLIP Connector LLM Connector (c) design of SpatialVLM (CVPR'24) (a) our proposed design Visual Encoder ...
...
- **p. 4 / 3.3. Compound 3D-Informed Design - extractive body cue:** While previous approaches focused on generating 3D-informed instruction tuning data to enable LMMs with the abilities to estimate distances, depths, or spatial relationships [14, 16], ...
- **p. 5 / 3.3.1. Design space - extractive body cue:** In a multimodal LLM, a pre-trained CLIP visual encoder extracts grid features from the input image.
...
- **p. 4 / 3.3. Compound 3D-Informed Design - extractive body cue:** 3.2.1, we consider two main aspects in our compound 3D-informed design - the architecture design that leads to visual encoders with strong 3D awareness and ...
- **p. 6 / 3.3.1. Design space - extractive body cue:** We investigate the 3D-awareness of mixed visual encoders, and incorporate 3D-informed data at each training stage across all architecture components.
- **p. 7 / 4.2. Results - extractive body cue:** 3D-informed pretraining of vision encoder?
...
- **p. 7 / 4.2. Results - extractive body cue:** 3D-informed pretraining of vision encoder?
- **p. 7 / 4.2. Results - extractive body cue:** We observe that pre-pretraining in stage 0 reduces performance, suggesting that tuning vi17255
...
- **Evidence anchors reviewed:** method p. 5 (3.3.1. Design space), p. 3 (3.1. Preliminary of LMMs), p. 3 (3.1. Preliminary of LMMs), p. 5 (3.3.1. Design space), p. 4 (3.3. Compound 3D-Informed Design), p. 6 (3.3.1. Design space), objective p. 3 (3.1. Preliminary of LMMs), p. 3 (3.1. Preliminary of LMMs), p. 5 (3.3.1. Design space), p. 8 (Model), p. 8 (Model), temporal p. 3 (3.1. Preliminary of LMMs), p. 3 (3.1. Preliminary of LMMs), p. 7 (4.2. Results), p. 8 (Model).
