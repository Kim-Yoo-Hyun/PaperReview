...
- **p. 3 / 3. Method - extractive body cue:** Following SDFusion [5], we employ the volumetric Truncated Signed Distance Field (T-SDF) to model the distribution across 3D shapes and a 3D variant of the ...
- **p. 5 / 3.3. Policy Training - extractive body cue:** For reward function setting, since the final output shape is not predicted, we design it to be the difference in the diffusion model's loss values.
- **p. 5 / 3.3. Policy Training - extractive body cue:** The loss function for reinforcement learning is as follows: Lrl = [R + ϖ max an+1 Q(T0, ..., Tn+1, an+1) ↓Q(T0, ..., Tn, an)]2, (6) ...
...
- **p. 5 / 3.3. Policy Training - extractive body cue:** For reward function setting, since the final output shape is not predicted, we design it to be the difference in the diffusion model's loss values.
- **p. 5 / 3.3. Policy Training - extractive body cue:** The loss function for reinforcement learning is as follows: Lrl = [R + ϖ max an+1 Q(T0, ..., Tn+1, an+1) ↓Q(T0, ..., Tn, an)]2, (6) ...
- **p. 4 / 3.1. Touch-conditioned Diffusion Model - extractive body cue:** The objective is to pulling together with matching touch-shape pairs while pushing unmatched pairs apart.
- **p. 4 / 3.1. Touch-conditioned Diffusion Model - extractive body cue:** The loss function is: Lrl = ↓log eq·kp/ε !K i=0 eq·ki/ε , (3) where q is the query feature, k is the key feature, ki ...
...
- **p. 4 / 3. Method - extractive body cue:** The policy model receives the denoised vector as input and is trained using reinforcement learning (Section 3.2).
- **p. 2 / 1. Introduction - extractive body cue:** In this work, we employ a simulated robotic arm guided by a trained policy model to touch the target, enabling the acquisition of tactile images ...
- **p. 4 / 3.1. Touch-conditioned Diffusion Model - extractive body cue:** The loss function for diffusion model training is as follows: Ldiff(t, n) = //Eω(zt, r(t), C(T0, ..., Tn→1)) ↓ωt//2, (2) where ωt is the added ...
- **p. 2 / 1. Introduction - extractive body cue:** The main contributions of this article are as follows: • We propose Touch2Shape, a touch-conditioned 3D diffusion model for shape exploration and reconstruction, utilizing the ...
- **p. 5 / 3.3. Policy Training - extractive body cue:** The loss function for reinforcement learning is as follows: Lrl = [R + ϖ max an+1 Q(T0, ..., Tn+1, an+1) ↓Q(T0, ..., Tn, an)]2, (6) ...
- **p. 5 / 3.3. Policy Training - extractive body cue:** Subsequently, we construct an action embedding module to derive the embedding for the potential actions.
- **p. 3 / 3. Method - extractive body cue:** In test stage, we gather tactile images (T0, ..., Tn→1) from the target, utilizing the trained diffusion model to obtain a lowdimensional representation for predicting ...
...
- **p. 6 / 4.1. Experimental Settings - extractive body cue:** The diffusion model was trained for 1 million iterations with an initial learning rate of 0.00001 and batch size of 12, while the touch shape ...
- **p. 5 / 3.3. Policy Training - extractive body cue:** We first employ the pre-trained latent encoder in Figure 2 (c) to encode both the initial and current latent vectors of the touch-conditioned diffusion model.
...
- **p. 5 / 4. Experiment - extractive body cue:** Through the ablation study, we validate the necessity of each module.
- **p. 7 / 4.4. Ablation Study - extractive body cue:** We design the ablation study to further validate the necessity of our proposed reconstruction modules.
- **p. 8 / 4.4. Ablation Study - extractive body cue:** Ablation study results on dataset ABC.
- **p. 6 / 4.1. Experimental Settings - extractive body cue:** The evaluation metric is EMD (lower is better). touch shape fusion can be trained concurrently since they do not share any components.
...
- **Evidence anchors reviewed:** method p. 4 (3.1. Touch-conditioned Diffusion Model), p. 4 (3.1. Touch-conditioned Diffusion Model), p. 5 (3.3. Policy Training), p. 5 (3.3. Policy Training), p. 3 (3. Method), p. 3 (3. Method), objective p. 5 (3.3. Policy Training), p. 5 (3.3. Policy Training), p. 4 (3.1. Touch-conditioned Diffusion Model), p. 4 (3.1. Touch-conditioned Diffusion Model), temporal p. 5 (3.3. Policy Training), p. 5 (3.3. Policy Training), p. 4 (3.1. Touch-conditioned Diffusion Model), p. 1 (body section not recovered), p. 4 (3. Method), p. 6 (4.2. Evaluation on Reconstruction Performance).
