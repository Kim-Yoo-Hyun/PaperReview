...
- **p. 5 / 3.4.1. Model Inference - extractive body cue:** Given the visual and language input, the model outputs the predicted action 푎0.
...
- **p. 5 / 3.4.2. Interaction Strategy - extractive body cue:** If provided with 푎푚 0 , we then follow the predicted moving direction 푎푀 0 to determine the subsequent movements after contact.
- **p. 6 / 3.4.2. Interaction Strategy - extractive body cue:** Meanwhile, regarding each key-frame step as compositions, we can group them into arbitrary combinations, enabling the model to handle diverse manipulation tasks.
- **p. 4 / 3.3.2. Policy Learning - extractive body cue:** The aforementioned losses are trained simultaneously under the total objective function: L = 휆1 ∗L푇+휆2 ∗L푂+휆3 ∗L푃.
...
- **p. 4 / 3.3.2. Policy Learning - extractive body cue:** The aforementioned losses are trained simultaneously under the total objective function: L = 휆1 ∗L푇+휆2 ∗L푂+휆3 ∗L푃.
- **p. 4 / 3.3.2. Policy Learning - extractive body cue:** We formulate pose prediction as a classification task by discretizing the continuous numbers in the normalized 3D direction vector into 100 discrete bins [-50,50], with ...
- **p. 3 / 3.1. Problem Formulation - extractive body cue:** The objective of the model is to generate an action 푎0 = (푎푝′ 0 , 푎푍 0 , 푎푌 0 , 푎푀 0 ), where ...
...
- **p. 6 / 3.4.2. Interaction Strategy - extractive body cue:** The benefit of this approach revolves around breaking down the complexity of long-horizon tasks and allowing us to optimize the success rate of each key-frame ...
...
- **p. 5 / 3.4.1. Model Inference - extractive body cue:** Given the visual and language input, the model outputs the predicted action 푎0.
- **p. 4 / 3.3.2. Policy Learning - extractive body cue:** Therefore, we introduce the following losses to guide the policy training: Text Supervision Loss L푇: This loss ensures the effective alignment of the model's visual ...
- **p. 2 / 1. Introduction - extractive body cue:** Execution Input end effector pose: z-axis y-axis contact point t1 t2 CrayonRobo(ours) t1 t1 t2 t2 initial state end state next moving direction: RT-trajectory t ...
- **p. 4 / 3.3.2. Policy Learning - extractive body cue:** To establish an explicit connection between the input 2D directional prompt and the output 3D directions, we introduce a projection loss designed to guide their ...
...
- **p. 2 / 1. Introduction - extractive body cue:** Based on the input goal prompt, the model determines the 6 DoF contact pose, enabling it to interact with the object as required.
- **p. 3 / 3.2. Data Collection - extractive body cue:** These are then served as information embedded in the input language prompt.
...
- **p. 4 / 3.3.2. Policy Learning - extractive body cue:** Therefore, we introduce the following losses to guide the policy training: Text Supervision Loss L푇: This loss ensures the effective alignment of the model's visual ...
- **p. 3 / 3.3.1. Model Architecture - extractive body cue:** This approach aims to preserve the inherent existing pre-trained knowledge, particularly in simto-real transfer, while enhancing the model's ability to comprehend visual prompts and perform ...
- **p. 3 / 3.3.1. Model Architecture - extractive body cue:** Simultaneously, text prompts 푃are encoded into text features using LLaMa's pre-trained tokenizer [51].
...
- **p. 6 / 4. Experiment - extractive body cue:** In our experiments, we mainly focus on exploring the following questions: • Section 4.3.1: What is the effect of different types of prompts on model ...
- **p. 7 / 4.3. Ablation Study - extractive body cue:** Additionally, to investigate the differential effects of visual and language prompts, in the last row of Table 2, we enable the model to learn from ...
...
- **p. 8 / 4.4. Real-world Experiment - extractive body cue:** 3, we further explore whether it is possible to fine-tune the model using prompts provided during the first set of executions for a specific task, ...
- **p. 6 / 4.3. Ablation Study - extractive body cue:** Analysis on The Effect of Different Types of Prompt In Table 2 Ex1-Ex3, since our model is able to handle various input patterns thanks to ...
