...
- **p. 1 / Abstract - extractive PDF cue:** We also evaluate the impact of Graph2Nav via integration with SayNav, a state-of-the-art planner based on large language models, on an unmanned ground robot to ...
- **p. 3 / III. GRAPH2NAV - extractive PDF cue:** 3D Semantic Object Extraction We assume that a sensor system, which is composed of an RGBD camera or a LiDAR-camera suite, is equipped on a ...
- **p. 4 / IV. INTEGRATION WITH SAYNAV - extractive PDF cue:** To accomplish SayNav in the actual physical world, we use Graph2Nav to replace the original scene graph generation module in SayNav.
- **p. 3 / III. GRAPH2NAV - extractive PDF cue:** It separately models the objects and relations in the form of queries from two Transformer decoders, followed by a prompting-like relation-object matching mechanism.
- **p. 4 / IV. INTEGRATION WITH SAYNAV - extractive PDF cue:** It includes three modules: (1) Incremental Scene Graph Generation, (2) High-Level LLM-based Dynamic Planner, and (3) Low-Level Planner.
- **p. 2 / III. GRAPH2NAV - extractive PDF cue:** Graph2Nav (Figure 2) includes three major components: (1) A SLAM system that maps and labels 3D objects in the surrounding environment using sensors (including a ...
...
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** Our framework fully generates and exploits both 3D objects and ∗The first two authors have equal contribution.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** Our main contributions are summarized as follows: • We propose Graph2Nav, a new real-time 3D objectrelation graph generation framework that combines strengths of 2D object-relation ...
...
- **p. 1 / Abstract - extractive PDF cue:** This approach avoids previous training data constraints in learning 3D scene graphs directly from 3D data.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** The main advantage of a 3D scene graph over other object-based 3D scene representations is its capability also to represent semantic relationships (e.g. "beside", "in ...
- **p. 3 / III. GRAPH2NAV - extractive PDF cue:** Inconsistent labels and masks will be corrected and updated.
- **p. 3 / III. GRAPH2NAV - extractive PDF cue:** Then, it updates the labels of voxels that are previously updated by P.
- **p. 4 / III. GRAPH2NAV - extractive PDF cue:** We will also update existing nodes in G using correspondent information from Gi.
...
| Action/output | framework, fully, generates, exploits, objects, first, authors, have, equal, contribution | collision-free trajectory 또는 velocity command | body cue; unit/decoder verify |
...
- **p. 3 / III. GRAPH2NAV - extractive PDF cue:** 3D Semantic Object Extraction We assume that a sensor system, which is composed of an RGBD camera or a LiDAR-camera suite, is equipped on a ...
- **p. 3 / III. GRAPH2NAV - extractive PDF cue:** The panoptic segmentation image Ii used in the real-time 3D semantic object extraction process (Section III-A) is formed by combining M and Q outputted from ...
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** The figure also shows examples of 2D input images and the 3D point clouds generated by Graph2Nav.
- **p. 1 / Abstract - extractive PDF cue:** We also evaluate the impact of Graph2Nav via integration with SayNav, a state-of-the-art planner based on large language models, on an unmanned ground robot to ...
- **p. 2 / III. GRAPH2NAV - extractive PDF cue:** Graph2Nav (Figure 2) includes three major components: (1) A SLAM system that maps and labels 3D objects in the surrounding environment using sensors (including a ...
- **p. 4 / IV. INTEGRATION WITH SAYNAV - extractive PDF cue:** Each LLM-planned step is executed by the Low-Level Planner to generate a series of control commands for execution during navigation.
...
- **p. 5 / V. EXPERIMENTS - extractive PDF cue:** We use Nvidia AGX Orin as our onboard computer for Graph2Nav processing and SayNav inference.
...
- **p. 6 / V. EXPERIMENTS - extractive PDF cue:** It means the robot does two trials (one uses the graph without relations, and the other uses the entire graph with object relations from Graph2Nav) ...
- **p. 6 / V. EXPERIMENTS - extractive PDF cue:** Without object-relations, the LLM generates the initial search plan based on (1) the distances to different large objects inside the environment, and (2) the likelihoods ...
...
- **PDF anchors reviewed:** method p. 1 (I. INTRODUCTION), p. 1 (Abstract), p. 3 (III. GRAPH2NAV), p. 4 (IV. INTEGRATION WITH SAYNAV), p. 3 (III. GRAPH2NAV), p. 4 (IV. INTEGRATION WITH SAYNAV), objective p. 1 (Abstract), p. 1 (I. INTRODUCTION), p. 3 (III. GRAPH2NAV), p. 3 (III. GRAPH2NAV), p. 4 (III. GRAPH2NAV), temporal p. 5 (V. EXPERIMENTS), p. 1 (Abstract), p. 1 (I. INTRODUCTION), p. 4 (V. EXPERIMENTS), p. 5 (V. EXPERIMENTS), p. 2 (II. RELATED WORK).
