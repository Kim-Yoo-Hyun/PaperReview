...
| Observation / input | 3D Semantic Object Extraction We assume that a sensor system, which is composed of an RGBD camera or a LiDAR-camera suite, is ... | camera/depth stream, pose, map와 language goal | exact sensor/frame/preprocessing from PDF body |
...
| Decision / output variable | path/waypoint/velocity; body terms: main, contributions, summarized, follows, Graph2Nav, real-time, objectrelation, graph | p. 1 (I. INTRODUCTION), p. 1 (I. INTRODUCTION), p. 3 (III. GRAPH2NAV) |
...
PDF body contribution framing (p. 1 (I. INTRODUCTION), p. 1 (I. INTRODUCTION), p. 3 (III. GRAPH2NAV)): Our main contributions are summarized as follows: • We propose Graph2Nav, a new real-time 3D objectrelation graph generation framework that combines strengths of 2D object-relation graphs and 3D semantic mapping ...
...
navigation writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. Evidence interface anchors: p. 3 (III. GRAPH2NAV), p. 3 (III. GRAPH2NAV), p. 1 (I. INTRODUCTION), p. 1 (Abstract). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **Evidence anchors reviewed:** problem p. 1 (I. INTRODUCTION), p. 1 (I. INTRODUCTION), interface p. 3 (III. GRAPH2NAV), p. 3 (III. GRAPH2NAV), p. 1 (I. INTRODUCTION), p. 1 (Abstract), objective p. 1 (Abstract), p. 3 (III. GRAPH2NAV), p. 3 (III. GRAPH2NAV), p. 4 (III. GRAPH2NAV).
