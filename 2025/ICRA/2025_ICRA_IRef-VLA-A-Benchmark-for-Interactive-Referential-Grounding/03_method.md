...
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** To advance the path towards more intelligent interaction in natural language navigation, we propose the IRef-VLA dataset as a benchmark for both the referential objectgrounding ...
...
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** As we progress towards generalizable embodied intelligence, there is a need for methods that are capable of reasoning in 3Dspace and interacting with humans.
...
| Metric / failure reporting | An agent that can 1) similarly solve such a problem, 2) handle imperfect or ambiguous language, and 3) interact with humans to ... | p. 1 (I. INTRODUCTION), p. 5 (Figure/Table caption) |
...
- **PDF anchors reviewed:** method p. 1 (I. INTRODUCTION), p. 1 (Abstract), objective p. 1 (Abstract), p. 1 (I. INTRODUCTION), temporal 본문 anchor 없음.
