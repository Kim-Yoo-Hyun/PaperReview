...
PDF body method statement (p. 2 (I. INTRODUCTION), p. 2 (I. INTRODUCTION), p. 3 (III. METHOD), p. 4 (2) Computing open-vocabulary features for the scene repre), p. 1 (Abstract), p. 4 (2) Computing open-vocabulary features for the scene repre)): Object-centric open-vocabulary 3D segmentation methods typically first extract a set of class-agnostic 3D object instance masks and then compute a feature representation per object, represented in the joint vision-language embedding ...
...
- **p. 3 / 2) Computing open-vocabulary features for the scene repre - extractive body cue:** These masks represent the object nodes at the first level of our hierarchical scene representation.
- **p. 4 / 2) Computing open-vocabulary features for the scene repre - extractive body cue:** For each 3D segment, neighboring segments within the same object that exhibit similar features are identified and merged based on two constraints: 1) Proximity: The ...
...
- **p. 4 / 2) Computing open-vocabulary features for the scene repre - extractive body cue:** For each 3D segment, neighboring segments within the same object that exhibit similar features are identified and merged based on two constraints: 1) Proximity: The ...
- **p. 4 / 2) Computing open-vocabulary features for the scene repre - extractive body cue:** This refinement updates the 3D segments in the hierarchical scene representation, reflecting the semantic merging.
...
- **p. 3 / III. METHOD - extractive body cue:** This representation is built upon 3D scenes reconstructed using posed RGB-D image sequences, as shown in Fig.
...
- **p. 1 / I. INTRODUCTION - extractive body cue:** Ultimately, systems designed for such real-world interactions must be able to identify scene entities based on flexible and user-defined descriptions.
- **p. 3 / 2) Computing open-vocabulary features for the scene repre - extractive body cue:** Given the 3D scene Pscene ∈RN×3 where N is the number of points, it outputs M binary instance masks M = Fobj(Pscene) = {m3D 1 ...
- **p. 4 / 2) Computing open-vocabulary features for the scene repre - extractive body cue:** Given an input query such as "seat of a chair", we first encode the query using the SigLIP text encoder to obtain an embedding vector ...
- **p. 4 / 2) Computing open-vocabulary features for the scene repre - extractive body cue:** Object-features 4 are extracted using a method inspired by [7] and [8], leveraging class-agnostic object masks to identify optimal views for semantic feature extraction.
- **p. 5 / IV. DATA - extractive body cue:** Using the SceneFun3D annotation tool [5], we performed fine-grained semantic annotation on high-resolution point clouds, and extended it to incorporate object-part hierarchy information.
...
- **Body-defined terms:** Object-centric, open-vocabulary, segmentation, methods, typically, first, extract, class-agnostic, object, instance, masks, then, compute, feature, representation, represented, joint, vision-language, embedding, space.
...
- **p. 6 / V. EXPERIMENTS - extractive body cue:** Aggr. search AP AP50 AP25 (1) Ours ✓ 4.7 8.2 17.6 (2) Ours ✓ ✓ 6.6 11.4 23.7 (3) Ours ✓ ✓ ✓(max.) 7.5 13.5 ...
- **p. 5 / V. EXPERIMENTS - extractive body cue:** First, we evaluate the quality of our segment features for identifying object parts using an oracle mask experiment, isolating feature quality from the effect of ...
- **p. 5 / V. EXPERIMENTS - extractive body cue:** Additionally, we validate our design choices through corresponding ablation studies.
- **p. 6 / V. EXPERIMENTS - extractive body cue:** IV emphasize the importance of those components.
