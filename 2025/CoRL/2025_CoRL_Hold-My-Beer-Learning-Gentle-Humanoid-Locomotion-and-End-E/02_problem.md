...
- **p. 1 / Abstract - extractive PDF cue:** Can your humanoid walk up and hand you a full cup of beer-without spilling a drop?
- **p. 1 / Abstract - extractive PDF cue:** While humanoids are increasingly featured in flashy demos-dancing, delivering packages, traversing rough terrain-fine-grained control during locomotion remains a significant challenge.
- **p. 1 / Abstract - extractive PDF cue:** In particular, stabilizing a filled end-effector (EE) while walking is far from solved, due to a fundamental mismatch in task characteristics: locomotion demands slow-timescale, robust ...
- **p. 1 / Abstract - extractive PDF cue:** To address this, we propose SoFTA, a Slow-Fast Two-Agent framework that decouples upper-body and lower-body control into separate agents operating at different frequencies and with ...
- **p. 1 / Abstract - extractive PDF cue:** This temporal and objective separation mitigates policy interference and enables coordinated whole-body behavior.
- **p. 2 / 1 Introduction - extractive PDF cue:** The nature of ground contacts makes it more susceptible to the sim-to-real gap, demanding greater robustness against noise and disturbances.
...
PDF contribution framing (p. 2 (1 Introduction), p. 2 (1 Introduction), p. 16 (A.1 Training Details), p. 16 (A.1 Training Details)): Our key contributions are: • We introduce SoFTA, a novel slow-fast two-agent RL framework that decouples control for locomotion and EE stabilization in both temporal and task objective space, enabling ...
...
humanoid writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 15 (A.1 Training Details), p. 15 (A.1 Training Details), p. 17 (A.1 Training Details), p. 2 (1 Introduction). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (1 Introduction), p. 2 (1 Introduction), interface p. 15 (A.1 Training Details), p. 15 (A.1 Training Details), p. 17 (A.1 Training Details), p. 2 (1 Introduction), objective p. 17 (A.1 Training Details), p. 16 (A.1 Training Details).
