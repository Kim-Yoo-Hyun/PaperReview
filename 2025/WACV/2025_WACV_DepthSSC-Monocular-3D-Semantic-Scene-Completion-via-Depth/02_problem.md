...
| Observation / input | The final output Yt ∈RH×W ×Z×(M+1) represents the semantic segmentation map, where H×W ×Z is the output resolution and M +1 indicates ... | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF body |
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. Evidence interface anchors: p. 3 (3.1. Preliminary), p. 3 (3.1. Preliminary), p. 1 (1. Introduction), p. 2 (1. Introduction). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **Evidence anchors reviewed:** problem p. 1 (1. Introduction), p. 2 (1. Introduction), p. 1 (1. Introduction), interface p. 3 (3.1. Preliminary), p. 3 (3.1. Preliminary), p. 1 (1. Introduction), p. 2 (1. Introduction), objective p. 4 (3.2. Spatially-Transformed Graph Fusion), p. 4 (3.2. Spatially-Transformed Graph Fusion), p. 3 (3.1. Preliminary).
