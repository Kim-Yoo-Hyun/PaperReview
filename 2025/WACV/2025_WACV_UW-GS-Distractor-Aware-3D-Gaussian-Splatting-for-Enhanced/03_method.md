...
- **p. 3 / 3.1. Problem formulation - extractive PDF cue:** Therefore, we propose a new color appearance model and a physical-based density control module in UW-GS.
- **p. 4 / 3.1. Problem formulation - extractive PDF cue:** In the splatting process, the physical-based density control module addresses densification failures and the binary motion mask handle distractors. we propose a pixel-level mask, named ...
- **p. 5 / 3.5. Binary Motion Mask - extractive PDF cue:** Inspired by RobustNeRF [35], we introduce a Binary Motion Mask (BMM) ω into our reconstruction loss function to eliminate the distractors as the follows: LRec ...
- **p. 6 / 3.6. Loss Function - extractive PDF cue:** We use a synthetic ground truth depth map D that is predicted by DepthAnything [47], a novel monocular depth estimation model that can adapt to ...
- **p. 4 / 3.3. Color Appearance Model - extractive PDF cue:** The modified color for Gaussians is then sent to 3DGS module.
- **p. 3 / 3.1. Problem formulation - extractive PDF cue:** Recent studies [31, 44, 48] have utilized temporal information to achieve 4D rendering; however, these methods typically model foreground motions with predictable motion trajectory and ...
- **p. 5 / 3.6. Loss Function - extractive PDF cue:** The final loss function, L, used for optimization, comprises a reconstruction loss LRec (see subsection 3.5), a 3284
- **p. 4 / 3.4. Physical-based Density Control - extractive PDF cue:** However, in underwater scenes, Equation 3 suggests that δLRec δcolor is calculated from T D · δLRec δcolorobject so that each Gaussian color gradient has ...

## Design Rationale

- **p. 4 / 3.3. Color Appearance Model - extractive PDF cue:** To address this issue, we propose a novel approach for color appearance formation.
- **p. 1 / 1. Introduction - extractive PDF cue:** To address the aforementioned issues, we propose a new Gaussian Splatting (GS)-based method, UW-GS, specifically for underwater scenes.
- **p. 2 / 1. Introduction - extractive PDF cue:** We also incorporated pseudo-depth maps generated from DepthAnything [47], trained with more general scenes, to enhance the robustness of our method.
...
- **p. 4 / 3.4. Physical-based Density Control - extractive PDF cue:** However, in underwater scenes, Equation 3 suggests that δLRec δcolor is calculated from T D · δLRec δcolorobject so that each Gaussian color gradient has ...
- **p. 6 / 3.6. Loss Function - extractive PDF cue:** This loss is developed based on the underwater image formation model Equation 3, resulting in the approximate depth per Gaussian can be calculated in the ...
- **p. 5 / 3.6. Loss Function - extractive PDF cue:** The final loss function, L, used for optimization, comprises a reconstruction loss LRec (see subsection 3.5), a 3284
- **p. 5 / 3.4. Physical-based Density Control - extractive PDF cue:** Furthermore, by adjusting the 2D position gradient, this method optimizes regions where object color information is substantially attenuated, thereby enhancing 3DGS performance in areas characterized ...
- **p. 3 / 3.1. Problem formulation - extractive PDF cue:** However, Equation 2 is insufficient for representing the underwater scenes [39].
- **p. 3 / 3.1. Problem formulation - extractive PDF cue:** During training, the 2D position gradient, an intermediate result in backward propagation, is used as evidence to indicate "color under-represented" issues [23].
...
- **p. 4 / 3.2. Overview of UW-GS - extractive PDF cue:** The 3D Gaussians with modified color will be sent to do 2D projection and then generate pixel color in rasterization module to output the final ...
- **p. 4 / 3.3. Color Appearance Model - extractive PDF cue:** Similar to [25], we use an additional MLP f with positon encoded depth and viewing direction input to estimate medium properties: (T D i , ...
- **p. 6 / 3.6. Loss Function - extractive PDF cue:** Lca imposes restrictions on T D and T B, which are closely related to the depth.
- **p. 6 / 3.6. Loss Function - extractive PDF cue:** In 3DGS, we can render depth maps in a similar way as image: ˆD = X i∈N αizi i-1 Y j=1 (1 -αj).
- **p. 3 / 3.1. Problem formulation - extractive PDF cue:** 3D Gaussian Splatting (3DGS) is a point-based approach that uses discrete Gaussian point clouds to represent a 3D scene.
- **p. 3 / 3.1. Problem formulation - extractive PDF cue:** Based on its average value, the sparse point cloud obtained from COLMAP [36] is adaptively grown by splitting and cloning.
- **p. 5 / 3.4. Physical-based Density Control - extractive PDF cue:** As a result, the point clouds at larger scales exhibit improved representational capability, thereby reducing both blur and needle-like artifacts.
...
- **p. 6 / 4. Experiment Configuration - extractive PDF cue:** For training, we carried out 15,000 iterations in a single RTX3090 GPU.
...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Figure 7. Examples of rendering results from Composite and Sar- dine scenes. From left to right: raw videos, results without and with BMM, respectively. restored ...
- **p. 8 / 5. Results and Discussion - extractive PDF cue:** We isolate our contributions using a set of modified architectures: (V1) solely using spherical harmonics to represent view-dependent color (note that MLP will also assist ...
