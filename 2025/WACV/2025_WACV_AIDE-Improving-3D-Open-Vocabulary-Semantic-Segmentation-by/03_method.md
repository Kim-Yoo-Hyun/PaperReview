...
- **p. 5 / 3.5. Training Objective - extractive body cue:** The training objective of our proposed AIDE is a weighted linear combination of segmentation loss (Eq.
- **p. 5 / 3.5. Training Objective - extractive body cue:** (2)), confidence calibration loss (Eq.
- **p. 5 / 3.4. Adaptive Segmentation-Text Modeling - extractive body cue:** Sequentially, for each transformer layer, trainable tokens are merged with the output of the previous layer as the input of the current layer, i.e., concat([TOKENSi, ...
- **p. 4 / 3.1. Problem Definition - extractive body cue:** To automatically find the most suitable prompt for adapting text encoders into 3D scenarios, AIDE extends prompt tuning [38] by incorporating learnable tokens in the ...
- **p. 4 / 3.1. Problem Definition - extractive body cue:** We first generate numerous captions using the temperature-based generation strategy, then sample captions based on their similarity to the images (CLIP-rewarded sampling), and finally align ...
...
- **p. 4 / 3.1. Problem Definition - extractive body cue:** We first generate numerous captions using the temperature-based generation strategy, then sample captions based on their similarity to the images (CLIP-rewarded sampling), and finally align ...
- **p. 1 / 1. Introduction - extractive body cue:** Due to the lack of large-scale 3D-image-text pairs, instead of training a 3D-language model from scratch, recent works [13, 23, 57] propose to transfer the ...
- **p. 2 / 1. Introduction - extractive body cue:** Furthermore, to adapt text encoders for 3D semantics, the adaptive segmentation module extends beyond the popular visual prompt tuning methods [31, 38, 86] by incorporating ...
- **p. 2 / 1. Introduction - extractive body cue:** The adaptive segmentation module adapts the text encoder by integrating learnable prompts across the input space and each layer of text encoder. • Extensive experiments ...
- **p. 5 / 3.4. Adaptive Segmentation-Text Modeling - extractive body cue:** Specifically, at the input layer, we directly concatenate the trainable tokens with the text as the input of the text encoder.
...
- **p. 5 / 3.4. Adaptive Segmentation-Text Modeling - extractive body cue:** During inference, we use the trainable tokens and the category names as the input of the text encoder ftext(·) to generate the category embedding C ...
- **p. 6 / 4. Experiments - extractive body cue:** Four trainable tokens are used in adapting text encoders.
- **p. 5 / 3.4. Adaptive Segmentation-Text Modeling - extractive body cue:** One possible solution is fine-tuning VLMs to handle 3D data for better alignment between text encoders of VLMs and 3D models.
...
- **p. 6 / 4.3. Ablation Studies - extractive body cue:** In this part, we present the ablation studies on the effects of two proposed modules (Tab.
...
- **p. 7 / 4.3. Ablation Studies - extractive body cue:** Ablation studies on different numbers of learnable tokens of AIDE on ScanNet (B15/N4). conduct a series of experiments as shown in Tab.
...
- **Evidence anchors reviewed:** method p. 5 (3.4. Adaptive Segmentation-Text Modeling), p. 5 (3.4. Adaptive Segmentation-Text Modeling), objective p. 5 (3.5. Training Objective), p. 5 (3.5. Training Objective), temporal p. 1 (Abstract), p. 8 (4.4. Qualitative Results-Generalization).
