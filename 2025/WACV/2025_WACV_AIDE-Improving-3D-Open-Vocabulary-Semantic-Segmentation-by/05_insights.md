...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: In this paper, to address these issues and improve generalization performance, we propose an AlIgned 3D Open-Vocabulary S Emantic Segmentation framework, called A I D E, with two ...
- 출발 문제 단서: Due to the lack of large-scale 3D-vision-language segmentation data, instead of training models from scratch, the current solutions distill knowledge from pre-trained 2D vision-language models (VLMs) into 3D ...
- 주장된 효과 단서: Extensive experiments show that A I D E outperforms previous methods by a large margin on three representative benchmarks, demonstrating its effectiveness.
...
- Result cue: Extensive experiments show that A I D E outperforms previous methods by a large margin on three representative benchmarks, demonstrating its effectiveness.
