...
- **p. 1 / Abstract - extractive PDF cue:** Vision-Language-Action (VLA) models demonstrate remarkable potential for generalizable robotic manipulation.
- **p. 1 / Abstract - extractive PDF cue:** The performance of VLA models can be improved by integrating with action chunking, a critical technique for effective control.
- **p. 1 / Abstract - extractive PDF cue:** However, action chunking linearly scales up action dimensions in VLA models with increased chunking sizes.
- **p. 1 / Abstract - extractive PDF cue:** This reduces the inference efficiency.
- **p. 1 / Abstract - extractive PDF cue:** Therefore, accelerating VLA integrated with action chunking is an urgent need.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** The pursuit of robust and generalizable robotic manipulation policies remains a fundamental challenge in embodied AI research [1].
- **p. 2 / I. INTRODUCTION - extractive PDF cue:** To address the above challenges, we present a novel parallel decoding framework for the mainstream VLA model with action chunking, called Parallel Decoding for VLA ...
...
| Observation / input | It takes two images as input, a static image Istatic and a gripper image Igripper, to get a comprehensive observation. | image/video, language instruction, proprioception과 history | exact sensor/frame/preprocessing from PDF |
...
- **p. 2 / I. INTRODUCTION - extractive PDF cue:** To address the above challenges, we present a novel parallel decoding framework for the mainstream VLA model with action chunking, called Parallel Decoding for VLA ...
- **p. 2 / I. INTRODUCTION - extractive PDF cue:** It preserves action performance while eliminating the bottlenecks in the efficiency of autoregressive decoding. • We design a decoding-process-only acceleration strategy for VLA inference.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** This emerging paradigm shows strong effectiveness and generalization in diverse scenarios.

## What the Paper Changes

PDF contribution framing (p. 3 (III. METHOD), p. 2 (I. INTRODUCTION), p. 2 (I. INTRODUCTION), p. 3 (III. METHOD), p. 4 (III. METHOD)): In this section, we introduce the details of our method PD-VLA.
...
- **p. 3 / III. METHOD - extractive PDF cue:** Finally, we present parallel decoding to accelerate inference in subsection III-C.
...
vla writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 3 (III. METHOD), p. 3 (III. METHOD), p. 1 (I. INTRODUCTION), p. 1 (I. INTRODUCTION). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (I. INTRODUCTION), p. 2 (I. INTRODUCTION), p. 2 (I. INTRODUCTION), p. 1 (I. INTRODUCTION), interface p. 3 (III. METHOD), p. 3 (III. METHOD), p. 1 (I. INTRODUCTION), p. 1 (I. INTRODUCTION), objective p. 4 (III. METHOD), p. 4 (III. METHOD), p. 3 (III. METHOD).
