...
- **p. 1 / Abstract - extractive PDF cue:** Collaborative perception in unknown environments is crucial for multi-robot systems.
- **p. 1 / Abstract - extractive PDF cue:** With the emergence of foundation models, robots can now not only perceive geometric information but also achieve open-vocabulary scene understanding.
- **p. 1 / Abstract - extractive PDF cue:** However, existing map representations that support open-vocabulary queries often involve large data volumes, which becomes a bottleneck for multi-robot transmission in communication-limited environments.
- **p. 1 / Abstract - extractive PDF cue:** To address this challenge, we develop a method to construct a graph-structured 3D representation called COGraph, where nodes represent objects with semantic features and edges ...
- **p. 1 / Abstract - extractive PDF cue:** Before transmission, a data-driven feature encoder is applied to compress the feature dimensions of the COGraph.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** However, current open-vocabulary 3D map representations demand significant data storage [9][11], which becomes a communication bottleneck for multi-robot mapping systems.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** This data explosion makes it difficult for multiple robots to share and update maps in real time.
...
| Observation / input | 3D back projection is conducted using FO images, depth images, and poses derived from SLAM. | camera/depth stream, pose, map와 language goal | exact sensor/frame/preprocessing from PDF |
...
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** This data explosion makes it difficult for multiple robots to share and update maps in real time.
- **p. 2 / I. INTRODUCTION - extractive PDF cue:** mapping works [14] [18] have explored the collaborative construction of 3D scene graphs, they do not consider open-vocabulary capabilities and have yet to address the ...
...
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** Recent advances in visual foundation models (e.g., SAM [2]) and vision-language models (e.g., CLIP [3]) have enabled the development of open-vocabulary 3D map representations.
...
navigation writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 4 (III. METHOD), p. 4 (III. METHOD), p. 1 (I. INTRODUCTION), p. 2 (I. INTRODUCTION). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (I. INTRODUCTION), p. 1 (I. INTRODUCTION), p. 2 (I. INTRODUCTION), interface p. 4 (III. METHOD), p. 4 (III. METHOD), p. 1 (I. INTRODUCTION), p. 2 (I. INTRODUCTION), objective p. 4 (III. METHOD), p. 4 (III. METHOD).
