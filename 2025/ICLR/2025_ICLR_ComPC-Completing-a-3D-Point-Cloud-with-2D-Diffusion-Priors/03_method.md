...
- **p. 3 / 3 METHODOLOGY - extractive body cue:** 2, the whole completion process is composed of Partial Gaussian Initialization (PGI), Zero-shot Fractal Completion (ZFC), and Point Cloud Extraction (PCE).
...
- **p. 4 / 3 METHODOLOGY - extractive body cue:** Given that the centers of Gin are anchored to Pin, we can estimate Vp by minimizing: Vp = arg min Vn CD(Pin[h(Gin, Vn)], Pin) + ...
- **p. 4 / 3 METHODOLOGY - extractive body cue:** Additionally, it incorporates a Preservation Constraint computed with respect to Vp.
- **p. 4 / 3 METHODOLOGY - extractive body cue:** For any point cloud to be completed, we first determine an reference camera pose Vp, that captures its most completed observation.
- **p. 4 / 3 METHODOLOGY - extractive body cue:** Finally, we extract uniform completed point clouds Pout from the centers of Gall with Point Cloud Extraction.
- **p. 5 / 3 METHODOLOGY - extractive body cue:** Upon estimating the reference camera pose Vp, we render a reference image Iin from 3D Gaussians Gin initialized from partial point cloud Pin.
...
- **p. 4 / 3 METHODOLOGY - extractive body cue:** Finally, we extract uniform completed point clouds Pout from the centers of Gall with Point Cloud Extraction.
- **p. 5 / 3 METHODOLOGY - extractive body cue:** Upon estimating the reference camera pose Vp, we render a reference image Iin from 3D Gaussians Gin initialized from partial point cloud Pin.
- **p. 2 / 1 INTRODUCTION - extractive body cue:** Inspired by the amodal perception (Lehar, 1999; Breckon & Fisher, 2005), we aim to complete a point cloud by utilizing the observation from a reference ...
- **p. 3 / 1 INTRODUCTION - extractive body cue:** 2.2 POINT CLOUD COMPLETION Point cloud completion aims to recover completed point clouds from partial input point clouds.
- **p. 3 / 1 INTRODUCTION - extractive body cue:** The Fractal approach focuses on predicting only the missing regions of point clouds, preserving existing details by retaining the shapes from the partial input.
...
| Rate / latency | training update와 iterative sampling/inference rate가 분리된다. | In this work, we propose a test-time framework for completing partial point clouds across unseen categories without any requirement for training. | Hz/fps, inference time and control rate |
...
|---|---|---|
| Data / condition representation | By introducing abundant priors from 2D diffusion model (Liu et al., 2023), our method can achieve robust completion for objects across different ... | p. 9 (4 EXPERIMENTS), p. 9 (4 EXPERIMENTS) |
...
- **p. 10 / 4 EXPERIMENTS - extractive body cue:** We also provide quantitative ablation study for our proposed components in Table 4.
- **p. 9 / 4 EXPERIMENTS - extractive body cue:** 5 without any prompts and related geometries.
- **p. 9 / 4 EXPERIMENTS - extractive body cue:** Depth Coordinates Normal(Ours) CD 2.25 2.01 1.96 EMD 2.88 2.64 2.60 Table 4: Ablation for ZFC and PCE.
- **p. 10 / 4 EXPERIMENTS - extractive body cue:** More detailed ablation study can be found in the appendix A.
- **p. 14 / Figure/Table caption - extractive body cue:** Figure 9: Ablation study for Grid Pulling module. Far, Near, and Merge denote the Lfar, Lnear, and merge layer gm(·), respectively. Vanilla Gaussian + Opacity ...
- **p. 14 / Figure/Table caption - extractive body cue:** Table 5: The setting of mentioned hyper-parameters in Sec. 3. Hyper-parameters w0 ∼w3 1e-3, 1e3, 1e2, 0.1 δ, σ0, σn 0.01, 0.005, 0.05 Iterations 1000 ...
- **p. 16 / Figure/Table caption - extractive body cue:** Figure 11: Ablation Study for the Fractal completion strategy. W/ Fractal and W/O Fractal denote using and not using Fractal completion strategy, respectively.
...
- **Evidence anchors reviewed:** method p. 4 (3 METHODOLOGY), p. 3 (3 METHODOLOGY), p. 4 (3 METHODOLOGY), objective p. 4 (3 METHODOLOGY), p. 4 (3 METHODOLOGY), temporal p. 5 (2) The color Gc), p. 1 (ABSTRACT), p. 1 (1 INTRODUCTION), p. 3 (1 INTRODUCTION), p. 5 (1) The opacity Go), p. 6 (2) The color Gc).
