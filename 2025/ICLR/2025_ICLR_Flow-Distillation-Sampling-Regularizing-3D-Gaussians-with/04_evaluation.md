...
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** 4.1.2 DATASETS AND METRICS We evaluate our method for 3D reconstruction and novel view synthesis tasks on Mushroom (Ren et al., 2024), ScanNet (v2) (Dai ...
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** We select 5 scenes from the ScanNet (V2) 7
- **p. 8 / 4 EXPERIMENTS - extractive PDF cue:** 3D Reconstruction and novel view synthesis results on ScanNet dataset.
- **p. 8 / 4.2 RESULTS - extractive PDF cue:** The qualitative comparisons on the Mushroom and ScanNet dataset are illustrated in Fig.
- **p. 9 / 4.2 RESULTS - extractive PDF cue:** The comparison results on Mushroom dataset are shown in Tab.
...
- **p. 10 / 4.2 RESULTS - extractive PDF cue:** (5) utilizing ground truth depth in dataset.
- **p. 10 / 4.2 RESULTS - extractive PDF cue:** We apply our method to the 3DGS-based framework, and the geometry is enhanced on the Mushroom, ScanNet, and Replica datasets.
...
- **p. 2 / Figure/Table caption - extractive PDF cue:** Figure 1. Pipeline of the proposed FDS. For each input view, we apply the FDS camera sampling scheme to generate corresponding unobserved sampled view. We ...
...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Table 1. 3D Reconstruction and novel view synthesis results on Mushroom dataset. * Represents that FDS uses the Raft model.
- **p. 8 / Figure/Table caption - extractive PDF cue:** Table 2. 3D Reconstruction and novel view synthesis results on ScanNet dataset.
- **p. 9 / Figure/Table caption - extractive PDF cue:** Table 3. Ablation study on geometry priors.
- **p. 9 / Figure/Table caption - extractive PDF cue:** Figure 4. The error map of Radiance Flow and Prior Flow. RF: Radiance Flow, PF: Prior Flow, * means that there is no FDS loss ...
...
- **p. 10 / 4.2 RESULTS - extractive PDF cue:** Due to the significant movement between images, the Prior Flow fails to accurately match the pixel between them, leading to a further degradation in geometric ...
- **p. 9 / 4.2 RESULTS - extractive PDF cue:** The multi-view depth prior, hindered by the limited feature overlap between input views, fails to offer reliable geometric information.
- **p. 10 / 4.2 RESULTS - extractive PDF cue:** 4.4 LIMITATION AND FURTHER WORK Firstly, our FDS faces challenges in scenes with significant lighting variations between different views, as shown in the lamp of ...
...
- **p. 8 / 4.2 RESULTS - extractive PDF cue:** This demonstrates the robustness and effectiveness of the FDS method across different datasets.
