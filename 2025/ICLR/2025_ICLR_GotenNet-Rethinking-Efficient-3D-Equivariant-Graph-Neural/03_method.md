...
- **p. 2 / B L - extractive body cue:** Most current architectures (Han et al., 2024; Wang et al., 2024; Liao & Smidt, 2023; Liao et al., 2024) either compromise on expressiveness for efficiency ...
- **p. 1 / ABSTRACT - extractive body cue:** We evaluated models on QM9, rMD17, MD22, and Molecule3D datasets, where the proposed model consistently outperforms state-of-the-art methods in both scalar and high-degree property predictions, ...
- **p. 2 / B L - extractive body cue:** Through rigorous evaluations on benchmark datasets-QM9, Molecule3D, rMD17, and MD22-our approach consistently outperforms state-of-the-art methods, even in its smallest configuration, establishing GotenNet as a versatile ...
...
- **p. 9 / 4 EXPERIMENTS - extractive body cue:** The x-axis shows the node count, while the y-axis shows the training time per batch in milliseconds.
- **p. 9 / 4 EXPERIMENTS - extractive body cue:** We analyze computational efficiency by measuring training time across varying node counts (10-140 nodes per graph).
...
- **p. 10 / 4 EXPERIMENTS - extractive body cue:** The removal of any one of these components results in a significant degradation in performance, particularly in the cases without geometric encoding (row 4) or ...
- **p. 10 / 4 EXPERIMENTS - extractive body cue:** 4.5 ABLATION STUDY Table 5: Ablation study on QM9 dataset. # L Lmax SE SEA GE HTR std log 4 2 ✓ ✓ ✓ ✓ ...
- **p. 8 / 4 EXPERIMENTS - extractive body cue:** As shown in Table 1, even our smallest variant GotenNetS outperforms baseline methods on nine out of twelve targets while surpassing baselines on std.
- **p. 8 / 4 EXPERIMENTS - extractive body cue:** We evaluate three model variants - small (S), base (B), and large (L) - to analyze both performance and scaling behavior, with detailed specifications in ...
- **p. 9 / 4 EXPERIMENTS - extractive body cue:** Both GotenNetS and GotenNetB variants maintain consistent efficiency across all node counts, demonstrating their suitability for large-scale applications where computational overhead is critical.
- **p. 9 / 4 EXPERIMENTS - extractive body cue:** Following the data splits from (Chmiela et al., 2023), we evaluate GotenNet against several baselines, including sDGML (Chmiela et al., 2018), ET (Thölke & De ...
...
- **Evidence anchors reviewed:** method p. 2 (B L), p. 2 (B L), p. 1 (ABSTRACT), p. 1 (ABSTRACT), objective p. 1 (ABSTRACT), p. 2 (B L), temporal p. 7 (4 EXPERIMENTS), p. 8 (4 EXPERIMENTS), p. 1 (ABSTRACT), p. 2 (B L), p. 2 (B L), p. 3 (2 RELATED WORK).
