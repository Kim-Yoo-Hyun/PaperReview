...
- **Method cue:** For rendered image quality on evaluation views (views not provided in training), we use common metrics (PSNR, SSIM, and LPIPS ) as Kerbl et al. .
...
- **Registry interface:** `Robotics, 3D Vision, Gaussian Splatting` is the paper's recorded topic/interface, not evidence that the full robotics loop was evaluated.
...
- Registry position: `Language-Grounded 3D Scene Representations`; tags: `Robotics, 3D Vision, Gaussian Splatting`.
...
- **Protocol carried forward from the legacy note (candidate, not a verified paper evaluation):**
  - 논문 내 evaluation 단서: COCO, Tanks and Temples / accuracy, mIoU, AP, PSNR, SSIM
