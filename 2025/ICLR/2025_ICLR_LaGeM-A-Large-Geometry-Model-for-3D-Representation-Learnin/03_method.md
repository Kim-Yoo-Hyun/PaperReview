...
- **p. 6 / 3 METHODOLOGY - extractive body cue:** As shown in the previous section, we use cross-attention for resampling (both down-sampling and upsampling).
...
- **p. 5 / 3 METHODOLOGY - extractive body cue:** We do not need an explicit loss to regularize the latent space.
- **p. 5 / 3 METHODOLOGY - extractive body cue:** Features to Latents (FtoL) Latent Loss Latents to Features (LtoF) VAE µ = FCµ(x) z = µ + σ ⊙ϵ KL Divergence x′ = FCup(z) ...
...
- **p. 5 / 3 METHODOLOGY - extractive body cue:** We do not need an explicit loss to regularize the latent space.
- **p. 5 / 3 METHODOLOGY - extractive body cue:** Features to Latents (FtoL) Latent Loss Latents to Features (LtoF) VAE µ = FCµ(x) z = µ + σ ⊙ϵ KL Divergence x′ = FCup(z) ...
- **p. 4 / 3 METHODOLOGY - extractive body cue:** (2) This compression step is also regularized by KL divergence.
- **p. 6 / 3 METHODOLOGY - extractive body cue:** Formally, the optimization goal (for our three-level implementation) is as follows, min D3
...
- **p. 4 / 3 METHODOLOGY - extractive body cue:** The process first downsamples the 3D input point cloud PInput = {pi}i=1,...,N with furthest point sampling (FPS), P = FPS(PInput, r), where r is the ...
- **p. 5 / 3 METHODOLOGY - extractive body cue:** For notational convenience, we denote the input point cloud as level 0.
- **p. 5 / 3 METHODOLOGY - extractive body cue:** In the i-th level, we first obtain a lower resolution of the point clouds in the (i -1)-th level, FPS(Pi-1, ri-1) = Pi where P0 ...
- **p. 6 / 3 METHODOLOGY - extractive body cue:** For diffusion-based image super-resolution methods, this is often done by bilinearly interpolating small images and concatenating them with denoising networks' inputs.
- **p. 3 / 1 INTRODUCTION - extractive body cue:** Point Cloud Output Init Diffusion Level 2 Diffusion Level 1 Diffusion Level 3 C Figure 2: Pipeline.
- **p. 4 / 3 METHODOLOGY - extractive body cue:** We also write CA(P, PInput) for short.
- **p. 6 / 3 METHODOLOGY - extractive body cue:** Specifically, assuming we are training a denoising network for Z2, the input of the network is ˜Z2(t), CA( ˜Z2(t), Z3).
...
- **p. 6 / 3 METHODOLOGY - extractive body cue:** When using a small number (512) of latent vectors, our model uses 0.87x time and 0.66x memory during training.
- **p. 7 / 4 EXPERIMENTS - extractive body cue:** We can see that, LaGeM-ShapeNet has almost the same number of parameters as VecSet, but with much shorter training time and less training memory.
- **p. 9 / 4 EXPERIMENTS - extractive body cue:** For Objaverse-10k, due to limited training GPU resources, we select a subset of 10k models from Objaverse and train the unconditional generative model.
- **p. 4 / 3 METHODOLOGY - extractive body cue:** This severely affects the training time when M is large.
- **p. 6 / 3 METHODOLOGY - extractive body cue:** VecSet LaGeM VecSet LaGeM VecSet LaGeM Batch Size 64 8 4 Self Attn Layers 24 8/8/8 24 8/8/8 24 8/8/8 Attn Channels 512 512/512/512 1k ...
...
- **Evidence anchors reviewed:** method p. 5 (3 METHODOLOGY), p. 5 (3 METHODOLOGY), p. 4 (3 METHODOLOGY), p. 4 (3 METHODOLOGY), p. 6 (3 METHODOLOGY), p. 6 (3 METHODOLOGY), objective p. 5 (3 METHODOLOGY), p. 5 (3 METHODOLOGY), p. 4 (3 METHODOLOGY), p. 6 (3 METHODOLOGY), p. 6 (3 METHODOLOGY), temporal p. 6 (3 METHODOLOGY), p. 4 (3 METHODOLOGY), p. 4 (3 METHODOLOGY), p. 5 (3 METHODOLOGY), p. 6 (3 METHODOLOGY), p. 7 (4 EXPERIMENTS).
