...
- **p. 8 / 4 EXPERIMENTS - extractive PDF cue:** Overall, HEPi generalizes well to unseen objects, performs consistently across resolutions, and handles noise effectively, making it suitable for real-world tasks.
- **p. 6 / 4 EXPERIMENTS - extractive PDF cue:** 4.1 EXPERIMENTAL SETUP Task Design Our task design, illustrated in Figure 2, emphasizes testing the role of geometric structure and information exchange between objects and ...
- **p. 6 / 4 EXPERIMENTS - extractive PDF cue:** We then introduce a novel task, Rope-Shaping, which increases complexity by requiring the rope to form a specific shape (a "W" from the LASA dataset ...
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** 4.2 RESULTS AND DISCUSSIONS In the main evaluations, we generate 1000 scenes per task (sampled according to Appendix B) and compute the undiscounted return over ...
- **p. 8 / 4 EXPERIMENTS - extractive PDF cue:** Finally, we evaluate the generalization of these models to unseen objects on two rigid tasks: rigidsliding and rigid-insertion.
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** HEPi and EMPN perform similarly on the 2D tasks, but as tasks scale up to 3D environments, such as cloth-hanging-3D, HEPi shows a significant advantage, ...
- **p. 9 / 4 EXPERIMENTS - extractive PDF cue:** The computational overhead is measured as the ratio of training time per iteration (over seven tasks) relative to HEPi. and three (plus, star, pentagon)-and tested ...
- **p. 9 / 4 EXPERIMENTS - extractive PDF cue:** However, in tasks with a lower-dimensional action space, such as 2D environments, well-tuned PPO performs comparably to TRPL in terms of final average return, though ...
...
- **p. 2 / Figure/Table caption - extractive PDF cue:** Figure 1: Left: A Cloth-Hanging task represented by a heterogeneous graph that comprises two disjoint node sets, objects, and actuators, connected through directed, fully-connected inter-edges. ...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Figure 2: Illustration of our diverse and challenging manipulation tasks, involving both rigid and deformable objects. These tasks require precise control under complex geometric constraints, ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Figure 3: Evaluation curves for our seven manipulation tasks, comparing HEPi (ours), EMPN, and Transformer baselines. Results are averaged over 10 seeds, using IQM with ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Figure 4: Performance of different models on the Cloth-Hanging task across varying sample spaces. Overall, performance improves as the sample space decreases. In terms of ...
...
- **p. 9 / Figure/Table caption - extractive PDF cue:** Figure 7: Performance comparison on tasks with and without attention mechanisms over 10 seeds. Adding attention significantly increases the training time but does not improve ...
- **p. 10 / Figure/Table caption - extractive PDF cue:** Figure 8: Performance comparison between HEPi and Transformer models with TRPL and PPO over 10 seeds. TRPL shows stable performance across all tasks, while PPO ...
- **p. 16 / Figure/Table caption - extractive PDF cue:** Figure 9: Demonstration of graph with overlapping and non-overlapping nodes. Actuator nodes are in red, object nodes are in either white or orange. messages from ...
...
| Published as a conference paper at ICLR 2025 HEPi (Ours) EMPN Transformer HeteroGNN GNN 0 1 2 3 4 5 Env Steps (1M) -1.75 ... | definition/direction/unit from same section | p. 8 (4 EXPERIMENTS) |
| Published as a conference paper at ICLR 2025 HEPi (Ours) HEPi + Attention EMPN + Attention EMPN -1.40 -1.20 -1.00 IQM Return rigid-sliding-2D -1.40 ... | definition/direction/unit from same section | p. 9 (4 EXPERIMENTS) |
...
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** Results are averaged over 10 seeds, using IQM with 95% confidence intervals.
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** 4.2 RESULTS AND DISCUSSIONS In the main evaluations, we generate 1000 scenes per task (sampled according to Appendix B) and compute the undiscounted return over ...
- **p. 9 / 4 EXPERIMENTS - extractive PDF cue:** Published as a conference paper at ICLR 2025 HEPi (Ours) HEPi + Attention EMPN + Attention EMPN -1.40 -1.20 -1.00 IQM Return rigid-sliding-2D -1.40 -1.20 ...
...
| Published as a conference paper at ICLR 2025 HEPi (Ours) EMPN Transformer HeteroGNN GNN 0 1 2 3 4 5 Env Steps (1M) -1.75 ... | p. 8 (4 EXPERIMENTS) |
...
- **p. 10 / 6 CONCLUSION - extractive PDF cue:** Limitation In our current setup, we abstract away the robot body, focusing solely on end-effector movements.
- **p. 10 / 6 CONCLUSION - extractive PDF cue:** This limitation could be addressed by integrating state-of-the-art computer vision techniques to extract keypoints from cameras (Tumanyan et al., 2024; Hou et al., 2024), using ...
- **p. 8 / 4 EXPERIMENTS - extractive PDF cue:** As shown in Figure 5 (left), HEPi maintains high performance across resolutions with only mild degradation at higher noise levels, demonstrating its scalability and robustness ...
- **p. 9 / 4 EXPERIMENTS - extractive PDF cue:** Overall, as depicted in Figure 8, in tasks requiring high exploration such as cloth-hanging-3D, PPO struggles to maintain conservative updates, often resulting in unstable performance.
