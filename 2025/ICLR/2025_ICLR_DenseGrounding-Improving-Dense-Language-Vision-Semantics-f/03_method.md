...
- **p. 5 / 4 METHOD - extractive body cue:** 4.1 HIERARCHICAL SCENE SEMANTIC ENHANCER To mitigate the semantic loss inherent in sparse point clouds, we introduce the Hierarchical Scene Semantic Enhancer (HSSE) module, which ...
...
- **p. 7 / 4 METHOD - extractive body cue:** 4.3 ENHANCED BASELINE In this work, we introduce a strong baseline by building upon the state-of-the-art method established by EmbodiedScan for ego-centric multiview 3D visual ...
- **p. 7 / 4 METHOD - extractive body cue:** This approach enables LLMs to better understand the spatial and relational dynamics of objects in a given scene, leading to a more holistic comprehension of ...
- **p. 15 / A.5 IMPLEMENTATION DETAILS OF LSE - extractive body cue:** In this section, we provide the implementation details of the Language Semantic Enhancer (LSE) module, focusing on how the LLM is prompted.
- **p. 5 / 4 METHOD - extractive body cue:** This enriched information is then unprojected to the depth reconstructed point cloud during fusion, minimizing the semantic loss.
...
- **p. 5 / 4 METHOD - extractive body cue:** This enriched information is then unprojected to the depth reconstructed point cloud during fusion, minimizing the semantic loss.
- **p. 5 / 4 METHOD - extractive body cue:** 4.1 HIERARCHICAL SCENE SEMANTIC ENHANCER To mitigate the semantic loss inherent in sparse point clouds, we introduce the Hierarchical Scene Semantic Enhancer (HSSE) module, which ...
- **p. 7 / 4 METHOD - extractive body cue:** By instructing the LLM to utilize positional relationships among objects in the scene, the prompt ensures sufficient and reliable anchors, encouraging the description of the ...
...
- **p. 4 / 3 PRELIMINARIES - extractive body cue:** (2024), we formalize the ego-centric 3D visual grounding task as follows: Given a language description L ∈RT , together with V views of RGB-D images ...
- **p. 3 / 1 INTRODUCTION - extractive body cue:** HSSE enables interaction between textual and visual features, ensuring the model captures both global context and detailed object semantics from ego-centric inputs. • Our method ...
- **p. 5 / 3 PRELIMINARIES - extractive body cue:** View V 😀 🤔 (a) Overview of DenseGrounding 3D Detection Model OR Bbox Labels Object Location Branch Object Relation Branch Language Desc Select K Scene ...
- **p. 6 / 4 METHOD - extractive body cue:** 4.2 LLM-BASED LANGUAGE SEMANTIC ENHANCER Human interactions with intelligent agents often involve casual and vague language descriptions, resulting in input that lacks clear anchors, limited ...
- **p. 4 / 3 PRELIMINARIES - extractive body cue:** They often rely on ego-centric observations, such as multi-view RGB-D images, rather than pre-established scene-level priors like pre-reconstructed 3D point clouds of the entire scene, ...
- **p. 3 / 1 INTRODUCTION - extractive body cue:** By addressing the ambiguities in annotated text and enhancing the model's capacity to capture scenelevel global visual semantics from ego-centric multi-view inputs, DenseGrounding, advances the ...
- **p. 5 / 4 METHOD - extractive body cue:** Then, it fuses these aggregated view semantics with language semantics, facilitating scene-level multi-view semantic interaction and cross-modal feature fusion.
...
- **p. 9 / 5 EXPERIMENTS - extractive body cue:** We conduct an ablation analysis to assess the effectiveness of each component, as shown in Tab.
- **p. 9 / 5 EXPERIMENTS - extractive body cue:** 4, we conduct an ablation study to determine the optimal number of self-attention layers needed for effective learning of the scenefeature representation.
- **p. 8 / 5 EXPERIMENTS - extractive body cue:** For inference, our model processes descriptions directly, without any enhancement, aligning with our baseline methods for fair comparison.
- **p. 8 / 5 EXPERIMENTS - extractive body cue:** The "Dep" and "Indep" metrics further challenge spatial understanding ability by assessing its performance with and without perspective-specific descriptions.
- **p. 10 / 5 EXPERIMENTS - extractive body cue:** In real-life applications, vague or ambiguous descriptions from human instructions pose challenges, as the model struggles without the necessary information to resolve ambiguities.
- **p. 10 / 5 EXPERIMENTS - extractive body cue:** Lscene Easy Hard Overall 1 40.15 35.44 39.77 2 40.76 32.70 40.11 3 41.95 34.38 41.34 4 41.06 33.96 40.49 6 40.66 33.23 40.06 Table ...
...
- **Evidence anchors reviewed:** method p. 6 (4 METHOD), p. 5 (4 METHOD), p. 6 (4 METHOD), p. 5 (4 METHOD), p. 7 (4 METHOD), p. 7 (4 METHOD), objective p. 5 (4 METHOD), p. 5 (4 METHOD), p. 7 (4 METHOD), temporal p. 3 (1 INTRODUCTION), p. 3 (1 INTRODUCTION), p. 5 (3 PRELIMINARIES).
