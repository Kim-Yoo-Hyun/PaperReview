...
- **p. 4 / 2 METHODOLOGY - extractive body cue:** For the semantic feature map, we use the feature map from AM-RADIO (Ranzinger et al., 2024) as supervision.
- **p. 22 / C.2 PRE-TRAINING DETAILS - extractive body cue:** Following standard protocols for embodied evaluation, we use the [CLS] token as the feature representation.
- **p. 24 / C.2 PRE-TRAINING DETAILS - extractive body cue:** In this study, we maintained all settings identical-data loading, training techniques, hyperparameters, and the encoder-while replacing the volume neural rendering decoder with a multiview transformer-based ...
- **p. 4 / 2 METHODOLOGY - extractive body cue:** We then render depth, RGB, and semantic maps for loss computation.
- **p. 4 / 2 METHODOLOGY - extractive body cue:** (7) Additionally, we incorporate the Eikonal regularization loss Leikonal, near-surface SDF supervision loss Lsdf, and free space SDF loss Lfree, which are standard in neural ...
...
- **p. 3 / 2 METHODOLOGY - extractive body cue:** Finally, we explain the image rendering from the feature volume and loss functions for network optimization in Sec.
- **p. 4 / 2 METHODOLOGY - extractive body cue:** (7) Additionally, we incorporate the Eikonal regularization loss Leikonal, near-surface SDF supervision loss Lsdf, and free space SDF loss Lfree, which are standard in neural ...
- **p. 22 / C.2 PRE-TRAINING DETAILS - extractive body cue:** We employ the Mean Squared Error (MSE) loss function and optimize the model using the AdamW optimizer with a OneCycle learning rate scheduler.
- **p. 3 / 2 METHODOLOGY - extractive body cue:** Following MAE, we apply random masking to input images to enhance robustness, but without a ViT decoder and MAE's pixel reconstruction objective.
- **p. 4 / 2 METHODOLOGY - extractive body cue:** Detailed definitions of these losses are provided in Appendix A.
...
- **p. 3 / 2 METHODOLOGY - extractive body cue:** In this section, we first describe our process for handling multi-view image inputs and feature extraction in Sec.
- **p. 3 / 2 METHODOLOGY - extractive body cue:** 2.1 INPUT PROCESS AND FEATURE EXTRACTION Given a set of multi-view images I = {I1, I2, . . . , IN}, where each Ii ∈R3×H×W ...
- **p. 1 / 1 INTRODUCTION - extractive body cue:** Unlike explicit 3D representations like point clouds or meshes-which prior work (Wang et al., 2024b;a; Ze et al., 2024; Zhu et al., 2024) has shown ...
- **p. 23 / C.2 PRE-TRAINING DETAILS - extractive body cue:** (2023) to use DrQ-v2 (Yarats et al., 2021), a state-of-the-art off-policy actor-critic approach for continuous vision-based control.
- **p. 4 / 2 METHODOLOGY - extractive body cue:** Given multi-view images, we randomly mask patches and input the remaining into a Vision Transformer.
- **p. 22 / C.1 DATASET DETAILS - extractive body cue:** Consequently, we resize the images to 1024 × 1024 before feeding them into RADIO, which outputs a feature map of size 64 × 64.
- **p. 23 / C.2 PRE-TRAINING DETAILS - extractive body cue:** (2021), which train a neural network to detect the 7-DoF grasp poses on monocular image observations.
...
- **p. 22 / C.2 PRE-TRAINING DETAILS - extractive body cue:** Given an image pair from different viewpoints, we first extract features from each image using a frozen, pre-trained Vision Transformer (ViT) encoder.
- **p. 24 / C.2 PRE-TRAINING DETAILS - extractive body cue:** In this study, we maintained all settings identical-data loading, training techniques, hyperparameters, and the encoder-while replacing the volume neural rendering decoder with a multiview transformer-based ...
- **p. 23 / C.2 PRE-TRAINING DETAILS - extractive body cue:** We train for a total of 1.1M frames and all other hyperparameters including random seeds are kept as default and same.
- **p. 24 / C.2 PRE-TRAINING DETAILS - extractive body cue:** In this study, we maintained all settings identical-data loading, training techniques, hyperparameters, and the encoder-while replacing the volume neural rendering decoder with a multiview transformer-based ...
...
- **p. 8 / Figure/Table caption - extractive body cue:** Table 6: Additional ablations on VC-1. Methods SPA-B SPA-MAE RADIO E-RADIO VC-1 AD
...
- **p. 24 / Figure/Table caption - extractive body cue:** Table 11: Additional Ablation Study on Neural Rendering. The models are evaluated on two subsets of the VC-1 benchmark. The model architectures are both ViT-base.
- **p. 9 / Figure/Table caption - extractive body cue:** Table 7: Mask ratio and loss components. C., D., S. denote color, depth, and semantic. Mask Ratio Loss VC-1 Benchmark Mean S.R.
- **p. 22 / C.2 PRE-TRAINING DETAILS - extractive body cue:** To facilitate faster convergence and improve stability, we initialize the encoder with ImageNet pretrained weights from the Masked Autoencoder (MAE), applying a learning rate layer ...
