...
- **p. 8 / 1. I have a blue - extractive body cue:** 4.3 EXPERIMENTS WITH A REAL-WORLD UR5 ROBOT ARM We fine-tune our VLAS-Base by utilizing both the Berkeley UR5 demonstration dataset and our own cup-picking dataset.
- **p. 8 / 1. I have a blue - extractive body cue:** Particularly, this benchmark includes three task categories: (1) Object Ownership Tasks: The robot must interact with the appropriate objects according to their ownership.
- **p. 6 / 1. I have a blue - extractive body cue:** CALVIN with Speech Instructions (CSI) Dataset Given that conventional robot manipulation datasets contain only textual task instructions, we utilized the aforementioned TTS model to generate ...
- **p. 6 / 1. I have a blue - extractive body cue:** Stage III: Robot Manipulation Fine-tuning, where the model is further fine-tuned to execute robot manipulation tasks using both speech and text instructions. instructions, paired with ...
- **p. 7 / 1. I have a blue - extractive body cue:** 4.1 ROBOT MANIPULATION WITH SPEECH INSTRUCTIONS To quantitatively assess the performance of our proposed model for robot manipulation tasks, we conduct experiments on the CALVIN ...
- **p. 7 / 1. I have a blue - extractive body cue:** Each sample in this dataset contains a complete motion trajectory, represented as a sequence of robot actions, along with visual observations from two distinct views ...
- **p. 9 / 1. I have a blue - extractive body cue:** Ownership Ownership Ownership Preference Preference Preference Figure 5: Demonstration of object ownership tasks (top row) and user preference tasks (bottom row) for customized robot manipulation.
- **p. 9 / 1. I have a blue - extractive body cue:** For the speech recognition benchmark, we employ the state-ofthe-art Whisper large-v2 model as the baseline.
...
- **p. 4 / Figure/Table caption - extractive body cue:** Figure 2: Overall Framework of VLAS. VLAS encodes visual and speech inputs via encoders and MLP layers to obtain respective embeddings. The Voice RAG module ...
- **p. 5 / Figure/Table caption - extractive body cue:** Figure 3: Data collection process for the SQA and CSI datasets. speech tokens may impose a significant computational burden when directly input into the LLM, ...
- **p. 6 / Figure/Table caption - extractive body cue:** Figure 4: Training paradigm of VLAS. The training process of VLAS is divided into three stages. Stage I: Speech Alignment, where the model aligns speech ...
- **p. 7 / Figure/Table caption - extractive body cue:** Table 1: Performance of different robot policy models on the CALVIN benchmark. +: Evaluated with the ground truth textual instructions. *: Evaluated with the speech ...
- **p. 8 / Figure/Table caption - extractive body cue:** Table 2: Performance of three types of customized tasks for robot manipulation. +: Evaluated with the ground truth textual instructions. *: Evaluated with the speech ...
- **p. 9 / Figure/Table caption - extractive body cue:** Figure 5: Demonstration of object ownership tasks (top row) and user preference tasks (bottom row) for customized robot manipulation. Speaker id: 260 Meta: I have ...
- **p. 9 / Figure/Table caption - extractive body cue:** Figure 6: Demonstration of compound tasks for customized robot manipulation. 4.4 ANALYSIS FOR THE VLAS-BASE FOUNDATION MODEL The multimodal understanding capability of the VLAS-Base is ...
...
| Figure 9: Demonstration of failure cases of VLA on the customization benchmark. We conducted additional analysis on the failure cases of VLAS and VLA ... | definition/direction/unit from same section | p. 16 (Figure/Table caption) |
...
| Figure 3: Data collection process for the SQA and CSI datasets. speech tokens may impose a significant computational burden when directly input into the ... | definition/direction/unit from same section | p. 5 (Figure/Table caption) |
...
| As can be observed, VLAS-Base obtains nearly the same performance to LLaVA, while significantly outperforming other VLMs. | comparison identity and matched condition | p. 9 (1. I have a blue) |
...
| Stage I: Speech Alignment, where the model aligns speech with text through MLP fine-tuning. | component/input/data sensitivity | p. 6 (1. I have a blue) |
...
- **p. 7 / 1. I have a blue - extractive body cue:** 4.1 ROBOT MANIPULATION WITH SPEECH INSTRUCTIONS To quantitatively assess the performance of our proposed model for robot manipulation tasks, we conduct experiments on the CALVIN ...
- **p. 8 / 1. I have a blue - extractive body cue:** 4.2 ROBOT MANIPULATION FOR CUSTOMIZED TASKS Table 2: Performance of three types of customized tasks for robot manipulation. +: Evaluated with the ground truth textual ...

## Generalization and Failure Cases

| Body cue type | Observed cue or missing regime | Anchor |
|---|---|---|
| body limitation/failure cue | Figure 9: Demonstration of failure cases of VLA on the customization benchmark. We conducted additional analysis on the failure cases of VLAS and VLA ... | p. 16 (Figure/Table caption) |
...
| body limitation/failure cue | It can be seen from Table 2 that when the RAG module is removed, the performance of VLAS significantly degrades on the customized benchmark. | p. 8 (1. I have a blue) |
...
- **Evidence anchors reviewed:** datasets p. 8 (1. I have a blue), p. 8 (1. I have a blue), p. 6 (1. I have a blue), p. 6 (1. I have a blue), p. 7 (1. I have a blue), p. 7 (1. I have a blue), metrics p. 10 (Figure/Table caption), p. 8 (1. I have a blue), p. 8 (1. I have a blue), p. 16 (Figure/Table caption), p. 2 (Figure/Table caption), p. 10 (Figure/Table caption), baselines p. 7 (1. I have a blue), p. 7 (1. I have a blue), p. 10 (Figure/Table caption), p. 8 (1. I have a blue), p. 9 (1. I have a blue), p. 9 (1. I have a blue), results p. 10 (Figure/Table caption), p. 8 (1. I have a blue), p. 9 (1. I have a blue), p. 8 (1. I have a blue), p. 7 (1. I have a blue), p. 7 (1. I have a blue).
