...
- Result/evaluation cue: Our method achieves over a 10% improvement in success rates compared to leading baselines.

위 cue는 기존 official abstract 또는 local text extraction에서 보존한 것이다. 수치·조건·인과적 해석은 full-text 정독 전까지 `UNVERIFIED`다.

## Dependency Position

`RoboPack: Learning Tactile-Informed Dynamics Models for Dense Packing → DexTrack: Towards Generalizable Neural Tracking Control for Dexterous Manipulation from Human References → 이 track의 frontier 연결`
