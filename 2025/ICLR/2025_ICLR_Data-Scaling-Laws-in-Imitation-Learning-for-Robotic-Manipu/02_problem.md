...
| Observation / input | There are several key observations: (1) As the number of training objects increases, the policy's performance on unseen objects consistently improves across ... | observation history와 expert trajectory/action | exact sensor/frame/preprocessing from PDF body |
...
PDF body contribution framing (p. 1 (1 INTRODUCTION), p. 2 (1 INTRODUCTION), p. 4 (3 APPROACH), p. 3 (3 APPROACH), p. 2 (1 INTRODUCTION)): To answer this, we present a comprehensive empirical study on data scaling in imitation learning, which is a predominant method for learning real-world manipulation skills (Shafiullah et al., 2024).
...
il writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. Evidence interface anchors: p. 5 (3 APPROACH), p. 4 (3 APPROACH), p. 4 (3 APPROACH), p. 5 (3 APPROACH). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **Evidence anchors reviewed:** problem p. 1 (1 INTRODUCTION), p. 1 (1 INTRODUCTION), p. 2 (1 INTRODUCTION), p. 2 (1 INTRODUCTION), interface p. 5 (3 APPROACH), p. 4 (3 APPROACH), p. 4 (3 APPROACH), p. 5 (3 APPROACH), objective p. 7 (3 APPROACH), p. 8 (3 APPROACH).
