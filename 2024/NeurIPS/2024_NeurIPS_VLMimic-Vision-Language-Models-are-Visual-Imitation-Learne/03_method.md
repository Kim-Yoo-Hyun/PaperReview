...
- **Paper-specific method/interface:** (II) We build an effective human-object interaction grounding algorithm to enhance fine-grained action recognition capabilities, and propose hierarchical constraint representations for VLM reasoning to reduce information redundancy and ... (p. 3, 1 Introduction).
- **Objective/update evidence:** In manipulation constraint learning, keypoints are obtained by uniformly sampling 10 points. (p. 15, A Implementation details).
- **Temporal/runtime evidence:** R3M-DP and DP are trained using the robot demonstrations with paired observation and action sequences. (p. 7, 4 Experiments).
