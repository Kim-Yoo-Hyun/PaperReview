...
- **p. 1 / Abstract - extractive PDF cue:** Leveraging massive knowledge from large language models (LLMs), recent machine learning models show notable successes in generalpurpose task solving in diverse domains such as computer ...
- **p. 1 / Abstract - extractive PDF cue:** However, several significant challenges remain: (i) most of these models rely on 2D images yet exhibit a limited capacity for 3D input; (ii) these models ...
- **p. 1 / Abstract - extractive PDF cue:** We argue these limitations significantly hinder current models from performing real-world tasks and approaching general intelligence.
- **p. 1 / Abstract - extractive PDF cue:** To this end, we introduce LEO, an embodied multimodal generalist agent that excels in perceiving, grounding, reasoning, planning, and acting in the 3D world.
- **p. 1 / Abstract - extractive PDF cue:** LEO is trained with a unified task interface, model architecture, and objective in two stages: (i) 3D vision-language (VL) alignment and (ii) 3D vision-language-action (VLA) ...
...
- **p. 1 / 1. Introduction - extractive PDF cue:** This limitation stands as an obstacle that prevents current models from solving realworld tasks and approaching general intelligence.
...
| Observation / input | The leading design principles of LEO are two-fold: 1) It should handle the multi-modal input of egocentric 2D, global 3D, and textual ... | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF |
...
PDF contribution framing (p. 7 (4.3. Embodied Action in 3D World), p. 1 (1. Introduction), p. 1 (1. Introduction), p. 7 (4.3. Embodied Action in 3D World)): We present the results of CLIPort manipulation and object navigation in Tabs.
...
- **p. 7 / 4.3. Embodied Action in 3D World - extractive PDF cue:** Underlined figures indicate zero-shot results on novel scenes (3RScan).
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 3 (2. Model), p. 4 (2.3. Training & Inference), p. 6 (4.2. Scene-grounded Dialogue and Planning), p. 3 (2.1. Tokenization). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (1. Introduction), p. 1 (1. Introduction), interface p. 3 (2. Model), p. 4 (2.3. Training & Inference), p. 6 (4.2. Scene-grounded Dialogue and Planning), p. 3 (2.1. Tokenization), objective p. 3 (2. Model), p. 3 (2.3. Training & Inference), p. 7 (4.3. Embodied Action in 3D World), p. 7 (4.3. Embodied Action in 3D World).
