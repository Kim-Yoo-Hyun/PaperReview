...
- **p. 7 / 3 Method - extractive PDF cue:** Within the decoder layer l, the instance queries Ql retrieve task-relevant information by first attending to the scene features {V, I, P} in parallel and ...
- **p. 6 / 3 Method - extractive PDF cue:** 3: The model architecture of PQ3D, which consists of Task Prompt Encoding, 3D Scene Encoding, and Prompt-guided Query Learning modules.
- **p. 8 / 3 Method - extractive PDF cue:** To support flexible inference when only some representations are available, we randomly drop out some scene features with rate 0.6 in masked-attention computation during training.
- **p. 7 / 3 Method - extractive PDF cue:** Then, we encode these scene representations by the corresponding encoders and pool the features to the segments in total of M.
- **p. 6 / 3 Method - extractive PDF cue:** In scene encoding, point clouds, voxel grids, and multi-view images of a scene are first encoded by corresponding encoders and then aligned into a shared ...
- **p. 8 / 3 Method - extractive PDF cue:** During training, we follow [54, 70] to apply Hungarian Matching between queries and ground-truth objects, then calculate the mask loss: \ma t hcal {L} _ ...
- **p. 5 / 3 Method - extractive PDF cue:** Next, we will explain the details of each module.
- **p. 8 / 3 Method - extractive PDF cue:** During training, if text responses are provided as supervision for dense caption and QA task, we calculate the cross-entropy loss as the generation loss Lgen.
...
- **p. 8 / 3 Method - extractive PDF cue:** During training, if text responses are provided as supervision for dense caption and QA task, we calculate the cross-entropy loss as the generation loss Lgen.
- **p. 8 / 3 Method - extractive PDF cue:** Formally, we have: p_ \ mathrm {grd} = \sigma (f_g(\mathbf {Q})) (6) During training, if grounding labels are provided as supervision, we calculate a binary ...
- **p. 6 / 3 Method - extractive PDF cue:** 3.2 3D Scene Encoding There are three widely used representations for 3D scenes: point clouds, voxel grids, and multi-view images, which have their unique advantages ...
- **p. 6 / 3 Method - extractive PDF cue:** The prompt-guided query learning module takes in zero-initialized instance queries and progressively retrieves task-relevant information from aligned scene features under the guidance of task prompts.
...
- **p. 8 / 3 Method - extractive PDF cue:** Generation head We choose the decoder of a pre-trained T5-small [12,50] as the generation head to generate a text response, using all instance queries as ...
- **p. 8 / 3 Method - extractive PDF cue:** 3.4 Output Heads and Losses We adopt the following three output heads to support a variety of 3D-VL tasks: Mask head For each instance query, ...
- **p. 6 / 3 Method - extractive PDF cue:** In scene encoding, point clouds, voxel grids, and multi-view images of a scene are first encoded by corresponding encoders and then aligned into a shared ...
...
- **p. 7 / 3 Method - extractive PDF cue:** Point Cloud To process the point cloud of a 3D scene, we first partition the full point cloud into the pre-generated segments.
...
- **p. 8 / 3 Method - extractive PDF cue:** To support flexible inference when only some representations are available, we randomly drop out some scene features with rate 0.6 in masked-attention computation during training.
- **p. 8 / 3 Method - extractive PDF cue:** During training, we follow [54, 70] to apply Hungarian Matching between queries and ground-truth objects, then calculate the mask loss: \ma t hcal {L} _ ...
- **p. 5 / 3 Method - extractive PDF cue:** We encode the textual and visual prompts by the pre-trained CLIP
...
- **p. 12 / Figure/Table caption - extractive PDF cue:** Table 7: Results on ObjNav from CortexBench [42]. Note we reproduce the result of "VC-1 (ViT-B)" ourselves due to the slight mismatch we have found. ...
- **p. 13 / Figure/Table caption - extractive PDF cue:** Table 8: Ablation study of scene features. Each entry denotes PQ3D "trained with specific scene features" and "trained with all features but some removed during ...
- **p. 12 / 4 Experiments - extractive PDF cue:** 4: Ablation study of query decoder depth.
- **p. 13 / 4 Experiments - extractive PDF cue:** Voxel Point Image Refer QA Caption ✓ 46.1 / 47.1 43.7 / 44.2 67.8 / 68.1 ✓ ✓ 49.2 / 49.4 45.4 / 45.8 74.6 ...
- **p. 14 / 4 Experiments - extractive PDF cue:** Vacuum or sweep the floor to remove any dirt or debris.
...
- **PDF anchors reviewed:** method p. 7 (3 Method), p. 6 (3 Method), p. 8 (3 Method), p. 7 (3 Method), p. 6 (3 Method), p. 8 (3 Method), objective p. 8 (3 Method), p. 8 (3 Method), p. 6 (3 Method), p. 6 (3 Method), temporal p. 1 (1 Introduction), p. 5 (2 Related Work), p. 5 (2 Related Work), p. 14 (4. Adjust the temperature or settings of the heater).
