...
- **p. 9 / 3 Method - extractive PDF cue:** At first, we train the model with the color reconstruction loss as in original 3DGS [20] together with the monocular cue related losses in Sec.
- **p. 10 / 3 Method - extractive PDF cue:** After the optimization, we use 3D Gaussian means and normals for Poisson surface reconstruction [18] to extract the reconstructed meshes.
- **p. 5 / 3 Method - extractive PDF cue:** 3.1 and then elaborate on the technical details of each core module.
- **p. 6 / 3 Method - extractive PDF cue:** To address the above issues, our first thought is to leverage the monocular geometry cues [11] to guide the 3DGS training, including both surface normal ...
- **p. 9 / 3 Method - extractive PDF cue:** \mat hcal {L}_{\texttt {joi nt}} = \s um _{\bm {x}, \bm {\mu }_l} (\/F_\text {IMLS}(\bm {x}) - F_\text {MLP}(\bm {x})\/_2 + \/\frac {\nabla F_\text {MLP}(\bm ...
- **p. 7 / 3 Method - extractive PDF cue:** With a slight abuse of notation, we denote the adjusted rendered normal as ˆ N ′ and apply the following normal consistency loss with monocular ...
- **p. 8 / 3 Method - extractive PDF cue:** 2: Joint optimization of 3DGS and neural implicit representation.
- **p. 9 / 3 Method - extractive PDF cue:** The loss function will backpropagate the gradients to both the Gaussian Splatting field and the neural SDF.

## Design Rationale

- **p. 3 / 1 Introduction - extractive PDF cue:** To address this, we propose a novel regularizer that leverages a neural implicit network to approximate the signed distance values of the MLS function at ...
- **p. 8 / 3 Method - extractive PDF cue:** We propose a novel strategy to further align the Gaussians with the surface.
- **p. 3 / 1 Introduction - extractive PDF cue:** Moreover, to ensure geometry consistency, we propose regularizing the MLS-based function prediction with a jointly learned neural implicit field.
...
- **p. 9 / 3 Method - extractive PDF cue:** The loss function will backpropagate the gradients to both the Gaussian Splatting field and the neural SDF.
- **p. 9 / 3 Method - extractive PDF cue:** \mat hcal {L}_{\texttt {joi nt}} = \s um _{\bm {x}, \bm {\mu }_l} (\/F_\text {IMLS}(\bm {x}) - F_\text {MLP}(\bm {x})\/_2 + \/\frac {\nabla F_\text {MLP}(\bm ...
- **p. 6 / 3 Method - extractive PDF cue:** Following [20], the means of 3D Gaussians are initialized from Structure-from-Motion (SfM) [41] points, and their attributes are optimized by the differentiable reconstruction loss on ...
- **p. 7 / 3 Method - extractive PDF cue:** Without loss of the generality, we assume s1 is the minimum scaling value, and define our regularization term using this minimum value and the harmonic ...
- **p. 7 / 3 Method - extractive PDF cue:** \su m _ {i} z_i \alpha _i\prod _{j=1}^{i-1}(1-\alpha _j), \label {eqn:depth} (5) and optimizing it using a depth loss with monocular depth hint ¯D: \mat ...
- **p. 6 / 3 Method - extractive PDF cue:** As the sparsely initialized Gaussians may fail to represent scene details, 3DGS introduces a densification operation that performs splits and merges for Gaussians based on ...
...
- **p. 9 / 3 Method - extractive PDF cue:** In addition to the vanilla IMLS definition, we further introduce a Robust IMLS (RIMLS) by applying a 1-D Gaussian kernel inputted with the norm of ...
- **p. 1 / 1 Introduction - extractive PDF cue:** Given multi-view images with corresponding camera poses, 3DGS initializes Gaussian primitives from a sparse point cloud that comes from COLMAP [41] and renders a novel ...
- **p. 5 / 3 Method - extractive PDF cue:** Given a set of M posed RGB images I = {I1, . . . , IM} with corresponding camera parameters, 3DGS represents the scene as ...
- **p. 5 / 3 Method - extractive PDF cue:** Our primary goal is to reconstruct the scene geometry from 3D Gaussians by aligning Gaussians with the real-world surface.
- **p. 6 / 3 Method - extractive PDF cue:** 3.2 Monocular Geometry Cue for 3DGS Optimization While 3DGS mainly focuses on the image quality of view synthesis, it lags behind in scene surface reconstruction.
- **p. 6 / 3 Method - extractive PDF cue:** To address the above issues, our first thought is to leverage the monocular geometry cues [11] to guide the 3DGS training, including both surface normal ...
- **p. 7 / 3 Method - extractive PDF cue:** In addition to the local geometry cue like surface normal to guide the covariance learning, we can also adopt the monocular depth to guide the ...
...
- **p. 11 / 4 Experiments - extractive PDF cue:** We term these two variants as SuGaR (SDF)/(density).
- **p. 11 / 4 Experiments - extractive PDF cue:** 1 where our method outperforms both of SuGaR's variants with a clear margin.
- **p. 13 / 4 Experiments - extractive PDF cue:** To delve into its functionality, we show the results by ablation study over the MLS-based joint optimization.
- **p. 14 / 4 Experiments - extractive PDF cue:** Although the MonoSDF (MLP) adopts pure MLP structure which shows robustness to the camera noise, the training time of such a variant gets much longer ...
- **p. 12 / 4 Experiments - extractive PDF cue:** 4: Reconstructed surface by ablating proposed components on Replica [44].
...
- **PDF anchors reviewed:** method p. 9 (3 Method), p. 10 (3 Method), p. 5 (3 Method), p. 6 (3 Method), p. 9 (3 Method), p. 7 (3 Method), objective p. 9 (3 Method), p. 9 (3 Method), p. 6 (3 Method), p. 7 (3 Method), p. 7 (3 Method), p. 6 (3 Method), temporal p. 5 (3 Method), p. 6 (3 Method), p. 7 (3 Method), p. 10 (4 Experiments), p. 12 (4 Experiments), p. 12 (4 Experiments).
