...
- **p. 12 / 4 Experiments - extractive PDF cue:** We show qualitative examples of our system controlling geometry by programmatically 1) interpolating between preset blend shapes, 2) editing of geometry node graphs, and 3) ...
- **p. 13 / 4 Experiments - extractive PDF cue:** In Figure 8(a), BlenderAlchemy is presented with a scene layout editing task, where assets are initially dropped out of camera view and must be iteratively ...
- **p. 13 / 4 Experiments - extractive PDF cue:** 4.3 Lighting Setup Editing We show that BlenderAlchemy can be used to adjust the lighting of scenes according to language instructions as well.
- **p. 14 / 4 Experiments - extractive PDF cue:** As mentioned in Section 3.1, we can consider iteratively optimizing two separate programs, one controlling the lighting of the whole scene and another controlling the ...
- **p. 9 / 4 Experiments - extractive PDF cue:** We compare against BlenderGPT, the most recent open-sourced Blender AI agent that use GPT-4 to execute actions within the Blender environment through the Python API.
- **p. 9 / 4 Experiments - extractive PDF cue:** In reality, this is a very challenging task, since this may require a wide range in the size of edits even if the language describing ...
- **p. 11 / 4 Experiments - extractive PDF cue:** BlenderGPT for the text-based material editing task.
- **p. 11 / 4 Experiments - extractive PDF cue:** Image-based material-editing Given an image of a desired material, the task is to convert the code of the starter material into a material that contains ...
...
- **p. 2 / Figure/Table caption - extractive PDF cue:** Fig. 1: Overview of BlenderAlchemy. Given an input Blender state and a user intention specified using either language or reference images, BlenderAlchemy edits the Blender ...
...
- **p. 10 / Figure/Table caption - extractive PDF cue:** Fig. 3: Text-based Material Editing Results. The step-by-step edits of a 4x8 version of BlenderAlchemy to the same wooden material, given the text description on ...
- **p. 11 / Figure/Table caption - extractive PDF cue:** Fig. 4: The edit discovery process of turning a wooden material into "mar- bled granite". Each column shows the hypotheses generated by G, with the ...
- **p. 11 / Figure/Table caption - extractive PDF cue:** Table 1: CLIP scores of BlenderAlchemy vs. BlenderGPT for the text-based material editing task. We find that a version of our system that has no ...
- **p. 12 / Figure/Table caption - extractive PDF cue:** Fig. 5: Comparisons between our method and BlenderGPT for the text- based material editing task setting. Note how our materials align better with the original ...
- **p. 12 / Figure/Table caption - extractive PDF cue:** Fig. 6: Material editing based on image inputs. Our edit intention is described by the target image shown on the right. 5 different Infinigen [32] ...
- **p. 13 / Figure/Table caption - extractive PDF cue:** Fig. 7: BlenderAlchemy editing geometry using blend shapes. Edits are made to match a description or a script line. Input shapes from BlenderKit.
