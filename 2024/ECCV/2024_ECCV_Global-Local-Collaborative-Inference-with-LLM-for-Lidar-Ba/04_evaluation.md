...
- **p. 11 / 4 Experiments - extractive body cue:** SUN RGB-D is a large 3D object detection and scene understanding dataset, which contains 10335 samples with around 800 object classes.
- **p. 10 / 4 Experiments - extractive body cue:** 4.1 Datasets and Metrics We conduct experiments on two datasets: ScanNetV2 [7] and SUN RGB-D [38].
- **p. 11 / 4 Experiments - extractive body cue:** For a fair comparison, we evaluate our GLIS on the top-20 object classes in ScanNetV2 and SUN RGB-D respectively, following OV-3DET [26].
- **p. 11 / 4 Experiments - extractive body cue:** We use the mean Average Precision (mAP) at the IoU threshold of 0.25 to evaluate the detection performance.
...
- **p. 11 / 4 Experiments - extractive body cue:** The base learning rate is set as 1e-4.
- **p. 6 / Figure/Table caption - extractive body cue:** Fig. 2: The training pipeline of GLIS. local branch, the bounding boxes are extracted by the Background-Aware Ob- ject Localization (BAOL) module, while the object ...
- **p. 14 / Figure/Table caption - extractive body cue:** Fig. 5: Visualizations of detection results. the LLM's answer, our scheme automatically recognizes the bed as false detec- tion. Subsequently, as the confidence of the ...
...
- **p. 2 / Figure/Table caption - extractive body cue:** Fig. 1: (a) The previous 3D OVD paradigm determines the class of an object proposal by comparing its point cloud feature with the class text ...
- **p. 6 / Figure/Table caption - extractive body cue:** Fig. 2: The training pipeline of GLIS. local branch, the bounding boxes are extracted by the Background-Aware Ob- ject Localization (BAOL) module, while the object ...
- **p. 9 / Figure/Table caption - extractive body cue:** Fig. 3: The inference pipeline of GLIS. With the extracted local and global information, we can conduct global- local collaborative inference (GLCI) with LLM and ...
- **p. 11 / Figure/Table caption - extractive body cue:** Table 1: Comparisons with other methods on ScanNetV2
- **p. 12 / Figure/Table caption - extractive body cue:** Table 2: Comparisons with other methods on SUN RGB-D
- **p. 12 / Figure/Table caption - extractive body cue:** Table 3: Ablation study on ScanNetV2
- **p. 13 / Figure/Table caption - extractive body cue:** Fig. 4: Visualizations of GLIS. The score of each proposal is the confidence that the proposal is truly a foreground object. These proposals, as well ...
- **p. 14 / Figure/Table caption - extractive body cue:** Fig. 5: Visualizations of detection results. the LLM's answer, our scheme automatically recognizes the bed as false detec- tion. Subsequently, as the confidence of the ...
...
- **p. 11 / 4 Experiments - extractive body cue:** SUN RGB-D is a large 3D object detection and scene understanding dataset, which contains 10335 samples with around 800 object classes.
- **p. 11 / 4 Experiments - extractive body cue:** For a fair comparison, we evaluate our GLIS on the top-20 object classes in ScanNetV2 and SUN RGB-D respectively, following OV-3DET [26].
- **p. 11 / 4 Experiments - extractive body cue:** The training of phase 1 lasts for 400 epochs with a total batch size of 32 (i.e., a single batch size 4 × 8 GPUs).
- **p. 11 / 4 Experiments - extractive body cue:** The training of phase 2 lasts for 50 epochs with a total batch size of 16.
- **p. 3 / X. Peng et al - extractive body cue:** Global-Local Collaborative Inference 3 scene-level information.
