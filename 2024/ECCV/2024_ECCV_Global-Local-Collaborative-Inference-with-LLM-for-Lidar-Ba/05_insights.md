...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: In this paper, we propose a Global-Local Collaborative Scheme (GLIS) for the lidar-based OVD task, which contains a local branch to generate object-level detection result and a global ...
- 출발 문제 단서: In this way, the detection model fails to detect objects not belonging to the training object classes.
- 주장된 효과 단서: Extensive experiments on ScanNetV2 and SUN RGB-D demonstrate the superiority of our methods.
...
## 이 논문이 끝난 지점
- 논문이 도달한 지점: Extensive experiments on ScanNetV2 and SUN RGB-D demonstrate the superiority of our methods.
...
## 근거가 되는 논문 단서
- Problem cue: In this way, the detection model fails to detect objects not belonging to the training object classes.
- Method cue: In this paper, we propose a Global-Local Collaborative Scheme (GLIS) for the lidar-based OVD task, which contains a local branch to generate object-level detection result and a global ...
