...
- **p. 5 / 3 Method - extractive PDF cue:** Then, we use a multi-view Transformer with selfand cross-attention layers to exchange information between different views.
- **p. 5 / 3 Method - extractive PDF cue:** For better efficiency, we use Swin Transformer's local window attention [22] in our Transformer architecture.
- **p. 7 / 3 Method - extractive PDF cue:** 3.3 Training Loss Our model predicts a set of 3D Gaussian parameters {(µj, αj, Σj, cj)}H×W ×K j=1 , which are then used for rendering ...
- **p. 6 / 3 Method - extractive PDF cue:** Given the near and far depth ranges, we first uniformly sample D depth candidates {dm}D m=1 in the inverse depth domain and then warp view ...
- **p. 6 / 3 Method - extractive PDF cue:** The U-Net takes the concatenation of Transformer features F i and cost volume Ci as inputs, and outputs a residual ∆Ci ∈R H 4 × ...
- **p. 7 / 3 Method - extractive PDF cue:** MVSplat 7 refinement is performed with a very lightweight 2D U-Net, which takes multiview images, features, and current depth predictions as input, and outputs perview ...
- **p. 5 / 3 Method - extractive PDF cue:** 2, is trained end-to-end using only a simple rendering loss for supervision.
- **p. 5 / 3 Method - extractive PDF cue:** Note that we construct K cost volumes for K input views to predict K depth maps.

## Design Rationale

- **p. 5 / 3 Method - extractive PDF cue:** In this paper, we present MVSplat, a Gaussian-based feed-forward model for novel view synthesis.
...
- **p. 5 / 3 Method - extractive PDF cue:** 2, is trained end-to-end using only a simple rendering loss for supervision.
- **p. 5 / 3 Method - extractive PDF cue:** Note that we construct K cost volumes for K input views to predict K depth maps.
- **p. 6 / 3 Method - extractive PDF cue:** (4) Overall, we obtain K cost volumes {Ci}K i=1 for K input views.
- **p. 6 / 3 Method - extractive PDF cue:** Chen et al. we take view i's cost volume construction as an example.
- **p. 7 / 3 Method - extractive PDF cue:** The training loss is calculated as a linear combination of ℓ2 and LPIPS [49] losses, with loss weights of 1 and 0.05, respectively.
- **p. 7 / 3 Method - extractive PDF cue:** 3.3 Training Loss Our model predicts a set of 3D Gaussian parameters {(µj, αj, Σj, cj)}H×W ×K j=1 , which are then used for rendering ...
...
- **p. 6 / 3 Method - extractive PDF cue:** The U-Net takes the concatenation of Transformer features F i and cost volume Ci as inputs, and outputs a residual ∆Ci ∈R H 4 × ...
- **p. 5 / 3 Method - extractive PDF cue:** Note that we construct K cost volumes for K input views to predict K depth maps.
- **p. 5 / 3 Method - extractive PDF cue:** Our depth model includes multi-view feature extraction, cost volume construction, cost volume refinement, depth estimation, and depth refinement, as introduced next.
- **p. 7 / 3 Method - extractive PDF cue:** The residual depths are then added to the current depth predictions as the final depth outputs.
- **p. 2 / 1 Introduction - extractive PDF cue:** Specifically, it predicts a probabilistic depth distribution for each input view and then samples depths from that predicted distribution.
- **p. 6 / 3 Method - extractive PDF cue:** (4) Overall, we obtain K cost volumes {Ci}K i=1 for K input views.
...
- **p. 7 / 3 Method - extractive PDF cue:** 3.3 Training Loss Our model predicts a set of 3D Gaussian parameters {(µj, αj, Σj, cj)}H×W ×K j=1 , which are then used for rendering ...
- **p. 8 / 4 Experiments - extractive PDF cue:** The inference time and model parameters are also reported to enable thorough comparisons of speed and accuracy trade-offs.
- **p. 10 / 4 Experiments - extractive PDF cue:** 1, apart from attaining superior image quality, MVSplat also shows the fastest inference time among all the compared models, accompanied by a lightweight model size, ...
- **p. 8 / 4 Experiments - extractive PDF cue:** All models are trained on a single A100 GPU for 300,000 iterations with the Adam [19] optimizer.
- **p. 9 / 4 Experiments - extractive PDF cue:** This includes pixelNeRF [46], GPNR [33], AttnRend [10] and pixelSplat [1], with results taken directly from the pixelSplat [1] paper, and the recent state-of-the-art NeRF-based ...
...
- **p. 10 / 4 Experiments - extractive PDF cue:** 4 demonstrates the feed-forward geometry reconstruction results of MVSplat, without any extra fine-tuning.
- **p. 11 / 4 Experiments - extractive PDF cue:** Models trained on RE10K (indoor scenes) are directly used to test on scenes from ACID (outdoor scenes) and DTU (objectcentric scenes), without any further fine-tuning.
- **p. 12 / 4 Experiments - extractive PDF cue:** All other ablations are conducted on the "base" model w/o depth refinement.
- **p. 12 / 4 Experiments - extractive PDF cue:** Setup PSNR↑SSIM↑LPIPS↓ base + refine 26.39 0.869 0.128 base 26.12 0.864 0.133 w/o cost volume 22.83 0.753 0.197 w/o cross-view attention 25.19 0.852 0.152 w/o ...
- **p. 13 / 4 Experiments - extractive PDF cue:** Our full model without "depth refinement" (Sec.
- **p. 13 / 4 Experiments - extractive PDF cue:** 4.3 Ablations We conduct thorough ablations on RealEstate10K to analyze MVSplat.
...
- **PDF anchors reviewed:** method p. 5 (3 Method), p. 5 (3 Method), p. 7 (3 Method), p. 6 (3 Method), p. 6 (3 Method), p. 7 (3 Method), objective p. 5 (3 Method), p. 5 (3 Method), p. 6 (3 Method), p. 6 (3 Method), p. 7 (3 Method), p. 7 (3 Method), temporal p. 6 (3 Method), p. 7 (4 Experiments), p. 9 (4 Experiments), p. 10 (4 Experiments), p. 12 (4 Experiments), p. 14 (4 Experiments).
