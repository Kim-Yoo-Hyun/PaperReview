...
- **p. 11 / 5 Experiments - extractive PDF cue:** Additionally, it focuses on natural scenes with image pairs as input, and its performance significantly decreases when applied to object-centric datasets [1,29].
- **p. 10 / 5 Experiments - extractive PDF cue:** Following MVSNeRF [6], we train the generalizable model on the DTU training set [1] and evaluate it on the DTU test set.
- **p. 10 / 5 Experiments - extractive PDF cue:** Subsequently, we conduct further evaluations on the Real Forward-facing [28], NeRF Synthetic [29], and Tanks and Temples [21] datasets.
- **p. 11 / 5 Experiments - extractive PDF cue:** MVSGaussian 11 Table 2: Quantitative results of generalization on Real Forward-facing [28], NeRF Synthetic [29], and Tanks and Temples [21] datasets.
- **p. 12 / 5 Experiments - extractive PDF cue:** PSNRdtu, PSNRllff, PSNRnerf, and PSNRtnt are the PSNR metrics for different datasets [1,21,28,29].
- **p. 13 / 5 Experiments - extractive PDF cue:** Especially on the Real Forward-facing dataset, our method achieves superior performance with only 45 seconds of optimization, compared to 10 minutes for 3D-GS and 10 ...
- **p. 13 / 5 Experiments - extractive PDF cue:** Regarding color representation, we directly decode RGB values instead of spherical harmonic (SH) coefficients (No.5), as decoding coefficients may result in a degradation of generalization, ...
- **p. 14 / 5 Experiments - extractive PDF cue:** We report the quantitative results obtained with different strategies on the Real Forward-facing dataset [28].
...
- **p. 2 / Figure/Table caption - extractive PDF cue:** Fig. 1: Comparison with existing methods. (a) We present the generalizable re- sults on the Real Forward-facing dataset [28]. Compared with other competitors, our method ...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Fig. 2: Overview of MVSGaussian. We first extract features {fi}N i=1 from input source views {Ii}N i=1 using FPN. These features are then aggregated into ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Fig. 3: Consistent aggregation. With depth maps and point clouds produced by the generalizable model, we first conduct geometric consistency checks on depths to derive ...
- **p. 10 / Figure/Table caption - extractive PDF cue:** Table 1: Quantitative results of generalization on the DTU test set [1]. FPS and Mem are measured under a 3-view input, while FPS∗and Mem∗are measured ...
- **p. 11 / Figure/Table caption - extractive PDF cue:** Table 2: Quantitative results of generalization on Real Forward-facing [28], NeRF Synthetic [29], and Tanks and Temples [21] datasets. Due to the signif- icant memory ...
- **p. 11 / Figure/Table caption - extractive PDF cue:** Table 3: Quantitative results after per-scene optimization. Timeft represents the time for fine-tuning. The best result is in bold, and second-best one is in underlined.
- **p. 12 / Figure/Table caption - extractive PDF cue:** Fig. 4: Qualitative comparison of rendering quality under generalization and 3-view settings with state-of-the-art methods [6, 9, 22].
...
| Output/decision | point map, pose, scene graph, affordance 또는 query result | action frame, controller and termination | p. 1 (4 Shanghai AI Laboratory), p. 3 (1 Introduction) |
...
- **p. 10 / 5 Experiments - extractive PDF cue:** The generalizable model is trained using the Adam optimizer [20] on four RTX 3090 GPUs.
- **p. 11 / 5 Experiments - extractive PDF cue:** For NeRF-based methods, ENeRF [22] enjoys promising speeds by sampling only 2 points per ray, however, its performance is limited and consumes higher memory overhead.
- **p. 13 / 5 Experiments - extractive PDF cue:** Especially on the Real Forward-facing dataset, our method achieves superior performance with only 45 seconds of optimization, compared to 10 minutes for 3D-GS and 10 ...
- **p. 14 / 5 Experiments - extractive PDF cue:** Aggregation PSNR ↑ SSIM ↑ LPIPS ↓ Timeft ↓ FPS ↑ direct concatenation 26.18 0.901 0.122 90s 220 downsampling 26.72 0.909 0.121 60s 340 consistency ...
- **p. 2 / 1 Introduction - extractive PDF cue:** 0 0.5 10 20 21 22 23 24 25 PSNR ↑ IBRNet (0.1, 21.79) MVSNeRF (0.2, 21.93) MatchNeRF (0.5, 22.43) ENeRF (11.7, 23.63) Ours (14.1, ...
...
| Specifically, 1) we leverage MVS to encode geometry-aware Gaussian representations and decode them into Gaussian parameters. | p. 1 (4 Shanghai AI Laboratory) |
