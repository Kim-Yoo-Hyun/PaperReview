...
| Observation / input | We introduce the task of localizing an input image within a multi-modal reference map represented by a collection of 3D scene graphs. | camera/depth stream, pose, map와 language goal | exact sensor/frame/preprocessing from PDF body |
| State / latent | introduce, task, localizing, input, image, within, multi-modal, reference, represented, collection | robot pose, free-space/semantic map와 local goal | notation and tensor shape require body check |
| Output / action | SceneGraphLoc, coarse, localization, input, image, given, reference, represented | collision-free trajectory 또는 velocity command | exact unit/frame/decoder require body check |
...
|---|---|---|
| State / observation variable | sensor/map state and goal; body terms: introduce, task, localizing, input, image, within, multi-modal, reference, represented, collection | p. 1 (Body text (section not recovered)), p. 1 (Body text (section not recovered)), p. 3 (1 Introduction) |
| Decision / output variable | path/waypoint/velocity; body terms: enables, creation, small, efficient, databases, significantly, accelerates, coarse | p. 2 (1 Introduction), p. 3 (1 Introduction), p. 1 (Body text (section not recovered)) |
...
PDF body contribution framing (p. 2 (1 Introduction), p. 3 (1 Introduction), p. 1 (Body text (section not recovered)), p. 2 (1 Introduction), p. 3 (1 Introduction)): This method enables the creation of small, efficient databases and significantly accelerates the coarse localization process.
...
navigation writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. Evidence interface anchors: p. 1 (Body text (section not recovered)), p. 1 (Body text (section not recovered)), p. 3 (1 Introduction), p. 2 (1 Introduction). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **Evidence anchors reviewed:** problem p. 2 (1 Introduction), p. 2 (1 Introduction), p. 3 (1 Introduction), interface p. 1 (Body text (section not recovered)), p. 1 (Body text (section not recovered)), p. 3 (1 Introduction), p. 2 (1 Introduction), objective p. 2 (1 Introduction).
