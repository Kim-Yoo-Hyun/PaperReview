...
PDF body method statement (p. 2 (1 Introduction), p. 2 (1 Introduction), p. 3 (1 Introduction)): The training phase is represented by orange arrows, while blue arrows denote the inference phase.
...
- **p. 3 / 1 Introduction - extractive PDF cue:** With images, SceneGraphLoc achieves performance close to that of state-of-theart image-based methods while requiring three orders-of-magnitude less storage and operating orders-of-magnitude faster.
...
- **p. 3 / 1 Introduction - extractive PDF cue:** SceneGraphLoc, a new method for the coarse localization of an input image given a reference map represented by a database of 3D scene graphs.
- **p. 2 / 1 Introduction - extractive PDF cue:** Cross-modal approaches, such as [100,142], which attempt to bridge different types of data, often restrict their scope to connecting two modalities at a time (e.g., ...
...
- **p. 2 / 1 Introduction - extractive PDF cue:** This method enables the creation of small, efficient databases and significantly accelerates the coarse localization process.
...
|---|---|---|---|
| Input/observation | SceneGraphLoc, coarse, localization, input, image, given, reference, represented, database, scene, graphs, challenge, current, state-of-the-art | camera/depth stream, pose, map와 language goal | body cue; exact tensor/frame verify |
| State/latent | SceneGraphLoc, coarse, localization, input, image, given, reference, represented, database, scene | robot pose, free-space/semantic map와 local goal | body cue; notation verify |
...
- **p. 2 / 1 Introduction - extractive PDF cue:** The training phase is represented by orange arrows, while blue arrows denote the inference phase.
- **p. 10 / 4 Experiments - extractive PDF cue:** These methods offer advanced performance but demand significant storage for image descriptors and exhibit slower inference times.
- **p. 11 / 4 Experiments - extractive PDF cue:** Additionally, we will report the inference time and storage requirements.
...
- **p. 12 / 4 Experiments - extractive PDF cue:** Also, the storage of SceneGraphLoc with and without images is the same due to its design of distilling knowledge into fixed-sized embeddings.
- **p. 13 / 4 Experiments - extractive PDF cue:** OpenMask3D attains an accuracy comparable to our proposed method without incorporating the image modality.
- **p. 14 / Figure/Table caption - extractive PDF cue:** Table 4: Ablation study performed on the val. split of 3RScan [123], analysing map modalities (P - point cloud, I - image, A - attributes, ...
...
- **PDF anchors reviewed:** method p. 2 (1 Introduction), p. 2 (1 Introduction), p. 3 (1 Introduction), objective p. 2 (1 Introduction), p. 2 (1 Introduction), temporal p. 12 (4 Experiments), p. 12 (4 Experiments), p. 13 (4 Experiments), p. 13 (4 Experiments), p. 8 (2 Related Work), p. 10 (4 Experiments).
