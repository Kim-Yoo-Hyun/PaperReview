...
- **p. 1 / 1 Introduction - extractive PDF cue:** 3D panoptic segmentation is a crucial task in computer vision with many realworld applications, most notably in autonomous driving.
- **p. 1 / 1 Introduction - extractive PDF cue:** It combines 3D semantic and instance segmentation to produce per-point predictions for two different types of objects: things (e.g., car) and stuff (e.g., road).
- **p. 1 / 1 Introduction - extractive PDF cue:** To date, there has been significant progress in 3D panoptic segmentation [27, 40, 42, 47, 52, 58].
- **p. 1 / 1 Introduction - extractive PDF cue:** Most recently, methods such as [47] produce panoptic segmentation predictions directly from point clouds by leveraging learned queries to represent objects and ∗Work done while ...
- **p. 2 / 1 Introduction - extractive PDF cue:** Transformer-based [45] architectures [2, 4] to perform the modeling.
- **p. 2 / 1 Introduction - extractive PDF cue:** However, these methods are only possible due to the vast amounts of paired image-text data available, making it difficult to train similar models for 3D ...
- **p. 2 / 1 Introduction - extractive PDF cue:** However, existing models only predict panoptic segmentation results for a closed-set of objects.
...
| Observation / input | The LiDAR encoder is a model which takes an unordered set of points as input and extracts per-point features. | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF |
...
- **p. 4 / 3 Method - extractive PDF cue:** The overview of our method is presented in Fig.
- **p. 5 / 3 Method - extractive PDF cue:** The architecture of our method is shown in Fig.
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 5 (3 Method), p. 5 (3 Method), p. 6 (3 Method), p. 6 (3 Method). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (1 Introduction), p. 2 (1 Introduction), interface p. 5 (3 Method), p. 5 (3 Method), p. 6 (3 Method), p. 6 (3 Method), objective p. 7 (3 Method), p. 4 (3 Method), p. 5 (3 Method), p. 7 (3 Method), p. 8 (3 Method), p. 8 (3 Method).
