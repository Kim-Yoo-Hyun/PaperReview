...
PDF body method statement (p. 2 (X. Xu et al), p. 4 (X. Xu et al), p. 8 (X. Xu et al), p. 3 (X. Xu et al), p. 6 (X. Xu et al), p. 8 (X. Xu et al)): Point clouds are first processed by several Multi-Layer Perception (MLP) layers and thus get a point cloud feature map, and then this point cloud feature map is processed by a ...

## Method Body Digest

- **p. 2 / X. Xu et al - extractive PDF cue:** Point clouds are first processed by several Multi-Layer Perception (MLP) layers and thus get a point cloud feature map, and then this point cloud feature ...
...
- **p. 8 / X. Xu et al - extractive PDF cue:** Finally, we use the pseudo labels Y to supervise the model, and the green dashed lines denote back-propagation of the loss La.
- **p. 3 / X. Xu et al - extractive PDF cue:** We first process these multi-view images using the image encoder of the pretrained off-the-shelf 2D OVSS model such as Openseg [12] to get the 2D ...
- **p. 6 / X. Xu et al - extractive PDF cue:** 2, we first implement dense 2D embeddings extraction for each RGB image via the frozen visual encoder of Openseg [12], and back-project them onto the ...
- **p. 8 / X. Xu et al - extractive PDF cue:** For (a), we first utilize the text encoder εtext of Openseg to obtain embeddings of the category labels FC, which are frozen during the training ...
...
- **p. 8 / X. Xu et al - extractive PDF cue:** The red dashed lines denote back-propagation of the loss Ls. a classification cross-entropy loss La is introduced to supervise the procedure.
...
|---|---|---|---|---|---|---|
| Geometry / pose extraction | image·depth·point input에서 spatial state를 만든다 | RGB/RGB-D, point cloud, camera pose 또는 multi-view input | depth, pose, correspondence, point, mesh, Gaussian 또는 feature representation을 추정 | geometry/map/pose | Point clouds are first processed by several Multi-Layer Perception (MLP) layers and thus get a point cloud feature map, and then this ... | p. 2 (X. Xu et al), p. 4 (X. Xu et al) |
| Semantic / temporal fusion | geometry에 semantics와 history를 정렬한다 | geometry, visual/language feature와 temporal context | feature lifting, scene graph, map update, tracking 또는 temporal fusion을 수행 | queryable 3D state | In summary, the main contributions of this paper are as follows: - We propose a weakly supervised method 3DSS-VLG for 3D WSSS, ... | p. 4 (X. Xu et al), p. 8 (X. Xu et al) |
| Robot query / planning handoff | 3D state를 task decision에 전달한다 | map/feature와 task query | target grounding, affordance, collision/free-space 또는 action cue를 생성 | goal, pose, path 또는 policy input | Finally, we use the pseudo labels Y to supervise the model, and the green dashed lines denote back-propagation of the loss La. | p. 8 (X. Xu et al), p. 3 (X. Xu et al) |
...
- **p. 8 / X. Xu et al - extractive PDF cue:** The red dashed lines denote back-propagation of the loss Ls. a classification cross-entropy loss La is introduced to supervise the procedure.
- **p. 8 / X. Xu et al - extractive PDF cue:** The cosine similarity loss Ls will be integrated to train the model.
...
- **p. 4 / X. Xu et al - extractive PDF cue:** Moreover, we propose Embeddings Specialization Stage to make the embedding space to be more robust based on the pseudo label filtering with indoor point cloud ...
- **p. 3 / X. Xu et al - extractive PDF cue:** Specifically, for the input 3D point cloud, the dataset also provides a set of multi-view images corresponding to it.
- **p. 6 / X. Xu et al - extractive PDF cue:** Given an input point cloud with multi-view images as shown in Fig.
- **p. 6 / X. Xu et al - extractive PDF cue:** The inputs of 3DSS-VLG comprise a scene with 3D point cloud, scene-level labels and the associated multi-view RGB images
- **p. 4 / X. Xu et al - extractive PDF cue:** In summary, the main contributions of this paper are as follows: - We propose a weakly supervised method 3DSS-VLG for 3D WSSS, which takes 2D ...
...
- **p. 7 / X. Xu et al - extractive PDF cue:** After ranking the filtered classification logits Lf, we can get the more precise pseudo label Y ∈RN of the input point cloud.
...
- **p. 8 / X. Xu et al - extractive PDF cue:** For (a), we first utilize the text encoder εtext of Openseg to obtain embeddings of the category labels FC, which are frozen during the training ...
- **p. 3 / X. Xu et al - extractive PDF cue:** We first process these multi-view images using the image encoder of the pretrained off-the-shelf 2D OVSS model such as Openseg [12] to get the 2D ...
- **p. 7 / X. Xu et al - extractive PDF cue:** Similarly, we also freeze the text encoder and directly load the pretrained Openseg parameters.

- Training inputs, privileged information, data augmentation and inference-time feedback must be recorded separately; they are not interchangeable.

## Method-Specific Formal Details

- **Body-defined terms:** Point, clouds, first, processed, several, Multi-Layer, Perception, MLP, layers, thus, cloud, feature, then, Global, Average, Pooling, GAP, classification, prediction, trained.
...
- **p. 9 / 4 Experiments - extractive PDF cue:** Finally, ablation studies are provided to further demonstrate the necessity and effectiveness of each component of our framework.
...
- **PDF anchors reviewed:** method p. 2 (X. Xu et al), p. 4 (X. Xu et al), p. 8 (X. Xu et al), p. 3 (X. Xu et al), p. 6 (X. Xu et al), p. 8 (X. Xu et al), objective p. 8 (X. Xu et al), p. 8 (X. Xu et al), temporal p. 9 (4 Experiments), p. 10 (X. Xu et al), p. 12 (X. Xu et al), p. 13 (X. Xu et al), p. 14 (X. Xu et al).
