...
- **p. 10 / 5 Experiments - extractive body cue:** We conduct our experiments on SG-FRONT dataset [58], which provides scene-graph annotations for the high-quality 3D-FRONT [16] with household environments.
- **p. 10 / 5 Experiments - extractive body cue:** SG-FRONT contains 15 relationship types and 45K object instances from three types of scenes.
...
- **p. 10 / 5 Experiments - extractive body cue:** We follow the metrics used in [32, 57, 58], to measure the fidelity and diversity of generated scenes, where we adopt Fréchet Inception Distance (FID) ...
...
- **p. 2 / Figure/Table caption - extractive body cue:** Fig. 1: EchoScene Schematic. EchoScene uses a dual-branch diffusion model to generate 3D scenes from scene graphs. In both branches, each node is allocated a ...
...
- **p. 13 / Figure/Table caption - extractive body cue:** Table 2: Scene graph constraints (higher is better). Top: Relationship change mode. Middle: Node addition mode. Bottom: No manipulation (i.e., generation only). The decrease in ...
- **p. 14 / Figure/Table caption - extractive body cue:** Fig. 5: Off-the-shelf texture creation. A bedroom (top) and living room (bottom) generated by EchoScene and textured in different styles by SceneTex [9].
...
- **p. 8 / 4 Method - extractive body cue:** Initially, each bounding box bi 0 has 7 parameters, e.g., location (x, y, z), size (l, h, w), and a yaw angle ✓.
- **p. 8 / 4 Method - extractive body cue:** Thereby, the final representation contains 8 parameters: bi 0 = {x, y, z, l, h, w, sin ✓, cos ✓}, as shown in Fig.
- **p. 9 / 4 Method - extractive body cue:** Shape Branch A.3 Parameter Diffusion B.3 Dimension Alignment Denoiser γθ Denoiser γθ Denoiser εθ Denoiser εθ Fig.
- **p. 11 / 11 Method - extractive body cue:** 5.1 Scene Fidelity As a generation task, the fidelity of scene synthesis is essential to measure.
