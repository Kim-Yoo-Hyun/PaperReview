...
- **p. 3 / 4. Image-conditioned 3D Gaussian Inference - extractive PDF cue:** Our method consists of a two-view image encoder and a pixel-aligned Gaussian prediction module.
- **p. 4 / 4.1. Resolving Scale Ambiguity - extractive PDF cue:** Note that for brevity, we use h to represent the function that computes depths from bucket indices (see equations 6 and 7). date per-pixel features ...
- **p. 3 / 4.1. Resolving Scale Ambiguity - extractive PDF cue:** We first encode each view separately into feature volumes F and ˜F via a per-image feature encoder.
...
- **p. 6 / 4.2. Gaussian Parameter Prediction - extractive PDF cue:** ACID RealEstate10k Inference Time (s) Memory (GB) PSNR ↑ SSIM ↑ LPIPS ↓ PSNR ↑ SSIM ↑ LPIPS ↓ Encode ↓ Render ↓ Training ↓ ...
- **p. 5 / 4.2. Gaussian Parameter Prediction - extractive PDF cue:** This means that in each backward pass, we assign the gradients of the loss L with respect to the opacities α to the gradients of ...
...
- **p. 5 / 4.2. Gaussian Parameter Prediction - extractive PDF cue:** This means that in each backward pass, we assign the gradients of the loss L with respect to the opacities α to the gradients of ...
- **p. 1 / 1. Introduction - extractive PDF cue:** Next, optimizing primitive parameters directly via gradient descent suffers from local minima.
- **p. 2 / 1. Introduction - extractive PDF cue:** When receiving a gradient that would increase the opacity of a Gaussian at a 3D location, our model increases the probability that the Gaussian will ...
- **p. 5 / 4.2. Gaussian Parameter Prediction - extractive PDF cue:** In this case, gradient descent decreases the opacity of the Gaussian, lowering the probability of further incorrect depth predictions.
...
- **p. 1 / 1. Introduction - extractive PDF cue:** Given a pair of input images, pixelSplat reconstructs a 3D radiance field parameterized via 3D Gaussian primitives.
- **p. 4 / 4.1. Resolving Scale Ambiguity - extractive PDF cue:** For every pixel feature F[u] in the input feature map, a neural network f predicts Gaussian primitive parameters Σ and S.
- **p. 3 / 4.1. Resolving Scale Ambiguity - extractive PDF cue:** In practice, this means that a neural network making predictions about the geometry of a scene from a single image cannot possibly predict the depth ...
- **p. 2 / 1. Introduction - extractive PDF cue:** We significantly outperform previous black-box based light field transformers on the real-world ACID and RealEstate10k datasets while drastically reducing both training and rendering cost and ...
- **p. 4 / 4.1. Resolving Scale Ambiguity - extractive PDF cue:** Note that this mechanism can be extended to more than two input views.
- **p. 6 / 4.2. Gaussian Parameter Prediction - extractive PDF cue:** We outperform all baseline methods in terms PSNR, LPIPS, and SSIM for novel view synthesis on the real-world RealEstate10k and ACID datasets.
...
- **p. 2 / 1. Introduction - extractive PDF cue:** We significantly outperform previous black-box based light field transformers on the real-world ACID and RealEstate10k datasets while drastically reducing both training and rendering cost and ...
- **p. 6 / 4.2. Gaussian Parameter Prediction - extractive PDF cue:** ACID RealEstate10k Inference Time (s) Memory (GB) PSNR ↑ SSIM ↑ LPIPS ↓ PSNR ↑ SSIM ↑ LPIPS ↓ Encode ↓ Render ↓ Training ↓ ...
- **p. 6 / 5.2. Results - extractive PDF cue:** Our method also uses significantly less memory per ray at training time.
- **p. 6 / 4.2. Gaussian Parameter Prediction - extractive PDF cue:** ACID RealEstate10k Inference Time (s) Memory (GB) PSNR ↑ SSIM ↑ LPIPS ↓ PSNR ↑ SSIM ↑ LPIPS ↓ Encode ↓ Render ↓ Training ↓ ...
- **p. 1 / Abstract - extractive PDF cue:** Our model features real-time and memory-efficient rendering for scalable training as well as fast 3D reconstruction at inference time.
...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Figure 7. Ablations. Without the epipolar transformer, our model is unable to resolve scale ambiguity, leading to ghosting artifacts. Without our sampling approach, our model ...
...
- **p. 6 / 5. Experiments - extractive PDF cue:** In this section, we describe our experimental setup, evaluate our method on wide-baseline novel view synthesis from image pairs, and perform ablations to validate our ...
- **p. 7 / 5.3. Ablations and Analysis - extractive PDF cue:** We perform ablations on RealEstate10k to answer the following questions: • Question 1a: Is our epipolar encoder responsible for our model's ability to handle scale ...
- **p. 8 / 5.3. Ablations and Analysis - extractive PDF cue:** To measure our epipolar encoding scheme's importance, we compare pixelSplat to a variant (No Epipolar Encoder) that eschews epipolar encoding.
- **p. 7 / 5.2. Results - extractive PDF cue:** We visualize point clouds using the version of our model that has been fine-tuned with a depth regularizer.
