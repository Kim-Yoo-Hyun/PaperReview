...
- **p. 5 / Method - extractive PDF cue:** Thus, it is not beneficial to exhaustively enforce our depth model to produce exactly the same features as the frozen encoder.
- **p. 5 / Method - extractive PDF cue:** The feature alignment loss is formulated as: \ma t h c a l {L } _{f eat} = 1 - \frac {1}{HW}\sum _{i=1}^{HW}\cos (f_i, f'_i), ...
- **p. 6 / 4.3. Fine-tuned to Metric Depth Estimation - extractive PDF cue:** In this part, we use our ViT-L encoder for fine-tuning.
- **p. 7 / Method - extractive PDF cue:** We use Mask2Former as our segmentation model. since the labeled images are already sufficient.
- **p. 7 / Method - extractive PDF cue:** Following ZoeDepth, we use the model trained on NYUv2 for indoor generalization, while use the model trained on KITTI for outdoor evaluation.
- **p. 6 / 4.3. Fine-tuned to Metric Depth Estimation - extractive PDF cue:** We initialize the encoder of downstream MDE models with our pre-trained encoder parameters and leave the decoder randomly initialized.
- **p. 8 / Method - extractive PDF cue:** Since our finally produced encoder (from large-scale MDE training) is finetuned from DINOv2 [43], we compare our encoder with the original DINOv2 encoder in Table ...
- **p. 5 / Method - extractive PDF cue:** Best, second best results. depth model with an auxiliary feature alignment loss.

## Design Rationale

- **p. 5 / Method - extractive PDF cue:** This allows our method to enjoy both the semantic-aware representation from DINOv2 and the part-level discriminative representation from depth supervision.
...
- **p. 5 / Method - extractive PDF cue:** Best, second best results. depth model with an auxiliary feature alignment loss.
- **p. 5 / Method - extractive PDF cue:** Finally, our overall loss is an average combination of the three losses Ll, Lu, and Lfeat.
...
- **p. 7 / Method - extractive PDF cue:** Effectiveness of 1) challenging the student model when learning unlabeled images, and 2) semantic constraint.
- **p. 7 / Method - extractive PDF cue:** Moreover, with our used semantic constraint Lfeat, the power of unlabeled images can be further amplified for the depth estimation task.
...
- **p. 8 / Method - extractive PDF cue:** Meantime, we use ControlNet to synthesize new images from the depth map.
- **p. 6 / 4.4. Fine-tuned to Semantic Segmentation - extractive PDF cue:** Similar observations hold on the ADE20K dataset [89] in Table 8.
- **p. 1 / 1. Introduction - extractive PDF cue:** In this work, our goal is to build a foundation model for MDE capable of producing high-quality depth information for any images under any circumstances.
- **p. 2 / 1. Introduction - extractive PDF cue:** labeled images from depth sensors, our used monocular unlabeled images exhibit three advantages: (i) (simple and cheap to acquire) Monocular images exist almost everywhere, thus ...
- **p. 5 / Method - extractive PDF cue:** Thus, it is not beneficial to exhaustively enforce our depth model to produce exactly the same features as the frozen encoder.
- **p. 6 / Method - extractive PDF cue:** We highlight best, second best results, as well as most discriminative metrics. ∗: Reproduced by us. benchmarks KITTI and NYUv2, although MiDaS v3.1 uses the ...
- **p. 7 / Method - extractive PDF cue:** Moreover, with our used semantic constraint Lfeat, the power of unlabeled images can be further amplified for the depth estimation task.
...
- **p. 6 / 4.3. Fine-tuned to Metric Depth Estimation - extractive PDF cue:** We initialize the encoder of downstream MDE models with our pre-trained encoder parameters and leave the decoder randomly initialized.
- **p. 8 / Method - extractive PDF cue:** Since our finally produced encoder (from large-scale MDE training) is finetuned from DINOv2 [43], we compare our encoder with the original DINOv2 encoder in Table ...
- **p. 5 / 4.1. Implementation Details - extractive PDF cue:** In both stages, the base learning rate of the pre-trained encoder is set as 5e-6, while the randomly initialized decoder uses a 10× larger learning ...
- **p. 9 / 6. More Implementation Details - extractive PDF cue:** The batch size is 16 and the model is trained for 5 epochs.
...
- **p. 5 / 4.1. Implementation Details - extractive PDF cue:** All labeled datasets are simply combined together without re-sampling.
- **p. 6 / Figure/Table caption - extractive PDF cue:** Table 3. Fine-tuning and evaluating on NYUv2 [55] with our pre-trained MDE encoder. We highlight best, second best results, as well as most discriminative metrics. ...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Table 4. Fine-tuning and evaluating on KITTI [18] with our pre-trained MDE encoder. ∗: Reproduced by us. coder with metric depth information from NYUv2 [55] ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Table 9. Ablation studies of: 1) challenging the student with strong perturbations (S) when learning unlabeled images, and 2) semantic constraint (Lfeat). Limited by space, ...
- **p. 9 / 6. More Implementation Details - extractive PDF cue:** The model is trained for 160K iterations on ADE20K and 80K iterations on Cityscapes both with batch size 16, without any COCO [36] or Mapillary ...
- **p. 9 / Figure/Table caption - extractive PDF cue:** Table 12. Ablation studies on different values of the tolerance margin α for the feature alignment loss Lfeat. Limited by space, we only report the ...
- **p. 4 / Figure/Table caption - extractive PDF cue:** Figure 2. Our pipeline. Solid line: flow of labeled images, dotted line: unlabeled images. We especially highlight the value of large-scale unlabeled images. The S ...
