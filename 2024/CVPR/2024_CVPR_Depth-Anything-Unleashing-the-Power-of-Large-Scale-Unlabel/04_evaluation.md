...
- **p. 5 / 4.2. Zero-Shot Relative Depth Estimation - extractive PDF cue:** Moreover, our ViT-S model, whose scale is less than 1/10 of the MiDaS model, even outperforms MiDaS on several unseen datasets, including Sintel, DDAD, and ...
- **p. 5 / 4.2. Zero-Shot Relative Depth Estimation - extractive PDF cue:** Therefore, we comprehensively validate the zero-shot depth estimation capability of our Depth Anything model on six representative unseen datasets: KITTI [18], NYUv2 [55], Sintel [7], ...
- **p. 8 / 4.6. Qualitative Results - extractive PDF cue:** We visualize our model predictions on the six unseen datasets in Figure 3.
- **p. 9 / 9. More Qualitative Results - extractive PDF cue:** Please refer to the following pages for comprehensive qualitative results on six unseen test sets (Figure 5 for KITTI [18], Figure 6 for NYUv2 [55], ...
- **p. 9 / 9. More Qualitative Results - extractive PDF cue:** Our model exhibits higher depth estimation accuracy and stronger robustness.
- **p. 5 / 4.2. Zero-Shot Relative Depth Estimation - extractive PDF cue:** As shown in Table 2, both with a ViT-L encoder, our Depth Anything surpasses the strongest MiDaS model tremendously across extensive scenes in terms of ...
- **p. 5 / 4.2. Zero-Shot Relative Depth Estimation - extractive PDF cue:** The performance advantage of these small-scale models demonstrates their great potential in computationally-constrained scenarios.
- **p. 8 / 4.6. Qualitative Results - extractive PDF cue:** Our model produces more accurate depth estimation than MiDaS, as well as better synthesis results.
...
- **p. 1 / Figure/Table caption - extractive PDF cue:** Figure 1. Our model exhibits impressive generalization ability across extensive unseen scenes. Left two columns: COCO [36]. Middle two: SA-1B [27] (a hold-out unseen set). ...
- **p. 3 / Figure/Table caption - extractive PDF cue:** Table 1. In total, our Depth Anything is trained on 1.5M labeled images and 62M unlabeled images jointly. our easy-to-acquire and diverse unlabeled images will ...
...
- **p. 5 / Figure/Table caption - extractive PDF cue:** Table 2. Zero-shot relative depth estimation. Better: AbsRel ↓, δ1 ↑. We compare with the best model from MiDaS v3.1. Note that MiDaS does not ...
...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Table 5. Zero-shot metric depth estimation. The first three test sets in the header are indoor scenes, while the last two are outdoor scenes. Following ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Table 6. Examine the zero-shot transferring performance of each labeled training set (left) to six unseen datasets (top). Better performance: AbsRel ↓, δ1 ↑. We ...
...
- **p. 5 / 4.1. Implementation Details - extractive PDF cue:** In the first stage, we train a teacher model on labeled images for 20 epochs.
- **p. 9 / 6. More Implementation Details - extractive PDF cue:** All images are cropped to 518×518 during training.
- **p. 9 / 6. More Implementation Details - extractive PDF cue:** Concretely, the training resolution is 392×518 on NYUv2 [55] and 384×768 on KITTI [18] to match the patch size of our encoder.
- **p. 9 / 6. More Implementation Details - extractive PDF cue:** The batch size is 16 and the model is trained for 5 epochs.
- **p. 9 / 6. More Implementation Details - extractive PDF cue:** The training resolution is set as 896×896 on both ADE20K [89] and Cityscapes [15].
- **p. 9 / 6. More Implementation Details - extractive PDF cue:** All images are cropped to 518×518 during training.
