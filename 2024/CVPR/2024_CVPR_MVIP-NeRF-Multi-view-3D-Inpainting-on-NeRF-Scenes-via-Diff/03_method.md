...
- **p. 4 / 3.2. Problem formulation and overview - extractive PDF cue:** Then, a latent diffusion model is employed as the appearance and geometry prior.
- **p. 4 / 3.3. Appearance Diffusion Prior - extractive PDF cue:** In this work, we use the stablediffusion-inpainting model [21] as our guidance model.
- **p. 3 / 3.1. Preliminary - extractive PDF cue:** Formally, let x = g(θ) represent an image rendered by a differentiable generator g with parameter θ, then SDS minimizes density distillation loss [18] which ...
- **p. 3 / 3.1. Preliminary - extractive PDF cue:** Therefore, the NeRF reconstruction loss can be formulated as La =  r∈R // ˆC(r) -C(r)//2, (1) where ˆC(r) represents the rendered color blended from ...
- **p. 5 / 3.4. Geometry Diffusion Prior - extractive PDF cue:** To update θ, we again employ the SDS loss that computes the gradient w.r.t. θ as: ∇θLg masked = w(t)  ϵω φ(zt; m, y, ...
- **p. 5 / 3.5. Multi-view Score Distillation - extractive PDF cue:** Intuitively, this function implies that when updating θ, we take into account the interactions with other sampled views, thereby promoting view consistency.
- **p. 6 / 3.5. Multi-view Score Distillation - extractive PDF cue:** its multi-view version, and jointly train the loss as: L = La unmasked + λ1Lg unmasked + λ2Lma masked + λ3Lg masked.
- **p. 4 / 3.2. Problem formulation and overview - extractive PDF cue:** Rather than directly utilizing inconsistent 2D inpainting results as supervisions and resolving these inconsistencies post hoc, we employ two SDS losses to compute a gradient ...

## Design Rationale

- **p. 2 / 1. Introduction - extractive PDF cue:** To this end, we present MVIP-NeRF, a novel approach that performs multiview-consistent inpainting in NeRF scenes via diffusion priors.
- **p. 2 / 1. Introduction - extractive PDF cue:** (iv) Extensive experiments to show the effectiveness of our method over existing NeRF inpainting techniques.
- **p. 4 / 3.2. Problem formulation and overview - extractive PDF cue:** To further enhance consistency for large-view motion, we introduce a multi-view score function.
...
- **p. 3 / 3.1. Preliminary - extractive PDF cue:** Formally, let x = g(θ) represent an image rendered by a differentiable generator g with parameter θ, then SDS minimizes density distillation loss [18] which ...
- **p. 4 / 3.2. Problem formulation and overview - extractive PDF cue:** Rather than directly utilizing inconsistent 2D inpainting results as supervisions and resolving these inconsistencies post hoc, we employ two SDS losses to compute a gradient ...
- **p. 3 / 3.1. Preliminary - extractive PDF cue:** For an efficient implementation, SDS updates the parameter θ by randomly choosing timesteps t ∼U(tmin, tmax) and forward x = g(θ) with noise ϵ ∼N(0, ...
- **p. 4 / 3.2. Problem formulation and overview - extractive PDF cue:** In the optimization process, for unmasked regions, we employ direct pixel-wise RGB and depth reconstruction losses.
- **p. 5 / 3.4. Geometry Diffusion Prior - extractive PDF cue:** The second column displays the normal map derived from the density field gradient and the corresponding optimized depth map.
...
- **p. 4 / 3.2. Problem formulation and overview - extractive PDF cue:** Input views, masks, and camera poses Shared diffusion priorࣦ ௨௡௠௔௦௞௘ௗ ௔ Multi-view appearance SDS ࣦ ௠௔௦௞௘ௗ ௔ Geometry SDS ࣦ ௠௔௦௞௘ௗ ௚ࣦ ௨௡௠௔௦௞௘ௗ ௚ ࠁ ...
- **p. 5 / 3.4. Geometry Diffusion Prior - extractive PDF cue:** In the first column, we present the input image with a mask (black region) and the depth map generated by NeRF, optimized with unmasked pixels.
- **p. 4 / 3.4. Geometry Diffusion Prior - extractive PDF cue:** In our work, we have two observations: (i) text-to-image diffusion models have a strong shape prior due to their training on diverse objects, and (ii) ...
- **p. 3 / 3.2. Problem formulation and overview - extractive PDF cue:** Given a set of RGB images, I = {Ii}n i=1, with corresponding 3D poses G = {Gi}n i=1, 2D masks M = {mi}n i=1, and ...
- **p. 5 / 3.4. Geometry Diffusion Prior - extractive PDF cue:** In particular, we search the K view 38 view 18 Inputs view 48 w/o multi-view w/ multi-view Figure 4.
- **p. 2 / 1. Introduction - extractive PDF cue:** Besides, they share the common limitation of neglecting the correlation between inpainted RGB images and inpainted depth maps, resulting in less pleasing geometry completion.
- **p. 2 / 1. Introduction - extractive PDF cue:** We summarize our contributions as follows: (i) A diffusion prior guided approach for high-quality NeRF inpainting, achieved without the need for explicit supervision of inpainted ...
...
- **p. 3 / 3.1. Preliminary - extractive PDF cue:** Therefore, the NeRF reconstruction loss can be formulated as La =  r∈R // ˆC(r) -C(r)//2, (1) where ˆC(r) represents the rendered color blended from ...
- **p. 6 / 3.5. Multi-view Score Distillation - extractive PDF cue:** its multi-view version, and jointly train the loss as: L = La unmasked + λ1Lg unmasked + λ2Lma masked + λ3Lg masked.
- **p. 6 / 4.1. Experimental Setup - extractive PDF cue:** We implemented our NeRF inpainting model built upon SPIn-NeRF [17] and trained it on 4 NVIDIA V100 GPUs for 10, 000 iterations using the Adam ...
- **p. 6 / 4.1. Experimental Setup - extractive PDF cue:** In addition, we implement an annealing timestep scheduling strategy [39], which allocates more training steps to lower values of t.
...
- **p. 6 / 4.1. Experimental Setup - extractive PDF cue:** Additionally, 40 test images without the object are provided for quantitative evaluations.
- **p. 6 / 4.2. Results - extractive PDF cue:** In total, we compare two NeRF inpainting approaches: SPIn-NeRF [17] with LaMa [27], and Remove-NeRF [35] with LaMa [27].
- **p. 7 / 4.2. Results - extractive PDF cue:** Remove-NeRF SPIn-NeRF Input views GT (two novel views) Ours Masks Remove-NeRF SPIn-NeRF Input views GT (two novel views) Ours Masks Figure 5.
...
- **PDF anchors reviewed:** method p. 4 (3.2. Problem formulation and overview), p. 4 (3.3. Appearance Diffusion Prior), p. 3 (3.1. Preliminary), p. 3 (3.1. Preliminary), p. 5 (3.4. Geometry Diffusion Prior), p. 5 (3.5. Multi-view Score Distillation), objective p. 5 (3.4. Geometry Diffusion Prior), p. 3 (3.1. Preliminary), p. 4 (3.2. Problem formulation and overview), p. 3 (3.1. Preliminary), p. 4 (3.2. Problem formulation and overview), p. 5 (3.4. Geometry Diffusion Prior), temporal p. 3 (3.1. Preliminary), p. 4 (3.3. Appearance Diffusion Prior), p. 6 (4.1. Experimental Setup), p. 2 (1. Introduction), p. 6 (4.1. Experimental Setup), p. 2 (1. Introduction).
