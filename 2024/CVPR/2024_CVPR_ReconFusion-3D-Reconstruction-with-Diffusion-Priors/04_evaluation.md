...
- **p. 5 / 4.1. Experiment Setup - extractive PDF cue:** For the mip-NeRF 360 dataset, we retain its original test set and select the input views from the training set using a heuristic to encourage ...
- **p. 5 / 4.1. Experiment Setup - extractive PDF cue:** For the real-world object-centric scenes from CO3D we evaluate on a subset of 20 scenes from 10 categories.
- **p. 6 / 4.2. Comparison Results - extractive PDF cue:** A visual comparison of rendered images and depth maps on scenes from the RealEstate10K [71], LLFF [31], DTU [23], CO3D [39], and mip-NeRF 360 [1] ...
- **p. 7 / 4.2. Comparison Results - extractive PDF cue:** However, they fall short on 360-degree scenes (e.g. the CO3D dataset), where a large portion of the scene is undersampled or even unobserved due to ...
- **p. 7 / 4.2. Comparison Results - extractive PDF cue:** Baselines that we additionally tuned for the task of few-view reconstruction are indicated with ∗. eRF and SimpleNeRF) are able to significantly improve the baseline ...
- **p. 8 / 4.4. Scaling to More Views - extractive PDF cue:** Our learned diffusion prior improves performance over the Zip-NeRF baseline up to as many as 81 input views on the kitchenlego scene from the mip-NeRF ...
- **p. 8 / 4.4. Scaling to More Views - extractive PDF cue:** Though most results in this paper focus on the challenging case of 3-9 input views, achieving a high-quality reconstruction of a real-world scene often requires ...
- **p. 6 / 4.2. Comparison Results - extractive PDF cue:** Both the appearance and geometry of our method are of higher quality than the baselines in these examples-typical failure modes exhibited by the baselines include ...
...
- **p. 3 / Figure/Table caption - extractive PDF cue:** Figure 2. (a) We optimize a NeRF to minimize a reconstruction loss Lrecon between renderings and a limited set of input images, alongside a sample ...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Figure 3. A visual comparison of rendered images and depth maps on scenes from the RealEstate10K [71], LLFF [31], DTU [23], CO3D [39], and mip-NeRF ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Table 1. Quantitative evaluation of few-view 3D reconstruction methods. Datasets are ordered in terms of sparsity from easier (novel views are close to observed views) ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Table 2. We ablate two aspects of our model: pretrained dif- fusion weights (PT) and conditioning. For PT, we initialize the diffusion model weights from ...
...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Figure 5. Comparing diffusion losses for 3D reconstruction. Note the "blotchy" texture on the placemat and background chair when using SDS, and improved background detail ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Figure 6. Our learned diffusion prior improves performance over the Zip-NeRF baseline up to as many as 81 input views on the kitchenlego scene from ...
...
- **p. 5 / 3.3. Implementation Details - extractive PDF cue:** Regardless of t, we always sample the denoised image with k = 10 steps.
- **p. 5 / 4.1. Experiment Setup - extractive PDF cue:** For Objaverse, we render each 3D asset from 16 randomly sampled views at resolution 512×512 and composite the rendering onto a randomly selected solid color ...
- **p. 5 / 4.1. Experiment Setup - extractive PDF cue:** For training, we sample 3 frames of the same scene as input views and sample another frame as the target view.
- **p. 5 / 4.1. Experiment Setup - extractive PDF cue:** For the real-world object-centric scenes from CO3D we evaluate on a subset of 20 scenes from 10 categories.
