...
- **p. 3 / 3. The Multisensory-Universe Dataset - extractive body cue:** As presented in Figure 2, we begin by explaining how we input interactive objects into the scene to construct object-centric 3D scenes for our dataset ...
- **p. 3 / 3.1. Inputting Interactive Objects into 3D Scenes - extractive body cue:** We build our scenes on top of the Habitat-Matterport 3D (HM3D) semantics dataset [45, 62], which has 216 3D Context (Bounding box, material, temperature, hardness……): ...
- **p. 6 / 5. Experiments - extractive body cue:** We ensure that no scenes and objects in the Multisensory Universe appear in the evaluation setup.
- **p. 6 / 5.1. Object Retrieval - extractive body cue:** The scene setup is different from the Multisensory Universe as we place more distracting objects to retrieve from (while in Multisensory Universe most scenes have ...
- **p. 4 / 3.3. Embodied Agents for Data Collection - extractive body cue:** Then we place an embodied agent to interact with the objects in 3D environments to perform the task and collect interaction data.
- **p. 7 / 5.2. Tool Use - extractive body cue:** Similar to the object retrieval task, we place some objects from different categories, and also objects from the same categories but with different materials/haptic/thermal information ...
- **p. 8 / 5.4. Task Decomposition - extractive body cue:** In our setting, we place several possible choice combinations in the environment, we also place object combinations unseen from the Multisensory Universe.
- **p. 4 / 3.3. Embodied Agents for Data Collection - extractive body cue:** Inspired by [58], we utilize LLM-powered embodied agents to collect the data in the constructed scenes.
...
- **p. 1 / Figure/Table caption - extractive body cue:** Figure 1. We propose MultiPLY, a multisensory embodied LLM that encodes object-centric multisensory representations (e.g., visual, audio, tactile, and thermal), by deploying an embodied agent ...
- **p. 3 / Figure/Table caption - extractive body cue:** Figure 2. Multisensory-Universe Generation Pipelines. We first add a set of new interactive objects in the embodied environments, then prompt ChatGPT to generate diverse tasks ...
- **p. 5 / Figure/Table caption - extractive body cue:** Figure 3. Overview of our MultiPLY. We first encode the scene as an abstracted object-centric representation, while multisensory details of objects can only be unveiled ...
- **p. 6 / Figure/Table caption - extractive body cue:** Table 1. Experimental Results of Object Retrieval. -I denotes the models utilize oracle action tokens to interact with the environ- ment. (Finetuned) means finetuned on ...
- **p. 7 / Figure/Table caption - extractive body cue:** Table 2. Experimental Results of Tool Use.
- **p. 7 / Figure/Table caption - extractive body cue:** Table 3. Experimental Results of Multisensory Captioning. Analysis Table 3 shows the result. From the table, we could see that 3D-based LLMs overall outshine 2D ...
- **p. 8 / Figure/Table caption - extractive body cue:** Figure 4. Qualitative Examples of our MultiPLY. MultiPLY could interact with the objects in the embodied environments and gather multisensory information. able foods in the ...
- **p. 8 / Figure/Table caption - extractive body cue:** Table 4. Experimental Results of Task Decomposition. Analysis Table 4 shows the task decomposition results. From the table, we observe that models without interaction have ...
