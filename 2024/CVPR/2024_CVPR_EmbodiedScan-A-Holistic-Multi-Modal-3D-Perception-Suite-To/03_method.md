...
- **p. 6 / 4.1. Multi-Modal 3D Encoder - extractive PDF cue:** Given the multi-level sparse visual features F S k and text features from the text encoder, we use a multi-modal fusion transformer model [20, 61] ...
- **p. 5 / 4.1. Multi-Modal 3D Encoder - extractive PDF cue:** Next, we first present how we aggregate multi-view inputs and then introduce different fusion approaches for dense and sparse feature extraction.
- **p. 6 / 4.2. Sparse & Dense Decoder - extractive PDF cue:** We use cross-entropy loss and sceneclass affinity loss [55] for training.
- **p. 1 / Abstract - extractive PDF cue:** This necessitates the ability to fully understand 3D scenes given their first-person observations and contextualize them into language for interaction.
- **p. 4 / 3.2. Annotation - extractive PDF cue:** We used the Segment Anything Model (SAM) [22] and a customized annotation tool based on [24] (Fig.
- **p. 5 / 4.1. Multi-Modal 3D Encoder - extractive PDF cue:** For the sparse case, we use multi-level features as seeds instead of a single dense feature map to predict 3D objects.
- **p. 3 / 3.1. Data Collection & Processing - extractive PDF cue:** We first unified the format into a general multi-view case to fit Matterport3D by adding randomness when loading images but maintaining sequential continuity for ScanNet ...
- **p. 6 / 4.2. Sparse & Dense Decoder - extractive PDF cue:** Training objectives include the original classification loss, centerness loss, and a disentangled Chamfer Distance (CD) loss for eight corners [3, 44].

## Design Rationale

- **p. 1 / Abstract - extractive PDF cue:** Building upon this database, we introduce a baseline framework named Embodied Perceptron.
- **p. 1 / Abstract - extractive PDF cue:** To address the gap, we introduce EmbodiedScan, a multi-modal, ego-centric 3D perception dataset and benchmark for holistic 3D scene understanding.
...
- **p. 6 / 4.2. Sparse & Dense Decoder - extractive PDF cue:** Training objectives include the original classification loss, centerness loss, and a disentangled Chamfer Distance (CD) loss for eight corners [3, 44].
- **p. 6 / 4.2. Sparse & Dense Decoder - extractive PDF cue:** We use cross-entropy loss and sceneclass affinity loss [55] for training.
- **p. 5 / 4. Embodied Perceptron - extractive PDF cue:** In addition, we customize the output's parameterization and training objectives to fit the formulation of oriented 3D bounding boxes in the sparse decoder.
- **p. 5 / 4.1. Multi-Modal 3D Encoder - extractive PDF cue:** The initial attempt of still query features from Fup or raw images I for these seeds is unstable due to inconsistent features for fusion and ...
- **p. 4 / 3.2. Annotation - extractive PDF cue:** Given updated 3D bounding boxes annotated with orientations, we derive the language prompts that describe the spatial relationships among objects following SR3D [1].
...
- **p. 4 / 4. Embodied Perceptron - extractive PDF cue:** Given this dataset, we can take multi-modality input, including RGB images, point clouds derived from depth maps as well as language prompts, to extract multi-modal ...
- **p. 1 / 1. Introduction - extractive PDF cue:** Most previous studies have primarily revolved around scene-level input and output problems from a global view [13, 34, 40], i.e., taking reconstructed 3D point clouds ...
- **p. 1 / Abstract - extractive PDF cue:** This necessitates the ability to fully understand 3D scenes given their first-person observations and contextualize them into language for interaction.
- **p. 4 / 3.1. Data Collection & Processing - extractive PDF cue:** A global coordinate system is necessary to aggregate multi-view observations and serve as a reference for outputs.
- **p. 5 / 4.1. Multi-Modal 3D Encoder - extractive PDF cue:** Next, we first present how we aggregate multi-view inputs and then introduce different fusion approaches for dense and sparse feature extraction.
- **p. 2 / Dataset - extractive PDF cue:** We establish two series of benchmarks on EmbodiedScan: 1) fundamental 3D perception benchmarks focusing on traditional tasks, including 3D detection and semantic occupancy prediction under ...
- **p. 5 / 4.1. Multi-Modal 3D Encoder - extractive PDF cue:** Formally, the input aggregated points P ∈RNp×3 (first voxelized) and Ni images as I ∈RNi×H×W are processed via a Minkowski ResNet and a shared 2D ...
...
- **p. 4 / 3.2. Annotation - extractive PDF cue:** Semantic occupancy necessitates accurate boundaries across semantic regions without considering object pose or recalling all the objects, so the original point cloud segmentation annotations were ...
- **p. 7 / 5.1. Fundamental 3D Perception Benchmarks - extractive PDF cue:** If a category lacks instances, it is removed when calculating mAP and mIoU.
- **p. 7 / 5.1. Fundamental 3D Perception Benchmarks - extractive PDF cue:** Variants of our baselines exhibit a performance trend akin to embodied benchmarks.
- **p. 8 / 5.1. Fundamental 3D Perception Benchmarks - extractive PDF cue:** Ablation with conventional settings.
- **p. 8 / 5.2. Language-Grounded Benchmark - extractive PDF cue:** As an initial step, this setup takes multi-view RGB-D images as input without considering differing prompt timestamps.
