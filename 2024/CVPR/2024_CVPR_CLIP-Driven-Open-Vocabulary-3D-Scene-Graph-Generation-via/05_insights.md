...
- **Problem cue:** However, current 3DSGG methods struggle with two main challenges.
- **Claim/result cue:** Lastly, the recognition of novel object and predicate classes is achieved by calculating the cosine similarity between prompts and 3DSG features.
...
- **Registry interface:** `3D Scene Graph, CLIP, Graph Reasoning` is the paper's recorded topic/interface, not evidence that the full robotics loop was evaluated.
...
- Registry position: `3D Vision-Language Understanding`; tags: `3D Scene Graph, CLIP, Graph Reasoning`.
- A direct citation predecessor/successor is not recorded in the legacy note; confirm it from references and the track synthesis before asserting lineage.
- Recorded scope boundary/future cue:
  - 논문이 도달한 지점: Lastly, the recognition of novel object and predicate classes is achieved by calculating the cosine similarity between prompts and 3DSG features.
...
- **Protocol carried forward from the legacy note (candidate, not a verified paper evaluation):**
  - 논문 내 evaluation 단서: ScanNet, ScanRefer / accuracy, collision
