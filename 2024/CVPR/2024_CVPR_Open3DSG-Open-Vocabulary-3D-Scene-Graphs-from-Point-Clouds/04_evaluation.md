...
|---|---|---|---|---|---|---|---|
| geometric accuracy, semantic consistency와 planning/manipulation utility | setting not found in current note | ScanNet; split/role unresolved | 3D scene/object와 robot coordinate frame | not found in current note | accuracy; Our experiments show that Open3DSG is effective at predicting arbitrary object classes as well as their complex inter-object relationships describing spatial, supportive, semantic and comparative relationships. | trials: not reported; seeds: not reported | 04_evaluation.md `Evaluation Protocol and Results`; exact table/figure/page 확인 필요 |
...
| LERF | legacy dataset/benchmark cue; train/eval/pretraining/auxiliary role unresolved | not reported | 04_evaluation.md `Dataset / Benchmark`; exact table/figure/page 확인 필요 |
...
|---|---|---|---|
| accuracy | direction/unit not reported | legacy keyword cue; metric role and direction not verified | 04_evaluation.md `Metrics`; exact table/figure/page 확인 필요 |
...
|---|---|---|---|---|
| geometric accuracy, semantic consistency와 planning/manipulation utility | Our experiments show that Open3DSG is effective at predicting arbitrary object classes as well as their complex inter-object relationships describing spatial, supportive, semantic and comparative relationships. | EMPIRICAL / LEARNING OR SIMULATION | legacy protocol cue; exact main table/figure and conditions require verification | 04_evaluation.md `Evaluation Protocol and Results`; exact table/figure/page 확인 필요 |

## Generalization and Failure Cases

| Assumption / regime | Failure or stress test | Status | Source |
|---|---|---|---|
| foundation semantic feature가 novel object/relation 의미를 보존 | domain-specific geometry/contact relation은 semantic prior만으로 혼동 | profile/formulation-derived stress test; not necessarily paper-reported | 02 problem profile; exact failure evidence verify |
| point cloud가 object boundary와 relative geometry를 보존 | occlusion·sparse scan은 missing node/edge | profile/formulation-derived stress test; not necessarily paper-reported | 02 problem profile; exact failure evidence verify |
...
- **Research-facing limitation:** point cloud segmentation/instance grouping이 안정적이고, open-vocabulary embedding과 relation predicate가 동일 scene coordinate에서 해석되어야 한다.

- **Source anchor:** 본문의 fixed-category 3D scene graph bottleneck, open-vocabulary node/edge prediction과 queryable graph formulation.; exact dataset table, split, baseline configuration, ablation table and result figure must be located.
