...
- **p. 5 / 4. Experiments - extractive PDF cue:** We test the zero-shot generalization ability of the proposed method on the dataset of MVImgNet [65], which is a real-world dataset with 220k object videos ...
- **p. 6 / 4.2. Results on MVImgNet - extractive PDF cue:** 4, our method can well generalize to more various categories of objects than in the CO3D-v2 dataset.
- **p. 5 / 4. Experiments - extractive PDF cue:** We also contribute a dataset with 100k cleaned point clouds from MVImgNet.
- **p. 6 / 4. Experiments - extractive PDF cue:** Results on CO3D-v2 We show the evaluation results of the proposed method on the CO3D-v2 dataset in Tab.
- **p. 7 / 4.3. Ablation Study - extractive PDF cue:** Notably, when addressing objects with Table 2.
- **p. 8 / 4.3. Ablation Study - extractive PDF cue:** Visualization of reconstructions by different methods on CO3D-v2 unseen categories.
- **p. 5 / 4. Experiments - extractive PDF cue:** The metrics can be divided into two groups for measuring (i) the absolute distance: the Chamfer distance (CD) and its two components that measure the ...
- **p. 6 / 4. Experiments - extractive PDF cue:** With PVCNN, our method improves the performance of the baseline PC2-depth by 19.2% on Chamfer distance and 7.8% on F-score.
...
- **p. 1 / Figure/Table caption - extractive PDF cue:** Figure 1. Our work focuses on the task of generalizable 3D object reconstruction from a single RGB-D image. The proposed method conducts implicit field learning ...
- **p. 4 / Figure/Table caption - extractive PDF cue:** Figure 2. Overview of the proposed method. The network takes a single-view image and a partial point cloud unprojected from the image according to the ...
- **p. 5 / Figure/Table caption - extractive PDF cue:** Fig. 3. The UDF prediction ν′ is first computed and sent into the the other decoder. As ν′ ∈RN×1 provides point- wise information, we simply ...
...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Table 1. Results on CO3D-v2, averaged on all samples from 10 held-out categories. The best results are highlighted in bold font.
- **p. 6 / Figure/Table caption - extractive PDF cue:** Fig. 6. As shown, the proposed method can produce better generations than the baseline method (Ours1 v.s. PC2-depth and Ours2 v.s. NU-MCC) on both higher ...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Figure 4. Visualization on MVImgNet data. Upper: generalization results by Ours2; Lower: the comparison of generalization results before and after fine-tuning on cleaned MVImgNet data. ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Figure 5. Visualization of the denoising process (t={1000, 750, 500, 250, 0}) of our method in inferring. Note we only sample 2k points in each ...
...
- **p. 5 / 4. Experiments - extractive PDF cue:** It consists of around 37k videos of 51 object categories, of which 10 are held out for evaluation and the remaining 41 for training.
- **p. 5 / 4. Experiments - extractive PDF cue:** Note that videos of MVImgNet are captured in N×3 Embed N×e t Embed scale shift Linear 1×e' 𝑋𝑋𝑡𝑡 N×e Linear P (M+N)×d 𝐸𝐸𝑋𝑋 I M×3 ...
- **p. 6 / 4. Experiments - extractive PDF cue:** Method Backbone Acc↓ Comp↓ CD↓ Prec↑ Recall↑ F1↑ PC2 [35] PVCNN 0.342 0.214 0.556 24.2 56.2 33.0 PC2-depth PVCNN 0.209 0.103 0.312 61.7 87.6 70.7 ...
- **p. 6 / 4. Experiments - extractive PDF cue:** Besides, T is set as 1,000 in our diffusion model, λ=1.0 in training, and N=50k, the distance threshold ρ=0.1 for evaluation.
...
- **p. 8 / 5. Conclusion - extractive PDF cue:** We also develop a self-conditioning mechanism to leverage implicit predictions to reversely assist the noise estimation in diffusion learning, which eventually forges a cooperative system.
- **p. 1 / Figure/Table caption - extractive PDF cue:** Figure 1. Our work focuses on the task of generalizable 3D object reconstruction from a single RGB-D image. The proposed method conducts implicit field learning ...
- **p. 5 / 4. Experiments - extractive PDF cue:** In CO3D-v2, the object shape annotations are obtained via COLMAP [50, 51] and thus inevitably contain noise and voids.
