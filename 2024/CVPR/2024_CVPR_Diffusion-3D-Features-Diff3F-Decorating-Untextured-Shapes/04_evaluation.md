...
- **p. 6 / 4.1. Datasets and Benchmarks - extractive body cue:** We also evaluate our method on the FAUST benchmark [8].
- **p. 7 / 4.4. Evaluation on Human Shapes - extractive body cue:** We present results on the SHREC'19 dataset.
...
- **p. 8 / 4.6. Ablations - extractive body cue:** Although our complete approach produces the second-best score in every category, incorporating all of our parts together (including fusion with DINO) resulted in the best ...
- **p. 6 / 4.2. Evaluation Metrics - extractive body cue:** The correspondence accuracy is measured as the fraction of correct correspondences within a threshold tolerance distance: acc( \ e p s ilon ) = \fr ...
- **p. 6 / 4.2. Evaluation Metrics - extractive body cue:** We use the average correspondence error and the correspondence accuracy as our evaluation criteria.
...
- **p. 2 / Figure/Table caption - extractive body cue:** Table 1. Comparison of DIFFUSION 3D FEATURES to related methods. Unlike traditional geometric feature detectors (e.g., WKS), modern learning-based approaches require training and can struggle ...
...
- **p. 6 / Figure/Table caption - extractive body cue:** Table 3. Generalization. We compare generalization capabilities of DIFF3F vs others by training on one dataset and testing on a different set. For DPC and ...
- **p. 7 / Figure/Table caption - extractive body cue:** Figure 4. Comparisons. We compare our DIFF3F (bottom) against SOTA methods (i.e., DPC [30] and SE-ORNet [14]) for the task of point-to-point shape correspondence. Corresponding ...
...
- **p. 6 / 4.1. Datasets and Benchmarks - extractive body cue:** TOSCA comprises of 80 objects representing a mixture of animals and humans, formed by deforming template meshes.
- **p. 7 / 4.5. Evaluation on Animal Shapes - extractive body cue:** While the evaluation is on a subset of only about 50 points as the number of annotated points is very limited, we show dense correspondence ...
- **p. 5 / 3.3. Distilling 2D Features to 3D - extractive body cue:** Moreover, we render the 3D shape from several views (n = 100), which further stabilizes the aggregation, resulting in descriptors that mostly capture semantic meaning: ...
