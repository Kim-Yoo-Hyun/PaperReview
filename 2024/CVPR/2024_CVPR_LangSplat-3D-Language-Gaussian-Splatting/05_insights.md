...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: Modeling a 3D language field to support open-ended language queries in 3D has gained increasing attention recently.
- 출발 문제 단서: Unlike existing methods that ground CLIP language embeddings in a NeRF model, LangSplat advances the field by utilizing a collection of 3D Gaussians, each encoding language features distilled ...
- 주장된 효과 단서: Humans live in a 3D world and commonly use natural language to interact with a 3D scene.
...
- Result cue: Humans live in a 3D world and commonly use natural language to interact with a 3D scene.
