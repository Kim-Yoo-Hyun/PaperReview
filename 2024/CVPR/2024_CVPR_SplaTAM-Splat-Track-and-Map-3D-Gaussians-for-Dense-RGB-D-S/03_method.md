...
- **p. 4 / 3. Method - extractive PDF cue:** E.g. the camera parameters are initialized using the following: E_ { t+ 1} = E_t + (E_t - E_{t \text {-} 1} ) (7) The ...
- **p. 4 / 3. Method - extractive PDF cue:** We begin with a brief overview and then describe each module in detail.
- **p. 3 / 3. Method - extractive PDF cue:** By modeling the world as a collection of 3D Gaussians which can be rendered into highfidelity color and depth images, we are able to directly ...
- **p. 3 / 3. Method - extractive PDF cue:** We made a number of simplifications to the representation proposed in [14], by using only view-independent color and forcing Gaussians to be isotropic.
- **p. 5 / 3. Method - extractive PDF cue:** Instead of starting from scratch, we warm-start the optimization from the most recently constructed map.
- **p. 5 / 3. Method - extractive PDF cue:** This phase optimizes a similar loss as during tracking, except we don't use the silhouette mask as we want to optimize over all pixels.
...
- **p. 5 / 3. Method - extractive PDF cue:** This is done again by differentiablerendering and gradient-based-optimization, however unlike tracking, in this setting the camera poses are fixed, and the parameters of the Gaussians ...

## Design Rationale

- **p. 2 / 1. Introduction - extractive PDF cue:** We show across all our experiments on simulated and real data that our approach, SplaTAM, achieves state-of-the-art results compared to all previous approaches for camera ...
...
- **p. 3 / 3. Method - extractive PDF cue:** By modeling the world as a collection of 3D Gaussians which can be rendered into highfidelity color and depth images, we are able to directly ...
- **p. 5 / 3. Method - extractive PDF cue:** This phase optimizes a similar loss as during tracking, except we don't use the silhouette mask as we want to optimize over all pixels.
- **p. 3 / 3. Method - extractive PDF cue:** Each Gaussian influences a point in 3D space x ∈R3 according to the standard (unnormalized) Gaussian equation weighted by its opacity: f( \ math b ...
...
- **p. 4 / 3. Method - extractive PDF cue:** We propose to similarly differentiably render depth: D( \ m a thb f {p}) = \ s um _{i = 1}^{n} d_i f_i(\mathbf {p}) \prod ...
- **p. 4 / 3. Method - extractive PDF cue:** We add new Gaussians to the map based on the rendered silhouette and input depth.
- **p. 3 / 3. Method - extractive PDF cue:** By modeling the world as a collection of 3D Gaussians which can be rendered into highfidelity color and depth images, we are able to directly ...
- **p. 2 / 1. Introduction - extractive PDF cue:** We show across all our experiments on simulated and real data that our approach, SplaTAM, achieves state-of-the-art results compared to all previous approaches for camera ...
- **p. 5 / 3. Method - extractive PDF cue:** Overlap is determined by taking the point cloud of the current frame depth map and determining the number of points inside the frustum of each ...
- **p. 1 / 1. Introduction - extractive PDF cue:** Visual simultaneous localization and mapping (SLAM)- the task of estimating the pose of a vision sensor (such as a depth camera) and a map of ...
...
- **p. 5 / 5. Results & Discussion - extractive PDF cue:** Finally, we discuss pipeline ablations and provide a runtime comparison.
- **p. 6 / 5. Results & Discussion - extractive PDF cue:** On ScanNet++ [49], both SOTA SLAM approaches Point-SLAM [30] and ORB-SLAM3 [3] (RGB-D variant) completely fail to correctly track the camera pose due to the ...
- **p. 7 / 5. Results & Discussion - extractive PDF cue:** Color & Depth Loss Ablation on Replica/Room 0.
- **p. 7 / 5. Results & Discussion - extractive PDF cue:** Camera Tracking Ablations on Replica/Room 0. depth loss.
- **p. 8 / 5. Results & Discussion - extractive PDF cue:** Silhouette is critical as without it tracking completely fails.
- **p. 8 / 5. Results & Discussion - extractive PDF cue:** Although SplaTAM achieves state-of-the-art performance, we find our method to show some sensitivity to motion blur, large depth noise, and aggressive rotation.
...
- **PDF anchors reviewed:** method p. 4 (3. Method), p. 4 (3. Method), p. 3 (3. Method), p. 3 (3. Method), p. 5 (3. Method), p. 5 (3. Method), objective p. 4 (3. Method), p. 4 (3. Method), p. 5 (3. Method), p. 3 (3. Method), p. 5 (3. Method), p. 3 (3. Method), temporal p. 4 (3. Method), p. 4 (3. Method), p. 3 (2. Related Work), p. 3 (3. Method), p. 5 (4. Experimental Setup), p. 5 (4. Experimental Setup).
