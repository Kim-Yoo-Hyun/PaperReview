...
- **p. 3 / 3. Method - extractive PDF cue:** Then, we dive into the details of our time-varying two-stream architecture, including the encoder part, latent point Transformer, time mask generator, and decoder part.
- **p. 4 / 3.3. Latent Point Cloud Transformer - extractive PDF cue:** Following [29], we use dual PatchNorm to project the latent point cloud into tokens, which place LayerNorm before and after an MLP layer for more ...
- **p. 4 / 3.2. Noisy Point Cloud Encoder - extractive PDF cue:** Specifically, a noisy point cloud encoder E : RN×3 →RM×d transforms the noisy point cloud Xt ∈RN×3 at timestep t into a latent point cloud ...
- **p. 3 / 3.2. Noisy Point Cloud Encoder - extractive PDF cue:** This representation works well with both CNN and Transformer architecture as we can downsample the voxel-grid following [11] to 9464
- **p. 5 / 3.4. Time Mask Generator - extractive PDF cue:** We apply a depth-2 PVCNN [30] to get local feature Xc ∈ RM×D and our latent point cloud Transformer to get global feature Xtr ∈RM×D.
- **p. 7 / Method - extractive PDF cue:** 2, our model trains much faster than LION, with only a quarter of its training time and a third of its inference time.
- **p. 5 / 3.4. Time Mask Generator - extractive PDF cue:** Time mask generator module scales the local and global features by the time variable.
- **p. 3 / 3.1. Problem Formulation - extractive PDF cue:** In training, we optimize the MSE loss: Lsimple = Et∼[1,T ]∥µ -µθ(Xt, t)∥2 2 = Et∼[1,T ]∥ϵ -ϵθ(Xt, t)∥2 2, (3) where ϵ is the ...

## Design Rationale

- **p. 2 / 1. Introduction - extractive PDF cue:** Our main contributions include: • We propose a novel two-stream denoising model, which uses timestep to optimally reweigh the global feature from Transformer and the ...
- **p. 1 / 1. Introduction - extractive PDF cue:** We propose to merge these two properties across different timesteps in the diffusion process. plore and develop efficient and effective model architectures for 3D point ...
- **p. 4 / 3.3. Latent Point Cloud Transformer - extractive PDF cue:** We propose two novel 3D space continuous position encoding methods: Phase Shift Position Encoding (PSPE) and Baseλ Position Encoding (BλPE).
...
- **p. 3 / 3.1. Problem Formulation - extractive PDF cue:** In training, we optimize the MSE loss: Lsimple = Et∼[1,T ]∥µ -µθ(Xt, t)∥2 2 = Et∼[1,T ]∥ϵ -ϵθ(Xt, t)∥2 2, (3) where ϵ is the ...
- **p. 4 / 3.3. Latent Point Cloud Transformer - extractive PDF cue:** We maximize the phase shift difference ( 4π 3 ) of different axes to distinguish them and combine Sine and Cosine to guarantee the linear ...
- **p. 5 / 3.4. Time Mask Generator - extractive PDF cue:** With this constraint, the trade-off of local and global features can be learned by the network.
- **p. 3 / 3.1. Problem Formulation - extractive PDF cue:** By giving a sequence of pre-defined noising scales β1, β2, ..., βT , the transition probability can be expressed as : q(Xt/Xt-1) = N( p ...
...
- **p. 2 / 1. Introduction - extractive PDF cue:** Our main contributions include: • We propose a novel two-stream denoising model, which uses timestep to optimally reweigh the global feature from Transformer and the ...
- **p. 4 / 3.2. Noisy Point Cloud Encoder - extractive PDF cue:** We use furthest point sampling algorithm [11] to downsample the input noisy point cloud Xt ∈RN×3 into a sparser point cloud Xs t ∈RM×3 (M ...
- **p. 1 / 1. Introduction - extractive PDF cue:** Once trained, the model can be used to generate new point clouds by iterating the reverse process over a sequence of time steps, with each ...
- **p. 2 / 1. Introduction - extractive PDF cue:** Empirically we show that TIGER achieves state-ofthe-art (SoTA) performance in 3D point cloud generation.
- **p. 5 / 3.4. Time Mask Generator - extractive PDF cue:** We apply a depth-2 PVCNN [30] to get local feature Xc ∈ RM×D and our latent point cloud Transformer to get global feature Xtr ∈RM×D.
- **p. 6 / Method - extractive PDF cue:** TIGER generates high-quality and diverse 3D point clouds. where WD×3 is the projection matrix to map the noise into 3D space and Xt is the ...
- **p. 1 / 1. Introduction - extractive PDF cue:** However, compared to 2D images, the cost and complexity of acquiring 3D point clouds make it crucial to exFigure 1.
...
- **p. 4 / 3.3. Latent Point Cloud Transformer - extractive PDF cue:** Following [29], we use dual PatchNorm to project the latent point cloud into tokens, which place LayerNorm before and after an MLP layer for more ...
- **p. 7 / Method - extractive PDF cue:** 2, our model trains much faster than LION, with only a quarter of its training time and a third of its inference time.
- **p. 7 / Method - extractive PDF cue:** Training Time (GPU hours) Inference Time (s) PVD [55] 142 8.46 LION [53] 550 27.12 Tiger 164 9.73 Table 2.
- **p. 7 / Method - extractive PDF cue:** 2, our model trains much faster than LION, with only a quarter of its training time and a third of its inference time.
...
- **p. 7 / 4.3. Ablation and Analysis - extractive PDF cue:** Ablation of Transformer backbones, position encoding, and self-attention strategies.
- **p. 7 / 4.3. Ablation and Analysis - extractive PDF cue:** In this ablation, we also compare the performance of our time masking with scalar value setting and channel-wise value setting.
