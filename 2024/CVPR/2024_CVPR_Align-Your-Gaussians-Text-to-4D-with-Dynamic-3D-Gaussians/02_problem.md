...
- **p. 1 / Abstract - extractive PDF cue:** Text-guided diffusion models have revolutionized image and video generation and have also been successfully used for optimization-based 3D object synthesis.
- **p. 1 / Abstract - extractive PDF cue:** Here, we instead focus on the underexplored text-to-4D setting and synthesize dynamic, animated 3D objects using score distillation methods with an additional temporal dimension.
...
- **p. 1 / Abstract - extractive PDF cue:** Our method, called Align Your Gaussians (AYG), leverages dynamic 3D Gaussian Splatting with deformation fields as 4D representation.
- **p. 1 / Abstract - extractive PDF cue:** Crucial to AYG is a novel method to regularize the distribution of the moving 3D Gaussians and thereby stabilize the optimization and induce motion.
- **p. 2 / 1. Introduction - extractive PDF cue:** We also propose a new view-guidance method to generate consistent 3D scenes for initialization of the 4D stage, and we leverage the concurrent classifier score ...
...
| Observation / input | Compared to previous work, we pursue a novel compositional generation-based approach, and combine text-to-image, text-to-video, and 3D-aware multiview diffusion models to provide ... | conditioning observation와 noisy/intermediate sample | exact sensor/frame/preprocessing from PDF |
...
| Decision / output variable | sample/action x̂ or trajectory; body terms: Align, Your, Gaussians, AYG, novel, content, creation, scale | p. 2 (1. Introduction), p. 2 (1. Introduction), p. 1 (1. Introduction) |
...
PDF contribution framing (p. 2 (1. Introduction), p. 2 (1. Introduction), p. 1 (1. Introduction), p. 1 (Abstract), p. 4 (3. Align Your Gaussians)): We propose Align Your Gaussians (AYG), a novel method for 4D content creation.
...
- **p. 1 / Abstract - extractive PDF cue:** Our method, called Align Your Gaussians (AYG), leverages dynamic 3D Gaussian Splatting with deformation fields as 4D representation.
- **p. 4 / 3. Align Your Gaussians - extractive PDF cue:** 3.1, we present AYG's 4D representation, and in Sec.
...
generative writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 1 (Abstract), p. 4 (3.2. Text-to-4D as Compositional Generation), p. 6 (3.3. AYG's Score Distillation in Practice), p. 2 (1. Introduction). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (1. Introduction), interface p. 1 (Abstract), p. 4 (3.2. Text-to-4D as Compositional Generation), p. 6 (3.3. AYG's Score Distillation in Practice), p. 2 (1. Introduction), objective p. 3 (2. Background), p. 2 (1. Introduction), p. 2 (1. Introduction), p. 3 (2. Background), p. 4 (3.2. Text-to-4D as Compositional Generation), p. 5 (3.2. Text-to-4D as Compositional Generation).
