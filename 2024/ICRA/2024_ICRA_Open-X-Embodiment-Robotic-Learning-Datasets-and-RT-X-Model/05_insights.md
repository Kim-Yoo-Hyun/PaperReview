...
- Result/evaluation cue: We show that a high-capacity model trained on this data, which we call RT-X, exhibits positive transfer and improves the capabilities of multiple robots by leveraging experience from ...

위 cue는 기존 official abstract 또는 local text extraction에서 보존한 것이다. 수치·조건·인과적 해석은 full-text 정독 전까지 `UNVERIFIED`다.

## Dependency Position

`VoxPoser: Composable 3D Value Maps for Robotic Manipulation with Language Models → Open X-Embodiment: Robotic Learning Datasets and RT-X Models → Octo: An Open-Source Generalist Robot Policy`
