...
- **Claim/result cue:** For legged robots that can robustly traverse various terrains , such local planners have become a routine and demonstrated remarkable performance in different tasks , .
...
- **Registry interface:** `Robotics, legged locomotion, Navigation, robust perception` is the paper's recorded topic/interface, not evidence that the full robotics loop was evaluated.
...
- Registry position: `Locomotion, Whole-Body, and Mobile Manipulation`; tags: `Robotics, legged locomotion, Navigation, robust perception`.
