...
- **p. 3 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** We use a neural network to represent the policy for mapping observation sequences to actions.
- **p. 2 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** Below, we describe the observation space, action space, architecture of the policy, and training methods.
...
- **p. 4 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** The model parameters are randomized per episode to simulate a range of robot models and also provide a wide range of state space that the ...
- **p. 2 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** Control Policy Design Observation Space.
- **p. 4 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** Parameters Range Unit Simulation Model Joint Damping [0.5, 2.5] % Mass [-0.25, 0.25] % Center of Mass Location [-0.01, 0.01] m Passive Spring Stiffness [-500, ...
- **p. 4 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** However, due to the nature of RL, this term only acts as a soft constraint that the robot may violate in favor of not falling ...
- **p. 2 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** The main control objective is to follow speed and heading commands while maintaining balance over possibly challenging terrains.

## Design Rationale

- **p. 2 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** The relative encoding means that the heights vary as the robot moves up and down during its gait, but enables us to avoid using global ...
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** The key contribution of our work is the sim-to-real pipeline and the system integration for these components, which allows the overall locomotion controller to transfer ...
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** The proposed approach enables bipedal robot Cassie traversing over challenging terrains, including random high blocks, stairs, 0.5m step up (∼60% leg length), with speed up ...
...
- **p. 4 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** However, due to the nature of RL, this term only acts as a soft constraint that the robot may violate in favor of not falling ...
- **p. 2 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** The main control objective is to follow speed and heading commands while maintaining balance over possibly challenging terrains.
- **p. 3 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** First, we modified the PPO loss function to include a mirror loss over robot propriocepive inputs as well as visual inputs.
- **p. 3 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** The conditions correspond to undesirable robot behavior and implicitly punish the robot by causing it to not receive future rewards.
- **p. 4 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** This reward helps prevent fast swing leg motions, which we found could arise during training on difficult terrain.
...
- **p. 3 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** The input to the vision-based modulator includes all of the available observations, including the heightmap, in addition to the action produced by the blind policy.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** In particular, our architecture is composed of two primary learned components: 1) a control policy whose input is proprioceptive information and a heightmap of a ...
- **p. 2 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** Below, we describe the observation space, action space, architecture of the policy, and training methods.
- **p. 3 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** We use a neural network to represent the policy for mapping observation sequences to actions.
- **p. 2 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** The policy input includes: 1) proprioceptive information containing the orientation (in quaternion) and angular velocity of the floating base, and position and velocity for all ...
- **p. 4 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** Parameters Range Unit Simulation Model Joint Damping [0.5, 2.5] % Mass [-0.25, 0.25] % Center of Mass Location [-0.01, 0.01] m Passive Spring Stiffness [-500, ...
- **p. 4 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** The visual inputs are randomized to simulate noise from the heightmap estimator and prevent the policy from overfitting to the exact simulated heightmaps.
...
- **p. 2 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** Below, we describe the observation space, action space, architecture of the policy, and training methods.
- **p. 5 / VI. SIMULATION RESULTS - extractive PDF cue:** LSTM has implicit history, Transformer has a fixed window size of 0.6 seconds to allow reasonable inference during runtime, and MLP does not have history.
...
- **p. 5 / VI. SIMULATION RESULTS - extractive PDF cue:** Policy Performance We use the trained policy, along with a number of different policy setups, to evaluate the performance in simulation for the ablation study.
- **p. 5 / VI. SIMULATION RESULTS - extractive PDF cue:** We also implemented other architectures to use for ablations, including an MLP model and a transformer-based model, and they all have robot states and depth ...
- **p. 6 / VI. SIMULATION RESULTS - extractive PDF cue:** Ablation study on policy with different heightmap predictor architectures.
- **p. 6 / VI. SIMULATION RESULTS - extractive PDF cue:** Each ablation study uses data collected from a range of terrains defined in Table I.
