...
- **p. 1 / Abstract - extractive PDF cue:** Reinforcement learning (RL) for bipedal locomotion has recently demonstrated robust gaits over moderate terrains using only proprioceptive sensing.
- **p. 1 / Abstract - extractive PDF cue:** However, such blind controllers will fail in environments where robots must anticipate and adapt to local terrain, which requires visual perception.
- **p. 1 / Abstract - extractive PDF cue:** In this paper, we propose a fully-learned system that allows bipedal robots to react to local terrain while maintaining commanded travel speed and direction.
- **p. 1 / Abstract - extractive PDF cue:** Our approach first trains a controller in simulation using a heightmap expressed in the robot's local frame.
- **p. 1 / Abstract - extractive PDF cue:** Next, data is collected in simulation to train a heightmap predictor, whose input is the history of depth images and robot states.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** Robustly achieving such an integration of vision and locomotion remains an open problem for bipedal robots.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** For this purpose, bipedal robots have the potential to match human locomotion capabilities, but currently are far inferior.
...
| Observation / input | The input to the vision-based modulator includes all of the available observations, including the heightmap, in addition to the action produced by ... | proprioception, terrain/perception observation과 velocity command | exact sensor/frame/preprocessing from PDF |
...
- **p. 3 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** 3: Policy consists of a blind policy and a vision-based modulator. cos (2π(ϕt + γi t)).
- **p. 3 / IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY - extractive PDF cue:** This allows the policy to gain some experience on easier terrains, which is useful early in learning, but focuses most of the learning effort on ...
...
locomotion writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 3 (IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY), p. 1 (I. INTRODUCTION), p. 2 (IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY), p. 3 (IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (I. INTRODUCTION), p. 1 (I. INTRODUCTION), interface p. 3 (IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY), p. 1 (I. INTRODUCTION), p. 2 (IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY), p. 3 (IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY), objective p. 2 (IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY), p. 3 (IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY), p. 3 (IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY), p. 4 (IV. LEARNING A TERRAIN-AWARE LOCOMOTION POLICY).
