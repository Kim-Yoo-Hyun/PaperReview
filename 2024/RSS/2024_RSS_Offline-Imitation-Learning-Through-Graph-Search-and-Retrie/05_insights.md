...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: To overcome this problem, we propose GSR, a simple yet effective algorithm that learns from suboptimal demonstrations through Graph Search and Retrieval.
- 출발 문제 단서: Existing works typically use offline deep reinforcement learning (RL) to solve this challenge, but in practice these algorithms are unstable and fragile due to the deadly triad issue.
- 주장된 효과 단서: GSR can achieve a 10% to 30% higher success rate and over 30% higher proficiency compared to baselines.
...
- Result cue: GSR can achieve a 10% to 30% higher success rate and over 30% higher proficiency compared to baselines.
