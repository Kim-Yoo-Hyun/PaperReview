...
- Problem cue: With HumanoidBench, we provide the robotics community with a platform to identify the challenges arising when solving diverse tasks with humanoid robots, facilitating prompt verification of algorithms and ...
- Method cue: To accelerate the progress of research for humanoid robots, we present the first-of-its-kind humanoid robot benchmark, HumanoidBench, with a diverse set of locomotion and manipulation tasks.
- Result/evaluation cue: Our findings reveal that state-of-the-art reinforcement learning algorithms struggle with most tasks, whereas a hierarchical learning baseline achieves superior performance when supported by robust low-level policies, such as ...

위 cue는 기존 official abstract 또는 local text extraction에서 보존한 것이다. 수치·조건·인과적 해석은 full-text 정독 전까지 `UNVERIFIED`다.

## Dependency Position

`ANYmal Parkour: Learning Agile Navigation for Quadrupedal Robots → HumanoidBench: Simulated Humanoid Benchmark for Whole-Body Locomotion and Manipulation → OmniH2O: Universal and Dexterous Human-to-Humanoid Whole-Body Teleoperation and Learning`
