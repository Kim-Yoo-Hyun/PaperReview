...
- **Target problem:** This poses several challenges, as outlined in prior work [108], stemming from variations in kinematic configuration, control scheme, camera viewpoint, and end-effector morphology. (p. 2, I. INTRODUCTION).
- **Formulation-changing contribution:** To summarize, our key contributions are: (p. 2, I. INTRODUCTION).
- **Assumption/failure evidence:** Less robust source policies leave little room for error, while more robust ones tend to retry even if the target robot fails to grasp the object the first time. (p. 5, IV. STATE-BASED TRANSFER EXPERIMENTS).
