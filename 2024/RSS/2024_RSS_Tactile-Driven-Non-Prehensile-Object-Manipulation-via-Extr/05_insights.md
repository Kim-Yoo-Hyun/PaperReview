...
- Problem cue: —In this paper, we consider the problem of nonprehensile manipulation using grasped objects.
- Method cue: Here, we present an algorithmic approach for non-prehensile manipulation leveraging a gripper with highly compliant and high-resolution tactile sensors.
- Result/evaluation cue: We evaluate 4 variants of controllers that attempt to realize these plans and demonstrate a number of complex skills including non-prehensile planar sliding and pivoting on a variety ...

위 cue는 기존 official abstract 또는 local text extraction에서 보존한 것이다. 수치·조건·인과적 해석은 full-text 정독 전까지 `UNVERIFIED`다.

## Dependency Position

`Global Planning for Contact-Rich Manipulation via Local Smoothing of Quasi-Dynamic Contact Models → Tactile-Driven Non-Prehensile Object Manipulation via Extrinsic Contact Mode Control → RoboPack: Learning Tactile-Informed Dynamics Models for Dense Packing`
