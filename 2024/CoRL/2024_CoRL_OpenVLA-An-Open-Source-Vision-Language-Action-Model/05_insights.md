...
- Result/evaluation cue: As a product of the added data diversity and new model components, OpenVLA demonstrates strong results for generalist manipulation, outperforming closed models such as RT-2-X (55B) by 16.5% ...

위 cue는 기존 official abstract 또는 local text extraction에서 보존한 것이다. 수치·조건·인과적 해석은 full-text 정독 전까지 `UNVERIFIED`다.

## Dependency Position

`Octo: An Open-Source Generalist Robot Policy → OpenVLA: An Open-Source Vision-Language-Action Model → π0: A Vision-Language-Action Flow Model for General Robot Control`
