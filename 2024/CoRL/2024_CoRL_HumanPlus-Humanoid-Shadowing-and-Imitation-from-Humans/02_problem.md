...
| Observation / input | Using state-of-the-art human body and hand pose estimation algorithms [58, 81], we can estimate real-time human motion and retarget it to humanoid ... | proprioception, reference pose/motion, visual or language command | exact sensor/frame/preprocessing from PDF |
...
PDF contribution framing (p. 2 (1. Introduction), p. 3 (1. Introduction), p. 3 (1. Introduction), p. 4 (4. Human Body and Hand Data), p. 5 (5. Shadowing of Human Motion)): In this paper, we present a full-stack system for humanoids to learn motion and autonomous skills from human data.
...
humanoid writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 2 (1. Introduction), p. 5 (5. Shadowing of Human Motion), p. 2 (1. Introduction), p. 3 (1. Introduction). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (1. Introduction), p. 2 (1. Introduction), p. 3 (1. Introduction), interface p. 2 (1. Introduction), p. 5 (5. Shadowing of Human Motion), p. 2 (1. Introduction), p. 3 (1. Introduction), objective p. 7 (6. Imitation of Human Skills).
