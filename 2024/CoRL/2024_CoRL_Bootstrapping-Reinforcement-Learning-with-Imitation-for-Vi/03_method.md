...
- **p. 3 / 3 Methodology - extractive body cue:** In stage II, we use IL to learn a student distillation policy using visual inputs.
- **p. 8 / 3 Methodology - extractive body cue:** In this work, we introduced a novel approach by fusing the strengths of Reinforcement Learning (RL) and Imitation Learning (IL) for vision-based agile quadrotor flight, ...
- **p. 3 / 3 Methodology - extractive body cue:** The drone perceives the environment solely through a single RGB camera, and the learned policy network utilizes egocentric vision input op to output Collective Thrust ...
- **p. 3 / 3 Methodology - extractive body cue:** The drone racing task can be formulated as an optimization problem where the objective is to minimize the time required to navigate through a predefined ...
...
- **p. 3 / 3 Methodology - extractive body cue:** The drone racing task can be formulated as an optimization problem where the objective is to minimize the time required to navigate through a predefined ...
- **p. 4 / 3 Methodology - extractive body cue:** The reward at time t, denoted as rt, is defined as the sum of various components rt = rprog t + rperc t + ract ...
- **p. 5 / 3 Methodology - extractive body cue:** Once the policy achieves high-reward action sequences, the policy update rate also increases.
...
- **p. 4 / 3 Methodology - extractive body cue:** The RL policy training rewards are adjusted based on [38].
- **p. 7 / 3 Methodology - extractive body cue:** It is evident that at 60%, the collected rewards achieve a peak (>100) in the curve representing the best performance.
...
- **p. 4 / 3 Methodology - extractive body cue:** In the case of BC, the state-based teacher policy is executed for a fixed number of steps, generating a dataset that encompasses corresponding perceptual observations ...
- **p. 3 / 3 Methodology - extractive body cue:** The drone perceives the environment solely through a single RGB camera, and the learned policy network utilizes egocentric vision input op to output Collective Thrust ...
- **p. 8 / 3 Methodology - extractive body cue:** In that work, the policy observation was an explicit state computed from images and an inertial sensor.
- **p. 4 / 3 Methodology - extractive body cue:** Phase I: State-based Teacher Policy Training The teacher policy πteacher processes state observations s = h p, ˜R, v, ω, i, d i , where ...
- **p. 1 / 1 Introduction - extractive body cue:** The intrinsic high dimensionality of visual input makes the policy exploration and learning process more inefficient than using low-dimensional input, such as robot states.
- **p. 12 / A.3 Training Configurations - extractive body cue:** For state-based teacher training, we employ a policy network consisting of a two-layer MLP, each layer containing 256 neurons, with a final layer outputting a ...
- **p. 2 / 1 Introduction - extractive body cue:** The RL policy for state-of-the-art autonomous drone racing [7], which outperformed worldchampion pilots, still relies on explicit state estimates, including position, velocity, and orientation.
...
- **p. 7 / 3 Methodology - extractive body cue:** Training Effectiveness with different RL configurations To demonstrate the effectiveness of our visuomotor policy learning approach, we ablate the training performance of our approach with ...
- **p. 3 / 3 Methodology - extractive body cue:** The drone perceives the environment solely through a single RGB camera, and the learned policy network utilizes egocentric vision input op to output Collective Thrust ...
- **p. 13 / A.3 Training Configurations - extractive body cue:** We incorporate a linear decay in the learning rate, starting at 1e-3 and decreasing to 1e-5 at 50 epochs, remaining unchanged for the remainder of ...
- **p. 12 / A.2 Reward Formulations for RL Trainings - extractive body cue:** In our experiments, we employ identical hyperparameters for both state-based teacher training and vision-based RL fine-tuning to ensure a fair comparison.
...
- **p. 7 / Figure/Table caption - extractive body cue:** Figure 5: Left: Reward comparison between our approach and the other RL configurations. Ours is the only approach that is able to learn to perform ...
- **p. 15 / Figure/Table caption - extractive body cue:** Table 6: Ablation study on history length of the policy observations using raw pixels. We could clearly find out by using more history observations, that ...
...
- **p. 6 / 3 Methodology - extractive body cue:** This limitation arises because the student policy is trained only on the explicit actions of the expert, without understanding the underlying context that the expert ...
- **p. 7 / 3 Methodology - extractive body cue:** This once again underscores the difficulty of RL exploration in high-dimensional time series without bootstrapping.
- **p. 15 / A.8 Unobservable States Illustration - extractive body cue:** Approach Slow IL policy Our Finetuned Policy Champion-level Policy LT [s] SR [%] LT [s] SR [%] LT [s] SR [%] Nominal Simulation 9.53 39 ...
- **p. 13 / A.5 Ablation study on Asymmetric Critic Formulation - extractive body cue:** In stage III of our approach, the visuomotor policy undergoes fine-tuning using an asymmetric critic setup.
...
- **Evidence anchors reviewed:** method p. 4 (3 Methodology), p. 4 (3 Methodology), p. 7 (3 Methodology), p. 7 (3 Methodology), p. 3 (3 Methodology), p. 8 (3 Methodology), objective p. 3 (3 Methodology), p. 4 (3 Methodology), p. 5 (3 Methodology), p. 13 (A.3 Training Configurations), p. 4 (3 Methodology), p. 7 (3 Methodology), temporal p. 4 (3 Methodology), p. 4 (3 Methodology), p. 5 (3 Methodology), p. 7 (3 Methodology), p. 7 (3 Methodology), p. 2 (1 Introduction).
