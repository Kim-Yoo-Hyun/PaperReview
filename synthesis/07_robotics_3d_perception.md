...
| [RLBench](../2020/RA-L/2020_RA-L_RLBench-The-Robot-Learning-Benchmark-and-Learning-Environm/01_overview.md) | 2020 / RA-L | benchmark; simulator | arm; RGB-D/language; simulation; manipulation benchmark | standardized multi-task RGB-D manipulation substrate | simulated scene/task state; policy action; task success objective; per-step update | common task substrate for 3D representation and policy comparison | simulated tabletop; data/split=task/object splits; metric=task success; real=no/trials=UNVERIFIED | `CURATION-SEED`; overview + queue | simulator/calibration/contact gap, visual bias | benchmark과 representation contribution을 분리 |
