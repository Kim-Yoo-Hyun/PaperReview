...
Point-cloud representation, feed-forward geometry, SLAM, semantic mapping, 3D scene graphs, active perception, articulation flow와 3D-aware VLA를 downstream robot behavior 관점에서 비교한다.

## Reading Path

PointNet/3DGS/DUSt3R/VGGT → DROID-SLAM → ConceptFusion/VLMaps/Open3DSG → Where2Act/FlowBot3D/RVT → PointVLA/Any3D-VLA/ActiveVLA.
