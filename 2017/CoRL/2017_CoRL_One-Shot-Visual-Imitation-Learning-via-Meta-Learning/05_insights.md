...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: Our proposed architecture consists of three modules: the demonstration network, the context network, and the manipulation network.
- 출발 문제 단서: To accomplish this, we must solve two broad problems.
- 주장된 효과 단서: Our experiments show that the use of soft attention allows the model to generalize to conditions and tasks unseen in the training data.
...
- Result cue: Our experiments show that the use of soft attention allows the model to generalize to conditions and tasks unseen in the training data.
