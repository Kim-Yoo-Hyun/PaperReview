...
- **p. 2 / 1 Introduction - extractive PDF cue:** (a) Traditional Imitation Learning Task A e.g. stack blocks into towers of height 3 Many demonstrations Imitation Learning Algorithm Policy for task A action Environment ...
- **p. 6 / B C - extractive PDF cue:** We then apply standard soft attention over the current state to produce fixed-dimensional vectors, where the memory content only consists of positions of each block, ...
- **p. 1 / Abstract - extractive PDF cue:** A neural net is trained such that when it takes as input the first demonstration demonstration and a state sampled from the second demonstration, it ...
- **p. 2 / 1 Introduction - extractive PDF cue:** When conditioned on both the first demonstration and this observation, the network is trained to output the corresponding action. systems are not yet at a ...
- **p. 6 / B C - extractive PDF cue:** Attention over demonstration: The context network starts by computing a query vector as a function of the current state, which is then used to attend ...
- **p. 3 / 1 Introduction - extractive PDF cue:** To make this model work, we made essential use of soft attention [6] for processing both the (potentially long) sequence of states and action that ...
- **p. 5 / B C - extractive PDF cue:** D E F G H I J Attention over Current State Context Network Demonstration Network Manipulation Network Context Embedding Figure 2: Illustration of the network ...
- **p. 2 / 1 Introduction - extractive PDF cue:** 1, where the objective is to maximize the expected performance of the learned policy when faced with a new, previously unseen, task, and having received ...
...
- **p. 2 / 1 Introduction - extractive PDF cue:** When conditioned on both the first demonstration and this observation, the network is trained to output the corresponding action. systems are not yet at a ...
- **p. 1 / Abstract - extractive PDF cue:** A neural net is trained such that when it takes as input the first demonstration demonstration and a state sampled from the second demonstration, it ...
- **p. 2 / 1 Introduction - extractive PDF cue:** We note that any pair of demonstrations for the same task provides a supervised training example for the neural net policy, where one demonstration is ...
- **p. 5 / B C - extractive PDF cue:** Therefore, we need an operation that can map variable-dimensional inputs to outputs with comparable dimensions.
- **p. 3 / 1 Introduction - extractive PDF cue:** To make this model work, we made essential use of soft attention [6] for processing both the (potentially long) sequence of states and action that ...
- **p. 5 / B C - extractive PDF cue:** Soft attention is a natural operation which maps variable-dimensional inputs to fixed-dimensional outputs.
...
- **p. 1 / Abstract - extractive PDF cue:** A neural net is trained such that when it takes as input the first demonstration demonstration and a state sampled from the second demonstration, it ...
- **p. 2 / 1 Introduction - extractive PDF cue:** When conditioned on both the first demonstration and this observation, the network is trained to output the corresponding action. systems are not yet at a ...
- **p. 1 / Abstract - extractive PDF cue:** At training time, our algorithm is presented with pairs of demonstrations for a subset of all tasks.
...
- **p. 8 / 5 Experiments - extractive PDF cue:** We plan to extend the framework to demonstrations in the form of image data, which will allow more end-to-end learning without requiring a separate perception ...
- **p. 8 / 5 Experiments - extractive PDF cue:** Another interesting finding was that training with behavioral cloning has the same level of performance as training with DAGGER, which suggests that the entire training ...
- **p. 6 / 5 Experiments - extractive PDF cue:** 1In principle, one can replace this module with an RNN module.
...
- **PDF anchors reviewed:** method p. 2 (1 Introduction), p. 6 (B C), p. 1 (Abstract), p. 2 (1 Introduction), p. 6 (B C), p. 3 (1 Introduction), objective p. 2 (1 Introduction), p. 2 (1 Introduction), temporal p. 4 (2 Related Work), p. 7 (5 Experiments), p. 8 (5 Experiments), p. 8 (5 Experiments), p. 4 (2 Related Work), p. 6 (B C).
