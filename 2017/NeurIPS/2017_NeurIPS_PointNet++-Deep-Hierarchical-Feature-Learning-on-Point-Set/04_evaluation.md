...
- **p. 5 / 4 Experiments - extractive PDF cue:** We use five fold cross validation to acquire classification accuracy on this dataset. • ScanNet: 1513 scanned and reconstructed indoor scenes.
- **p. 5 / 4 Experiments - extractive PDF cue:** Datasets We evaluate on four datasets ranging from 2D objects (MNIST [11]), 3D objects (ModelNet40 [31] rigid object, SHREC15 [12] non-rigid object) to real 3D ...
- **p. 5 / 4 Experiments - extractive PDF cue:** In MNIST, we see a relative 60.8% and 34.6% error rate reduction 1See supplementary for more details on network architecture and experiment preparation.
- **p. 6 / Figure/Table caption - extractive PDF cue:** Figure 4: Left: Point cloud with random point dropout. Right: Curve showing advantage of our density adaptive strategy in dealing with non-uniform density. DP means ...
- **p. 5 / 4 Experiments - extractive PDF cue:** Object classification is evaluated by accuracy.
- **p. 6 / Figure/Table caption - extractive PDF cue:** Figure 5: Scannet labeling accuracy. To validate that our approach is suitable for large scale point cloud analysis, we also evaluate on semantic scene labeling ...
...
- **p. 2 / Figure/Table caption - extractive PDF cue:** Figure 1: Visualization of a scan captured from a Structure Sensor (left: RGB; right: point cloud). One issue that still remains is how to generate ...
...
- **p. 4 / Figure/Table caption - extractive PDF cue:** Figure 3: (a) Multi-scale grouping (MSG); (b) Multi- resolution grouping (MRG). As discussed earlier, it is common that a point set comes with non- uniform ...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Table 1: MNIST digit classification.
- **p. 6 / Figure/Table caption - extractive PDF cue:** Table 2: ModelNet40 shape classification. 1024 points 512 points 256 points 128 points
- **p. 6 / Figure/Table caption - extractive PDF cue:** Figure 4: Left: Point cloud with random point dropout. Right: Curve showing advantage of our density adaptive strategy in dealing with non-uniform density. DP means ...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Figure 5: Scannet labeling accuracy. To validate that our approach is suitable for large scale point cloud analysis, we also evaluate on semantic scene labeling ...
...
- **p. 5 / 4 Experiments - extractive PDF cue:** We follow the experiment setting in [5] and use 1201 scenes for training, 312 scenes for test.
- **p. 5 / 4 Experiments - extractive PDF cue:** 4.1 Point Set Classification in Euclidean Metric Space We evaluate our network on classifying point clouds sampled from both 2D (MNIST) and 3D (ModleNet40) Euclidean ...
- **p. 5 / 4 Experiments - extractive PDF cue:** In default we use 512 points for MNIST and 1024 points for ModelNet40.
- **p. 5 / 4 Experiments - extractive PDF cue:** In last row (ours normal) in Table 2, we use face normals as additional point features, where we also use more points (N = 5000) ...
- **p. 5 / 3 Method - extractive PDF cue:** 3.4 Point Feature Propagation for Set Segmentation In set abstraction layer, the original point set is subsampled.
...
- **PDF anchors reviewed:** datasets p. 5 (4 Experiments), p. 5 (4 Experiments), metrics p. 5 (4 Experiments), p. 6 (Figure/Table caption), p. 5 (4 Experiments), p. 6 (Figure/Table caption), p. 13 (Figure/Table caption), p. 2 (Figure/Table caption), baselines p. 6 (Figure/Table caption), p. 14 (Figure/Table caption), results p. 5 (4 Experiments), p. 6 (Figure/Table caption), p. 3 (Figure/Table caption), p. 14 (Figure/Table caption), p. 5 (4 Experiments), p. 6 (Figure/Table caption).
