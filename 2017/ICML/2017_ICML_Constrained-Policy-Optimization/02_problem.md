...
| Observation / input | Constrained Policy Optimization Algorithm 1 Constrained Policy Optimization Input: Initial policy π0 ∈Πθ tolerance α for k = 0, 1, 2, ... ... | observation, uncertainty/risk estimate와 task command | exact sensor/frame/preprocessing from PDF |
...
PDF contribution framing (p. 1 (1. Introduction), p. 5 (5.3. Trust Region Optimization for Constrained MDPs), p. 5 (5.3. Trust Region Optimization for Constrained MDPs), p. 1 (1. Introduction), p. 2 (1. Introduction)): In this work, we propose the first such algorithm, allowing applications to constrained deep RL.
