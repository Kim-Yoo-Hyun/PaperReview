...
- **p. 1 / Abstract - extractive PDF cue:** This paper focuses on semantic scene completion, a task for producing a complete 3D voxel representation of volumetric occupancy and semantic labels for a scene ...
- **p. 1 / Abstract - extractive PDF cue:** Previous work has considered scene completion and semantic labeling of depth maps separately.
- **p. 1 / Abstract - extractive PDF cue:** However, we observe that these two problems are tightly intertwined.
- **p. 1 / Abstract - extractive PDF cue:** To leverage the coupled nature of these two tasks, we introduce the semantic scene completion network (SSCNet), an end-to-end 3D convolutional network that takes a ...
- **p. 1 / Abstract - extractive PDF cue:** Our network uses a dilation-based 3D context module to efficiently expand the receptive field and enable 3D context learning.
- **p. 2 / 1. Introduction - extractive PDF cue:** However, if we consider the context due to surrounding objects, such as the table and floor, the problem is much easier.
- **p. 2 / 1. Introduction - extractive PDF cue:** First, how do we effectively capture contextual information from 3D volumetric data, where the signal is sparse and lacks high frequency detail?
...
| Observation / input | (a) Input single-view depth map (b) Visible surface from the depth map; color is for visualization only. | RGB-D, image set, point cloud, depth와 camera pose | exact sensor/frame/preprocessing from PDF |
...
- **p. 2 / 1. Introduction - extractive PDF cue:** First, how do we effectively capture contextual information from 3D volumetric data, where the signal is sparse and lacks high frequency detail?
- **p. 1 / 1. Introduction - extractive PDF cue:** Prior work is limited to address only part of this problem as shown in FigFigure 1.
- **p. 1 / 1. Introduction - extractive PDF cue:** Therefore, the two problems of predicting voxel occupancy and identifying object semantics are strongly coupled.

## What the Paper Changes

PDF contribution framing (p. 2 (1. Introduction), p. 1 (1. Introduction), p. 2 (1. Introduction), p. 5 (4. Synthesizing training data), p. 5 (4. Synthesizing training data)): To provide the training data for our network, we introduce SUNCG, a manually created large-scale dataset of synthetic 3D scenes with dense occupancy and semantic annotations.
...
- **p. 5 / 4. Synthesizing training data - extractive PDF cue:** In this paper, we present a new large-scale synthetic 3D scene dataset, from which we obtain a large amount of training data with synthetically rendered ...
- **p. 5 / 4. Synthesizing training data - extractive PDF cue:** During the task, we show a set of top view renderings of each floor and ask turkers to vote whether this is a valid apartment ...
...
3d_perception writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 1 (1. Introduction), p. 1 (1. Introduction), p. 5 (4.2. Synthetic depth map generation), p. 4 (3.2. Network architecture). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (1. Introduction), p. 2 (1. Introduction), p. 1 (1. Introduction), p. 1 (1. Introduction), interface p. 1 (1. Introduction), p. 1 (1. Introduction), p. 5 (4.2. Synthetic depth map generation), p. 4 (3.2. Network architecture), objective p. 5 (3.2. Network architecture), p. 4 (3.2. Network architecture), p. 4 (3.2. Network architecture).
