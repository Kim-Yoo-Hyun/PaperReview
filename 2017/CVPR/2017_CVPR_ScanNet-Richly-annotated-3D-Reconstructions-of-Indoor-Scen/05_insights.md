...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: To address this issue, we introduce ScanNet, an RGB-D video dataset containing 2.5M views in 1513 scenes annotated with 3D camera poses, surface reconstructions, and semantic segmentations.
- 출발 문제 단서: Thus, existing work on 3D datasets often fall back to polygon or bounding box annotations on 2.5D RGB-D images , rather than directly annotating in 3D.
- 주장된 효과 단서: We show that using this data helps achieve state-of-the-art performance on several 3D scene understanding tasks, including 3D object classification, semantic voxel labeling, and CAD model retrieval.
...
- Result cue: We show that using this data helps achieve state-of-the-art performance on several 3D scene understanding tasks, including 3D object classification, semantic voxel labeling, and CAD model retrieval.
