...
| Observation / input | Note that inputs computed by solving this QP are, in general, not equal to those computed by thresholding the output of the ... | proprioception, reference pose/motion, visual or language command | exact sensor/frame/preprocessing from PDF body |
...
PDF body contribution framing (p. 4 (3.1 Footstep planning as a mixed-integer convex), p. 1 (1 Introduction), p. 1 (1 Introduction), p. 2 (1 Introduction), p. 2 (1 Introduction)): Unfortunately, the set of safe terrain is unlikely to be convex or even connected: in an environment as simple as a staircase, the safe terrain consists of the top surface ...
