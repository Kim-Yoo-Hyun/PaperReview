...
- **p. 5 / 3.1.1 Convex decomposition - extractive PDF cue:** We use the polytope representation in our planner, since it is always of larger volume than the (inscribed) ellipsoid and can be represented as a ...
- **p. 8 / 3.2 Dynamic motion planning - extractive PDF cue:** As will be discussed below, we use a redundant multiple-force description of the total wrench acting on a rigid body because it permits the use ...
- **p. 11 / 4.4 Additional costs and constraints - extractive PDF cue:** For this we used a simple logic to determine what contact force variables should be included in the 123
- **p. 8 / 3.2 Dynamic motion planning - extractive PDF cue:** By simultaneously optimizing the states and inputs along the trajectory, transcription methods avoid numerical issues that can be present in shooting methods (in which small ...
- **p. 10 / 4 Controller design - extractive PDF cue:** In the following sections, we write the problem in a general form first, then describe particular implementations used in conjunction with the planners described previously.
- **p. 3 / 3.1 Footstep planning as a mixed-integer convex - extractive PDF cue:** If the objective function and constraints are convex, then such an optimization can be solved extremely efficiently (Boyd and Vandenberghe 2004).
- **p. 3 / 3 Motion planning - extractive PDF cue:** Then we solve an optimization problem that assigns contacts to these regions in a way that minimizes cost while respecting kinematic and dynamic constraints.
- **p. 6 / 3.1.3 Determining the number of footsteps - extractive PDF cue:** Adding a negative cost on each ρ j to the objective in our optimization allows us to reward the planner for taking fewer footsteps without ...

## Design Rationale

- **p. 4 / 3.1 Footstep planning as a mixed-integer convex - extractive PDF cue:** Unfortunately, the set of safe terrain is unlikely to be convex or even connected: in an environment as simple as a staircase, the safe terrain ...
...
- **p. 3 / 3 Motion planning - extractive PDF cue:** Then we solve an optimization problem that assigns contacts to these regions in a way that minimizes cost while respecting kinematic and dynamic constraints.
- **p. 6 / 3.1.3 Determining the number of footsteps - extractive PDF cue:** Adding a negative cost on each ρ j to the objective in our optimization allows us to reward the planner for taking fewer footsteps without ...
- **p. 10 / 4.1 General formulation - extractive PDF cue:** Next we define a quadratic cost function, g(x(t), u(t), t) = ¯xT (t)Q¯x(t) + ¯uT (t)R¯u(t), (15) and solve the constrained minimization problem, minimize u(t)
- **p. 10 / 4.1 General formulation - extractive PDF cue:** (16) By the Hamilton-Jacobi-Bellman (HJB) equation (Bertsekas 1995), we know that the optimal controller satisfies ¯u∗(t) = arg min ¯u ℓ(¯x, ¯u, t), (17) ℓ(¯x, ...
- **p. 13 / 4.5 Efficient QP solver - extractive PDF cue:** (2008) consider the MPC problems where the cost function and dynamic constraints are the same at each time step; i.e., the QPs solved at iteration ...
- **p. 3 / 3.1 Footstep planning as a mixed-integer convex - extractive PDF cue:** If the objective function and constraints are convex, then such an optimization can be solved extremely efficiently (Boyd and Vandenberghe 2004).
...
- **p. 10 / 4.1 General formulation - extractive PDF cue:** Note that inputs computed by solving this QP are, in general, not equal to those computed by thresholding the output of the closed-form LQR policy.
- **p. 11 / 4.4 Additional costs and constraints - extractive PDF cue:** Given the current robot state, q, v, we can compute the equations of motion, H(q)˙v + C(q, v) = Bτ + JT λ, (25) H ...
- **p. 14 / 5.1 Requirements and approach - extractive PDF cue:** LQR solutions can be recomputed online (typically in a separate thread) using the current state of the robot to reduce the systems sensitivity to deviations ...
- **p. 2 / 1 Introduction - extractive PDF cue:** Inputs to the controller are computed by a low-drift state estimator that fuses kinematic, inertial, and LIDAR information (Sect.
- **p. 2 / 1 Introduction - extractive PDF cue:** We describe an efficient active-set algorithm capable of finding solutions in less than 1 millisecond for Atlas (68 states and 28 inputs).
- **p. 8 / 3.2 Dynamic motion planning - extractive PDF cue:** Transcription methods, on the other hand, include a finite set of states along the trajectory as decision variables and incorporate the dynamics of the system ...
- **p. 8 / 3.2 Dynamic motion planning - extractive PDF cue:** By simultaneously optimizing the states and inputs along the trajectory, transcription methods avoid numerical issues that can be present in shooting methods (in which small ...
