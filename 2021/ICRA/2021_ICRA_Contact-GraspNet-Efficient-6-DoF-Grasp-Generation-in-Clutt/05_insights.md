...
- Problem cue: Despite recent progress in full 6-DoF grasp learning, existing approaches often consist of complex sequential pipelines that possess several potential failure points and run-times unsuitable for closed-loop grasping.
- Method cue: Therefore, we propose an end-to-end network that efficiently generates a recording of a scene.
- Result/evaluation cue: In a robotic grasping study of unseen objects in structured clutter we achieve over 90% success rate, cutting the failure rate in half compared to a recent state-of-the-art ...

위 cue는 기존 official abstract 또는 local text extraction에서 보존한 것이다. 수치·조건·인과적 해석은 full-text 정독 전까지 `UNVERIFIED`다.

## Dependency Position

`GraspNet-1Billion: A Large-Scale Benchmark for General Object Grasping → Contact-GraspNet: Efficient 6-DoF Grasp Generation in Cluttered Scenes → Factory: Fast Contact for Robotic Assembly`
