...
- **p. 3 / 2 BACKGROUND - extractive body cue:** 3.1 PERTURBING DATA WITH SDES Our goal is to construct a diffusion process txptquT t"0 indexed by a continuous time variable t P r0, Ts, ...
- **p. 7 / 2 BACKGROUND - extractive body cue:** As an example, we report negative log-likelihoods (NLLs) measured in bits/dim on the CIFAR-10 dataset in Table 2.
- **p. 9 / 2 BACKGROUND - extractive body cue:** Since the forward SDE is tractable, we can easily create training data pxptq, yq for the time-dependent classifier by first sampling pxp0q, yq from a ...
...
- **p. 3 / 2 BACKGROUND - extractive body cue:** (3) is also a weighted sum of denoising score matching objectives, which implies that the optimal model, sθ˚p˜x, iq, matches the score of the perturbed ...
- **p. 4 / 2 BACKGROUND - extractive body cue:** (7) uses denoising score matching, but other score matching objectives, such as sliced 4
...
- **p. 2 / Figure/Table caption - extractive body cue:** Figure 1: Solving a reverse- time SDE yields a score-based generative model. Transform- ing data to a simple noise dis- tribution can be accomplished with ...
- **p. 4 / Figure/Table caption - extractive body cue:** Figure 2: Overview of score-based generative modeling through SDEs. We can map data to a noise distribution (the prior) with an SDE (Section 3.1), and ...
- **p. 6 / Figure/Table caption - extractive body cue:** Table 1: Comparing different reverse-time SDE solvers on CIFAR-10. Shaded regions are obtained with the same computation (number of score function evaluations). Mean and standard ...
- **p. 7 / Figure/Table caption - extractive body cue:** Table 2: NLLs and FIDs (ODE) on CIFAR-10.
- **p. 8 / Figure/Table caption - extractive body cue:** Figure 3: Probability flow ODE enables fast sampling with adaptive stepsizes as the numerical precision is varied (left), and reduces the number of score function ...
- **p. 9 / Figure/Table caption - extractive body cue:** Figure 4: Left: Class-conditional samples on 32 ˆ 32 CIFAR-10. Top four rows are automobiles and bottom four rows are horses. Right: Inpainting (top two ...
- **p. 16 / Figure/Table caption - extractive body cue:** Figure 5: Discrete-time perturbation kernels and our continuous generalizations match each other almost exactly. (a) compares the variance of perturbation kernels for SMLD and VE ...
- **p. 19 / Figure/Table caption - extractive body cue:** Fig. 7. In Fig. 8, we show the dimension-wise differences and correlation coefficients between latent encodings on a total of 16 CIFAR-10 images. Our results ...
...
| Figure 4: Left: Class-conditional samples on 32 ˆ 32 CIFAR-10. Top four rows are automobiles and bottom four rows are horses. Right: Inpainting (top ... | definition/direction/unit from same section | p. 9 (Figure/Table caption) |
...
- **p. 3 / 2 BACKGROUND - extractive body cue:** (3), namely σ2 i and p1´αiq, are related to corresponding perturbation kernels in the same functional form: σ2 i 91{Er∥∇x log pσip˜x / xq∥2 2s ...
- **p. 5 / 2 BACKGROUND - extractive body cue:** In the limit of N Ñ 8, tσiuN i"1 becomes a function σptq, zi becomes zptq, and the Markov chain txiuN i"1 becomes a continuous ...
- **p. 6 / 2 BACKGROUND - extractive body cue:** Mean and standard deviation are reported over five sampling runs. "P1000" or "P2000": predictor-only samplers using 1000 or 2000 steps. "C2000": corrector-only samplers using 2000 ...
- **p. 7 / 2 BACKGROUND - extractive body cue:** (deep, sub-VP) 2.99 2.92 Table 3: CIFAR-10 sample quality.
- **p. 9 / 2 BACKGROUND - extractive body cue:** We provide class-conditional CIFAR-10 samples in Fig.
