...
- **p. 1 / Abstract - extractive PDF cue:** In this paper, we question if self-supervised learning provides new properties to Vision Transformer (ViT) [19] that stand out compared to convolutional networks (convnets).
- **p. 1 / Abstract - extractive PDF cue:** Beyond the fact that adapting self-supervised methods to this architecture works particularly well, we make the following observations: first, self-supervised ViT features contain explicit information ...
- **p. 1 / Abstract - extractive PDF cue:** Second, these features are also excellent k-NN classifiers, reaching 78.3% top-1 on ImageNet with a small ViT.
- **p. 1 / Abstract - extractive PDF cue:** Our study also underlines the importance of momentum encoder [33], multi-crop training [10], and the use of small patches with ViTs.
- **p. 1 / Abstract - extractive PDF cue:** We implement our findings into a simple self-supervised method, called DINO, which we interpret as a form of self-distillation with no labels.
- **p. 2 / 1. Introduction - extractive PDF cue:** text specific, many existing self-supervised methods have shown their potential on images with convnets [10, 12, 30, 33].
...
| Observation / input | Given an input image x, both networks output probability distributions over K dimensions denoted by Ps and Pt. | 논문이 명시한 observation과 task input | exact sensor/frame/preprocessing from PDF |
...
PDF contribution framing (p. 3 (3.1. SSL with Knowledge Distillation), p. 2 (1. Introduction), p. 2 (1. Introduction), p. 4 (3.1. SSL with Knowledge Distillation), p. 4 (3.1. SSL with Knowledge Distillation)): However, our method shares also similarities with knowledge distillation [35] and we present it under this angle.
...
- **p. 4 / 3.1. SSL with Knowledge Distillation - extractive PDF cue:** Of particular interest, using an exponential moving average (EMA) on the student weights, i.e., a momentum encoder [33], is particularly well suited for our framework.
- **p. 4 / 3.1. SSL with Knowledge Distillation - extractive PDF cue:** However, in our framework, its role differs since we do not have a queue nor a contrastive loss, and may be closer to the role ...
...
upstream writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 3 (3.1. SSL with Knowledge Distillation), p. 2 (1. Introduction), p. 3 (3.1. SSL with Knowledge Distillation), p. 4 (3.1. SSL with Knowledge Distillation). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 2 (1. Introduction), interface p. 3 (3.1. SSL with Knowledge Distillation), p. 2 (1. Introduction), p. 3 (3.1. SSL with Knowledge Distillation), p. 4 (3.1. SSL with Knowledge Distillation), objective p. 4 (3.1. SSL with Knowledge Distillation), p. 3 (3.1. SSL with Knowledge Distillation), p. 3 (3.1. SSL with Knowledge Distillation), p. 4 (3.1. SSL with Knowledge Distillation).
