...
- 사용자의 연구 키워드 중 `CLIP, Vision-Language Model, alignment`와 직접적으로 연결된다.

## Paper Claim
- 논문의 중심 claim은 기존 2D-only, closed-set, 또는 task-specific 접근보다 더 일반화 가능한 3D-aware representation/policy/reasoning을 제공한다는 것이다.
- Abstract result cue: State-of-the-art computer vision systems are trained to predict a fixed set of predetermined object categories.
...
## 내 관점
- 이 논문은 `Foundations: Vision-Language Models` 축에서 읽어야 한다.
