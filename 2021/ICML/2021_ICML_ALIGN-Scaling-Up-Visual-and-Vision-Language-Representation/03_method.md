...
- **p. 3 / 4.1. Pre-training on Noisy Image-Text Pairs - extractive PDF cue:** We use EfficientNet with global pooling (without training the 1x1 conv layer in the classification head) as the image encoder and BERT with [CLS] token ...
...
- **p. 9 / 8. Multilingual ALIGN Model - extractive PDF cue:** Model training follows the exact English configuration.
- **p. 4 / 4.3. Transferring to Visual Classification - extractive PDF cue:** We first apply zero-shot transfer of ALIGN to visual classification tasks on ImageNet ILSVRC-2012 benchmark (Deng et al., 2009) and its variants including ImageNet-R(endition) (Hendrycks ...
- **p. 9 / 8. Multilingual ALIGN Model - extractive PDF cue:** A multilingual model ALIGNmling is trained using this data.
- **p. 3 / 4.1. Pre-training on Noisy Image-Text Pairs - extractive PDF cue:** We minimize the sum of two losses: one for image-to-text classification Li2t = -1 N N X i log exp(x⊤ i yi/σ) PN j=1 exp(x⊤ ...
- **p. 3 / 4.1. Pre-training on Noisy Image-Text Pairs - extractive PDF cue:** The image and text encoders are optimized via normalized softmax loss (Zhai & Wu, 2019).

## Design Rationale

- **p. 2 / 1. Introduction - extractive PDF cue:** Moreover, such cross-modality matching naturally enables zero-shot image classification when feeding the classnames into the text encoder, achieving 76.4% top-1 accuracy in ImageNet without using ...
- **p. 3 / 4.1. Pre-training on Noisy Image-Text Pairs - extractive PDF cue:** The model consists of a pair of image and text encoders with a cosine-similarity combination function at the top.
- **p. 4 / 4.3. Transferring to Visual Classification - extractive PDF cue:** (2020), we also evaluate the robustness of our model on Visual Task Adaptation Benchmark (VTAB) (Zhai et al., 2019) which consists of 19 diverse (covering ...
...
- **p. 3 / 4.1. Pre-training on Noisy Image-Text Pairs - extractive PDF cue:** We minimize the sum of two losses: one for image-to-text classification Li2t = -1 N N X i log exp(x⊤ i yi/σ) PN j=1 exp(x⊤ ...
- **p. 3 / 4.1. Pre-training on Noisy Image-Text Pairs - extractive PDF cue:** The image and text encoders are optimized via normalized softmax loss (Zhai & Wu, 2019).
- **p. 9 / 8. Multilingual ALIGN Model - extractive PDF cue:** Given that, we further lift the language constraint of the conceptual caption data processing pipeline to extend the dataset to multilingual (covering 100+ languages) and ...
- **p. 9 / 8. Multilingual ALIGN Model - extractive PDF cue:** One advantage of ALIGN is that the model is trained on noisy web image text data with very simple filters, and none of the filters ...
...
- **p. 2 / 1. Introduction - extractive PDF cue:** The aligned image and text representations are naturally suited for cross-modality matching/retrieval tasks and achieve state-of-the-art (SOTA) results in corresponding benchmarks.
- **p. 1 / 1. Introduction - extractive PDF cue:** In this work, we leverage a dataset of over one billion noisy image alt-text pairs to scale visual and vision-language representation learning.
- **p. 1 / 1. Introduction - extractive PDF cue:** However, vision-language pre-training datasets such as Conceptual Captions (Sharma et al., 2018), Visual Genome Dense Captions (Krishna et al., 2016), and ImageBERT (Qi et al., ...
- **p. 2 / 1. Introduction - extractive PDF cue:** Visual and language representations are jointly learned from noisy image alt-text data.
- **p. 4 / 4.3. Transferring to Visual Classification - extractive PDF cue:** For this purpose, we use the ImageNet as well as a handful of smaller fine-grained classification datasets such as Oxford Flowers-102 (Nilsback & Zisserman, 2008), ...
- **p. 9 / 8. Multilingual ALIGN Model - extractive PDF cue:** One advantage of ALIGN is that the model is trained on noisy web image text data with very simple filters, and none of the filters ...
- **p. 9 / 8. Multilingual ALIGN Model - extractive PDF cue:** Given that, we further lift the language constraint of the conceptual caption data processing pipeline to extend the dataset to multilingual (covering 100+ languages) and ...
...
- **p. 4 / 5.1. Image-Text Matching & Retrieval - extractive PDF cue:** We also reduce the initial learning rate to 1e-5 and train for 3K and 6K steps (with linear decay) respectively on Flickr30K and MSCOCO.
...
- **p. 4 / 5. Experiments and Results - extractive PDF cue:** The image encoder is trained at resolution of 289 × 289 pixels no matter what EfficientNet variant is used.
- **p. 4 / 5. Experiments and Results - extractive PDF cue:** Unless in the ablation study, we use the results of ALIGN where the image encoder is EfficientNet-L2 and the text encoder is BERT-Large.
- **p. 5 / 5.2. Zero-shot Visual Classification - extractive PDF cue:** Top-1 Accuracy of zero-shot transfer of ALIGN to image classification on ImageNet and its variants.
- **p. 6 / 5.2. Zero-shot Visual Classification - extractive PDF cue:** So we list the baseline results in (Foret et al., 2021) without using SAM optimization for a fairer comparison.
- **p. 6 / 5.2. Zero-shot Visual Classification - extractive PDF cue:** Our result (average of three runs) is comparable to the SOTA results without tweaking on optimization algorithms.
- **p. 7 / 6.2. Pre-training Datasets - extractive PDF cue:** Ablation study of different training datasets.
