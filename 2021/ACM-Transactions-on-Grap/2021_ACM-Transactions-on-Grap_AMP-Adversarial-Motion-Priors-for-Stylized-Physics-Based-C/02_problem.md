...
| Observation / input | 6.2 Network Architecture Each policy 𝜋is modeled by a neural network that maps a given state s𝑡and goal g to a Gaussian ... | proprioception, reference pose/motion, visual or language command | exact sensor/frame/preprocessing from PDF body |
...
| Decision / output variable | joint/whole-body action; body terms: central, contribution, adversarial, learning, physics-based, character, animation, combines | p. 2 (1 INTRODUCTION), p. 1 (Body text (section boundary not confidently recovered)), p. 2 (1 INTRODUCTION) |
| Objective / loss / cost | tracking/balance/task objective; cue terms: Least-Squares, Discriminator, standard, GAN, objective, detailed, Equation, typically | p. 5 (4 BACKGROUND), p. 5 (4 BACKGROUND), p. 6 (4 BACKGROUND), p. 7 (4 BACKGROUND), p. 1 (Body text (section boundary not confidently recovered)), p. 1 (Body text (section boundary not confidently recovered)) |
| Constraint / feasibility | paper-specific constraints are recorded only where the body states them; otherwise unresolved | p. 1 (Body text (section boundary not confidently recovered)), p. 7 (4 BACKGROUND), p. 7 (4 BACKGROUND) |
...
PDF body contribution framing (p. 2 (1 INTRODUCTION), p. 1 (Body text (section boundary not confidently recovered)), p. 2 (1 INTRODUCTION), p. 1 (Body text (section boundary not confidently recovered)), p. 5 (4 BACKGROUND)): The central contribution of this work is an adversarial learning approach for physics-based character animation that combines goalconditioned reinforcement with an adversarial motion prior, which enables the style of a ...
...
humanoid writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. Evidence interface anchors: p. 7 (4 BACKGROUND), p. 4 (4 BACKGROUND), p. 5 (4 BACKGROUND), p. 4 (4 BACKGROUND). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **Evidence anchors reviewed:** problem p. 5 (4 BACKGROUND), p. 5 (4 BACKGROUND), p. 1 (1 INTRODUCTION), p. 1 (1 INTRODUCTION), p. 2 (1 INTRODUCTION), interface p. 7 (4 BACKGROUND), p. 4 (4 BACKGROUND), p. 5 (4 BACKGROUND), p. 4 (4 BACKGROUND), objective p. 5 (4 BACKGROUND), p. 5 (4 BACKGROUND), p. 6 (4 BACKGROUND), p. 7 (4 BACKGROUND), p. 1 (Body text (section boundary not confidently recovered)), p. 1 (Body text (section boundary not confidently recovered)).
...
- **Target problem:** However, it can be exceptionally difficult to design a style-reward 𝑟𝑆 that leads a character to learn naturalistic behaviors, or behaviors that conform to a particular style. (p. 5, 4 BACKGROUND).
- **Formulation-changing contribution:** We present one of the first adversarial learning systems that is able to produce high-quality full-body motions for physically simulated characters. (p. 2, 1 INTRODUCTION).
- **Assumption/failure evidence:** 9 DISCUSSION AND LIMITATIONS In this work, we presented an adversarial learning system for physicsbased character animation that enables characters to imitate diverse behaviors from large unstructured datasets, without the ... (p. 12, 8 RESULTS).
