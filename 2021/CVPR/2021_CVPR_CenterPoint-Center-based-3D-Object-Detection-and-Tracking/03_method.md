...
- **p. 3 / 3. Preliminaries - extractive PDF cue:** The 3D encoder then pools these features into its primary feature representation.
- **p. 3 / 3. Preliminaries - extractive PDF cue:** A point-based network [40] then extracts features for all points inside a bin.
- **p. 4 / 4. CenterPoint - extractive PDF cue:** Then, a 2D CNN architecture detection head finds object centers and regress to full 3D bounding boxes using center features.
- **p. 5 / 4.1. Two-Stage CenterPoint - extractive PDF cue:** For box regression, the model predicts a refinement on top of first stage proposals, and we train the model with L1 loss.
- **p. 2 / 1. Introduction - extractive PDF cue:** Thirdly, point-based feature extraction enables us to design an effective two-stage refinement module that is much faster than previous approaches [44-46].
- **p. 4 / 4. CenterPoint - extractive PDF cue:** At inference time, we use this offset to associate current detections to past ones in a greedy fashion.
- **p. 1 / Abstract - extractive PDF cue:** On the Waymo Open Dataset, CenterPoint outperforms all previous single model method by a large margin and ranks first among all Lidar-only submissions.
- **p. 4 / 4. CenterPoint - extractive PDF cue:** CenterPoint combines all heatmap and regression losses in one common objective and jointly optimizes them.

## Design Rationale

- **p. 1 / 1. Introduction - extractive PDF cue:** These marked differences between 2D and 3D detection made a transfer of ideas bea) Anchor-based t=1 c) Anchor-based t=2 b) Center-based t=1 d) Center-based t=2 ...
- **p. 3 / 3. Preliminaries - extractive PDF cue:** We introduce a novel center-based detection head but rely on existing 3D backbones (VoxelNet or PointPillars).
- **p. 2 / 1. Introduction - extractive PDF cue:** Thirdly, point-based feature extraction enables us to design an effective two-stage refinement module that is much faster than previous approaches [44-46].
...
- **p. 4 / 4. CenterPoint - extractive PDF cue:** CenterPoint combines all heatmap and regression losses in one common objective and jointly optimizes them.
- **p. 4 / 4.1. Two-Stage CenterPoint - extractive PDF cue:** The training is supervised with a binary cross entropy loss: Lscore = -It log(ˆIt) -(1 -It) log(1 -ˆIt) (2) 4
- **p. 2 / 1. Introduction - extractive PDF cue:** It recovers the lost local geometric information due to striding and a limited receptive field, and brings a decent performance boost with minor cost.
- **p. 3 / 3. Preliminaries - extractive PDF cue:** Without loss of generality, we use an egocentric coordinate system with sensor at (0, 0, 0) and yaw= 0.
- **p. 5 / 4.1. Two-Stage CenterPoint - extractive PDF cue:** For box regression, the model predicts a refinement on top of first stage proposals, and we train the model with L1 loss.
- **p. 2 / 1. Introduction - extractive PDF cue:** The center-based representation has several key advantages: First, unlike bounding boxes, points have no intrinsic orientation.
...
- **p. 3 / 3. Preliminaries - extractive PDF cue:** It takes an input image and predicts a w × h heatmap ˆY ∈[0, 1]w×h×K for each of K classes.
- **p. 3 / 3. Preliminaries - extractive PDF cue:** The output of a backbone network is a map-view feature-map M ∈RW ×L×F of width W and length L with F channels in a map-view ...
- **p. 4 / 4. CenterPoint - extractive PDF cue:** The velocity estimate is special, as it requires two input map-views the current and previous time-step.
- **p. 4 / 4.1. Two-Stage CenterPoint - extractive PDF cue:** For each point, we extract a feature using bilinear interpolation from the backbone map-view output M.
- **p. 1 / 1. Introduction - extractive PDF cue:** Strong 3D perception is a core ingredient in many stateof-the-art driving systems [1, 50].
- **p. 1 / 1. Introduction - extractive PDF cue:** Second, the resulting output is a threedimensional box that is often not well aligned with any global coordinate frame.
- **p. 2 / 1. Introduction - extractive PDF cue:** For 3D tracking, our model performs at 63.8 AMOTA outperforming the prior state-of-the-art by 8.8 AMOTA on nuScenes.
...
- **p. 5 / 4.1. Two-Stage CenterPoint - extractive PDF cue:** For box regression, the model predicts a refinement on top of first stage proposals, and we train the model with L1 loss.
- **p. 4 / 4. CenterPoint - extractive PDF cue:** At inference time, we use this offset to associate current detections to past ones in a greedy fashion.
- **p. 5 / 5. Experiments - extractive PDF cue:** The inference times are measured on an Intel Core i7 CPU and a Titan RTX GPU.
- **p. 5 / 5. Experiments - extractive PDF cue:** During inference, we run the second stage on the top 500 predictions after Non-Maxima Suppression (NMS).
- **p. 4 / 4. CenterPoint - extractive PDF cue:** At training time, only ground truth centers are supervised using an L1 regression loss.
- **p. 4 / 4. CenterPoint - extractive PDF cue:** At inference time, we use this offset to associate current detections to past ones in a greedy fashion.
...
- **p. 7 / 5.2. Ablation studies - extractive PDF cue:** Effects of different feature components In our two-stage CenterPoint model, we only use features from the 2D CNN feature map.
- **p. 8 / 5.2. Ablation studies - extractive PDF cue:** More importantly, our tracking is a simple nearest neighbor matching without any hidden-state computation.
...
- **p. 12 / Figure/Table caption - extractive PDF cue:** Table 14: Ablation studies for 3D detection on nuScenes validation. entries in the NeurIPS 2020 nuScenes detection challenge. In this section, we describe the details ...
