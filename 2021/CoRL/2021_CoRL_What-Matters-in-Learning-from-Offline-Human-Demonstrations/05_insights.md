...
- Problem cue: Offline human demonstration에서 algorithm, history, data quality와 checkpoint selection 중 무엇이 실제 manipulation 성능을 좌우하는지 재현 가능하게 비교해야 한다.
- Method cue: 여러 imitation learning과 batch offline RL 방법을 공통 codebase에서 비교한다.
...
`Domain Randomization for Transferring Deep Neural Networks from Simulation to the Real World → What Matters in Learning from Offline Human Demonstrations for Robot Manipulation → Implicit Behavioral Cloning`
