    ...
    for line in PAPER_MD.read_text(encoding="utf-8").splitlines():
        ...
        entries.append(
            PaperEntry(
                ...
                category=category,
                tags=cells[3],
                overview_path=Path(unquote(overview)),
                pdf_link=pdf,
                project=project,
            )
        )
...
def main() -> int:
    ...
    only_source_arg = next((arg for arg in sys.argv if arg.startswith("--only-source=")), None)
    only_source = Path(only_source_arg.split("=", 1)[1]).resolve() if only_source_arg else None
    report_arg = next((arg for arg in sys.argv if arg.startswith("--report=")), None)
    report_path = Path(report_arg.split("=", 1)[1]).resolve() if report_arg else REPORT_JSON

    entries = parse_registry()
    if only:
        entries = [entry for entry in entries if only in str(entry.folder)]
    if only_source:
        selected = json.loads(only_source.read_text(encoding="utf-8"))
        selected_titles = {item["title"] for item in selected}
        entries = [entry for entry in entries if entry.title in selected_titles]
