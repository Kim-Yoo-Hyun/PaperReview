...
- **p. 6 / 4. Experiments - extractive body cue:** We evaluate VoxelNet on the KITTI 3D object detection benchmark [11] which contains 7,481 training images/point clouds and 7,518 test images/point clouds, covering three categories: ...
- **p. 6 / 4. Experiments - extractive body cue:** Since the ground truth for the test set is not available and the access to the test server is limited, we conduct comprehensive evaluation using ...
- **p. 7 / 4.1. Evaluation on KITTI Validation Set - extractive body cue:** We would like to note that [21] reported 88.9%, 77.3%, and 72.7% for easy, moderate, and hard levels respectively, but these results are obtained based ...
- **p. 8 / 4.2. Evaluation on KITTI Test Set - extractive body cue:** We would like to note that many of the other leading methods listed in KITTI benchmark use both RGB images and LiDAR point clouds whereas ...
- **p. 8 / 4.2. Evaluation on KITTI Test Set - extractive body cue:** The inference time for the VoxelNet is 225ms where the voxel input feature computation takes 5ms, feature learning net takes 20ms, convolutional middle layers take ...
- **p. 7 / 4.1. Evaluation on KITTI Validation Set - extractive body cue:** Evaluation in 3D Compared to the bird's eye view detection, which requires only accurate localization of objects in the 2D plane, 3D detection is a ...
- **p. 7 / 4.1. Evaluation on KITTI Validation Set - extractive body cue:** The IoU threshold is the same for both bird's eye view and full 3D evaluation.
- **p. 7 / 4.1. Evaluation on KITTI Validation Set - extractive body cue:** Metrics We follow the official KITTI evaluation protocol, where the IoU threshold is 0.7 for class Car and is 0.5 for class Pedestrian and Cyclist.
...
- **p. 1 / Figure/Table caption - extractive body cue:** Figure 1. VoxelNet directly operates on the raw point cloud (no need for feature engineering) and produces the 3D detection re- sults using a single ...
- **p. 2 / Figure/Table caption - extractive body cue:** Figure 2. VoxelNet architecture. The feature learning network takes a raw point cloud as input, partitions the space into voxels, and transforms points within each ...
- **p. 3 / Figure/Table caption - extractive body cue:** Figure 3. Voxel feature encoding layer. point cloud is sparse and has highly variable point density throughout the space. Therefore, after grouping, a voxel will ...
- **p. 4 / Figure/Table caption - extractive body cue:** Figure 4. Region proposal network architecture. each fi with ˜f to form the point-wise concatenated feature as f out i = [f T i ,˜f ...
- **p. 5 / Figure/Table caption - extractive body cue:** Figure 5. Illustration of efficient implementation. mensions ∆l, ∆w, ∆h, and the rotation ∆θ, which are com- puted as: ∆x = xg c -xa c ...
- **p. 7 / Figure/Table caption - extractive body cue:** Table 1. Performance comparison in bird's eye view detection: average precision (in %) on KITTI validation set.
- **p. 7 / Figure/Table caption - extractive body cue:** Table 2. Performance comparison in 3D detection: average precision (in %) on KITTI validation set. and a validation set, which results in 3,712 data samples ...
- **p. 8 / Figure/Table caption - extractive body cue:** Figure 6. Qualitative results. For better visualization 3D boxes detected using LiDAR are projected on to the RGB images. state-of-the-art method MV (BV+FV+RGB) [5] based ...
...
- **p. 8 / 4.2. Evaluation on KITTI Test Set - extractive body cue:** The inference time for the VoxelNet is 225ms where the voxel input feature computation takes 5ms, feature learning net takes 20ms, convolutional middle layers take ...
- **p. 8 / 4.2. Evaluation on KITTI Test Set - extractive body cue:** Performance evaluation on KITTI test set. and region proposal net takes 30ms on a TitanX GPU and 1.7Ghz CPU.
- **p. 5 / 2.2. Loss Function - extractive body cue:** Voxel Input Feature Buffer Voxel Coordinate Buffer K T 7 Sparse Tensor K 3 1 Voxel-wise Feature K C 1 Point Cloud Indexing Memory Copy ...
- **p. 5 / 3.1. Network Details - extractive body cue:** We choose a voxel size of vD = 0.4, vH = 0.2, vW = 0.2 meters, which leads to D′ = 10, H′ = 400, ...
- **p. 6 / 3.1. Network Details - extractive body cue:** Unlike [5], we use only one anchor size, la = 3.9, wa = 1.6, ha = 1.56 meters, centered at za c = -1.0 meters ...
- **p. 6 / 3.1. Network Details - extractive body cue:** We use anchor size la = 0.8, wa = 0.6, ha = 1.73 meters centered at za c = -0.6 meters with 0 and 90 ...
