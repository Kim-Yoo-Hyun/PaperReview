...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: The variant of our method that uses on-policy joint finetuning has a failure rate more than four times lower than prior work on the test set, while using ...
- 출발 문제 단서: While grasping restricts the manipulation problem, it still retains many of its largest challenges: a grasping system should be able to pick up previously unseen objects with reliable ...
- 주장된 효과 단서: The first two questions are addressed through a set of rigorous real-world quantitative experiments, which we discuss in Section 6.1, question (3) is addressed through qualitative experiments, which ...
...
- Result cue: The first two questions are addressed through a set of rigorous real-world quantitative experiments, which we discuss in Section 6.1, question (3) is addressed through qualitative experiments, which ...
