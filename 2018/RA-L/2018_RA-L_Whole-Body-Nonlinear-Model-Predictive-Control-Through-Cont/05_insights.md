...
- Problem cue: In this field, centroidal dynamics approaches – become increasingly popular as they capture the core dynamics of the problem.
- Method cue: — In this work we present a whole-body Nonlinear Model Predictive Control approach for Rigid Body Systems subject to contacts.
- Result/evaluation cue: This outperforms the state of the art by at least one order of magnitude.

위 cue는 기존 official abstract 또는 local text extraction에서 보존한 것이다. 수치·조건·인과적 해석은 full-text 정독 전까지 `UNVERIFIED`다.

## Dependency Position

`Hierarchical Quadratic Programming: Fast Online Humanoid-Robot Motion Generation → Whole-Body Nonlinear Model Predictive Control Through Contacts for Quadrupeds → 이 track의 frontier 연결`
