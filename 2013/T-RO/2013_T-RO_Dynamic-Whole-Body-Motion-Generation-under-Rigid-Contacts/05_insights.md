...
- Problem cue: We propose a reduced formulation of the multiple rigid planar contact that keeps a low computation cost.
- Method cue: We propose a reduced formulation of the multiple rigid planar contact that keeps a low computation cost.
- Result/evaluation cue: —The most widely-used technique to generate wholebody motions on a humanoid robot accounting for various tasks and constraints is the inverse kinematics.

위 cue는 기존 official abstract 또는 local text extraction에서 보존한 것이다. 수치·조건·인과적 해석은 full-text 정독 전까지 `UNVERIFIED`다.

## Dependency Position

`PDDLStream: Integrating Symbolic Planners and Blackbox Samplers via Optimistic Adaptive Planning → Dynamic Whole-Body Motion Generation under Rigid Contacts and Other Unilateral Constraints → Hierarchical Quadratic Programming: Fast Online Humanoid-Robot Motion Generation`
