...
- **p. 1 / 1 Introduction - extractive PDF cue:** In this paper, we take the standpoint of the second approach, and introduce a new modeling which represents the dynamics of a robot with limited ...
- **p. 2 / 1 Introduction - extractive PDF cue:** Let (τr, τp, f) be the actuator torque and force associated with the state variables (θr, θp, r).
- **p. 3 / 1 Introduction - extractive PDF cue:** Therefore, the 3D-LIPM with zero input torque can be considered as a dynamics under the central force field.
- **p. 3 / 1 Introduction - extractive PDF cue:** Since the 3DLIPM is a dynamics under the central force field, the motion along Y ′ and X′ is also governed by the identical equations ...
- **p. 1 / 1 Introduction - extractive PDF cue:** Therefore, it mainly relies on the accuracy of the model data [3, 5, 10, 14].
- **p. 2 / 1 Introduction - extractive PDF cue:** (11) Therefore, we have the same dynamics of Eq.
- **p. 4 / 1 Introduction - extractive PDF cue:** The initial body state (x(n) i , v(n) i ) and the final body state (x(n) f , v(n) f ) have the relationship given ...
- **p. 1 / 1 Introduction - extractive PDF cue:** 2 Derivation of 3D Linear Inverted Pendulum Mode 2.1 Motion equation of a 3D inverted pendulum When a biped robot is supporting its body on ...

## Design Rationale

- **p. 1 / 1 Introduction - extractive PDF cue:** It allows a separate controller design for the sagittal (x-z) and the lateral (y-z) motions and simplifies a walking pattern generation a great deal.
...
- **p. 1 / 1 Introduction - extractive PDF cue:** 2 Derivation of 3D Linear Inverted Pendulum Mode 2.1 Motion equation of a 3D inverted pendulum When a biped robot is supporting its body on ...
- **p. 2 / 1 Introduction - extractive PDF cue:** (4) Substituting these constraints into Eqs.
- **p. 2 / 1 Introduction - extractive PDF cue:** (9) and (10) are independent linear equations.
- **p. 3 / 1 Introduction - extractive PDF cue:** (14) With a given initial condition, these equations determine trajectories in 3D space.
- **p. 3 / 1 Introduction - extractive PDF cue:** Since the 3DLIPM is a dynamics under the central force field, the motion along Y ′ and X′ is also governed by the identical equations ...
- **p. 4 / 1 Introduction - extractive PDF cue:** (16) into this definition and calculating the foothold of x(2) i which minimizes N, we obtain a proper control law, x(2) i = (aCT (xd ...
...
- **p. 2 / 1 Introduction - extractive PDF cue:** Let (τr, τp, f) be the actuator torque and force associated with the state variables (θr, θp, r).
- **p. 3 / 1 Introduction - extractive PDF cue:** Therefore, the 3D-LIPM with zero input torque can be considered as a dynamics under the central force field.
- **p. 3 / 1 Introduction - extractive PDF cue:** and Tani introduced a two-dimensional version of this dynamics mode[6], and Hara, Yokokawa and Sadao extended it to three dimensions in the case of zero ...
- **p. 1 / 1 Introduction - extractive PDF cue:** Since the controller knows little about the system structure, this approach much relies on a feedback control [1, 6, 9, 12, 13, 15].
- **p. 1 / Abstract - extractive PDF cue:** Experimental results of realtime walking control of a 12 d.o.f. biped robot HRP-2L using an input device such as a game pad are also shown.
- **p. 2 / 1 Introduction - extractive PDF cue:** (10) in the case of an inclined constraint plane when the following new constraint is introduced about the inputs as, urx + upy = 0.
- **p. 4 / 1 Introduction - extractive PDF cue:** The initial body state (x(n) i , v(n) i ) and the final body state (x(n) f , v(n) f ) have the relationship given ...
...
- **PDF anchors reviewed:** method p. 1 (1 Introduction), p. 2 (1 Introduction), p. 3 (1 Introduction), p. 3 (1 Introduction), p. 1 (1 Introduction), p. 2 (1 Introduction), objective p. 1 (1 Introduction), p. 2 (1 Introduction), p. 2 (1 Introduction), p. 3 (1 Introduction), p. 3 (1 Introduction), p. 4 (1 Introduction), temporal p. 6 (4 Experiments), p. 2 (1 Introduction), p. 5 (4 Experiments), p. 6 (4 Experiments), p. 5 (C D), p. 2 (1 Introduction).
