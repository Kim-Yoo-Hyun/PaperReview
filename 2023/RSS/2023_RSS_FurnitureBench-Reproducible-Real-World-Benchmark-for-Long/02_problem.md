...
| Observation / input | Our reproducible robot system (a) and visual observations from the front-view camera (b) and wrist camera (c). of long-horizon complex robotic manipulation ... | standardized observation, action, task state와 evaluation split | exact sensor/frame/preprocessing from PDF body |
...
| Output / action | Benchmarking, furniture, assembly, poses, address, many, robotic, manipulation | policy/controller trajectory 또는 measured result | exact unit/frame/decoder require body check |
...
|---|---|---|
| State / observation variable | standardized episode e and interface; body terms: reproducible, robot, system, visual, observations, front-view, camera, wrist, long-horizon, complex | p. 2 (I. INTRODUCTION), p. 7 (2) The furniture parts are rearranged using our provided), p. 1 (Body text (section boundary not confidently recovered)) |
...
| Objective / loss / cost | benchmark score and failure cost; cue terms: not stated or recoverable in the selected PDF body | no optimization/equation sentence selected |
...
PDF body contribution framing (p. 2 (I. INTRODUCTION), p. 2 (I. INTRODUCTION), p. 1 (Abstract), p. 1 (Abstract)): The main contributions of this paper are as follows: • We introduce FurnitureBench, a real-world furniture assembly benchmark, which allows robotics researchers to investigate RL, IL, and TAMP algorithms on ...
...
benchmark writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. Evidence interface anchors: p. 2 (I. INTRODUCTION), p. 7 (2) The furniture parts are rearranged using our provided), p. 1 (Body text (section boundary not confidently recovered)), p. 1 (Abstract). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **Evidence anchors reviewed:** problem p. 2 (I. INTRODUCTION), p. 2 (I. INTRODUCTION), p. 1 (I. INTRODUCTION), interface p. 2 (I. INTRODUCTION), p. 7 (2) The furniture parts are rearranged using our provided), p. 1 (Body text (section boundary not confidently recovered)), p. 1 (Abstract), objective no optimization/equation sentence selected.
...
- **Target problem:** Furniture assembly is a proper task suite to benchmark a difficult, long-horizon manipulation task through which many challenges in robotic manipulation must be addressed to solve. (p. 2, I. INTRODUCTION).
- **Formulation-changing contribution:** The main contributions of this paper are as follows: • We introduce FurnitureBench, a real-world furniture assembly benchmark, which allows robotics researchers to investigate RL, IL, and TAMP algorithms on ... (p. 2, I. INTRODUCTION).
- **Assumption/failure evidence:** On the other hand, both algorithms struggle at "inserting" skill, which shows from 0% to 20% success rates. "Inserting" requires precise control to correctly align a screw and a hole, ... (p. 7, VI. BENCHMARKING RESULTS).
