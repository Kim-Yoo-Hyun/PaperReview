...
- **p. 4 / B. Efficient Model-Free RL - extractive PDF cue:** Our choice of algorithm and implementation is aimed at enabling real-time synchronous training, which we expand on in Section V.
- **p. 4 / B. Efficient Model-Free RL - extractive PDF cue:** Actor-critic methods have recently become significantly more sample-efficient by improving the training of the critic, thereby allowing more updates to the critic network for the ...
- **p. 4 / B. Efficient Model-Free RL - extractive PDF cue:** These algorithms use up to 20 times the number of critic updates to speed up learning with respect to the number of samples collected, but ...
- **p. 4 / B. Efficient Model-Free RL - extractive PDF cue:** What all of these works have in common is that they add some sort of regularization or normalization method (or both) to mitigate the tendency ...
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** Reinforcement learning offers a promising alternative, acquiring effective control strategies directly through interaction with the real system, potentially right in the environment in which the ...
- **p. 2 / I. INTRODUCTION - extractive PDF cue:** Experimental Design Training Statistics Simulation Real World Hardware Actions Resets Terrains Samples Hours Samples Hours Ours A1 PD targets Learned In/Outdoor 0 0 20 · ...
- **p. 2 / I. INTRODUCTION - extractive PDF cue:** [29] Minitaur PMTG [26] parameters Unknown Indoor 0 0 45 · 103 0.16 Table I: Overview of experimental details (hardware platform used, the kinds of ...
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** Although large-scale robotic reinforcement learning experiments in the real world have been described in a number of prior works [1]-[5], many other researchers have sought ...
...
- **p. 4 / B. Efficient Model-Free RL - extractive PDF cue:** Our choice of algorithm and implementation is aimed at enabling real-time synchronous training, which we expand on in Section V.
- **p. 4 / B. Efficient Model-Free RL - extractive PDF cue:** Actor-critic methods have recently become significantly more sample-efficient by improving the training of the critic, thereby allowing more updates to the critic network for the ...
- **p. 4 / B. Efficient Model-Free RL - extractive PDF cue:** Because of this, a na¨ıve implementation cannot train as fast as the samples are collected.
- **p. 4 / B. Efficient Model-Free RL - extractive PDF cue:** Prior work has addressed this either by performing asynchronous training [23], [28] or training in-between trials [24].
...
- **p. 6 / V. SIMULATION ANALYSIS - extractive PDF cue:** In order to facilitate this kind of training synchronously, the updates must be inexpensive enough to be able to perform them between time-steps (of which ...
...
- **p. 6 / V. SIMULATION ANALYSIS - extractive PDF cue:** As such, we favor using the less computationally expensive DroQ variants over others in the real world.
- **p. 6 / V. SIMULATION ANALYSIS - extractive PDF cue:** Left to right: flat, solid ground covered in dense foam mats; a 5cm memory foam mattress; loose ground comprised of eucalyptus bark; a grassy lawn; ...
