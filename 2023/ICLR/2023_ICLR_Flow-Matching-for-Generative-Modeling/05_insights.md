...
- Problem cue: Continuous normalizing flow를 likelihood 계산이나 trajectory simulation 없이 대규모로 안정적으로 학습하는 방법이 필요하다.
- Method cue: 고정 conditional probability path의 vector field를 회귀하는 simulation-free objective를 정의한다.
...
`Denoising Diffusion Probabilistic Models → Flow Matching for Generative Modeling → Diffusion Policy: Visuomotor Policy Learning via Action Diffusion`
