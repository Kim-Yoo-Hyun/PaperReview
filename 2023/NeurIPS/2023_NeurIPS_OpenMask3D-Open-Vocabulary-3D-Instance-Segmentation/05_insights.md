...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: We propose OpenMask3D, the first open-vocabulary 3D instance 1 Our approach takes as input posed segmentation model.
- 출발 문제 단서: Hence, the second key problem with closed-vocabulary approaches is their inherent limitation to recognize only object classes that are predefined at training time.
- 주장된 효과 단서: Experiments and ablation studies on ScanNet200 and Replica show that OpenMask3D outperforms other open-vocabulary methods, especially on the long-tail distribution.
...
- Result cue: Experiments and ablation studies on ScanNet200 and Replica show that OpenMask3D outperforms other open-vocabulary methods, especially on the long-tail distribution.
