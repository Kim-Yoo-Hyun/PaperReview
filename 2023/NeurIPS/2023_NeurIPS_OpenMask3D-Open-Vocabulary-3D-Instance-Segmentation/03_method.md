...
- **p. 4 / 3 Method - extractive body cue:** We propose OpenMask3D, the first open-vocabulary 3D instance segmentation model.
...
- **p. 5 / 3 Method - extractive body cue:** In b⃝, we compute a 2D object mask in each selected frame, which is used to obtain multi-scale image-crops in order to extract effective CLIP ...
- **p. 8 / Model - extractive body cue:** To assess how well our model generalizes to other datasets, we use instance masks from the mask proposal module trained on ScanNet200, and test it ...
- **p. 6 / 3 Method - extractive body cue:** Algorithm 1 - 2D mask selection algorithm score∗←0, m2D ∗ ←0, r ←0 while r < krounds do Sample ksample points among the projected points ...
- **p. 9 / Model - extractive body cue:** For this experiment, we perform Hungarian matching between the predicted masks and oracle masks discarding all class-losses, and we only match based on the masks.
...
- **p. 3 / 3 Method - extractive body cue:** Our pipeline takes as input a collection of posed RGB-D images captured in an indoor scene, and the reconstructed point cloud representation of the scene.
- **p. 6 / 3 Method - extractive body cue:** 3.2.3 CLIP feature extraction and mask-feature aggregation For each instance mask, we collect k ⋅L images by selecting top-k views and obtaining L multi-level crops ...
- **p. 3 / 3 Method - extractive body cue:** Given a set of posed RGB-D images captured in a scene, along with the reconstructed scene point cloud 1⃝, OpenMask3D predicts 3D instance masks with ...
- **p. 4 / 3 Method - extractive body cue:** In the original setup, [58] produces two outputs: a set of M binary instance masks obtained from the predicted heatmaps, along with predicted class labels ...
- **p. 5 / 3 Method - extractive body cue:** SAM is sensitive to the set of input points (see Appendix A.2.3, A.2.4).
- **p. 5 / 3 Method - extractive body cue:** The output of SAM at a given iteration r is a 2D mask (m2D r ) and a mask confidence score (scorer).
...
- **p. 8 / Model - extractive body cue:** To assess how well our model generalizes to other datasets, we use instance masks from the mask proposal module trained on ScanNet200, and test it ...
- **p. 7 / Model - extractive body cue:** In order to compute image features on the mask-crops, we use CLIP [55] visual encoder from the ViT-L/14 model pre-trained at a 336 pixel resolution, ...
- **p. 3 / 3 Method - extractive body cue:** The mask-feature computation module leverages pre-trained CLIP [55] vision-language model in order to compute meaningful and flexible features for each mask.
...
- **p. 19 / Figure/Table caption - extractive body cue:** Table 7: Ablation study of the multi-scale cropping hyperparameters on the Replica dataset. We analyze the effect of varying number of levels, and the ratio ...
- **p. 6 / 4 Experiments - extractive body cue:** Furthermore, we provide an ablation study for OpenMask3D.
...
- **p. 19 / Figure/Table caption - extractive body cue:** Table 6: Ablation study of the top-k frame selection parameter k. This analysis is conducted on the ScanNet200 validation set. Levels Ratio of Exp. AP ...
- **p. 20 / Figure/Table caption - extractive body cue:** Figure 12: Visualization of the Replica RGB images. Original RGB images from the Replica dataset (left), and RGB images rendered from the scene point cloud ...
...
- **Evidence anchors reviewed:** method p. 4 (3 Method), p. 7 (Model), p. 4 (3 Method), p. 8 (Model), p. 5 (3 Method), p. 8 (Model), objective p. 9 (Model), temporal p. 4 (3 Method), p. 4 (3 Method), p. 5 (3 Method), p. 3 (3 Method), p. 3 (3 Method), p. 5 (3 Method).
