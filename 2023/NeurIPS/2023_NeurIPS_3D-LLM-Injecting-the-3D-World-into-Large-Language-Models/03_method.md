...
- **p. 2 / 5. Facing the mirror and dress - extractive body cue:** To this end, we propose to inject the 3D world into large language models, and introduce a whole new family of 3D-LLMs that could take ...
...
- **p. 5 / 5. Facing the mirror and dress - extractive body cue:** Using these alignment methods, we could use pretrained image encoders to extract image features, and then map the features to the 3D data.
- **p. 6 / 5. Facing the mirror and dress - extractive body cue:** Then we align 3D features in the rays and 2D features in the pixels using MSE loss.
...
- **p. 6 / 5. Facing the mirror and dress - extractive body cue:** Then we align 3D features in the rays and 2D features in the pixels using MSE loss.
- **p. 2 / 5. Facing the mirror and dress - extractive body cue:** By taking the 3D representations of scenes as input, LLMs are blessed with twofold advantages: (1) long-term memories about the entire scene can be stored ...
...
- **p. 2 / 5. Facing the mirror and dress - extractive body cue:** To this end, we propose to inject the 3D world into large language models, and introduce a whole new family of 3D-LLMs that could take ...
- **p. 6 / 5. Facing the mirror and dress - extractive body cue:** The 2D image features, output from frozen image encoders, are flattened and sent to the perceiver to generate a fixed-sized input.
- **p. 6 / 5. Facing the mirror and dress - extractive body cue:** After adding these additional location tokens, we unfreeze the weights for these tokens in the input and output embeddings of language models.
- **p. 2 / 5. Facing the mirror and dress - extractive body cue:** By taking the 3D representations of scenes as input, LLMs are blessed with twofold advantages: (1) long-term memories about the entire scene can be stored ...
...
- **p. 3 / 5. Facing the mirror and dress - extractive body cue:** In addition, we append a series of location tokens to the 3D-LLMs, and localization can be trained via outputting location tokens given the language descriptions ...
- **p. 3 / 5. Facing the mirror and dress - extractive body cue:** To better align these LLMs' predictions to human instructions, improve the models' generalization abilities on unseen tasks, a series of instruction tuning methods [35, 44] ...
...
- **p. 5 / 5. Facing the mirror and dress - extractive body cue:** Furthermore, for 3D scenes, there are no available pretrained encoders like those for 2D images (e.g., CLIP ViT encoders).
...
- **p. 7 / 5 Experiments - extractive body cue:** This shows that our model could perform visual reasoning about objects and their relationships even without explicit object representations.
- **p. 7 / 5 Experiments - extractive body cue:** Furthermore, 3D-based baselines use object detectors like VoteNet to segment the objects, and then send per-object features into their models, while our inputs are holistic ...
- **p. 8 / 5 Experiments - extractive body cue:** We add one language-only baseline: FlanT5, which examines LLMs' ability to complete these tasks without any visual input.
- **p. 13 / B.1 Implementation Details - extractive body cue:** Using Pretrained BLIP-2 as backbones, we train 3D-LLMs for 100K steps, and validate every 1K step.
- **p. 13 / B.1 Implementation Details - extractive body cue:** 3D-LLMs based on pretrained flamingo are trained using the AdamW optimizer with global norm clipping of 1, no weight decay for the perceiver resampler and ...
