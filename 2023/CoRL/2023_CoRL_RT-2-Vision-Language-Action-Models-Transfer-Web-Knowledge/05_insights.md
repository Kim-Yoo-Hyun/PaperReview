...
- Result/evaluation cue: With this solution, we can achieve a suitable frequency of control and also serve multiple robots using the same cloud service.

위 cue는 기존 official abstract 또는 local text extraction에서 보존한 것이다. 수치·조건·인과적 해석은 full-text 정독 전까지 `UNVERIFIED`다.

## Dependency Position

`RT-1: Robotics Transformer for Real-World Control at Scale → RT-2: Vision-Language-Action Models Transfer Web Knowledge to Robotic Control → VoxPoser: Composable 3D Value Maps for Robotic Manipulation with Language Models`
