...
- **p. 14 / A Implementation details - extractive body cue:** 2(b)), we specify the goal image gr t (gr t ∈Vr) as the frame H steps after the input observation or t in the robot ...
- **p. 2 / 1 Introduction - extractive body cue:** Conditioned on these latent plans, the low-level controller incorporates state information essential for fined-grained manipulation to generate the final actions.
- **p. 14 / A Implementation details - extractive body cue:** Based on the inputs, the latent planner generates a latent plan feature embedding pt of shape R1×d, which is used as guidance for the low-level ...
- **p. 2 / 1 Introduction - extractive body cue:** The methods for learning from such play data often seek to uncover such diverse behaviors by training a hierarchical policy [5], where the high-level planner ...
...
- **p. 1 / 1 Introduction - extractive body cue:** Imitation Learning (IL) has recently made considerable strides towards this goal, especially through supervised training using either human teleoperated demonstrations or trajectories of expert policies ...
...
|---|---|---|---|
| Horizon | single-step 또는 action chunk/trajectory horizon; exact chunk length는 exact value was not selected from the PDF body. | The embedding sequence of T time steps is represented as s[t:t+T]= [wt,et,pt,···,wt+T,et+T,pt+T], which passes through a transformer architecture [51]. | episode/sequence/action-chunk boundary |
...
- **p. 19 / C Supplementary Experiment Results - extractive body cue:** F Training hyperparameters We list the hyperparameters for training the models in Tab.
...
- **p. 17 / Figure/Table caption - extractive body cue:** Figure 8: System setups for the data collection. (a) Human play data collection. A human operator directly interacts with the scene with one of its ...
- **p. 16 / C Supplementary Experiment Results - extractive body cue:** (a) Feature visualization results of our method without using KL divergence loss.
...
- **p. 17 / C Supplementary Experiment Results - extractive body cue:** 7, we use t-SNE to process and visualize the learned feature embeddings generated by Ours and the model variant Ours (w/o KL) on the 2D ...
...
- **p. 7 / 5 Results - extractive body cue:** We visualize the trajectories generated by all of the model variants in the Appendix, where we found Ours (w/o GMM) has the worst quality of ...
...
- **Evidence anchors reviewed:** method p. 14 (A Implementation details), p. 14 (A Implementation details), objective 본문 anchor 없음, temporal p. 20 (C Supplementary Experiment Results), p. 20 (C Supplementary Experiment Results), p. 2 (1 Introduction), p. 16 (C Supplementary Experiment Results), p. 6 (2 Related Work), p. 6 (2 Related Work).
...
- **Paper-specific method/interface:** Based on the inputs, the latent planner generates a latent plan feature embedding pt of shape R1×d, which is used as guidance for the low-level robot policy. (p. 14, A Implementation details).
- **Objective/update evidence:** The robot policy model is a GPT-style transformer [52], which consists of four multi-head layers with four heads. (p. 14, A Implementation details).
- **Temporal/runtime evidence:** 2(b)), we specify the goal image gr t (gr t ∈Vr) as the frame H steps after the input observation or t in the robot demonstration. (p. 14, A Implementation details).
