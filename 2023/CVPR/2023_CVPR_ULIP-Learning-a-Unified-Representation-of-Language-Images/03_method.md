...
- **p. 6 / Model - extractive PDF cue:** It conducts zero-shot 3D classification by first converting a 3D point cloud into 6 orthogonal depth maps, then using CLIP's image encoder to get ensembled ...
- **p. 3 / 3.1. Creating Training Triplets for ULIP - extractive PDF cue:** Then a 3D encoder takes the augmented point cloud Pi as input and outputs its 3D representation hP i via
- **p. 5 / 4.3. Implementation Details - extractive PDF cue:** We use our pre-trained models as they are when performing zero-shot classification.
...
- **p. 8 / 4.7. Cross-Modal Retrieval - extractive PDF cue:** We use our pre-trained ULIP with PointBERT as the 3D encoder directly.
- **p. 3 / 3.1. Creating Training Triplets for ULIP - extractive PDF cue:** During each iteration of pre-training, we randomly select one image or depth map from each CAD model's 60 renderred candidates as Ii and take Ii ...
- **p. 6 / 4.6. Analyses - extractive PDF cue:** 5, ULIP by default aligns the 3D representation with both the text and image representations during pre-training.
- **p. 5 / 4.3. Implementation Details - extractive PDF cue:** We use 64 as the batch size, 10-3 as the learning rate, and AdamW as the optimizer.

## Design Rationale

- **p. 5 / 4.4. Standard 3D Classification - extractive PDF cue:** We present the standard 3D classification performances of our baselines and our methods on ScanObjectNN in Table 7.
...
- **p. 5 / 4.3. Implementation Details - extractive PDF cue:** We use 64 as the batch size, 10-3 as the learning rate, and AdamW as the optimizer.
- **p. 5 / 4.3. Implementation Details - extractive PDF cue:** As mentioned in Section 3.2, we freeze the image and text encoders and only update the 3D encoder's parameters during pre-training.
...
- **p. 5 / 4.3. Implementation Details - extractive PDF cue:** The inputs of image and text modalities are generated as described in Section 3.1.
- **p. 1 / 1. Introduction - extractive PDF cue:** ULIP improves 3D understanding by aligning features from images, texts, and point clouds in the same space.
...
- **p. 5 / 4.3. Implementation Details - extractive PDF cue:** 3We used the variants provided by [58] in our experiments.
...
- **p. 12 / Figure/Table caption - extractive PDF cue:** Table 9. ModelNet40 Medium Set. Hard Set: We remove both extract category names and their synonyms in our pre-training dataset. The final Hard Set is ...
- **p. 12 / Figure/Table caption - extractive PDF cue:** Table 8. ModelNet40 All Set. Medium Set: We remove categories whose exact category names exist in our pre-training dataset. The resulting cate- gories in this ...
...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Table 6. Analysis of aligning three vs. two modalities on zero-shot 3D classification on ScanObjectNN. Results show that aligning representations of three modalities always produces ...
