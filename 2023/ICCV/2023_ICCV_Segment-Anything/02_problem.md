...
- **p. 1 / Abstract - extractive PDF cue:** We introduce the Segment Anything (SA) project: a new task, model, and dataset for image segmentation.
- **p. 1 / Abstract - extractive PDF cue:** Using our efficient model in a data collection loop, we built the largest segmentation dataset to date (by far), with over 1 billion masks on ...
- **p. 1 / Abstract - extractive PDF cue:** The model is designed and trained to be promptable, so it can transfer zero-shot to new image distributions and tasks.
- **p. 1 / Abstract - extractive PDF cue:** We evaluate its capabilities on numerous tasks and find that its zero-shot performance is impressive - often competitive with or even superior to prior fully ...
- **p. 1 / 1. Introduction - extractive PDF cue:** Large language models pre-trained on web-scale datasets are revolutionizing NLP with strong zero-shot and few-shot generalization [10].
- **p. 1 / 1. Introduction - extractive PDF cue:** These "foundation models" [8] can generalize to tasks and data distributions beyond those seen during training.
- **p. 1 / 1. Introduction - extractive PDF cue:** Once trained, engineered text prompts enable zero-shot generalization to novel visual concepts and data distributions.
...
| Observation / input | The mask decoder efficiently maps the image embedding, prompt embeddings, and an output token to a mask. | 논문이 명시한 observation과 task input | exact sensor/frame/preprocessing from PDF |
...
PDF contribution framing (p. 2 (3. What data can power this task and model?), p. 1 (1. Introduction), p. 2 (3. What data can power this task and model?), p. 5 (3. Segment Anything Model), p. 1 (1. Introduction)): We introduce each interconnected component next, followed by the dataset we created and the experiments that demonstrate the effectiveness of our approach.
...
- **p. 5 / 3. Segment Anything Model - extractive PDF cue:** This runtime performance enables seamless, real-time interactive prompting of our model.
- **p. 1 / 1. Introduction - extractive PDF cue:** To develop them, we address the following questions about image segmentation:
...
upstream writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 5 (3. Segment Anything Model), p. 5 (3. Segment Anything Model), p. 2 (3. What data can power this task and model?), p. 2 (3. What data can power this task and model?). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (1. Introduction), p. 1 (1. Introduction), interface p. 5 (3. Segment Anything Model), p. 5 (3. Segment Anything Model), p. 2 (3. What data can power this task and model?), p. 2 (3. What data can power this task and model?), objective p. 2 (3. What data can power this task and model?), p. 2 (3. What data can power this task and model?), p. 5 (3. Segment Anything Model), p. 5 (3. Segment Anything Model).
