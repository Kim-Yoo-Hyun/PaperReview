...
- **Claim/result cue:** We evaluate its capabilities on numerous tasks and find that its zero-shot performance is impressive – often competitive with or even superior to prior fully supervised results.
...
- **Registry interface:** `segmentation, foundation model, prompting` is the paper's recorded topic/interface, not evidence that the full robotics loop was evaluated.
...
- Registry position: `Foundations: Vision and Language Models`; tags: `segmentation, foundation model, prompting`.
