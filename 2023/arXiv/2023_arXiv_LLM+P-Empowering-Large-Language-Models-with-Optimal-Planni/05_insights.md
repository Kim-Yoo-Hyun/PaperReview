...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: Specifically, they can be (relatively) easily fooled by, for example, asking for the result of a straightforward arithmetic problem that does not appear in their training corpus or ...
- 출발 문제 단서: A Failure Example of GPT-4 in Planning Problem (P1): You have 5 blocks.
- 주장된 효과 단서: Via a comprehensive set of experiments on these benchmark problems, we find that LLM+P is able to provide optimal solutions for most problems, while LLMs fail to provide ...
...
- Result cue: Via a comprehensive set of experiments on these benchmark problems, we find that LLM+P is able to provide optimal solutions for most problems, while LLMs fail to provide ...
