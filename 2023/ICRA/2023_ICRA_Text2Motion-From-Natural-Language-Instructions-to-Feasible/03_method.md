...
- **p. 4 / 4 Methods - extractive body cue:** We then introduce the full planning algorithm, Text2Motion, which synergistically integrates the strengths of both strategies.
- **p. 7 / 4.3 Search-based planning - extractive body cue:** See Figure 2 for a visualization of the shooting and greedy-search planners. are then multiplied to produce the overall skill score (Eq.
- **p. 5 / 4.2 Shooting-based planning - extractive body cue:** 13 15: if C == ∅then 16: raise planning failure 17: end if 18: j∗= arg maxj∈C p(j) success 19: return ψ(j∗) 1:t-1 ▷Return best ...
...
- **p. 5 / 4.2 Shooting-based planning - extractive body cue:** 13 15: if C == ∅then 16: raise planning failure 17: end if 18: j∗= arg maxj∈C p(j) success 19: return ψ(j∗) 1:t-1 ▷Return best ...
- **p. 4 / 3.3 Geometric feasibility planning - extractive body cue:** STAP resolves geometric dependencies across the skill sequence ψ1:H by maximizing the product of step reward probabilities of parameters a1:H: a∗ 1:H = arg max ...
- **p. 4 / 4 Methods - extractive body cue:** These strategies represent different ways of maximizing the overall planning objective in Eq.
- **p. 5 / 4.2 Shooting-based planning - extractive body cue:** Each candidate skill sequence is processed by the geometric feasibility planner which returns an estimate of the sequence's success probability (Eq.
- **p. 6 / 4.3 Search-based planning - extractive body cue:** This iterative approach can be described as a decomposition of the planning objective in Eq.
- **p. 6 / 4.3 Search-based planning - extractive body cue:** (8) Each planning iteration of greedy-search is responsible for finding the skill ψt that maximizes the skill score (Eq.
...
- **p. 5 / 4.1 Goal prediction - extractive body cue:** Both shooting and greedy-search planners use the LLM to predict the set of valid goal states given the user's natural language instruction and a description ...
...
- **p. 3 / 3.1 LLM and skill library - extractive body cue:** When a skill ψ is executed, an action a ∈A is sampled from its policy π(a/s) and fed to its primitive ϕ(a), which consumes the ...
- **p. 4 / 3.2 The planning objective - extractive body cue:** 3 represents the probability that skills ψ1:H achieve rewards r1:H when executed from initial state s1 with parameters a1:H; which is independent of the instruction ...
- **p. 4 / 3.2 The planning objective - extractive body cue:** This objective can be expressed as the joint probability of skill sequence ψ1:H and binary rewards r1:H given the instruction i and initial state s1: ...
...
- **p. 8 / 5.2 Large language model - extractive body cue:** We use two pretrained language models, both of which were accessed through the OpenAI API: i) text-davinci-003, a variant of the InstructGPT [61] language model ...
...
- **p. 10 / 6.1 Feasibility planning is required - extractive body cue:** Bottom: Methods without geometric feasibility planning tend to have high sub-goal completion rates but very low success rates.
- **p. 11 / 6.1 Feasibility planning is required - extractive body cue:** In this plot, we analyse the various types of failure modes that occur with Text2Motion, shooting and greedy-search when evaluated on tasks with partial affordance ...
- **p. 12 / 6.2 Search-based reasoning is - extractive body cue:** Plan Length 5.0 7.0 7.0 Table 1 Ablation on hybrid planning method.
- **p. 12 / 6.4 Plan termination is made - extractive body cue:** We test this hypothesis in an ablation experiment (Figure 7), comparing our plan termination method to that of SayCan and Inner Monologue's, while keeping all ...
