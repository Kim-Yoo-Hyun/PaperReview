...
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** CaP: Mobile Robot Navigation and Manipulation In this domain, a robot with a mobile base and a 7 DoF arm is tasked to perform navigation ...
- **p. 5 / IV. EXPERIMENTS - extractive PDF cue:** Hierarchical LMPs on Code-Generation Benchmarks We evaluate our code-generation approach on two codegeneration benchmarks: (i) a robotics-themed RoboCodeGen and (ii) HumanEval [1], which consists of ...
- **p. 5 / IV. EXPERIMENTS - extractive PDF cue:** RoboCodeGen: we introduce a new benchmark with 37 function generation problems with several key differences from previous code-gen benchmarks: (i) it is robotics-themed with questions ...
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** CaP: Pick & Place Policies for Table-Top Manipulation The table-top manipulation domain tasks a UR5e robot arm to pick and place various plastic toy objects ...
- **p. 1 / Figure/Table caption - extractive PDF cue:** Fig. 1: Given examples (via few-shot prompting), robots can use code-writing large language models (LLMs) to translate natural language commands into robot policy code which ...
- **p. 5 / IV. EXPERIMENTS - extractive PDF cue:** Within each model family, performance improves with larger models.
- **p. 5 / IV. EXPERIMENTS - extractive PDF cue:** Two Codex models [1]: cushman and davinci, trained to generate code. davinci is larger and better.
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** Examples of successful on-robot executions of unseen language commands are in Fig.
...
- **p. 1 / Figure/Table caption - extractive PDF cue:** Fig. 1: Given examples (via few-shot prompting), robots can use code-writing large language models (LLMs) to translate natural language commands into robot policy code which ...
- **p. 2 / Figure/Table caption - extractive PDF cue:** Fig. 2: Code as Policies can follow natural language instructions across diverse domains and robots: table-top manipulation (a)-(b), 2D shape drawing (c), and mobile manipulation ...
- **p. 11 / Figure/Table caption - extractive PDF cue:** Fig. 4: Robot Code-Generation Benchmark Performance across Generalization Types for Flat (top) and Hierarchical (middle) Code- Generation, as well as the performance improvements made by ...
- **p. 12 / Figure/Table caption - extractive PDF cue:** Fig. 5: LMPs can balance a cartpole LMPs can likewise be prompted to express impedance control: # define function: tau = ee_impedance_control(x_curr, x_goal, x_dot, K_x_mat, ...
- **p. 14 / Figure/Table caption - extractive PDF cue:** Fig. 6: Experiment Setup for mobile manipulation with a Everyday Robots robot. Prompts. • mobile_ui: the high-level UI for parsing user commands and calling other ...
- **p. 16 / Figure/Table caption - extractive PDF cue:** Fig. 7: LMPs inherit benefits of LLMs, such as parsing commands from non- English languages and emojis. # omnidirectional robot. # available actions: move_up(dist), move_right(dist), ...
...
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** We inherit all 8 tasks, referred as "long-horizon" tasks due to their multi-step nature (e.g., "put the blocks in matching bowls").
...
- **p. 6 / V. DISCUSSION AND LIMITATIONS - extractive PDF cue:** Our approach also assumes all given instructions are feasible, and we cannot tell if a response will be correct a priori.
- **p. 6 / IV. EXPERIMENTS - extractive PDF cue:** It also illustrates the ability to follow long-horizon reactive commands with control structures as well as precise spatial reasoning, which cannot be easily accomplished by ...
