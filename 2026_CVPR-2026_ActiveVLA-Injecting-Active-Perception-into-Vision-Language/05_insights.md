...
- 사용자의 연구 키워드 중 `VLA, active perception, 3D manipulation`와 직접적으로 연결된다.

## Paper Claim
- 논문의 중심 claim은 기존 2D-only, closed-set, 또는 task-specific 접근보다 더 일반화 가능한 3D-aware representation/policy/reasoning을 제공한다는 것이다.
- Abstract result cue: Additionally, ActiveVLA applies a 3D zoom-in to improve resolution in key areas.
