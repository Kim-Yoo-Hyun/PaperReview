...
- Abstract result cue: Experimentally, we show that convolutional neural networks with only generalized 3D sparse convolutions can outperform 2D or 2D-3D hybrid methods by a large margin.
...
## 내 관점
- 이 논문은 `Foundations: 3D Representation Learning` 축에서 읽어야 한다.
