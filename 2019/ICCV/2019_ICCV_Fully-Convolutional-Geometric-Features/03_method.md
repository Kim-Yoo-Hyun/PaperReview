...
- **p. 4 / 5. Implementation - extractive body cue:** As the input to the network requires unique coordinates C and corresponding features F, we first downsample the input point cloud using a fast GPU-based ...
- **p. 8 / 6.7. Runtime - extractive body cue:** The reported times include data preprocessing and feature extraction.
- **p. 4 / 4.2. Hardest-contrastive and Hardest-triplet Losses - extractive body cue:** Ii is short for I(i, ki, dt), which is an indicator function that returns 1 if the feature ki is located outside a sphere with ...
- **p. 4 / 4.2. Hardest-contrastive and Hardest-triplet Losses - extractive body cue:** Traditional contrastive and triplet losses use random sampling.
...
- **p. 3 / 4.2. Hardest-contrastive and Hardest-triplet Losses - extractive body cue:** Then, we use the pairwise loss for the mined quadruplet (fi, fj, f - i , f - j ) and form the fully8960
- **p. 4 / 4.2. Hardest-contrastive and Hardest-triplet Losses - extractive body cue:** Traditional contrastive and triplet losses use random sampling.
- **p. 4 / 4.2. Hardest-contrastive and Hardest-triplet Losses - extractive body cue:** The above equation finds the hardest negatives for both (i, j) ∈P (Fig.
- **p. 7 / 6.4. Hardest-contrastive and Hardest-triplet Losses - extractive body cue:** As the hardest-triplet loss tends to collapse Hardest-Triple Feature Match Recall STD Num.
...
| Action/output | section, metric, learning, losses, fully-convolutional, feature, most, accurate, fastest, does | point map, pose, scene graph, affordance 또는 query result | body cue; unit/decoder verify |
...
- **p. 4 / 5. Implementation - extractive body cue:** As the input to the network requires unique coordinates C and corresponding features F, we first downsample the input point cloud using a fast GPU-based ...
- **p. 8 / 6.7. Runtime - extractive body cue:** The reported times include data preprocessing and feature extraction.
- **p. 1 / 1. Introduction - extractive body cue:** A standard input representation for convolutional networks on 3D data is a dense 4D tensor: three spatial dimensions and one feature dimension.
- **p. 1 / 1. Introduction - extractive body cue:** Despite these advantages, fully-convolutional networks have not been widely used for 3D geometric feature extraction due to the characteristics of 3D data.
...
- **p. 8 / 6.5. Effect of Margins for Hardest-contrastive - extractive body cue:** This is because a high-resolution point cloud increases the specificity of the registration, which leads to lower translation error.
...
- **p. 7 / 6.4. Hardest-contrastive and Hardest-triplet Losses - extractive body cue:** We used L2 normalization to project features to the surface of a hypersphere and pass the gradient from the loss through the normalization layer to ...
- **p. 4 / 5. Implementation - extractive body cue:** All these preprocessing steps can be parallelized in data-loading parallel processes and consume a fraction of the training time.
...
- **p. 5 / 6.1. Datasets and Training - extractive body cue:** We found rotation augmentation to be a simple (SO(3) multiplication) and effective way to make FCGF invariant to relative camera pose change.
...
- **p. 4 / 5. Implementation - extractive body cue:** We use hash-based filtering to efficiently remove false negatives from the hard negative mining step to implement I(i, ji).
