...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: We used a simple parameterized controller that generates foot trajectories in the form of a sine wave; the corresponding joint positions were computed using inverse kinematics.
- 출발 문제 단서: Legged robotic systems are attractive alternatives to tracked/wheeled robots for applications in rough terrain and complex cluttered environments.
- 주장된 효과 단서: We used the same velocity command profile which results in the base velocity shown in fig.
...
- Result cue: We used the same velocity command profile which results in the base velocity shown in fig.
