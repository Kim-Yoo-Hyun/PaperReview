...
- **p. 6 / 4. Multi-level hierarchy - extractive body cue:** A humanoid robot must accomplish a collection of operational tasks while satisfying several constraints acting on the robot's body.
- **p. 6 / 4. Multi-level hierarchy - extractive body cue:** Let us suppose there are N behavioral primitives (constraints, operational tasks, and postures) controlling the robot's behavior at a given time.
- **p. 7 / 4. Multi-level hierarchy - extractive body cue:** (16) The dynamic behavior in task space can be obtained by projecting the robot's joint dynamics into the associated task space, i.e.
...
- **p. 9 / 4.3. Movement feasibility - extractive body cue:** In the case that an active task becomes ill-conditioned we say that the robot's movement is infeasible.
- **p. 9 / 4.3. Movement feasibility - extractive body cue:** This complex behavior is complemented with additional tasks to control the robot's center of gravity and to maintain a body symmetry posture.
...
- **p. 3 / Figure/Table caption - extractive body cue:** Fig. 1. Task and posture decomposition: In this sequence, we control the robot's hands to grab a box while maintaining body self-balance (based on the ...
- **p. 5 / Figure/Table caption - extractive body cue:** Fig. 2. Collision avoidance and control of multiple task primitives: This sequence depicts a robot avoiding an obstacle that is moved interactively towards several points ...
- **p. 10 / Figure/Table caption - extractive body cue:** Fig. 3. Hand position control under joint limit constraints: In this sequence, the robot is commanded to reach a target position with its right hand ...
- **p. 11 / Figure/Table caption - extractive body cue:** Fig. 4. Data recorded when the center of gravity and the hand position tasks share priority: When the knees flexion and right elbow joint-limits are ...
- **p. 12 / Figure/Table caption - extractive body cue:** Fig. 5. Data recorded when the center of gravity task precedes the hand position task: The center of gravity error stays small (a) when the ...
...
| Observation/sensor | proprioception, reference pose/motion, visual or language command | calibration, preprocessing, privileged input | p. 1 (1. Introduction), p. 1 (Body text (section not recovered)) |
| Output/decision | joint/whole-body action, motion target 또는 task trajectory | action frame, controller and termination | p. 2 (1. Introduction), p. 5 (3. Integration of constraints) |
...
- **p. 9 / 4.3. Movement feasibility - extractive body cue:** For this simulated experiment, we use a humanoid robot model consisting of 24 degrees of freedom: 2×6 for the legs, 2×4 for the arms, 2 ...
- **p. 12 / X Direction - extractive body cue:** When the hip, elbow, and knee flexion joint limits are reached at t = 0.9 s, 1 s, and 1.2 s respectively, the center of ...
- **p. 4 / 3. Integration of constraints - extractive body cue:** The control of robots under constraints has been investigated since the mid 1970s.
- **p. 8 / 4.1. Recursive null-spaces - extractive body cue:** December 19, 2005 17:13 WSPC/INSTRUCTION FILE ijhr-II-v4 8 This mathematical constraint leads to the following unique solution Nprec(k) = I - k-1 X i=1 Ji/prec(i)Ji/prec(i), ...
