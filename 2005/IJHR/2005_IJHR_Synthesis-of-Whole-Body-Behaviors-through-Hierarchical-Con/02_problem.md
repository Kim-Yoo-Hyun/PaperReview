...
- **p. 1 / 1. Introduction - extractive PDF cue:** Controlling humanoids in these environments requires us to synthesize and change complex whole-body behaviors on-demand in the presence of high uncertainty.
- **p. 1 / 1. Introduction - extractive PDF cue:** To synthesize whole-body behaviors on-demand we have developed a behavior-oriented methodology where multiple behavioral primitives are controlled simultaneously.
- **p. 1 / 1. Introduction - extractive PDF cue:** New behaviors are created by adding or removing individual, or collections of, pre-designed behavioral primitives, without the need to interrupt the movement.
...
- **p. 2 / 1. Introduction - extractive PDF cue:** Section 4 presents a multi-level prioritized framework that allows us to establish multiple priority levels among categories.
- **p. 2 / 1. Introduction - extractive PDF cue:** In this paper, we will describe in detail this hierarchy based on projecting the control of lower priority primitives into the motion null-space of higher ...
...
| Observation / input | Emerging applications of humanoids demand higher and higher degrees of autonomy for efficient interactions in human-populated environments. | proprioception, reference pose/motion, visual or language command | exact sensor/frame/preprocessing from PDF |
...
| Output / action | December, WSPC/INSTRUCTION, FILE, ijhr-II-v4, being, instantaneous, kinematic, solutions | joint/whole-body action, motion target 또는 task trajectory | exact unit/frame/decoder require body check |
...
|---|---|---|
| State / observation variable | whole-body pose/contact/reference state; body terms: Emerging, applications, humanoids, demand, higher, degrees, autonomy, efficient, interactions, human-populated | p. 1 (1. Introduction), p. 2 (1. Introduction), p. 5 (3. Integration of constraints) |
...
| Objective / loss / cost | tracking/balance/task objective; cue terms: accomplish, task, handle, constraint, efficiently, apply, control, described | p. 6 (3. Integration of constraints), p. 7 (4. Multi-level hierarchy), p. 7 (4. Multi-level hierarchy), p. 1 (1. Introduction), p. 2 (1. Introduction), p. 2 (1. Introduction) |
| Constraint / feasibility | paper-specific constraints are recorded only where the body states them; otherwise unresolved | p. 6 (4. Multi-level hierarchy), p. 1 (1. Introduction), p. 2 (1. Introduction) |
...
- **p. 6 / 4. Multi-level hierarchy - extractive PDF cue:** We propose a multi-level control hierarchy that extends the task and posture decomposition previously described.
- **p. 1 / 1. Introduction - extractive PDF cue:** New behaviors are created by adding or removing individual, or collections of, pre-designed behavioral primitives, without the need to interrupt the movement.
...
humanoid writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 1 (1. Introduction), p. 2 (1. Introduction), p. 5 (3. Integration of constraints), p. 8 (4.3. Movement feasibility). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (1. Introduction), p. 2 (1. Introduction), p. 2 (1. Introduction), interface p. 1 (1. Introduction), p. 2 (1. Introduction), p. 5 (3. Integration of constraints), p. 8 (4.3. Movement feasibility), objective p. 6 (3. Integration of constraints), p. 7 (4. Multi-level hierarchy), p. 7 (4. Multi-level hierarchy), p. 1 (1. Introduction), p. 2 (1. Introduction), p. 2 (1. Introduction).
