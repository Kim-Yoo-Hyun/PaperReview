...
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** We obtain the best FID results with a small amount of guidance (w = 0.1 or w = 0.3, depending on the dataset) and the ...
- **p. 8 / 4 EXPERIMENTS - extractive PDF cue:** Based on these findings, we conclude that only a relatively small portion of the model capacity of the diffusion model needs to be dedicated to ...
...
- **p. 5 / 4 EXPERIMENTS - extractive PDF cue:** We train diffusion models with classifier-free guidance on area-downsampled class-conditional ImageNet (Russakovsky et al., 2015), the standard setting for studying tradeoffs between FID and Inception ...
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** We consider w ∈{0, 0.1, 0.2, . . . , 4} and calculate FID and Inception Scores with 50000 samples for each value following the ...
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** At w = 0.3, our model's FID score on 128 × 128 ImageNet outperforms the classifier-guided ADM-G, and at w = 4.0, our model outperforms ...
- **p. 8 / 4 EXPERIMENTS - extractive PDF cue:** Because we used the same model architecture as ADM-G, the fair comparison in terms of sampling speed would be our T = 128 setting, which ...
- **p. 5 / 4 EXPERIMENTS - extractive PDF cue:** The purpose of our experiments is to serve as a proof of concept to demonstrate that classifier-free guidance is able to attain a FID/IS tradeoff ...
...
- **p. 1 / Figure/Table caption - extractive PDF cue:** Figure 1: Classifier-free guidance on the malamute class for a 64x64 ImageNet diffusion model. Left to right: increasing amounts of classifier-free guidance, starting from non-guided ...
- **p. 2 / Figure/Table caption - extractive PDF cue:** Figure 2: The effect of guidance on a mixture of three Gaussians, each mixture component represent- ing data conditioned on a class. The leftmost plot ...
- **p. 6 / Figure/Table caption - extractive PDF cue:** Figure 3: Classifier-free guidance on 128x128 ImageNet. Left: non-guided samples, right: classifier- free guided samples with w = 3.0. Interestingly, strongly guided samples such as ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Table 1: ImageNet 64x64 results (w = 0.0 refers to non-guided models). 50 100 150 200 250 0 10
- **p. 7 / Figure/Table caption - extractive PDF cue:** Figure 4: IS/FID curves over guidance strengths for ImageNet 64x64 models. Each curve represents a model with unconditional training probability puncond. Accompanies Table 1. 7
- **p. 8 / Figure/Table caption - extractive PDF cue:** Table 2: ImageNet 128x128 results (w = 0.0 refers to non-guided models). 8
- **p. 9 / Figure/Table caption - extractive PDF cue:** Figure 5: IS/FID curves over guidance strengths for ImageNet 128x128 models. Each curve represents sampling with a different number of timesteps T. Accompanies Table 2. ...
- **p. 12 / Figure/Table caption - extractive PDF cue:** Figure 6: Classifier-free guidance on ImageNet 64x64. Left: random classes. Right: single class (malamute). The same random seed was used for sampling in each subfigure. ...
...
- **p. 6 / 4 EXPERIMENTS - extractive PDF cue:** We apply our proposed classifier-free guidance to 64×64 and 128×128 class-conditional ImageNet generation.
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** We consider w ∈{0, 0.1, 0.2, . . . , 4} and calculate FID and Inception Scores with 50000 samples for each value following the ...
...
- **p. 7 / 4 EXPERIMENTS - extractive PDF cue:** The 64 × 64 models used sampler noise interpolation coefficient v = 0.3 and were trained for 400 thousand steps; the 128 × 128 models ...
