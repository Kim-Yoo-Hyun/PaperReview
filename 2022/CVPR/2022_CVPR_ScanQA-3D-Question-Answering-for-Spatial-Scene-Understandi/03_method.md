...
- **p. 6 / Model - extractive body cue:** We used a bottom-up top-down attention model [4] to extract the appearance features of the objects.
- **p. 6 / Model - extractive body cue:** The proposed method uses some of the modules used in MCAN, such as transformer encoder and decoder layers, to create 3D and language features.
- **p. 7 / Model - extractive body cue:** Thus, we used a pretrained ScanRefer model to identify the object corresponding to the question and then applied 2D-QA (MCAN) to the image surrounding the ...
- **p. 4 / 4. ScanQA Model - extractive body cue:** The 3D & language fusion layer combines multiple 3D object features guided by language information using transformerbased encoder and decoder layers [44, 51].
- **p. 5 / 4. ScanQA Model - extractive body cue:** To consider multiple answers, we compute final scores with the binary cross-entropy (BCE) loss function to train the module.
...
- **p. 5 / 4. ScanQA Model - extractive body cue:** To consider multiple answers, we compute final scores with the binary cross-entropy (BCE) loss function to train the module.
- **p. 5 / 4. ScanQA Model - extractive body cue:** To answer the 3D scene content, we additionally use the answer loss Lans of the answer classification module.
...
- **p. 4 / 4. ScanQA Model - extractive body cue:** The 3D-QA is formalized as follows: given inputs of the point cloud p ∈P and question q ∈Q about the 3D scene, the 3D-QA model ...
- **p. 4 / 4. ScanQA Model - extractive body cue:** We project a series of output states from the LSTM using a nonlinear layer with GELUs [21] activation to obtain the contextualized word representation Q′ ...
- **p. 5 / 4. ScanQA Model - extractive body cue:** Given a point cloud and RGB frame sequence that capture indoor scenes, the QA model outputs a corresponding answer by fusing 3D and language information ...
- **p. 1 / 1. Introduction - extractive body cue:** Given inputs of an entire 3D modeling and a linguistic question, models predict an answer phrase and the corresponding 3D-bounding boxes. suitcase located?", the existing ...
- **p. 5 / 4. ScanQA Model - extractive body cue:** Subsequently, the final outputs of the transformer layers Qenc and V dec are fused by a fusion layer that uses twolayer multi-layer perceptron (MLP) with ...
- **p. 7 / Model - extractive body cue:** Specifically, the input to the method is the object proposal feature of ScanRefer for VQA models; subsequently, the model predicts answers based on object box ...
- **p. 7 / Model - extractive body cue:** Unlike the ScanQA model, this uses the output of VoteNet separately for object localization and QA modules (although the information for both tasks is mutually ...
...
- **p. 7 / Model - extractive body cue:** Thus, we used a pretrained ScanRefer model to identify the object corresponding to the question and then applied 2D-QA (MCAN) to the image surrounding the ...
- **p. 5 / 5.1. Experimental Setup - extractive body cue:** To train the ScanQA model, we used Adam [26], a batch size of 16, and an initial learning rate of 5e-4.
...
- **p. 6 / Model - extractive body cue:** We applied pretrained 2D-QA models to these images and computed the answer scores for each image.
...
- **p. 13 / Figure/Table caption - extractive body cue:** Table 7. Feature ablation results on ScanQA (multiple) calization scores with the ground true boxes and consider positive predictions for the box with the highest ...
- **p. 7 / 5.2. Quantitative Analysis - extractive body cue:** We will clarify this point in the section on the ablation study.
- **p. 7 / 5.2. Quantitative Analysis - extractive body cue:** In addition, we observed that our 3D-QA model, ScanQA, is superior to a 2D-QA model, RandomImage+MCAN, which uses an effective pretrained model.
- **p. 3 / 3.2. Question-Answer Collection - extractive body cue:** Therefore, we decided to remove such questions as much as possible.
- **p. 3 / 3.1. 3D-QA Task - extractive body cue:** This prevents models from answering questions by relying on the textual priors of the trained questions without examining the scene.
- **p. 4 / 3.3. Dataset Statistics - extractive body cue:** Therefore, the ScanQA dataset includes two test sets with and without object annotations.
...
- **Evidence anchors reviewed:** method p. 4 (4. ScanQA Model), p. 5 (4. ScanQA Model), p. 5 (4. ScanQA Model), p. 6 (Model), p. 6 (Model), p. 7 (Model), objective p. 5 (4. ScanQA Model), p. 5 (4. ScanQA Model), temporal p. 4 (4. ScanQA Model), p. 5 (4. ScanQA Model), p. 1 (1. Introduction), p. 4 (4. ScanQA Model).
