...
- **p. 4 / 4. Experiments - extractive body cue:** We evaluate MonoScene on popular real-world SSC datasets being, indoor NYUv2 [58] and outdoor Se4
- **p. 5 / 4.2.1 Evaluation - extractive body cue:** 1 reports performance of MonoScene and RGBinferred baselines for NYUv2 (test set) and SemanticKITTI official benchmark (hidden test set).
- **p. 5 / 4.2.1 Evaluation - extractive body cue:** On individual classes, MonoScene performs either best or second, excelling on large structural classes for both datasets (e.g. floor, wall ; road, building).
- **p. 7 / 4.2.1 Evaluation - extractive body cue:** MonoScene better captures the scene layout on both datasets.
- **p. 7 / 4.2.1 Evaluation - extractive body cue:** In both, the input is shown left and the camera viewing frustum is shown in the ground truth (rightmost) with darker colors being parts of ...
- **p. 8 / 4.3. Ablation studies - extractive body cue:** Outputs of MonoScene when trained on Sem.KITTI having horizontal FOV of 82◦, and tested on datasets with decreasing (left) or increasing (right) FOV. when compared ...
- **p. 6 / 4.2.1 Evaluation - extractive body cue:** SC SSC Method Input IoU ■ceiling (1.37%) ■floor (17.58%) ■wall (15.26%) ■window (1.99%) ■chair (3.01%) ■bed (7.08%) ■sofa (4.70%) ■table (4.31%) ■tvs (0.47%) ■furniture (30.04%) ...
- **p. 6 / 4.2.1 Evaluation - extractive body cue:** 2b), the baselines clearly surpass us in all metrics which relates both to the lidar-originated 3D input having a much wider horizontal FOV than the ...
...
- **p. 1 / Figure/Table caption - extractive body cue:** Figure 1. RGB Semantic Scene Completion with MonoScene. Our framework infers dense semantic scenes, hallucinating scenery outside the field of view of the image (dark ...
...
- **p. 3 / Figure/Table caption - extractive body cue:** Figure 3. Features Line of Sight Projection (FLoSP). We project multi-scale 2D features F1:s 2D (here, s ∈{1, 2, 4, 8}) along their line of ...
- **p. 3 / Figure/Table caption - extractive body cue:** Figure 4. 2D illustration of 4-way relations. (a) We consider voxel↔voxel relations whether one is free or both are occupied, and if their semantics is ...
- **p. 4 / Figure/Table caption - extractive body cue:** Figure 5. 3D Context Relation Prior (3D CRP). We infer re- lation matrices ˆAm (here, 4), where each encodes a unique re- lation m ∈M ...
...
- **p. 6 / Figure/Table caption - extractive body cue:** Table 1. Performance on (a) NYUv2 [58] and (b) SemanticKITTI [3]. We report the performance on semantic scene completion (SSC - mIoU) and scene completion ...
- **p. 6 / Figure/Table caption - extractive body cue:** Table 2. 2.5/3D input baselines. Despite a single RGB, MonoScene still outperforms the mIoU of some indoor baselines. cars in rows 1-3 having better shapes. ...
...
- **p. 5 / 4. Experiments - extractive body cue:** We use RGB image of cam2 of size 1226x370, left cropped to 1220×370.
- **p. 5 / 4. Experiments - extractive body cue:** Unless otherwise mentioned, we use FLoSP at scales (1,2,4,8), 4 supervised relations for 3D CRP (i.e. n=4, with Lrel), and ℓ×ℓ=8×8 frustums for Lfp.
...
- **p. 6 / 4.2.1 Evaluation - extractive body cue:** 2a) we still beat the recent LMSCNet and AICNet in mIoU by NYUv2 SemanticKITTI IoU ↑ mIoU ↑ IoU ↑ mIoU ↑ Ours 42.51 ±0.15 ...
- **p. 8 / 4.3. Ablation studies - extractive body cue:** 2D scales (S) IoU ↑ mIoU ↑ 1, 2, 4, 8 42.51 ±0.15 26.94 ±0.10 1, 2, 4 42.08 ±0.69 26.28 ±0.24 1, 2 41.56 ...
- **p. 8 / 4.3. Ablation studies - extractive body cue:** Pool & Deconv E D Ours-light FLoSP E D (a) Architectures (2D or 3D) IoU ↑ mIoU ↑ CoReNet (1,2,4) 30.60 ±0.46 17.34 ±0.37 Ours-light ...
