...
- **p. 3 / 3.1. Features Line of Sight Projection (FLoSP) - extractive PDF cue:** We argue this enables 2D-3D disentangled representations, providing the 3D network with the freedom to use high-level 2D features for fine-grained 3D disambiguation.
- **p. 2 / 3. Method - extractive PDF cue:** First, a Scene-Class Affinity Loss (Sec.
- **p. 3 / 3.2. 3D Context Relation Prior (3D CRP) - extractive PDF cue:** We then optimize a weighted multilabel binary cross entropy loss: Lrel=- X m∈M,i [(1-Am i ) log(1- ˆAm i )+wmAm i log ˆAm i ], ...
- **p. 4 / 3.2. 3D Context Relation Prior (3D CRP) - extractive PDF cue:** The feature dimension is omitted for clarity.
- **p. 4 / 3.3. Losses - extractive PDF cue:** We now introduce new losses pursuing distinct global (Sec.
- **p. 4 / 3.3. Losses - extractive PDF cue:** For more generality, our loss Lscal maximizes the above class-wise metrics with: Lscal(ˆp, p) = -1
- **p. 4 / 3.4. Training strategy - extractive PDF cue:** MonoScene is trained end-to-end from scratch by optimizing our 4 losses and the standard cross-entropy (Lce): Ltotal = Lce + Lrel + Lsem scal + ...
...
- **p. 3 / 3.2. 3D Context Relation Prior (3D CRP) - extractive PDF cue:** We then optimize a weighted multilabel binary cross entropy loss: Lrel=- X m∈M,i [(1-Am i ) log(1- ˆAm i )+wmAm i log ˆAm i ], ...
- **p. 4 / 3.4. Training strategy - extractive PDF cue:** MonoScene is trained end-to-end from scratch by optimizing our 4 losses and the standard cross-entropy (Lce): Ltotal = Lce + Lrel + Lsem scal + ...
- **p. 2 / 3. Method - extractive PDF cue:** Second, a Frustum Proportion Loss (Sec.
- **p. 2 / 3. Method - extractive PDF cue:** First, a Scene-Class Affinity Loss (Sec.
- **p. 3 / 3.2. 3D Context Relation Prior (3D CRP) - extractive PDF cue:** This writes, V↔ν = {ν1↔ν, . . . , νs3↔ν}̸=, (2) where {·}̸= returns distinct elements of a set.
...
- **p. 3 / 3.1. Features Line of Sight Projection (FLoSP) - extractive PDF cue:** The output map F3D is used as 3D UNet input.
- **p. 2 / 3. Method - extractive PDF cue:** This has been almost exclusively addressed with 2.5D or 3D inputs [56], such as point cloud, depth or else, which act as strong geometrical cues.
- **p. 2 / 3. Method - extractive PDF cue:** The 2D UNet bases on a pre-trained EfficientNetB7 [61] taking as input the image xrgb.
- **p. 3 / 3.2. 3D Context Relation Prior (3D CRP) - extractive PDF cue:** It takes as input a 3D map of spatial dimension HxWxD, on which is applied a serie of ASPP convolutions [7] to gather a large ...
- **p. 1 / 1. Introduction - extractive PDF cue:** Here, we present MonoScene which - unlike the literature - relies on a single RGB image to infer the dense 3D voxelized semantic scene working ...
- **p. 4 / 3.2. 3D Context Relation Prior (3D CRP) - extractive PDF cue:** The matrices are multiplied with the supervoxels features to gather context, and later combined (concate, conv, DDR [40]) with input features.
- **p. 1 / 1. Introduction - extractive PDF cue:** 1, where it outperformed all comparable baselines and even some 3D input baselines.
...
- **p. 8 / 4.3. Ablation studies - extractive PDF cue:** To properly evaluate only the effect of features projection, we remove our other components, producing a light version (‘Ours-light') with the same 2D encoder (E), ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Figure 8. Type of 2D-3D features projections. (a) Comparing our FLoSP and ‘Ray-traced skip connections' from CoReNet [52] (cf. text) shows in (b) we get ...
- **p. 7 / 4.3. Ablation studies - extractive PDF cue:** We now study in-depth the effect of FLoSP (Sec.
- **p. 5 / 4. Experiments - extractive PDF cue:** Main results are from the hidden test set (online server), and ablations are from the validation set.
- **p. 6 / Figure/Table caption - extractive PDF cue:** Table 3. Architecture ablation. Our components boost perfor- mance on NYUv2 [58] (test set) and SemanticKitti [3] (val. set). a comfortable margin (+6.48 and +3.17), ...
- **p. 5 / 4.1. Baselines - extractive PDF cue:** We use the pretrained AdaBin [4] to infer a depth map (ˆxdepth) serving as input for AICNetrgb.
- **p. 6 / 4.2.1 Evaluation - extractive PDF cue:** (0.05%) ■fence (3.90%) ■pole (0.29%) ■traf.-sign (0.08%) mIoU LMSCNetrgb [55] ˆxocc 31.38 46.70 19.50 13.50 3.10 10.30 14.30 0.30 0.00 0.00 0.00 10.80 0.00 10.40 ...
...
- **PDF anchors reviewed:** method p. 2 (3. Method), p. 3 (3.1. Features Line of Sight Projection (FLoSP)), p. 2 (3. Method), p. 3 (3.2. 3D Context Relation Prior (3D CRP)), p. 4 (3.2. 3D Context Relation Prior (3D CRP)), p. 4 (3.3. Losses), objective p. 4 (3.3. Losses), p. 3 (3.2. 3D Context Relation Prior (3D CRP)), p. 4 (3.4. Training strategy), p. 2 (3. Method), p. 2 (3. Method), p. 3 (3.2. 3D Context Relation Prior (3D CRP)), temporal p. 3 (3.2. 3D Context Relation Prior (3D CRP)), p. 5 (4. Experiments), p. 5 (4. Experiments), p. 6 (4.2.1 Evaluation), p. 7 (4.3. Ablation studies), p. 8 (4.3. Ablation studies).
