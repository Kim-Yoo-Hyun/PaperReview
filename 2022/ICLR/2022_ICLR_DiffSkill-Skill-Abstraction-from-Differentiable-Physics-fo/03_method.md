...
PDF body method statement (p. 3 (2 METHOD), p. 3 (2 METHOD), p. 2 (2 METHOD), p. 4 (2 METHOD), p. 4 (2 METHOD), p. 5 (2 METHOD)): Given an initial state s0, a goal state sg and the transition dynamics p of a differentiable simulator, we use gradient-based trajectory optimization to solve for an open-loop action sequence ...
...
|---|---|---|---|---|---|---|
| Geometry / affordance state | object와 contact-relevant scene을 표현한다 | RGB-D, point cloud, object/task observation | pose, affordance, grasp/contact graph 또는 SE(3) descriptor를 구성 | object/contact state | Given an initial state s0, a goal state sg and the transition dynamics p of a differentiable simulator, we use gradient-based trajectory ... | p. 3 (2 METHOD), p. 3 (2 METHOD) |
| Grasp / trajectory generation | goal을 feasible manipulation candidate로 바꾼다 | geometry/contact state와 task goal | grasp sampling, pose planning, trajectory optimization 또는 policy decoding을 적용 | grasp, pose, force 또는 trajectory | Published as a conference paper at ICLR 2022 f(o, g) s0 sim s1 a0 ... sT sim back propagation Loss a1 policy ... | p. 3 (2 METHOD), p. 2 (2 METHOD) |
| Contact execution / correction | interaction outcome으로 action을 닫힌 loop로 수정한다 | candidate와 visual/force/tactile feedback | tracking, regrasp, correction, termination 또는 recovery를 수행 | next action/task state | Since it is not feasible to directly use a standalone differentiable physics solver to find an optimal solution for long-horizontal tasks, we ... | p. 2 (2 METHOD), p. 4 (2 METHOD) |
...
- **Equation/algorithm anchors:** p. 5 (2 METHOD), p. 5 (2 METHOD), p. 3 (2 METHOD), p. 14 (A IMPLEMENTATION DETAILS), p. 3 (2 METHOD), p. 4 (2 METHOD).
...
| Objective/constraint | Published, conference, ICLR, back, propagation, Loss, policy, feasibility, predictor, skill | task/contact/pose objective | equation anchor required |
...
- **PDF anchors reviewed:** method p. 3 (2 METHOD), p. 3 (2 METHOD), p. 2 (2 METHOD), p. 4 (2 METHOD), p. 4 (2 METHOD), p. 5 (2 METHOD), objective p. 3 (2 METHOD), p. 14 (A IMPLEMENTATION DETAILS), p. 5 (2 METHOD), p. 5 (2 METHOD), p. 3 (2 METHOD), p. 4 (2 METHOD), temporal p. 4 (2 METHOD), p. 3 (2 METHOD), p. 6 (3 EXPERIMENTS), p. 14 (A IMPLEMENTATION DETAILS), p. 2 (2 METHOD), p. 2 (2 METHOD).
