...
| Observation / input | Our neural skill abstraction consists of a goal-conditioned policy that takes a sensory observation (RGB-D images in our case) as input, a ... | RGB-D/point cloud, object state와 contact/task observation | exact sensor/frame/preprocessing from PDF |
...
| Output / action | Published, conference, ICLR, back, propagation, Loss, policy, feasibility | grasp, pose, force 또는 end-effector trajectory | exact unit/frame/decoder require body check |
...
|---|---|---|
| State / observation variable | object geometry/contact state; body terms: neural, skill, abstraction, consists, goal-conditioned, policy, takes, sensory, observation, RGB-D | p. 4 (2 METHOD), p. 2 (1 INTRODUCTION), p. 3 (2 METHOD) |
...
| Objective / loss / cost | task/contact/pose objective; cue terms: Published, conference, ICLR, back, propagation, Loss, policy, feasibility | p. 5 (2 METHOD), p. 5 (2 METHOD), p. 3 (2 METHOD), p. 14 (A IMPLEMENTATION DETAILS), p. 3 (2 METHOD), p. 4 (2 METHOD) |
| Constraint / feasibility | paper-specific constraints are recorded only where the body states them; otherwise unresolved | p. 5 (2 METHOD), p. 3 (2 METHOD), p. 4 (2 METHOD) |
...
PDF contribution framing (p. 2 (1 INTRODUCTION), p. 2 (1 INTRODUCTION), p. 4 (2 METHOD), p. 4 (2 METHOD), p. 1 (1 INTRODUCTION)): Our method consists of three components, (1) a trajectory optimizer that acts as an expert that applies gradient-based optimization on the differentiable simulator to obtain demonstration trajectories, which requires the ...
...
manipulation writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 4 (2 METHOD), p. 2 (1 INTRODUCTION), p. 3 (2 METHOD), p. 2 (2 METHOD). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (1 INTRODUCTION), p. 2 (1 INTRODUCTION), p. 2 (1 INTRODUCTION), p. 1 (1 INTRODUCTION), interface p. 4 (2 METHOD), p. 2 (1 INTRODUCTION), p. 3 (2 METHOD), p. 2 (2 METHOD), objective p. 5 (2 METHOD), p. 5 (2 METHOD), p. 3 (2 METHOD), p. 14 (A IMPLEMENTATION DETAILS), p. 3 (2 METHOD), p. 4 (2 METHOD).
