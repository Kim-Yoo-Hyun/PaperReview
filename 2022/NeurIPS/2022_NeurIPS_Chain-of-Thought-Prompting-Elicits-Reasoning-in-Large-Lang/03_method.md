...
- **p. 3 / 1 Introduction - extractive body cue:** First, chain of thought, in principle, allows models to decompose multi-step problems into intermediate steps, which means that additional computation can be allocated to problems ...
...
- **p. 3 / 1 Introduction - extractive body cue:** Strikingly, chainof-thought prompting when used with the 540B parameter language model performs comparably with task-specific finetuned models on several tasks, even achieving new state of ...
- **p. 2 / 1 Introduction - extractive body cue:** A prompting only approach is important because it does not require a large training dataset and because a single model checkpoint can perform many tasks ...
...
- **p. 2 / 1 Introduction - extractive body cue:** A prompting only approach is important because it does not require a large training dataset and because a single model checkpoint can perform many tasks ...
- **p. 2 / 1 Introduction - extractive body cue:** For rationale-augmented training and finetuning methods, it is costly to create a large set of high quality rationales, which is much more complicated than simple ...
...
- **p. 2 / 1 Introduction - extractive body cue:** That is, instead of finetuning a separate language model checkpoint for each new task, one can simply "prompt" the model with a few input-output exemplars ...
- **p. 2 / 1 Introduction - extractive body cue:** Specifically, we explore the ability of language models to perform few-shot prompting for reasoning tasks, given a prompt that consists of triples: ⟨input, chain of ...
- **p. 3 / 1 Introduction - extractive body cue:** (2020), in which a language model is given in-context exemplars of input-output pairs before outputting a prediction for a test-time example.
- **p. 3 / 1 Introduction - extractive body cue:** Strikingly, chainof-thought prompting when used with the 540B parameter language model performs comparably with task-specific finetuned models on several tasks, even achieving new state of ...
...
- **p. 4 / 1 Introduction - extractive body cue:** Math Word Problems (free response) Math Word Problems (multiple choice) CSQA (commonsense) StrategyQA Date Understanding Sports Understanding Last Letter Concatenation Coin Flip (state tracking) Q: ...
...
- **p. 2 / 1 Introduction - extractive body cue:** This work underscores how large language models can learn via a few examples with natural language data about the task (c.f. automatically learning the patterns ...
- **p. 4 / 1 Introduction - extractive body cue:** For AQuA, we used four exemplars and solutions from the training set, as given in Appendix Table 21.
- **p. 6 / 3.2 Results - extractive body cue:** To confirm that successful chain-of-thought prompting works for other sets of exemplars, we also run experiments with three sets of eight exemplars randomly sampled from ...
...
- **p. 6 / 3.2 Results - extractive body cue:** To isolate the effect of variable computation from chain-of-thought reasoning, we test a configuration where the model is prompted to output a only sequence of ...
- **p. 5 / 3.2 Results - extractive body cue:** 3.3 Ablation Study The observed benefits of using chain-of-thought prompting raises the natural question of whether the same performance improvements can be conferred via other ...
- **p. 6 / 3.2 Results - extractive body cue:** (2021).1 Figure 6 shows these results for LaMDA 137B on GSM8K and MAWPS (ablation results for other datasets are given in Appendix Table 6 / ...
- **p. 17 / Figure/Table caption - extractive body cue:** Figure 10: Examples of semantic understanding and one-step missing errors that were fixed by scaling PaLM from 62B to 540B. A.2 What is the role ...
- **p. 23 / Figure/Table caption - extractive body cue:** Table 7: Ablation and robustness results for four datasets in commonsense and symbolic reasoning. Chain of thought generally outperforms ablations by a large amount. Chain ...
...
- **Evidence anchors reviewed:** method p. 4 (1 Introduction), p. 2 (1 Introduction), p. 2 (1 Introduction), p. 4 (1 Introduction), p. 3 (1 Introduction), p. 8 (6 Discussion), objective p. 2 (1 Introduction), p. 2 (1 Introduction), temporal p. 1 (Abstract), p. 2 (1 Introduction), p. 2 (1 Introduction), p. 3 (1 Introduction), p. 3 (1 Introduction), p. 4 (1 Introduction).
