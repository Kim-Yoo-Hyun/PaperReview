...
- **p. 7 / 3 Experiments - extractive body cue:** For the DEV benchmarks that are used both to validate design decisions and hyperparameters, as well as to report final performance, we therefore use four ...
- **p. 7 / 3 Experiments - extractive body cue:** To account for this, we report performance on an additional set of 11 benchmarks, spanning captioning, video question-answering, as well as some less commonly explored ...
- **p. 31 / Figure/Table caption - extractive body cue:** Table 6: Summary of the evaluation benchmarks. DEV benchmarks were used to validate general design decision of the Flamingo models. Gen. stands for generative task ...
...
- **p. 7 / 3 Experiments - extractive body cue:** Performance estimates on the DEV benchmarks may be biased, as a result of model selection.
...
- **p. 5 / Figure/Table caption - extractive body cue:** Figure 4: GATED XATTN-DENSE layers. To condition the LM on visual inputs, we insert new cross-attention layers between existing pretrained and frozen LM layers. The ...
...
- **p. 2 / Figure/Table caption - extractive body cue:** Figure 1: Selected examples of inputs and outputs obtained from Flamingo-80B. Flamingo can rapidly adapt to various image/video understanding tasks with few-shot prompting (top). Out ...
- **p. 3 / Figure/Table caption - extractive body cue:** Figure 2: Flamingo results overview. Left: Our largest model, dubbed Flamingo, outperforms state-of-the-art fine-tuned models on 6 of the 16 tasks we consider with no ...
...
- **p. 8 / Figure/Table caption - extractive body cue:** Table 3: Ablation studies. Each row should be compared to the baseline Flamingo run (top row). Step time measures the time spent to perform gradient ...
- **p. 23 / Figure/Table caption - extractive body cue:** Figure 5: The Perceiver Resampler module maps a variable size grid of spatio-temporal visual features output by the Vision Encoder to a fixed number of ...
...
- **p. 7 / 3 Experiments - extractive body cue:** On six tasks, Flamingo even outperforms the fine-tuned SotA despite using a single set of model weights and only 32 task-specific examples.
- **p. 5 / 2 Approach - extractive body cue:** For video inputs, frames are sampled at 1 FPS and encoded independently to obtain a 3D spatio-temporal grid of features to which learned temporal embeddings ...
- **p. 6 / 2 Approach - extractive body cue:** From each document, we sample a random subsequence of 𝐿= 256 tokens and take up to the first 𝑁= 5 images included in the sampled ...
- **p. 6 / 2 Approach - extractive body cue:** 2.5 Task adaptation with few-shot in-context learning Once Flamingo is trained, we use it to tackle a visual task by conditioning it on a multimodal ...
...
- **Evidence anchors reviewed:** datasets p. 7 (3 Experiments), p. 7 (3 Experiments), metrics p. 31 (Figure/Table caption), p. 7 (Figure/Table caption), p. 7 (3 Experiments), p. 3 (Figure/Table caption), p. 5 (Figure/Table caption), p. 8 (Figure/Table caption), baselines p. 8 (Figure/Table caption), p. 35 (Figure/Table caption), p. 3 (Figure/Table caption), p. 35 (Figure/Table caption), p. 7 (3 Experiments), p. 7 (3 Experiments), results p. 3 (Figure/Table caption), p. 7 (Figure/Table caption), p. 8 (Figure/Table caption), p. 7 (3 Experiments), p. 5 (Figure/Table caption), p. 35 (Figure/Table caption).
