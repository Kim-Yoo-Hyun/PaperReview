...
| Observation / input | Training language models to follow instructions with human feedback Long Ouyang∗ Jeff Wu∗ Xu Jiang∗ Diogo Almeida∗ Carroll L. | 논문이 명시한 observation과 task input | exact sensor/frame/preprocessing from PDF body |
| State / latent | Training, language, models, follow, instructions, human, feedback, Long, Ouyang, Jeff | task state 또는 decision variable | notation and tensor shape require body check |
| Output / action | Specifically, reinforcement, learning, human, feedback, RLHF, Christiano, Stiennon | paper-specific output/action | exact unit/frame/decoder require body check |
...
|---|---|---|
| State / observation variable | source-defined input o; body terms: Training, language, models, follow, instructions, human, feedback, Long, Ouyang, Jeff | p. 1 (Body text (section not recovered)), p. 1 (Abstract), p. 2 (1 Introduction) |
...
PDF body contribution framing (p. 3 (1 Introduction), p. 4 (1 Introduction), p. 1 (Abstract)): See Section 3 for more details on our method. sizes (1.3B, 6B, and 175B parameters), and all of our models use the GPT-3 architecture.
...
upstream writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. Evidence interface anchors: p. 1 (Body text (section not recovered)), p. 1 (Abstract), p. 2 (1 Introduction), p. 3 (1 Introduction). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **Evidence anchors reviewed:** problem p. 4 (1 Introduction), p. 1 (1 Introduction), p. 4 (1 Introduction), interface p. 1 (Body text (section not recovered)), p. 1 (Abstract), p. 2 (1 Introduction), p. 3 (1 Introduction), objective p. 1 (1 Introduction), p. 2 (1 Introduction), p. 2 (1 Introduction), p. 3 (1 Introduction), p. 4 (1 Introduction).
