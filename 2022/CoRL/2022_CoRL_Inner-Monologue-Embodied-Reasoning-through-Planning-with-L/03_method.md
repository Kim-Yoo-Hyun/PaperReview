...
- **Paper-specific method/interface:** The input to the model consists of: (1) o0, the initial image observation, (2) of, the final image observation after the policy chose to terminate its chosen skill execution, and ... (p. 17, A.2 Inner Monologue for Real-World Tabletop Rearrangement).
- **Objective/update evidence:** To train this model, we use the symmetric contrastive loss as used in CLIP (Fig 7b). (p. 17, A.2 Inner Monologue for Real-World Tabletop Rearrangement).
- **Temporal/runtime evidence:** Unlike the evaluated settings in [76], the demonstrations cover all objects because the focus of this work is not on generalization across object instances but on novel long-horizon behaviors. (p. 15, A.1 Inner Monologue for Simulated Tabletop Rearrangement).
