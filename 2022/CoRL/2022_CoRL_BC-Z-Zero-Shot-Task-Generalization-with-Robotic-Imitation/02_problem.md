...
| Observation / input | Second, our system flexibly conditions the policy on different forms of task specification, including a language instruction or a video of a ... | image/video, language instruction, proprioception과 history | exact sensor/frame/preprocessing from PDF |
...
| Decision / output variable | action, pose, option or chunk a; body terms: properties, have, been, explored, previously, empirically, study, whether | p. 1 (1 Introduction), p. 2 (1 Introduction), p. 8 (7 Discussion) |
...
PDF contribution framing (p. 1 (1 Introduction), p. 2 (1 Introduction), p. 8 (7 Discussion), p. 2 (1 Introduction), p. 1 (1 Introduction)): These properties have been explored previously; our aim is to empirically study whether these ideas scale to a broad range of real-world tasks. *Equal Contribution †Work done while author was ...
...
vla writing domain maps to observation -> state/world model -> task and motion decision -> policy/control -> feedback. PDF interface anchors: p. 1 (1 Introduction), p. 2 (1 Introduction), p. 8 (7 Discussion), p. 1 (1 Introduction). The downstream handoff is claimed only when the body describes it.

## Verification Questions

- **PDF anchors reviewed:** problem p. 1 (1 Introduction), p. 1 (1 Introduction), p. 2 (1 Introduction), interface p. 1 (1 Introduction), p. 2 (1 Introduction), p. 8 (7 Discussion), p. 1 (1 Introduction), objective no optimization/equation sentence selected.
