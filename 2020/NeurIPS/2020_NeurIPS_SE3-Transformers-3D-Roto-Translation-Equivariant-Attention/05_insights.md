...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: We introduce the SE(3)-Transformer, a variant of the self-attention module for 3D point clouds and graphs, which is equivariant under continuous 3D rototranslations.
- 출발 문제 단서: In this paper, we find that the explicit imposition of equivariance constraints on the self-attention mechanism addresses these challenges.
- 주장된 효과 단서: We compare to publicly available, state-of-the-art results as well as a set of our own baselines.
...
- Result cue: We compare to publicly available, state-of-the-art results as well as a set of our own baselines.
