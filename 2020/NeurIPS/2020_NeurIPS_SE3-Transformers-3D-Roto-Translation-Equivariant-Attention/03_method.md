...
- **p. 6 / 3 Method - extractive PDF cue:** These weights are SE(3)-invariant due to the invariance of inner products of features, transforming under the same representation. wℓℓ i,c′c = MLP  M c,c′ ...
- **p. 6 / 3 Method - extractive PDF cue:** Channels, Self-interaction Layers, and Non-Linearities Analogous to conventional neural networks, the SE(3)-Transformer can straightforwardly be extended to multiple channels per representation degree ℓ, so far ...
- **p. 5 / 3 Method - extractive PDF cue:** 3.2 The SE(3)-Transformer The SE(3)-Transformer itself consists of three components.
- **p. 5 / 3 Method - extractive PDF cue:** If we remove the attention weights then we have a tensor field convolution, and if we instead remove the dependence of WV on (xj -xi), ...
- **p. 7 / 3 Method - extractive PDF cue:** Linear DeepSet [46] Tensor Field [28] Set Transformer [16] SE(3)-Transformer MSE x 0.0691 0.0639 0.0151 0.0139 0.0076 Position std - 0.0086 0.0011 0.0004 0.0002 ∆EQ ...
- **p. 1 / 1 Introduction - extractive PDF cue:** Furthermore, an important property is that these structures should be invariant to global changes in overall input pose; that is, 3D translations and rotations of ...
- **p. 6 / 3 Method - extractive PDF cue:** [25], output channels are a learned linear combination of input channels using one set of weights wℓℓ i,c′c = wℓℓ c′c per representation degree, shared ...
- **p. 5 / 3 Method - extractive PDF cue:** (7), and 3) a linear/attentive self-interaction layer.

## Design Rationale

- **p. 1 / 1 Introduction - extractive PDF cue:** In this paper, we propose the SE(3)-Transformer shown in Fig.
...
- **p. 1 / 1 Introduction - extractive PDF cue:** Finding neural structures which can adapt to the varying number of points in an input, while respecting the irregular sampling of point positions, is challenging.
- **p. 7 / 3 Method - extractive PDF cue:** Linear DeepSet [46] Tensor Field [28] Set Transformer [16] SE(3)-Transformer MSE x 0.0691 0.0639 0.0151 0.0139 0.0076 Position std - 0.0086 0.0011 0.0004 0.0002 ∆EQ ...
...
- **p. 8 / 4 Experiments - extractive PDF cue:** Our method sets itself apart by using roto-translation equivariant layers acting directly on the point cloud without prior projection onto a sphere [22, 45, 7].
- **p. 8 / 4 Experiments - extractive PDF cue:** We create an SO(2) invariant version of our algorithm by additionally feeding the z-component as an type-0 field and the x, y position as an ...
...
- **p. 7 / 4 Experiments - extractive PDF cue:** The dashed curves show the predicted locations of a perfectly equivariant model.
- **p. 7 / 4 Experiments - extractive PDF cue:** The N-body problem is an equivariant task: rotation of the input should result in rotated predictions of locations and velocities of the particles.
- **p. 9 / 4 Experiments - extractive PDF cue:** The table is split into non-equivariant (top) and equivariant models (bottom).
...
- **PDF anchors reviewed:** method p. 6 (3 Method), p. 6 (3 Method), p. 5 (3 Method), p. 5 (3 Method), p. 7 (3 Method), objective 본문 anchor 없음, temporal p. 7 (4 Experiments), p. 7 (4 Experiments), p. 5 (3 Method), p. 5 (3 Method), p. 6 (3 Method), p. 6 (3 Method).
