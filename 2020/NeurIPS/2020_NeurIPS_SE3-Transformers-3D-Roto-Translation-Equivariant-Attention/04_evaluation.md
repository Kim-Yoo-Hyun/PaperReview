...
- **p. 8 / 4 Experiments - extractive body cue:** To test our method, we choose ScanObjectNN, a recently introduced dataset for real-world object classification.
- **p. 7 / 4 Experiments - extractive body cue:** Next, we evaluate on a real-world object classification task.
- **p. 8 / 4 Experiments - extractive body cue:** Points 128 1024 128 1024 1024 1024 1024 128 1024 128 1024 1024 1024 Accuracy 63.1% 71.4% 72.8 % 73.8% 74.1% 79.2% 79.5% 81.0% 84.3% ...
- **p. 7 / 4 Experiments - extractive body cue:** [27], the error is not squared: ∆EQ = ∥LsΦ(f) -ΦLs(f)∥2 / ∥LsΦ(f)∥2 (14) 4.1 N-Body Simulations In this experiment, we use an adaptation of the ...
- **p. 9 / 4 Experiments - extractive body cue:** TASK α ∆ε εHOMO εLUMO µ Cν UNITS bohr3 meV meV meV D cal/mol K WaveScatt [11] .160 118 85 76 .340 .049 NMP [10] ...
- **p. 9 / 4 Experiments - extractive body cue:** We show results on the test set of Anderson et al.
- **p. 7 / 4 Experiments - extractive body cue:** The distance between the two, averaged over samples, yields the equivariance error.
- **p. 7 / 4 Experiments - extractive body cue:** The equivariance error shows that our approach is indeed fully rotation equivariant up to the precision of the computations. input label Set Transf. original rotated ...
...
- **p. 7 / Figure/Table caption - extractive body cue:** Table 1: Predicting future locations and velocities in an electron-proton simulation. Linear DeepSet [46] Tensor Field [28] Set Transformer [16] SE(3)-Transformer MSE x 0.0691
- **p. 7 / Figure/Table caption - extractive body cue:** Figure 3: A model based on conventional self-attention (left) and our rotation-equivariant version (right) predict future locations and velocities in a 5-body problem. The respective ...
- **p. 8 / Figure/Table caption - extractive body cue:** Figure 4: ScanObjectNN: x-axis shows data augmentation on the test set. The x-value corresponds to the maximum rotation around a random axis in the x-y-plane. ...
- **p. 8 / Figure/Table caption - extractive body cue:** Table 2: Classification accuracy on the 'object only' category of the ScanObjectNN dataset4. The performance of the SE(3)-Transformer is averaged over 5 runs (standard deviation ...
- **p. 9 / Figure/Table caption - extractive body cue:** Table 3: QM9 Mean Absolute Error. Top: Non-equivariant models. Bottom: Equivariant models. SE(3)-Tr. is averaged over 5 runs. TASK α ∆ε εHOMO εLUMO µ
- **p. 17 / Figure/Table caption - extractive body cue:** Figure 5: Spherical harmonics computation of our own implementation compared to the lie-learn library. We found that speeding up the computation of spherical harmonics is ...
...
- **p. 7 / 4 Experiments - extractive body cue:** The right-hand plots show predicted locations and velocities for rotations of the input in steps of 10 degrees.
- **p. 8 / 4 Experiments - extractive body cue:** The benchmark provides point clouds of 2902 objects across 15 different categories.
- **p. 9 / 4 Experiments - extractive body cue:** TASK α ∆ε εHOMO εLUMO µ Cν UNITS bohr3 meV meV meV D cal/mol K WaveScatt [11] .160 118 85 76 .340 .049 NMP [10] ...
...
- **p. 6 / 3 Method - extractive body cue:** at node i and a set of key vectors {kij}j∈Ni along each edge ij in the neighbourhood Ni where αij = exp(q⊤ i kij) P ...
