...
- **p. 11 / 3 Results - extractive PDF cue:** We omit the 4 Wikipedia-related tasks in that work because they are entirely contained in our training data, and we also omit the one-billion word ...
- **p. 29 / 3 Results - extractive PDF cue:** 4 Measuring and Preventing Memorization Of Benchmarks Since our training dataset is sourced from the internet, it is possible that our model was trained on ...
- **p. 18 / 3 Results - extractive PDF cue:** 3.7 SuperGLUE In order to better aggregate results on NLP tasks and compare to popular models such as BERT and RoBERTa in a more systematic ...
- **p. 33 / 3 Results - extractive PDF cue:** • Language modeling: We found the 4 Wikipedia language modeling benchmarks measured in GPT-2, plus the Children's Book Test dataset, to be almost entirely contained ...
- **p. 10 / 2.4 Evaluation - extractive PDF cue:** For most tasks we compare the per-token likelihood (to normalize for length), however on a small number of datasets (ARC, OpenBookQA, and RACE) we gain ...
- **p. 13 / 3 Results - extractive PDF cue:** We evaluate GPT-3 on the 3 datasets in [RRS20]: Natural Questions [KPR+19], WebQuestions [BCFL13], and TriviaQA [JCWZ17], using the same splits.
- **p. 10 / 2.4 Evaluation - extractive PDF cue:** For LAMBADA and Storycloze there is no supervised training set available so we draw conditioning examples from the development set and evaluate on the test ...
- **p. 13 / 3 Results - extractive PDF cue:** One note of caution is that an analysis of test set contamination identified that a significant minority of the LAMBADA dataset appears to be present ...
...
- **p. 3 / Figure/Table caption - extractive PDF cue:** Figure 1.1: Language model meta-learning. During unsupervised pre-training, a language model develops a broad set of skills and pattern recognition abilities. It then uses these ...
...
- **p. 5 / Figure/Table caption - extractive PDF cue:** Figure 1.3: Aggregate performance for all 42 accuracy-denominated benchmarks While zero-shot performance improves steadily with model size, few-shot performance increases more rapidly, demonstrating that larger ...
...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Figure 2.1: Zero-shot, one-shot and few-shot, contrasted with traditional fine-tuning. The panels above show four methods for performing a task with a language model - ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Figure 2.1 shows the four methods using the example of translating English to French. In this paper we focus on zero-shot, one-shot and few-shot, with ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Table 2.1: Sizes, architectures, and learning hyper-parameters (batch size in tokens and learning rate) of the models which we trained. All models were trained for ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Table 2.1 shows the sizes and architectures of our 8 models. Here nparams is the total number of trainable parameters, nlayers is the total number ...
...
- **p. 10 / 3 Results - extractive PDF cue:** For this graph we also include 6 additional extra-small models with as few as 100,000 parameters.
- **p. 11 / 3 Results - extractive PDF cue:** Our largest model sets a new SOTA on PTB by a substantial margin of 15 points, achieving a perplexity of 20.50.
- **p. 18 / 3 Results - extractive PDF cue:** On OpenBookQA [MCKS18], GPT-3 improves significantly from zero to few shot settings but is still over 20 points short of the overall SOTA.
- **p. 18 / 3 Results - extractive PDF cue:** GPT-3 performs best (within 3 points of the human baseline) on CoQA [RCM19] a free-form conversational dataset and performs worst (13 F1 below an ELMo ...
- **p. 20 / 3 Results - extractive PDF cue:** A value of K = 32 means that our model was shown 32 examples per task, for 256 examples total divided across the 8 tasks ...
- **p. 22 / 3 Results - extractive PDF cue:** In all 10 tasks the model must generate the correct answer exactly.
...
- **p. 33 / 3 Results - extractive PDF cue:** An important limitation of our contamination analysis is that we cannot be sure that the clean subset is drawn from the same distribution as the ...
- **p. 41 / 8 Conclusion - extractive PDF cue:** Despite many limitations and weaknesses, these results suggest that very large language models may be an important ingredient in the development of adaptable, general language ...
- **p. 2 / 3 Results - extractive PDF cue:** 21 4 Measuring and Preventing Memorization Of Benchmarks 29 5 Limitations 33 6 Broader Impacts 34 6.1 Misuse of Language Models . . . . ...
- **p. 4 / Figure/Table caption - extractive PDF cue:** Figure 1.2: Larger models make increasingly efficient use of in-context information. We show in-context learning performance on a simple task requiring the model to remove ...
- **p. 20 / 3 Results - extractive PDF cue:** On COPA and ReCoRD GPT-3 achieves near-SOTA performance in the one-shot and few-shot settings, with COPA falling only a couple points short and achieving second ...
- **p. 24 / 3 Results - extractive PDF cue:** This suggests that the model really does appear to learn these tasks at test time, as the model cannot perform them zero-shot and their artificial ...

- **PDF anchors reviewed:** datasets p. 11 (3 Results), p. 29 (3 Results), p. 18 (3 Results), p. 33 (3 Results), p. 10 (2.4 Evaluation), p. 13 (3 Results), metrics p. 18 (3 Results), p. 10 (2.4 Evaluation), p. 12 (3 Results), p. 19 (3 Results), p. 26 (3 Results), p. 26 (3 Results), baselines p. 18 (3 Results), p. 19 (3 Results), p. 18 (3 Results), p. 20 (3 Results), p. 19 (Figure/Table caption), p. 12 (3 Results), results p. 12 (3 Results), p. 16 (Figure/Table caption), p. 11 (3 Results), p. 13 (3 Results), p. 17 (3 Results), p. 5 (Figure/Table caption).
