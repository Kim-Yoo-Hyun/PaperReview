...
- **p. 4 / III. METHOD - extractive PDF cue:** Optimize surrogate L wrt θ, using ˆs, ˆa, ˆA if KL(πθ(ˆs, ˆa), πθold(ˆs, ˆa)) > klthresh then break end if end for end for end ...
- **p. 3 / III. METHOD - extractive PDF cue:** State Space and Action Space The policy's input consists of: Xt =          fvel desired forward speed ...
- **p. 4 / III. METHOD - extractive PDF cue:** Recurrent Proximal Policy Optimization We trained all policies with PPO, a model-free reinforcement learning algorithm.
- **p. 3 / III. METHOD - extractive PDF cue:** Reward Design Our learning process makes use of a reference trajectory produced by an expert walking controller to help the policy learn in the initial ...
- **p. 3 / III. METHOD - extractive PDF cue:** All policies were trained to maximize the following reward function: R =0.20 · exp(-qerr) +0.20 · exp(-˙xerr) +0.05 · exp(-xerr) +0.20 · exp(-˙yerr) +0.30 · ...
- **p. 3 / III. METHOD - extractive PDF cue:** It is important to note that while the reward function is partially based on a reference motion, very similarly to Xie et al.
- **p. 4 / III. METHOD - extractive PDF cue:** However, correctly calculating the gradient of an RNN requires the use of the backpropagation through time (BPTT) algorithm, which necessitates special measures when sampling from ...
- **p. 4 / III. METHOD - extractive PDF cue:** The recurrent critic was also an LSTM network with two layers of 128 units each, with a single output representing the value function.
...
- **p. 3 / III. METHOD - extractive PDF cue:** State Space and Action Space The policy's input consists of: Xt =          fvel desired forward speed ...
- **p. 2 / II. BACKGROUND - extractive PDF cue:** The policy is often a stochastic policy, in which case it is a function π(a/s) which takes in a state s and outputs the parameters ...
- **p. 3 / III. METHOD - extractive PDF cue:** The outputs of the policy are simply raw motor PD targets, much like Xie et al.
- **p. 4 / III. METHOD - extractive PDF cue:** Network Architecture All policies had an input dimension of length 49, and an output dimension of size 10.
- **p. 4 / III. METHOD - extractive PDF cue:** No information about the dynamics disturbances was provided to either the policy or the critic in the input space.
- **p. 1 / I. INTRODUCTION - extractive PDF cue:** Recent work in transferring these trained controllers from simulation onto real robots has also enjoyed encouraging results [1, 2, 3, 4, 5, 6], but many ...
- **p. 2 / II. BACKGROUND - extractive PDF cue:** The reward r = R(s, a) is a scalar signal that expresses how good a particular state-action pair is.
...
- **p. 4 / III. METHOD - extractive PDF cue:** Recurrent Proximal Policy Optimization We trained all policies with PPO, a model-free reinforcement learning algorithm.
- **p. 3 / III. METHOD - extractive PDF cue:** Reward Design Our learning process makes use of a reference trajectory produced by an expert walking controller to help the policy learn in the initial ...
- **p. 5 / IV. RESULTS - extractive PDF cue:** When training the feedforward policies, we used a batch size of 1024 timesteps.
- **p. 3 / III. METHOD - extractive PDF cue:** Though we believe that the recurrent policies do not have any theoretical reason for needing a clock input, we were not able to train any ...
...
- **p. 5 / IV. RESULTS - extractive PDF cue:** 5: Reward curve of LSTM and FF networks during training without dynamics randomization.
- **p. 5 / IV. RESULTS - extractive PDF cue:** As can be seen in Figure 5, when trained without dynamics randomization, LSTM networks attain a significantly higher reward than feedforward networks, with surprisingly little ...
- **p. 1 / Figure/Table caption - extractive PDF cue:** Fig. 1: We use recurrent neural networks and dynamics randomization to greatly improve the sim-to-real transfer rate and demonstrate learned, memory-based control on the bipedal ...
...
- **PDF anchors reviewed:** method p. 4 (III. METHOD), p. 3 (III. METHOD), p. 4 (III. METHOD), p. 3 (III. METHOD), objective p. 3 (III. METHOD), p. 3 (III. METHOD), p. 4 (III. METHOD), p. 4 (III. METHOD), temporal p. 3 (III. METHOD), p. 4 (III. METHOD), p. 4 (III. METHOD), p. 3 (III. METHOD), p. 1 (Abstract), p. 1 (I. INTRODUCTION).
