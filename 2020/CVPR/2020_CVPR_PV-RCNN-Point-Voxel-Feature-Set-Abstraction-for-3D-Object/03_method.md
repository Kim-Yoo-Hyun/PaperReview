...
- **p. 6 / 3.4. Training losses - extractive PDF cue:** The overall training loss are then the sum of these three losses with equal loss weights.
- **p. 8 / 4.3. 3D Detection on the Waymo Open Dataset - extractive PDF cue:** Effects of different feature components for VSA module. our proposed framework on various datasets.
- **p. 7 / 4.2. 3D Detection on the KITTI Dataset - extractive PDF cue:** Comparison with state-of-the-art methods.
- **p. 7 / Method - extractive PDF cue:** These proposals are further refined in the proposal refinement stage with aggregated keypoint features.
- **p. 8 / 4.3. 3D Detection on the Waymo Open Dataset - extractive PDF cue:** Hard RPN Baseline ✓ 90.46 80.87 77.30 Pool from Encoder ✓ ✓ 91.88 82.86 80.52 PV-RCNN ✓ ✓ ✓ 92.57 84.83 82.69 Table 7.
- **p. 6 / 3.4. Training losses - extractive PDF cue:** Further training loss details are provided in the supplementary file.
- **p. 1 / 1. Introduction - extractive PDF cue:** Our proposed PV-RCNN framework deeply integrates both the voxel-based and the PointNet-based networks via a twostep strategy including the voxel-to-keypoint 3D scene encoding and the ...
- **p. 2 / 1. Introduction - extractive PDF cue:** Therefore, to better integrate these two types of point cloud feature learning networks, we propose a two-step strategy with the first voxel-to-keypoint scene encoding step ...

## Design Rationale

- **p. 1 / 1. Introduction - extractive PDF cue:** We propose a novel 3D object detection framework, PVRCNN (Illustrated in Fig.
- **p. 2 / 1. Introduction - extractive PDF cue:** (2) We propose the voxelto-keypoint scene encoding scheme, which encodes multiscale voxel features of the whole scene to a small set of keypoints by the ...
- **p. 2 / 1. Introduction - extractive PDF cue:** Therefore, to better integrate these two types of point cloud feature learning networks, we propose a two-step strategy with the first voxel-to-keypoint scene encoding step ...
...
- **p. 2 / 1. Introduction - extractive PDF cue:** The features of each keypoint is aggregated by grouping the neighboring voxel-wise features via PointNet-based set abstraction for summarizing multi-scale point cloud information.
- **p. 7 / 4.2. 3D Detection on the KITTI Dataset - extractive PDF cue:** Similarly, as shown in Table 2, our method outperforms previous stateof-the-art methods with large margins.
...
- **p. 8 / 4.2. 3D Detection on the KITTI Dataset - extractive PDF cue:** (LEVEL 2) mAP mAPH mAP mAPH mAP mAPH mAP mAPH mAP mAPH mAP mAPH *StarNet [20] NeurIPSw 2019 53.70 - - - 66.80 - - ...
...
- **p. 6 / 4. Experiments - extractive PDF cue:** 4.4, we conduct extensive ablation studies to investigate each component of PV-RCNN to validate our design.
- **p. 8 / 4.3. 3D Detection on the Waymo Open Dataset - extractive PDF cue:** Effects of different feature components for VSA module. our proposed framework on various datasets.
