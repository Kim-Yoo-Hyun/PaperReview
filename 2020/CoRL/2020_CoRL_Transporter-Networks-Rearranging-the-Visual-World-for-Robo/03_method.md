...
- **p. 5 / 3 Method - extractive PDF cue:** Placing: Our placing model is a two-stream feed-forward FCN that takes as input the visual observation ot ∈RH×W×4 and outputs two dense feature maps: query ...
- **p. 5 / 3 Method - extractive PDF cue:** Picking: Our picking model is a single feed-forward FCN that takes as input the visual observation ot∈RH×W×4 and outputs dense pixel-wise values that correlate with ...
- **p. 4 / 3 Method - extractive PDF cue:** Spatially consistent visual representations enable us to perform visuo-spatial transporting, in which dense pixel-wise features from a partial crop ot[Tpick] centered on Tpick are rigidly ...
- **p. 4 / 3 Method - extractive PDF cue:** We formulate this as a template matching problem, using cross-correlation with dense feature embeddings ψ(·) and φ(·) from two deep models: Qplace(τ/ot,Tpick)=ψ(ot[Tpick])∗φ(ot)[τ], Tplace=argmax {τi} Qplace(τi/ot,Tpick) ...
- **p. 6 / 3 Method - extractive PDF cue:** Our training loss is simply the cross-entropy between these one-hot pixel maps and the outputs of the picking and placing models: L=-EYpick[logVpick]-EYplace[logVplace].
- **p. 6 / 3 Method - extractive PDF cue:** For SE(3) models involving regression, we use a Huber loss on each regression channel.
- **p. 3 / 3 Method - extractive PDF cue:** As in prior works [31, 32, 33], Transporter Networks use fully convolutional networks (FCNs, commonly used in vision for image segmentation tasks [34]) to model ...
- **p. 5 / 3 Method - extractive PDF cue:** 8-stride was chosen to balance between maximizing receptive field coverage per pixel prediction, while minimizing loss of resolution in the latent mid-level features of the ...

## Design Rationale

- **p. 3 / 3 Method - extractive PDF cue:** Transporter Networks decompose the problem into (i) picking and (ii) pick-conditioned placing: fpick(ot)→Tpick fplace(ot,Tpick)→Tplace We present a solution to both with Transporter Networks - but ...
...
- **p. 5 / 3 Method - extractive PDF cue:** 8-stride was chosen to balance between maximizing receptive field coverage per pixel prediction, while minimizing loss of resolution in the latent mid-level features of the ...
- **p. 6 / 3 Method - extractive PDF cue:** While we only have labels for a single pixel per dense probability map, gradients are passed to all other pixels via image-wide softmax.
- **p. 6 / 3 Method - extractive PDF cue:** Our training loss is simply the cross-entropy between these one-hot pixel maps and the outputs of the picking and placing models: L=-EYpick[logVpick]-EYplace[logVplace].
- **p. 3 / 3 Method - extractive PDF cue:** As in prior works [31, 32, 33], Transporter Networks use fully convolutional networks (FCNs, commonly used in vision for image segmentation tasks [34]) to model ...
- **p. 4 / 3 Method - extractive PDF cue:** We formulate this as a template matching problem, using cross-correlation with dense feature embeddings ψ(·) and φ(·) from two deep models: Qplace(τ/ot,Tpick)=ψ(ot[Tpick])∗φ(ot)[τ], Tplace=argmax {τi} Qplace(τi/ot,Tpick) ...
- **p. 5 / 3 Method - extractive PDF cue:** Note that this operation can be made fast if implemented with highly optimized matrix multiplication (where the crop is 5
...
- **p. 5 / 3 Method - extractive PDF cue:** Picking: Our picking model is a single feed-forward FCN that takes as input the visual observation ot∈RH×W×4 and outputs dense pixel-wise values that correlate with ...
- **p. 5 / 3 Method - extractive PDF cue:** Placing: Our placing model is a two-stream feed-forward FCN that takes as input the visual observation ot ∈RH×W×4 and outputs two dense feature maps: query ...
- **p. 2 / 3 Method - extractive PDF cue:** Consider the problem of learning pick-and-place actions at with a robot from visual observations ot: f(ot)→at=(Tpick,Tplace)∈A where Tpick is the pose of the end effector ...
- **p. 1 / 1 Introduction - extractive PDF cue:** In our experiments, Transporter Networks exhibit superior sample efficiency on a number of tabletop manipulation tasks that involve changing the state of the robot's environment ...
- **p. 4 / 3 Method - extractive PDF cue:** In the planar SE(2) case, we discretize the space of SO(2) rotations into k bins, then rotate the input visual observation ot for each bin. ...
- **p. 6 / 3 Method - extractive PDF cue:** 3.3 Training: Learning from Demonstrations We assume access to a dataset D = {ζ1,ζ2,...,ζn} of n expert demonstrations, where each trajectory ζi = {(o0,a0),(o1,a1),...} is ...
- **p. 6 / 3 Method - extractive PDF cue:** During training, we uniformly sample observation-action pairs from the dataset D, from which each action at can be unpacked into two training labels: Tpick and ...
...
- **p. 6 / 3 Method - extractive PDF cue:** Validation performance generally converges after a few hours of training on a single commodity GPU.
- **p. 8 / 4 Results - extractive PDF cue:** Despite COVID-19 lockdowns preventing physical access, we were still able to perform real experiments using our Unitybased [46] UI that enables people to remotely teleoperate ...
...
- **p. 6 / 4 Results - extractive PDF cue:** In this work, we do not use simulation for sim-to-real transfer - rather only as a means to provide a consistent and controlled environment for ...
- **p. 6 / 4 Results - extractive PDF cue:** Some tasks include multi-modality and permutations - for example, in stacking, a successful 6-block pyramid is invariant to the permutation of blocks within each row, ...
- **p. 7 / 4 Results - extractive PDF cue:** In our variant of the kit assembly task with unseen objects (shown in Fig.
- **p. 7 / 4 Results - extractive PDF cue:** We investigate two variants of experts in this setting: (a) one that provides deterministic demonstrations where Tpick, Tplace are fixed relative to the block and ...
...
- **p. 13 / Figure/Table caption - extractive PDF cue:** Figure 7. In Towers of Hanoi (a), the task is to sequentially pick-and-place 3 disks from the first peg to the third peg without placing ...
...
- **PDF anchors reviewed:** method p. 5 (3 Method), p. 5 (3 Method), p. 4 (3 Method), p. 4 (3 Method), p. 6 (3 Method), p. 6 (3 Method), objective p. 5 (3 Method), p. 6 (3 Method), p. 6 (3 Method), p. 3 (3 Method), p. 4 (3 Method), p. 5 (3 Method), temporal p. 3 (3 Method), p. 3 (3 Method), p. 5 (3 Method), p. 5 (3 Method), p. 6 (3 Method), p. 7 (4 Results).
