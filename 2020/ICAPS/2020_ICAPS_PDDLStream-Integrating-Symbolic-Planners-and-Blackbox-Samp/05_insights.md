...
- Problem cue: Symbolic action sequence와 continuous kinematic, collision, visibility, pose constraint를 한 planning problem에서 결합해야 한다.
- Method cue: PDDL에 declarative black-box sampling procedure인 stream을 추가한다.
...
`TrajOpt: A Sequential Convex Optimization Algorithm for Robot Motion Planning → PDDLStream: Integrating Symbolic Planners and Blackbox Samplers via Optimistic Adaptive Planning → Dynamic Whole-Body Motion Generation under Rigid Contacts and Other Unilateral Constraints`
