...
## 이 논문에서 가져갈 핵심 개념
- 핵심 방법 단서: Our architecture consists of two main modules: 1) detection & encoding; 2) fusion & localization (Fig.
- 출발 문제 단서: In recent years, there has been tremendous progress in both semantic understanding and localization of objects in 2D images from natural language (also known as visual grounding).
- 주장된 효과 단서: Results and analysis are conducted on the val split (except for results in Tab.
...
- Result cue: Results and analysis are conducted on the val split (except for results in Tab.
