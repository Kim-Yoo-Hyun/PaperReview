...
- **p. 13 / 5 Experiments - extractive PDF cue:** This Cross-Modal Attention PM+DA*+Aug model achieves an SPL of 0.35 on val-seen and 0.30 on val-unseen - succeeding on 32% of episodes in new environments.
- **p. 11 / 5 Experiments - extractive PDF cue:** Once again, we train to convergence on val-unseen (6 to 10 dataset collections, depending on the model).
- **p. 12 / 5 Experiments - extractive PDF cue:** Our baseline Seq2Seq model significantly outperforms the random and hand-crafted baselines, successfully reaching the goal in 20% of val-unseen episodes.
- **p. 14 / 5 Experiments - extractive PDF cue:** Qualitative examples of our Cross Modal Attention model taken in unseen validation environments.
- **p. 14 / 5 Experiments - extractive PDF cue:** Despite having similar cross-modal attention architectures, RCM [29] achieves an SPL of 0.38 in test environments while our model yields 5 Note that the VLN ...
- **p. 15 / 5 Experiments - extractive PDF cue:** Comparison on the VLN validation and test sets with existing models.
- **p. 11 / 5 Experiments - extractive PDF cue:** We report standard metrics for visual navigation tasks defined in [2,4,18] - trajectory length in meters (TL), navigation error in meters from goal at termination ...
- **p. 15 / 5 Experiments - extractive PDF cue:** Val-Seen (VLN) Val-Unseen (VLN) Test (VLN) Model TL ↓ NE ↓ OS ↑ SR ↑ SPL ↑ TL ↓ NE ↓ OS ↑ SR ↑ ...
...
- **p. 2 / Figure/Table caption - extractive PDF cue:** Fig. 1. The VLN setting (a) operates on a fixed topology of panoramic images (shown in blue) - assuming perfect navigation between nodes (often meters ...
- **p. 4 / Figure/Table caption - extractive PDF cue:** Table 1. Comparison of language-guided visual navigation tasks. Ours is the only to provide unconstrained navigation in real environments for crowdsourced instructions. Task Instructions Environment ...
- **p. 7 / Figure/Table caption - extractive PDF cue:** Fig. 2. We transfer nav-graph trajectories over panoramas (blue dots) from the Room-to- Room (R2R) dataset to locations in reconstructed Matterport3D (MP3D) environments. Some map ...
- **p. 8 / Figure/Table caption - extractive PDF cue:** Fig. 3. We successfully transfer 77% of the R2R trajectories. (a) Most panorama nodes transfer directly, but 3% require horizontal adjustment - with an average ...
...
- **p. 12 / Figure/Table caption - extractive PDF cue:** Table 2. No-learning baselines and input modality ablations for our baseline sequence- to-sequence model. Given the long trajectories involved, we find both random agents and ...
- **p. 13 / Figure/Table caption - extractive PDF cue:** Table 3. Performance in VLN-CE. We find that popular techniques in VLN have mixed benefit in VLN-CE; however, our best performing model combining all examined ...
- **p. 14 / Figure/Table caption - extractive PDF cue:** Fig. 5. Qualitative examples of our Cross Modal Attention model taken in unseen vali- dation environments. In the first example our agent successfully follows the ...
...
- **p. 11 / 5 Experiments - extractive PDF cue:** We collect 5, 000 trajectories at each stage and then perform 4 epochs of imitation learning (with inflection weighting) over all collected trajectories.
- **p. 15 / 5 Experiments - extractive PDF cue:** Further, state-of-the-art on the test set is near 0.47 SPL, over 2x what we report.
- **p. 3 / 1 Introduction - extractive PDF cue:** Agents in our task are free to navigate to any unobstructed point through a set of low-level actions (e.g. move forward 0.25m, turn-left 15 degrees) ...
...
- **p. 15 / 6 Discussion - extractive PDF cue:** In models presented here, we took an approach where observations were mapped directly to low-level control in an end-to-end manner; however, exploring modular approaches is ...
...
- **p. 15 / 5 Experiments - extractive PDF cue:** By default, our models cannot succeed on these.
- **p. 7 / Figure/Table caption - extractive PDF cue:** Fig. 2. We transfer nav-graph trajectories over panoramas (blue dots) from the Room-to- Room (R2R) dataset to locations in reconstructed Matterport3D (MP3D) environments. Some map ...

- **PDF anchors reviewed:** datasets p. 13 (5 Experiments), p. 11 (5 Experiments), p. 12 (5 Experiments), p. 14 (5 Experiments), p. 14 (5 Experiments), p. 15 (5 Experiments), metrics p. 11 (5 Experiments), p. 11 (5 Experiments), p. 12 (5 Experiments), p. 12 (5 Experiments), p. 13 (5 Experiments), p. 13 (5 Experiments), baselines p. 12 (5 Experiments), p. 13 (5 Experiments), p. 12 (5 Experiments), p. 14 (5 Experiments), p. 11 (5 Experiments), p. 11 (5 Experiments), results p. 12 (5 Experiments), p. 12 (5 Experiments), p. 11 (5 Experiments), p. 15 (5 Experiments), p. 13 (5 Experiments), p. 13 (5 Experiments).
